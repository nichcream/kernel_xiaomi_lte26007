...
int rtw_ieee80211_channel_to_frequency(int chan, int band)
{
...
static int rtw_cfg80211_inform_bss(_adapter *padapter, struct wlan_network *pnetwork)
{
	...
	if(bssinf_len > MAX_BSSINFO_LEN){
		printk("%s IE Length too long > 1000 byte \n",__FUNCTION__);
		goto exit;
	}
	...
	if (padapter->mlmepriv.scan_compensation)
		notify_signal += padapter->mlmepriv.scan_compensation * 100;
/*
	DBG_8192C("bssid: %2.2X:%2.2X:%2.2X:%2.2X:%2.2X:%2.2X ssid:%s\n",
...
#if (!(defined ANDROID_2X) && (defined CONFIG_PLATFORM_SPRD))
		/* remove the network entry in scanned_queue to fix ap power off, reconnect issue */
		_enter_critical_bh(&(pmlmepriv->scanned_queue.lock), &irqL);
		pwlan = rtw_find_network(&pmlmepriv->scanned_queue,
				pmlmepriv->cur_network.network.MacAddress);
		if (pwlan) {
			DBG_8192C("%s del this ssid:%s[%pM] from driver scan list\n",
					__func__, pwlan->network.Ssid.Ssid,
					pwlan->network.MacAddress);
			pwlan->fixed = _FALSE;
				    ...
				    u8 *mac, struct station_info *sinfo)
{
	...
	)
	{
		...
		sinfo->tx_packets= (u32)(pstats->tx_pkts);
		sinfo->filled |=STATION_INFO_RX_PACKETS;
		sinfo->rx_packets= (u32)(pstats->rx_mgnt_pkts + pstats->rx_ctrl_pkts + pstats->rx_data_pkts);
		sinfo->filled |=STATION_INFO_TX_BYTES;
		sinfo->tx_bytes =  (u32)(pstats->tx_bytes);
		sinfo->filled |=STATION_INFO_RX_BYTES;
		sinfo->rx_bytes =  (u32)(pstats->rx_bytes );
		sinfo->filled |=STATION_INFO_TX_FAILED;
		sinfo->tx_failed = (u32)(pstats->tx_drops );
		sinfo->filled |=STATION_INFO_RX_DROP_MISC;
		sinfo->rx_dropped_misc = (u32)(pstats->rx_drops );
	}
				...
				, struct cfg80211_scan_request *request)
{
	...
	struct	sta_priv *pbuddy_stapriv = NULL;
#endif
	int delaycount = 0;

	DBG_871X("%s set scan\n", __func__);

	if(padapter->hw_scan_disable)
	{
	...
	if (check_fwstate(&padapter->mlmepriv, WIFI_STATION_STATE)) {
		if (!padapter->mlmepriv.linked_lock_flag) {
	...
	if (padapter->pbuddy_adapter) {
		...
		pbuddy_stapriv = &(pbuddy_adapter->stapriv);
	}
#endif //CONFIG_CONCURRENT_MODE
	...
	if (padapter->registrypriv.mp_mode == 1)
	{
        ...
        if(strcmp(padapter->pnetdev->name,"p2p0")== 0 && padapter->pbuddy_adapter->registrypriv.mp_mode == 1)
        {
                ret = -EPERM;
                goto exit;
        }
	...
	if (pwrctrlpriv->in_early_suspend) {
		DBG_871X("%s dont scan when resume is in progress\n", __func__);
...
#ifdef CONFIG_BT_COEXIST
	{
		...
		printk("%s %d %d %d\n", __func__, delta_time, rtw_systime_to_ms(pwrctrlpriv->DelayLPSLastTimeStamp), rtw_systime_to_ms(curr_time));
	...
	{
#ifdef CONFIG_DEBUG_CFG80211
		DBG_871X("%s buddyAdapter under WIFI_AP_STATE\n", __FUNCTION__);
#endif

		if (check_fwstate(pbuddy_mlmepriv, WIFI_UNDER_WPS|_FW_UNDER_SURVEY|_FW_UNDER_LINKING) == _TRUE)
		{
			DBG_8192C("%s, buddy adapter fwstate=0x%x\n", __func__, pmlmepriv->fw_state);

			if(check_fwstate(pbuddy_mlmepriv, WIFI_UNDER_WPS))
			{
				DBG_8192C("buddy adapter AP mode process WPS \n");
			}
	...
	{
		DBG_871X("cfg8021_rtw_scan: Adapter is GO Case \n");
		if(pstapriv->asoc_sta_count <= 2)
		{
			DBG_871X("no sta is assoc \n");
			need_indicate_scan_done = _TRUE;
			goto check_need_indicate_scan_done;
		}
	...
	if(rtw_p2p_chk_role(pbuddy_wdinfo, P2P_ROLE_GO))
	{
		DBG_871X("cfg8021_rtw_scan: Buddy Adapter is GO Case \n");
		if(pbuddy_stapriv->asoc_sta_count <= 2)
		{
			DBG_871X("no sta is assoc \n");
			need_indicate_scan_done = _TRUE;
			goto check_need_indicate_scan_done;
...
check_need_indicate_scan_done:
	if(need_indicate_scan_done) {
		/* in suspend/resume longrun, before late resume */
		/* we should go here, but after some times all ssid */
		/* will time out, and scan list will empty */
		/* so we use fake_scantime to change last_scanned */
		/* to forbid ssid time out under this condition */
		pmlmepriv->fake_scantime = _TRUE;

		rtw_cfg80211_surveydone_event_callback(padapter);

		pmlmepriv->fake_scantime = _FALSE;
	}
				 ...
				 struct cfg80211_connect_params *sme)
{
	...
	if (sme->bssid) {
		...
		_rtw_memcpy(&pmlmepriv->assoc_bssid, sme->bssid, ETH_ALEN);
	} else {
		_rtw_memset(&pmlmepriv->assoc_bssid, 0, ETH_ALEN);
	}
...
static int rtw_cfg80211_monitor_if_xmit_entry(struct sk_buff *skb, struct net_device *ndev)
{
		...
		)
	{
		...
		#ifdef CONFIG_P2P
		if(rtw_p2p_check_action_frames(padapter, buf, len, _TRUE, &category, &action, &OUI_Subtype) < 0)
		{
			goto fail;
		}
		#ifdef CONFIG_WFD
...
static int rtw_cfg80211_add_monitor_if(_adapter *padapter, char *name, struct net_device **ndev)
{
	...
	//mon_ndev->destructor = rtw_ndev_destructor; binxia
...
static int rtw_add_beacon(_adapter *adapter, const u8 *head, size_t head_len, const u8 *tail, size_t tail_len)
{
	...
	//if(rtw_check_beacon_data(padapter, pbuf,  len) == _SUCCESS)
	{
		...
		if(got_p2p_ie == _TRUE)
		{
			...
			rtw_hal_set_hwreg(adapter, HW_VAR_H2C_FW_JOINBSSRPT, (u8 *)(&mstatus));
...
void rtw_cfg80211_rx_action_p2p(_adapter *padapter, u8 *pmgmt_frame, uint frame_len)
{
	...
	rtw_p2p_check_action_frames(padapter, pmgmt_frame, frame_len, _FALSE, &category, &action, &OUI_Subtype);
	DBG_871X("RTW_Rx:category(%u), action(%u), OUI_Subtype(%u)\n", category, action, OUI_Subtype);
...
//This must be revised when acquiring android4.4 SDK
//20140106
...
static int	cfg80211_rtw_mgmt_tx(struct wiphy *wiphy,
		...
		bool offchan,
#endif
#if(LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,30) && (LINUX_VERSION_CODE < KERNEL_VERSION(3,8,0)))
	enum nl80211_channel_type channel_type,
	#if (LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,37)) || defined(COMPAT_KERNEL_RELEASE)
			...
			u64 *cookie)
{
	...
	DBG_871X("====>%s()", __FUNCTION__);
	if(_FAIL == rtw_pwr_wakeup(padapter)) {
		ret = -EFAULT;
		goto exit;
	}
#if(LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,30) && (LINUX_VERSION_CODE < KERNEL_VERSION(3,8,0)))
	DBG_871X(FUNC_NDEV_FMT" len=%zu, ch=%d, ch_type=%d da="MAC_FMT"\n", FUNC_NDEV_ARG(ndev), len, tx_ch, channel_type, MAC_ARG(GetAddr1Ptr(buf)));
#endif
	/* cookie generation */
	*cookie = (unsigned long) buf;
	...
	if(GetFrameType(buf) != RTW_IEEE80211_FTYPE_MGMT)
	{
		goto exit;
	}

	frame_stype = GetFrameSubType(buf);
	if(frame_stype == RTW_IEEE80211_STYPE_ACTION)
	{
		rtw_p2p_check_action_frames(padapter, buf, len, _TRUE, &category, &action, &OUI_Subtype);
		if (category == RTW_WLAN_CATEGORY_PUBLIC)
			DBG_871X("RTW_Tx:%s\n", action_public_str(action));
		else
			DBG_871X("RTW_Tx:category(%u), action(%u), OUI_Subtype(%u)\n", category, action, OUI_Subtype);
	}
#ifdef CONFIG_WFDS
	else if(frame_stype == RTW_IEEE80211_STYPE_PROBE_RESP)
	{
		DBG_871X("WFDS: cfg80211_rtw_mgmt_tx: Probe Response\n");
		goto dump;
	}
#endif
	else
	...
	if(len>0)
	{
		if((wps_ie = rtw_get_wps_ie(buf, len, NULL, &wps_ielen)))
		{
			...
			if(check_fwstate(pmlmepriv, WIFI_UNDER_WPS))
			{
				...
				{
					DBG_871X("GO mode process WPS under site-survey,  sr no set\n");
					return ret;
				}
			}
...
static int cfg80211_rtw_set_hw_scan_disable(struct wiphy *wiphy,
				       struct wireless_dev *wdev,
				       bool state)
{
	_adapter *padapter = wiphy_to_adapter(wiphy);
	...
	//DBG_871X(FUNC_NDEV_FMT" state:%u\n", FUNC_NDEV_ARG(ndev),
	//	state);
	padapter->hw_scan_disable = state;
	return 0;
}
...
#endif
	.set_hw_scan_disable = cfg80211_rtw_set_hw_scan_disable,
};
