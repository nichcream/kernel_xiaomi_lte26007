...
*******************************************************/
#ifdef POLLING_TP
static enum hrtimer_restart synaptics_rmi4_timer_handler(struct hrtimer *timer)
{
	struct synaptics_rmi4_data *rmi4_data = container_of(timer, struct synaptics_rmi4_data, timer);


	queue_work(rmi4_data->poll_wq, &rmi4_data->work);

	hrtimer_forward_now(&rmi4_data->timer, ktime_set(0, (20)*1000000));
	return HRTIMER_RESTART;

	//hrtimer_start(&rmi4_data->timer, ktime_set(0, (20)*1000000), HRTIMER_MODE_REL);
		...
		bool enable)
{
	...
	if (enable) {
		...
		retval = request_threaded_irq(rmi4_data->irq,  NULL,   synaptics_rmi4_irq,
						bdata->irq_flags,PLATFORM_DRIVER_NAME, rmi4_data);
		...
		unsigned int intr_count)
{
	...
	dev_info(rmi4_data->pdev->dev.parent, "snaptics's resolution: sMax_x = %d, sMax_y = %d\n",
...
/*
 int synaptics_rmi4_set_gpio(struct synaptics_rmi4_data *rmi4_data)
{
...
static int __init synaptics_rmi4_probe(struct platform_device *pdev)
{
	...
	printk(KERN_DEBUG "initialize synaptics probe begin  \n");
	...
	rmi4_data->hw_if->board_data->power_ic(1);
...
#ifdef POLLING_TP
	rmi4_data->poll_wq = create_singlethread_workqueue("poll_wq");
	INIT_WORK(&rmi4_data->work, poll_work_func);
        hrtimer_init(&rmi4_data->timer, CLOCK_MONOTONIC, HRTIMER_MODE_REL);
        rmi4_data->timer.function = synaptics_rmi4_timer_handler;
	/*polling*/
...
static void synaptics_rmi4_early_suspend(struct early_suspend *h)
{
	...
	rmi4_data->hw_if->board_data->power_ic( 0 );
	//printk("synaptics early suspend end!\n");
	return;
}
...
static void synaptics_rmi4_late_resume(struct early_suspend *h)
{
	...
	rmi4_data->hw_if->board_data->power_ic( 1 ); // power off ic
	...
	if (rmi4_data->sensor_sleep == true) {
		//printk("sensor sleeped ,reinit the tp!\n");
	...
	rmi4_data->touch_stopped = false;
	//printk(" synaptics late resume end\n");
...
static struct platform_driver synaptics_rmi4_driver = {
	...
	.remove = __exit_p(synaptics_rmi4_remove),
};
