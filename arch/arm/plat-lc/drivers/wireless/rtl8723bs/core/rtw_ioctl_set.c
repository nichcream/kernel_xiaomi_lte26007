...
_func_enter_;
	...
	if (check_fwstate(pmlmepriv, _FW_LINKED|WIFI_ADHOC_MASTER_STATE) == _TRUE)
	{
		...
		if (_rtw_memcmp(&pmlmepriv->cur_network.network.MacAddress, bssid, ETH_ALEN) == _TRUE)
		{
			...
			rtw_disassoc_cmd(padapter,_TRUE);
...
_func_enter_;
	...
	// avoid scan blocked under WiFi idle circumstance.   -RJZ  2013.08.19
	if ((check_fwstate(pmlmepriv, _FW_UNDER_SURVEY|_FW_UNDER_LINKING) == _TRUE) ||
		   (pmlmepriv->LinkDetectInfo.bCanNotScan == _TRUE))
	{
...
u16 rtw_get_cur_max_rate(_adapter *adapter)
{
	...
	if((ht_cap == _TRUE) && (pregistrypriv->ht_enable))
	{
...
int rtw_set_country(_adapter *adapter, const char *country_code)
{
	...
	if(0 == strcmp(country_code, "US"))
		...
		channel_plan = RT_CHANNEL_DOMAIN_JAPAN;
#else
