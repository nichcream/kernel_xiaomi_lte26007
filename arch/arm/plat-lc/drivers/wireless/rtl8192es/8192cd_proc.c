...
#endif			
{
	...
	for (i=0; i<20; i++) {
		RTL_W32(REG_MBIDCAMCFG+4, 0x80000000|i<<24);
		CAM[0] = RTL_R32(REG_MBIDCAMCFG);
		CAM[1] = RTL_R32(REG_MBIDCAMCFG+4);
		PRINT_ONE("    Entry", "%s", 0);
		PRINT_ONE(i, " %2d:", 0);
...
#endif			
{
	...
	unsigned char tmpbuf[16];
	unsigned char *rate;
	unsigned char true_tx_bw =0;
#ifdef WIFI_11N_2040_COEXIST
	true_tx_bw = (pstat->tx_bw == HT_CHANNEL_WIDTH_20_40)
...
#ifdef WIFI_11N_2040_COEXIST
		rate = (unsigned char *)MCS_DATA_RATEStr[(pstat->ht_current_tx_info&BIT(0))?true_tx_bw:0][(pstat->ht_current_tx_info&BIT(1))?1:0][(pstat->current_tx_rate - HT_RATE_ID)];
#else		
...
//8812_client tx_bw rx_bw ??
	...
	CHECK_LEN_B;
#ifdef WIFI_11N_2040_COEXIST
	PRINT_SINGL_ARG("    tx_bw: ", (true_tx_bw? (pstat->tx_bw*40) : 20), "%dM");
#else
