...
//	ODM_RT_TRACE(pDM_Odm,ODM_COMP_CALIBRATION, ODM_DBG_LOUD, ("Path-A IQK setting!\n"));
	...
	ODM_SetBBReg(pDM_Odm, rTx_IQK_PI_A, bMaskDWord, 0x82140302);
...
//	ODM_SetRFReg(pDM_Odm, RF_PATH_A, RF_WE_LUT, 0x80000, 0x0);	               // 0xEF[19]   = 0x0
    ...
    if((pDM_Odm->ExtLNA) && !(pDM_Odm->ExtPA)){	//external LNA / external PA = 1 / 0
	    ODM_SetBBReg(pDM_Odm, rTx_IQK_PI_A, bMaskDWord, 0x8216001f);
...
{
	u4Byte regEAC, regEB4, regEBC, regEC4, regECC, tmp;
	u1Byte	result = 0x00;
#if !(DM_ODM_SUPPORT_TYPE & ODM_AP)
	HAL_DATA_TYPE	*pHalData = GET_HAL_DATA(pAdapter);
#if (DM_ODM_SUPPORT_TYPE == ODM_CE)
...
//	ODM_RT_TRACE(pDM_Odm,ODM_COMP_CALIBRATION, ODM_DBG_LOUD, ("Path-B IQK setting!\n"));
	...
	ODM_SetBBReg(pDM_Odm, rTx_IQK_PI_B, bMaskDWord, 0x82140302);
...
//	ODM_RT_TRACE(pDM_Odm,ODM_COMP_CALIBRATION, ODM_DBG_LOUD, ("Delay %d ms for One shot, path B LOK & IQK.\n", IQK_DELAY_TIME_92E));
	...
	if (!(regEAC & BIT31) &&
		...
		(((regEB4 & 0x03FF0000)>>16) <0x110) &&
		(((regEB4 & 0x03FF0000)>>16) >0xf0) &&
		(tmp <0xa))
		result |= 0x01;
	else
		return result;
...
{
	u4Byte regEAC, regEB4, regEBC, regECC, regEC4, u4tmp, tmp;
	u1Byte result = 0x00;
#if !(DM_ODM_SUPPORT_TYPE & ODM_AP)
	HAL_DATA_TYPE	*pHalData = GET_HAL_DATA(pAdapter);
#if (DM_ODM_SUPPORT_TYPE == ODM_CE)
...
//	ODM_SetRFReg(pDM_Odm, RF_PATH_A, RF_WE_LUT, 0x80000, 0x0);	               // 0xEF[19]   = 0x0
	...
	if((pDM_Odm->ExtLNA) && !(pDM_Odm->ExtPA)){	//external LNA / external PA = 1 / 0
   	  ODM_SetBBReg(pDM_Odm, rTx_IQK_PI_B, bMaskDWord, 0x8216001f);
	  ODM_SetBBReg(pDM_Odm, rRx_IQK_PI_B, bMaskDWord, 0x2816001f);
    }else if(pDM_Odm->ExtPA){	// external PA = 1
	  ODM_SetBBReg(pDM_Odm, rTx_IQK_PI_B, bMaskDWord, 0x8216001f);
	  ODM_SetBBReg(pDM_Odm, rRx_IQK_PI_B, bMaskDWord, 0x2816001f);
    }else{
...
//	ODM_RT_TRACE(pDM_Odm,ODM_COMP_CALIBRATION, ODM_DBG_LOUD, ("Delay %d ms for One shot, path B LOK & IQK.\n", IQK_DELAY_TIME_92E));
	...
	if (!(regEAC & BIT30) &&		//if Tx is OK, check whether Rx is OK
			(((regEC4 & 0x03FF0000) >> 16) != 0x132) &&
		(((regECC & 0x03FF0000)>>16) != 0x36) && 	
			(((regEC4 & 0x03FF0000)>>16) <0x110) &&
			(((regEC4 & 0x03FF0000)>>16) >0xf0) &&
			(tmp <0xa))
