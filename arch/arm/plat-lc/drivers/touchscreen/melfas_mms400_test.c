...
static ssize_t mms_dev_fs_write(struct file *fp, const char *wbuf, size_t cnt, loff_t *fpos)
{
	...
	if(cmd == 1){
		...
		//print_hex_dump(KERN_ERR, MMS_DEVICE_NAME" : input ", DUMP_PREFIX_OFFSET, 16, 1, wbuf, cnt, false);
		//print_hex_dump(KERN_ERR, MMS_DEVICE_NAME" : output ", DUMP_PREFIX_OFFSET, 16, 1, info->dev_fs_buf, buf[cnt - 2], false);
	}
	else if(cmd == 2){
...
* Process table data
*/
static int mms_proc_table_data(struct mms_ts_info *info, u8 size, u8 data_type_size, u8 data_type_sign, u8 buf_addr_h, u8 buf_addr_l, u8 row_num, u8 col_num, u8 buf_col_num, u8 rotate, u8 key_num)
{
	char data[10];
	int i_col, i_row;
	int i_x, i_y;
...
int mms_run_test(struct mms_ts_info *info, u8 test_type)
{
	...
	//print data
	if(mms_proc_table_data(info, size, data_type_size, data_type_sign, buf_addr_h, buf_addr_l, row_num, col_num, buffer_col_num, rotate, key_num)){
...
*/
static ssize_t mms_sys_fw_version(struct device *dev, struct device_attribute *attr, char *buf)
{
...
* Device enable
*/
static ssize_t mms_sys_device_enable(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct mms_ts_info *info = dev_get_drvdata(dev);
	struct i2c_client *client = info->client;
	u8 data[255];
...
* Device disable
*/
static ssize_t mms_sys_device_disable(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct mms_ts_info *info = dev_get_drvdata(dev);
	struct i2c_client *client = info->client;
	u8 data[255];
...
* Enable IRQ
*/
static ssize_t mms_sys_irq_enable(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct mms_ts_info *info = dev_get_drvdata(dev);
	struct i2c_client *client = info->client;
	u8 data[255];
...
* Disable IRQ
*/
static ssize_t mms_sys_irq_disable(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct mms_ts_info *info = dev_get_drvdata(dev);
	struct i2c_client *client = info->client;
	u8 data[255];
...
* Power on
*/
static ssize_t mms_sys_power_on(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct mms_ts_info *info = dev_get_drvdata(dev);
	struct i2c_client *client = info->client;
	u8 data[255];
...
* Power off
*/
static ssize_t mms_sys_power_off(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct mms_ts_info *info = dev_get_drvdata(dev);
	struct i2c_client *client = info->client;
	u8 data[255];
...
* Reboot chip
*/
static ssize_t mms_sys_reboot(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct mms_ts_info *info = dev_get_drvdata(dev);
	struct i2c_client *client = info->client;
	u8 data[255];
...
* Set glove mode
*/
static ssize_t mms_sys_glove_mode_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	struct mms_ts_info *info = dev_get_drvdata(dev);
	//struct i2c_client *client = info->client;
	//u8 data[255];
...
* Get glove mode
*/
static ssize_t mms_sys_glove_mode_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct mms_ts_info *info = dev_get_drvdata(dev);
	//struct i2c_client *client = info->client;
	u8 data[255];
...
* Set charger mode
*/
static ssize_t mms_sys_charger_mode_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	struct mms_ts_info *info = dev_get_drvdata(dev);
	//struct i2c_client *client = info->client;
	//u8 data[255];
...
* Get charger mode
*/
static ssize_t mms_sys_charger_mode_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct mms_ts_info *info = dev_get_drvdata(dev);
	//struct i2c_client *client = info->client;
	u8 data[255];
...
*/
static ssize_t mms_sys_window_mode_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	struct mms_ts_info *info = dev_get_drvdata(dev);
	//struct i2c_client *client = info->client;
	//u8 data[255];
...
*/
static ssize_t mms_sys_window_mode_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct mms_ts_info *info = dev_get_drvdata(dev);
	//struct i2c_client *client = info->client;
	u8 data[255];
...
*/
static ssize_t mms_sys_palm_rejection_mode_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	struct mms_ts_info *info = dev_get_drvdata(dev);
	//struct i2c_client *client = info->client;
	//u8 data[255];
...
*/
static ssize_t mms_sys_palm_rejection_mode_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct mms_ts_info *info = dev_get_drvdata(dev);
	//struct i2c_client *client = info->client;
	u8 data[255];
...
*/
static ssize_t mms_sys_intensity(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct mms_ts_info *info = dev_get_drvdata(dev);
	//struct i2c_client *client = info->client;
	int ret;
...
*/
static ssize_t mms_sys_rawdata(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct mms_ts_info *info = dev_get_drvdata(dev);
	//struct i2c_client *client = info->client;
	int ret;
...
*/
static ssize_t mms_sys_test_cm_delta(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct mms_ts_info *info = dev_get_drvdata(dev);
	//struct i2c_client *client = info->client;
	int ret;
...
*/
static ssize_t mms_sys_test_cm_abs(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct mms_ts_info *info = dev_get_drvdata(dev);
	//struct i2c_client *client = info->client;
	int ret;
...
*/
static ssize_t mms_sys_test_cm_jitter(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct mms_ts_info *info = dev_get_drvdata(dev);
	//struct i2c_client *client = info->client;
	int ret;
...
*/
static ssize_t mms_sys_test_short(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct mms_ts_info *info = dev_get_drvdata(dev);
	//struct i2c_client *client = info->client;
	int ret;
...
static DEVICE_ATTR(mode_charger, 0666, mms_sys_charger_mode_show, mms_sys_charger_mode_store);
static DEVICE_ATTR(mode_cover_window, 0666, mms_sys_window_mode_show, mms_sys_window_mode_store);
static DEVICE_ATTR(mode_palm_rejection, 0666, mms_sys_palm_rejection_mode_show, mms_sys_palm_rejection_mode_store);
