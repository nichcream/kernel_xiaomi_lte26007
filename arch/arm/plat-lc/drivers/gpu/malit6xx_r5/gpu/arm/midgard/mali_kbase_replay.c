		...
		struct job_head *job)
{
	...
	KBASE_LOG(2, kctx->kbdev->dev, "%s\n", head_str);
	KBASE_LOG(2, kctx->kbdev->dev, "addr               = %p\n"
			"status             = %x\n"
	...
	if (job->flags & JOB_FLAG_DESC_SIZE)
		KBASE_LOG(2, kctx->kbdev->dev, "next               = %llx\n",
				job->next._64);
	else
		KBASE_LOG(2, kctx->kbdev->dev, "next               = %x\n",
				job->next._32);
#endif
}
		...
		u16 hierarchy_mask, u32 default_weight)
{
	...
	KBASE_LOG(2, kctx->kbdev->dev, "fbd_address: %llx\n", fbd_address);
	...
	if (hierarchy_mask) {
		...
		KBASE_LOG(2, kctx->kbdev->dev, "Old hierarchy mask=%x  New hierarchy mask=%x\n",
				old_hierarchy_mask, hierarchy_mask);

		for (i = 0; i < HIERARCHY_WEIGHTS; i++)
			KBASE_LOG(2, kctx->kbdev->dev, " Hierarchy weight %02d: %08x\n",
					i, weights[i]);
		...
		for (i = 0; i < HIERARCHY_WEIGHTS; i++) {
			if (hierarchy_mask & (1 << i)) {
				...
				KBASE_LOG(2, kctx->kbdev->dev, " Writing hierarchy level %02d (%08x) to %d\n",
						i, weights[i], j);
	...
	KBASE_LOG(2, kctx->kbdev->dev, "heap_free_address=%llx flags=%x\n",
...
#ifdef CONFIG_MALI_DEBUG
	KBASE_LOG(2, kctx->kbdev->dev, "FBD tiler:\n"
			"flags = %x\n"
			"heap_free_address = %llx\n",
			fbd_tiler->flags,
		...
		mali_bool fragment_chain)
{
	...
	KBASE_LOG(2, kctx->kbdev->dev, "kbasep_replay_parse_jc: jc=%llx hw_job_id=%x\n",
			jc, hw_job_id_offset);

	while (jc) {
		KBASE_LOG(2, kctx->kbdev->dev, "kbasep_replay_parse_jc: parsing jc=%llx\n", jc);
...
static void kbasep_release_katom(struct kbase_context *kctx, int atom_id)
{
	...
	KBASE_LOG(2, kctx->kbdev->dev, "kbasep_release_katom: Released atom %d\n",
			atom_id);
...
static void payload_dump(struct kbase_context *kctx, base_jd_replay_payload *payload)
{
	...
	while (next) {
		...
		KBASE_LOG(2, kctx->kbdev->dev, "* jc_struct=%p jc=%llx next=%llx\n",
					      ...
					      struct base_jd_atom_v2 *f_atom)
{
	...
	KBASE_LOG(2, kctx->kbdev->dev, "kbasep_replay_parse_payload: replay_atom->jc = %llx sizeof(payload) = %zu\n",
...
#ifdef CONFIG_MALI_DEBUG
	KBASE_LOG(2, kctx->kbdev->dev, "kbasep_replay_parse_payload: payload=%p\n", payload);
	KBASE_LOG(2, kctx->kbdev->dev, "Payload structure:\n"
				  "tiler_jc_list            = %llx\n"
				  "fragment_jc              = %llx\n"
				  "tiler_heap_free          = %llx\n"
	...
	KBASE_LOG(2, kctx->kbdev->dev, "t_atom->jc=%llx f_atom->jc=%llx\n",
			t_atom->jc, f_atom->jc);
	ret = MALI_ERROR_NONE;
...
#ifdef CONFIG_MALI_DEBUG
	KBASE_LOG(2, dev, "kbase_replay_fault_check: payload=%p\n", payload);
	KBASE_LOG(2, dev, "\nPayload structure:\n"
		     "fragment_jc              = 0x%llx\n"
...
bool kbase_replay_process(struct kbase_jd_atom *katom)
{
	...
	if (katom->event_code == BASE_JD_EVENT_DONE) {
		KBASE_LOG(2, kctx->kbdev->dev, "Previous job succeeded - not replaying\n");
	...
	if (jctx->sched_info.ctx.is_dying) {
		KBASE_LOG(2, kctx->kbdev->dev, "Not replaying; context is dying\n");
