...
#elif defined(CONFIG_COMIP_PROJECT_K706_V1_0)
static struct comip_backlight_platform_data comip_backlight_data = {
	.ctrl_type = CTRL_PWM,
	.gpio_en = -1,
	.pwm_en = 1,
...
#endif
#if defined(CONFIG_VIDEO_SP0A20)
	{SP0A20_POWERDOWN_PIN,		MFP_PIN_MODE_GPIO},
	{SP0A20_RESET_PIN,		MFP_PIN_MODE_GPIO},
#endif
#if defined(CONFIG_VIDEO_SP2529_26M)
...
static int comip_lcd_power(int onoff)
{
	printk("k706 comip_lcd_power : %d\n", onoff);
	if (onoff) {
			gpio_request(LCD_PWR_CTRL, "LCD IF VCC Ctrl");
			gpio_direction_output(LCD_PWR_CTRL, 1);
			gpio_free(LCD_PWR_CTRL);	
			msleep(30);
	} else {
			gpio_request(LCD_PWR_CTRL, "LCD IF VCC Ctrl");
			gpio_direction_output(LCD_PWR_CTRL, 0);
			gpio_free(LCD_PWR_CTRL);	
			msleep(30);
	}
...
#if defined(CONFIG_VIDEO_SP0A20)
static int sp0a20_pwdn = mfp_to_gpio(SP0A20_POWERDOWN_PIN);
static int sp0a20_rst = mfp_to_gpio(SP0A20_RESET_PIN);

static int sp0a20_power(int onoff)
{
	printk("sp0a20 power : %d\n", onoff);

	gpio_request(sp0a20_pwdn, "sp0a20 Powerdown Pin");
	gpio_request(sp0a20_rst, "sp0a20 Reset");
	if (onoff) {
		gpio_direction_output(sp0a20_pwdn, 1);
		gpio_direction_output(sp0a20_rst, 1);
		mdelay(10);
		pmic_voltage_set(PMIC_POWER_CAMERA_IO, 0, PMIC_POWER_VOLTAGE_ENABLE);
		pmic_voltage_set(PMIC_POWER_CAMERA_ANALOG, 0, PMIC_POWER_VOLTAGE_ENABLE);
		gpio_direction_output(sp0a20_pwdn, 0);
		mdelay(10);
		gpio_direction_output(sp0a20_rst, 0);
		mdelay(50);
		gpio_direction_output(sp0a20_rst, 1);
	} else {
		pmic_voltage_set(PMIC_POWER_CAMERA_IO, 0, PMIC_POWER_VOLTAGE_DISABLE);
	...
	gpio_free(sp0a20_pwdn);
	gpio_free(sp0a20_rst);

	return 0;
}
static int sp0a20_reset(void)
{
	printk("sp0a20 reset\n");
...
static struct i2c_board_info sp0a20_board_info = {
	.type = "sp0a20-mipi-yuv",
	.addr = 0x21,
};
#endif
