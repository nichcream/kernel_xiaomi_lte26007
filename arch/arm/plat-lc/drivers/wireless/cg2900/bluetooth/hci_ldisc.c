...
#if 1
int cg2900_hci_uart_set_baudrate(struct hci_uart *hu, int baud, int first_time)
#else
...

{
...
#if 1
	hw_flow = ((tty->termios.c_cflag & CRTSCTS) == CRTSCTS);
	/* Finally inform the driver */
	if (first_time && hw_flow)
	...
	schedule_timeout_killable(msecs_to_jiffies(UART_BAUD_TIMEOUT));
#endif
