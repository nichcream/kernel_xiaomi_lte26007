...
int rtw_cmd_filter(struct cmd_priv *pcmdpriv, struct cmd_obj *cmd_obj)
{
	...
	if(	(adapter_to_pwrctl(pcmdpriv->padapter)->pwrctrlpriv.bHWPwrPindetect)
...
_func_enter_;
	...
	while(1)
	{
		...
		if (rtw_register_cmd_alive(padapter) != _SUCCESS)
		{
			...
			//open CONFIG_WAIT_PS_ACK will not _FAIL
			//close CONFIG_WAIT_PS_ACK:
			//if rtw_register_cmd_alive FAIL, we should continue here
			//and will wait under down(cmd_queue_sema), and cpwm will up(cmd_queue_sema)
			//and if cpwm interrupt lost, cmd_queue_sema may cause some issues.
			DBG_871X("%s: wait to leave LPS_LCLK!!!!!!!\n", __FUNCTION__);
			continue;
#endif
		}
#endif
...
u8 rtw_disassoc_cmd(_adapter*padapter,bool enqueue) // for sta_mode
{
	struct	cmd_obj*	pdisconnect_cmd;
	struct	disconnect_parm* pdisconnect=NULL;
	//struct 	mlme_priv *pmlmepriv = &padapter->mlmepriv;
	struct	cmd_priv   *pcmdpriv = &padapter->cmdpriv;
...
_func_enter_;
	...
	//if ((check_fwstate(pmlmepriv, _FW_LINKED)) == _TRUE) {
		pdisconnect = (struct	disconnect_parm*)rtw_zmalloc(sizeof(*pdisconnect));
		if(pdisconnect == NULL) {
	...
	if(enqueue)
		...
		if(pdisconnect_cmd == NULL){
			res=_FAIL;
			rtw_mfree((u8 *)pdisconnect, sizeof(*pdisconnect));
			goto exit;
			}
		...
		if (H2C_SUCCESS != disconnect_hdl(padapter, (u8 *)pdisconnect))
			res = _FAIL;
		rtw_mfree((u8 *)pdisconnect, sizeof(*pdisconnect));
		}
	//}
...
static void traffic_status_watchdog(_adapter *padapter)
{
	...
	{
		u32 rx_busy_thresh = RX_TRAFFIC_BUSY_THRESHOLD;
		u32 tx_busy_thresh = TX_TRAFFIC_BUSY_THRESHOLD;

		if(check_fwstate(&padapter->mlmepriv, WIFI_AP_STATE)) {
			rx_busy_thresh = AP_RX_TRAFFIC_BUSY_THRESHOLD;
			tx_busy_thresh = AP_TX_TRAFFIC_BUSY_THRESHOLD;
		}
		...
		if(pmlmepriv->LinkDetectInfo.NumRxUnicastOkInPeriod > rx_busy_thresh ||
				pmlmepriv->LinkDetectInfo.NumTxOkInPeriod > tx_busy_thresh)
		{
			bBusyTraffic = _TRUE;
...
#else
		if (!BT_IsBtControlLps(padapter) && !BT_IsBtLpsOn(padapter))
#endif
#endif
		{
...
#endif
		idle_count = 0;
		adapter_to_pwrctl(padapter)->WiFiOnlyLowTrafficTdma = _FALSE;
	}
	...
	//check if ethernet Byte rate > 300K B/s (600k B/2s)
	if (precvpriv->rx_bytes < pmlmepriv->LinkDetectInfo.LastRxBytesInPeriod) {
		pmlmepriv->LinkDetectInfo.MaxRxBytesInPeriod = 0;
	} else {
		cur_rx_bytes = precvpriv->rx_bytes - pmlmepriv->LinkDetectInfo.LastRxBytesInPeriod;
		if (cur_rx_bytes > pmlmepriv->LinkDetectInfo.MaxRxBytesInPeriod)
			pmlmepriv->LinkDetectInfo.MaxRxBytesInPeriod = cur_rx_bytes;

		//printk("MaxRxBytesInPeriod: %lld \n", pmlmepriv->LinkDetectInfo.MaxRxBytesInPeriod);
	}
	pmlmepriv->LinkDetectInfo.LastRxBytesInPeriod = precvpriv->rx_bytes;
...
void dynamic_chk_wk_hdl(_adapter *padapter, u8 *pbuf, int sz)
{
	...
	//
#ifdef CONFIG_WOWLAN
	if(adapter_to_pwrctl(padapter)->wowlan_mode != _TRUE)
#endif
...
_func_enter_;
	...
	switch(lps_ctrl_type)
	{
		...
		case LPS_CTRL_CONNECT:
			...
			mstatus = 1;//connect
			// Reset LPS Setting
			adapter_to_pwrctl(padapter)->LpsIdleCount = 0;
...
#ifdef CONFIG_BT_COEXIST
			BT_SpecialPacketNotify(padapter, 1); // 1 for DHCP
#endif
			LPS_Leave(padapter);
			break;
		case LPS_CTRL_LEAVE:
		...
		case LPS_CTRL_TRAFFIC_TDMA:
			BT_LowWiFiTrafficNotify(padapter);
			break;
#endif
#ifdef CONFIG_BT_COEXIST
		case LPS_CTRL_BT_WIFI_MEDIA_STATUS_NOTIFY:
			mstatus = 1;//connect
			BT_WifiMediaStatusNotify(padapter, mstatus);
			break;
#endif
...
u8 rtw_c2h_packet_wk_cmd(PADAPTER padapter, u8 *pbuf, u16 length)
{
	...
	pdrvextra_cmd_parm->type_size = length;
...
u8 rtw_drvextra_cmd_hdl(_adapter *padapter, unsigned char *pbuf)
{
	...
	switch(pdrvextra_cmd->ec_id)
	{
		case DYNAMIC_CHK_WK_CID:
			...
			{
				dynamic_chk_wk_hdl(padapter->pbuddy_adapter,pdrvextra_cmd->pbuf, pdrvextra_cmd->type_size);
			}
		...
		case C2H_WK_CID:
#ifdef CONFIG_C2H_PACKET_EN
			rtw_hal_set_hwreg_with_buf(padapter, HW_VAR_C2H_HANDLE, pdrvextra_cmd->pbuf, pdrvextra_cmd->type_size);
#else
