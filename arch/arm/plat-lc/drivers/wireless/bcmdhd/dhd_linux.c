...
#define max_mutlcast_index 5
#define max_mutlcast_cnt 10
static  int mutlcast_cnt[max_mutlcast_index][2];
...
dhd_set_multicast_list(struct net_device *dev)
{
	...
	// check if index exist in table
	for(i = 0; i < max_mutlcast_index; i++) {
		if(mutlcast_cnt[i][0] == 0)
			break;

		if(mutlcast_cnt[i][0] == ifidx + 10) {
			mutlcast_cnt[i][1] ++;
			is_found = 1;

		if(mutlcast_cnt[i][1] > max_mutlcast_cnt) {
			printk(KERN_DEBUG "ifidx %d,index %d, reach max mulcast cnt, return \n", ifidx,i);
			return;
		}
		else
			break;
	...
	// add new index in table
	if(is_found== 0) {
		for(i = 0; i < max_mutlcast_index; i++) {
			if(mutlcast_cnt[i][0] == 0) {
				mutlcast_cnt[i][0] = ifidx + 10;
				break;
			}
		}
	}
...

{
	...
	SMP_RD_BARRIER_DEPENDS();
	printk(KERN_DEBUG "get sema and reload firmware\n");
	dhd_reopen_dev(dev);
	//dhd_stop(dev);
	//dhd_open(dev);

	complete_and_exit(&tsk->completed, 0);
	printk(KERN_DEBUG "get sema and reload firmware exit\n");
}
#endif
...
dhd_open(struct net_device *net)
{
	...
	if((dhd->pub.op_mode&DHD_FLAG_HOSTAP_MODE) &&
		dhd->pub.wlfc_enabled && dhd->pub.wlfc_state == NULL){
		DHD_ERROR(("%s: wlfc_state is null\n",__FUNCTION__));
		ret = -1;
		goto exit;
...
static int dhd_preinit_proc(dhd_pub_t *dhd, int ifidx, char *name, char *value)
{
	...
	if (!strcmp(name, "country")) {
		...
		if (var_int == 0) {
			g_pm_control = TRUE;
			printk(KERN_DEBUG "%s var_int=%d don't control PM\n", __func__, var_int);
		} else {
			g_pm_control = FALSE;
			printk(KERN_DEBUG "%s var_int=%d do control PM\n", __func__, var_int);
		}
#endif
		...
		if (!strcmp(value, "auto"))
			...
			printk(KERN_DEBUG " set band value should be one of the a or b or all\n");
			var_int = WLC_BAND_AUTO;
		}
		if ((ret = dhd_wl_ioctl_cmd(dhd, WLC_SET_BAND, &var_int,
			sizeof(var_int), TRUE, 0)) < 0)
			printk(KERN_DEBUG " set band err=%d\n", ret);
		return ret;
...
static int dhd_preinit_config(dhd_pub_t *dhd, int ifidx)
{
	...
	printk(KERN_DEBUG "dhd_preinit_config : config path : %s \n", config_path);
...
#endif /* WLTDLS */
	memset(mutlcast_cnt, 0, sizeof(mutlcast_cnt));
...
#ifdef CUSTOM_MAX_ASSOC_CNT
	max_assoc = CUSTOM_MAX_ASSOC_CNT;
...
#endif
#else
	printk(KERN_DEBUG "start dhd_reopen_thread\n");
...
int dhd_os_send_hang_message(dhd_pub_t *dhdp)
{
	...
	if (dhdp) {
		if (!dhdp->hang_was_sent) {
			...
			printk(KERN_DEBUG "hang was sent. schedule dhd_hang_process\n");
...
int dhd_get_opmode(struct net_device *net)
{
	dhd_info_t *dhd = NULL;
	if (!net || !DEV_PRIV(net)) {
...
int dhd_reopen_dev(struct net_device *dev)
{
	...
	if(dhd->pub.op_mode == DHD_FLAG_HOSTAP_MODE) {
		...
		printk(KERN_DEBUG "dhd_reopen_dev enter\n");
		dhd_stop(dev);
		dhd_open(dev);
