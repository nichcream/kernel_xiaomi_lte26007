...
static u8 lcd_cmds_init[][ROW_LINE] = {
	#if 1
#if 0	
		{0x00, DCS_CMD, SW_PACK2, 0x02, 0x80,0xDD,},
		{0x00, DCS_CMD, SW_PACK2, 0x02, 0x81,0xDD,},
		{0x00, DCS_CMD, SW_PACK2, 0x02, 0x82,0xDD,},	
		{0x00, DCS_CMD, SW_PACK2, 0x02, 0x83,0xDD,},
		{0x00, DCS_CMD, SW_PACK2, 0x02, 0x84,0xDD,},
		{0x00, DCS_CMD, SW_PACK2, 0x02, 0x85,0xDD,},
		{0x00, DCS_CMD, SW_PACK2, 0x02, 0x86,0xDD,},						


//		{0x80, DCS_CMD, SW_PACK1, 0x01, 0x1},  //sw reset
//		{0x80, DCS_CMD, SW_PACK1, 0x01, 0x10},  //display on
//		{0x00, DCS_CMD, SW_PACK2, 0x02, 0xB0,0x80},						
//		{0x00, DCS_CMD, SW_PACK2, 0x02, 0xB2,0x40},						
#else
	{0xAa, DCS_CMD, SW_PACK1, 0x01, 0x11},
...
/* Byte0: command type, Byte1-x: command */
static u8 lcd_esd_cmd[][ROW_LINE] = {
	{DCS_CMD, 0xC5},
};

/* Byte0-1: Length, Byte2-x: Data */
static u8 lcd_esd_rsp[][ROW_LINE] = {
	{0x00, 0x02, 0x55, 0x17},
};
#endif

static int lcd_hs_ek79007_reset(struct comipfb_info *fbi)
{
	int gpio_rst = fbi->pdata->gpio_rst;
	printk("lcd_hs_ek79007_reset.....gpio %d..........\n",gpio_rst);
...
static int lcd_hs_ek79007_suspend(struct comipfb_info *fbi)
{
...
static int lcd_hs_ek79007_resume(struct comipfb_info *fbi)
{
	int ret=0;
	struct comipfb_dev_timing_mipi *mipi;
...
struct comipfb_dev lcd_hs_ek79007_dev = {
	.name = "lcd_hs_ek79007",
	...
	.cmds_suspend = NULL,//{ARRAY_AND_SIZE(lcd_cmds_suspend)},
	.cmds_resume = NULL,//{ARRAY_AND_SIZE(lcd_cmds_resume)},
#ifdef CONFIG_FB_COMIP_ESD
	.esd_cmd = NULL,//{ARRAY_AND_SIZE(lcd_esd_cmd)},
	.esd_rsp = NULL,//{ARRAY_AND_SIZE(lcd_esd_rsp)},
#endif
	.reset = lcd_hs_ek79007_reset,
	.suspend = lcd_hs_ek79007_suspend,
	.resume = lcd_hs_ek79007_resume,
	.backlight_info = NULL,/*/{{ARRAY_AND_SIZE(backlight_cmds)},
		.brightness_bit = 5
	},*/
};

static int __init lcd_hs_ek79007_init(void)
{
	return comipfb_dev_register(&lcd_hs_ek79007_dev);
}

subsys_initcall(lcd_hs_ek79007_init);
