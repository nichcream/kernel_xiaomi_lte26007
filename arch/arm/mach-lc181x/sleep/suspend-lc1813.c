...
#define A7CX_PD_ST(x)	(4 + (((x)-1)*4)) // X=1,2,3
#define PWR_ON		0
#define PWR_DOWN	7

static cpumask_var_t suspend_cpus_mask;
...
extern unsigned long comip_cpu_resume_phys;
...
extern void comip1813_resume(void);
extern void comip1813_continue(void);
...
static inline int
a7cx_is_warm_boot(unsigned int cpu)
{
	unsigned int val;

	val = __raw_readl(io_p2v(AP_PWR_A7_BOOT_FLAG));
	return !!(val & (1 << cpu));
}
...
{
	__raw_writeb(state,
		io_p2v(CPUX_SLP_STATE_BASE + cpu));
	dsb();
}
...
{
	smp_rmb();
	return __raw_readb(io_p2v(CPUX_SLP_STATE_BASE + cpu));
}
...
static int __init sym_reloc_init(void)
{
#ifdef CONFIG_SLEEP_CODE_IN_RAM
	void *sram_base;
	void *ddr_base = (void*)comip1813_resume;
	unsigned int val;
#endif

	symbol_reloc(&comip_cpu_resume_phys,
			(void *)virt_to_phys(cpu_resume));
	symbol_reloc(&secondary_startup_addr,
			(void *)virt_to_phys(secondary_startup));
	...
	val = symbol_sram_pa((void*)comip1813_continue);
	...
	val = symbol_sram_pa((void*)comip1813_resume);
...
static __init int suspend_mem_init(void)
{
	if (!alloc_cpumask_var(&suspend_cpus_mask, GFP_KERNEL|__GFP_ZERO))
		pr_err("suspend_cpus_mask allocation failed\n");
...
static inline int interrupt_is_pending(void)
{
	unsigned int val;
...
static inline int int_has_triggered(void)
{
	unsigned val;

	val = __raw_readl(io_p2v(CTL_A7_INTR_STA));
	if ((val & 0xff) != 0xff)
		return 1;
...
{
	reg_ctrl_set(AP_PWR_SLPCTL0, AP_PWR_A7C0_SLP_EN, 1);
}

static inline void  ap_sleep_disable(void)
{
	reg_ctrl_set(AP_PWR_SLPCTL0, AP_PWR_A7C0_SLP_EN, 0);
}

static inline void  ap_pd_enable(void)
{
	reg_ctrl_set(AP_PWR_A7_PD_CTL, AP_PWR_A7_PD_MK, 0);
}

static inline void  ap_pd_disable(void)
{
	reg_ctrl_set(AP_PWR_A7_PD_CTL, AP_PWR_A7_PD_MK, 1);
}

static inline void  ap_int_wake_enable(void)
{
	reg_ctrl_set(AP_PWR_A7_PD_CTL, AP_PWR_A7_INTR_MK, 0);
}

static inline void  ap_int_wake_disable(void)
{
	reg_ctrl_set(AP_PWR_A7_PD_CTL, AP_PWR_A7_INTR_MK, 1);
}

static inline void 
pd_ctl_reg_set(unsigned int cpu, unsigned int bit, bool s1c0)
{
	reg_ctrl_set(AP_PWR_A7Cx_PD_CTL(cpu), bit, s1c0);
}
...
{
	pd_ctl_reg_set(cpu, INTR_MK1, 0);
	pd_ctl_reg_set(cpu, INTR_MK0, 0);
}

static inline void  a7cx_pu_by_irq_dis(unsigned int cpu )
{
	pd_ctl_reg_set(cpu, INTR_MK1, 1);
	pd_ctl_reg_set(cpu, INTR_MK0, 1);
...
a7cx_is_pd_pu(int cpu, int state, int timeout)
{
	...
	do {
		dsb();
		val = __raw_readl(io_p2v(AP_PWR_PDFSM_ST));
...
static int cpu_poweroff_check(int cpu, int timeout)
{
	unsigned int val = 0;
...
static void poweroff_unused_cpu(int cpu)
{
	if (a7cx_is_pd_pu(cpu, PWR_DOWN, 0))
		return;

	if (cpu_poweroff_check(cpu, 10)) /*wait for 100 us*/
		pr_err("cpu%d is not ready to be powered-off!\n", cpu);
...
static int notrace cpu_suspend_finisher(unsigned long arg)
{
	unsigned int cpu;
	unsigned int flag;
...
{
	unsigned int val;
	void __iomem * clk_addr = io_p2v(AP_PWR_A7_CLK_CTL);

	val = (freq << 0) | (1 << (0 + BIT_WE_OFF));
	__raw_writel(val, clk_addr);
	dsb();
	udelay(1);
}
...
static void wakeup_irq_save_restore(int s1r0)
{
	static unsigned int mk0_val;
	static unsigned int mk1_val;
	void __iomem * mk0_addr = io_p2v(AP_PWR_A7INTIN_MK0);
	void __iomem * mk1_addr = io_p2v(AP_PWR_A7INTIN_MK1);
	void __iomem * gic_en_addr = io_p2v(GIC_DIST_BASE);
	unsigned int val;

	gic_en_addr += GIC_DIST_ENABLE_SET;

	if (s1r0) {
		mk0_val = __raw_readl(mk0_addr);
		mk1_val = __raw_readl(mk1_addr);
		val = __raw_readl(gic_en_addr + 4);  /*bit 32~ bit 63*/
		__raw_writel(~val, mk0_addr);
		val = __raw_readl(gic_en_addr + 8);  /*bit 64~ bit 95*/
		__raw_writel(~val, mk1_addr);
	} else {
		__raw_writel(mk0_val, mk0_addr);
		__raw_writel(mk1_val, mk1_addr);
	}
}
...
int suspend_idle_poweroff(unsigned int cpu)
{
	...
	if (CPU_ID_0 == cpu) {
		ap_int_wake_enable();
		ap_pd_enable();
		wakeup_irq_save_restore(1);
		barrier();
		ret = power_down_cpu0();
		barrier();
		ap_int_wake_disable();
		ap_pd_disable();
		wakeup_irq_save_restore(0);
		if (0 == ret)
...
static void choose_module_irq_print(unsigned int irq)
{
	...
	if (irq == INT_GPIO) {
		for (i = 0; i < 4; i++) {
			...
			if (status) {
				printk("exit comip suspend by gpio%d\n",
						fls(status) + i * 32 - 1);
				break;
			}
		}
		...
		printk("exit comip suspend by modem 0x%08x|0x%08x\n",
				__raw_readl(io_p2v(CTL_CP2AP_INTR_STA_RAW)),
				__raw_readl(io_p2v(CTL_CP2AP_INTR_STA_RAW1)));
	}else{
		printk("exit comip suspend by irq %d\n", irq);
	}
}
...
{
#define BUFLEN	2
	int nmaskbits = BUFLEN * BITS_PER_LONG;
	unsigned int wakeirq, pending;
	int cur, i;
	unsigned int irq[BUFLEN];

	for(i = 0; i < BUFLEN; i++){
		pending = __raw_readl(io_p2v(GIC_DIST_BASE +
			GIC_DIST_PENDING_SET + (i + 1) * 0x4));
		wakeirq = __raw_readl(io_p2v(AP_PWR_A7INTIN_MK0 + i * 0x4));
		irq[i] = pending & (~wakeirq);
	}
...
static inline unsigned int device_idle_state(void)
{
	dsb();
...
static int comip_suspend_prepare_late(void)
{
	cpumask_complement(suspend_cpus_mask, cpu_online_mask);
	return 0;
}

static void comip_suspend_wake(void)
{
	int cpu;

	if (!cpumask_empty(suspend_cpus_mask)) {
		for_each_cpu(cpu, suspend_cpus_mask) {
			poweroff_unused_cpu(cpu);
		}
		cpumask_complement(suspend_cpus_mask, suspend_cpus_mask);
		WARN(!cpumask_equal(suspend_cpus_mask, cpu_online_mask),
			"cpus are abnormally power-off/on in suspend/resume\n");
		cpumask_clear(suspend_cpus_mask);
	}
}

static struct platform_suspend_ops comip_suspend_ops = {
	.valid	= suspend_valid_only_mem,
	.prepare_late	= comip_suspend_prepare_late,
	.enter	= comip_suspend_enter,
	.wake	= comip_suspend_wake,
};
...
void suspend_smp_prepare(void)
{
	...
	__raw_writel(0x100, io_p2v(AP_PWR_A7C_PD_CNT1));
	__raw_writel(0x100, io_p2v(AP_PWR_A7C_PD_CNT2));
	__raw_writel(0x100, io_p2v(AP_PWR_A7C_PD_CNT3));

	val = virt_to_phys(comip_secondary_startup);
	__raw_writel(val, io_p2v(CPU1_CBOOT_ADDR));
	__raw_writel(val, io_p2v(CPU2_CBOOT_ADDR));
	__raw_writel(val, io_p2v(CPU3_CBOOT_ADDR));
	cpu_w_addr_reset = val;

	val = virt_to_phys(comip1813_continue);
	a7cx_warm_addr_set(CPU_ID_0, val);
	a7cx_warm_addr_set(CPU_ID_1, val);
	a7cx_warm_addr_set(CPU_ID_2, val);
	a7cx_warm_addr_set(CPU_ID_3, val);
	...
	val = virt_to_phys(comip1813_resume);
	cpu_w_addr_resume = val;
...
static inline void suspend_regs_init(void)
{
	...
	reg = 0 << AP_PWR_A7C0_SLP_EN | 
		1 << AP_PWR_A7C0_SLP_EN_WE;
	reg |= 1 << 1 | 1 << (1 + BIT_WE_OFF); //A7C1_SLP_EN=1
	reg |= 1 << 2 | 1 << (2 + BIT_WE_OFF); //A7C2_SLP_EN=1
	reg |= 1 << 3 | 1 << (3 + BIT_WE_OFF); //A7C3_SLP_EN=1

	reg |= 0 << AP_PWR_A7C1_SLP_MK | 
		1 << AP_PWR_A7C1_SLP_MK_WE;
	reg |= 0 << AP_PWR_A7C2_SLP_MK | 
		1 << AP_PWR_A7C2_SLP_MK_WE;
	reg |= 0 << AP_PWR_A7C3_SLP_MK | 
		1 << AP_PWR_A7C3_SLP_MK_WE;
	...
	reg = 1 << AP_PWR_A7_WK_ACK_MK | 
		1 << AP_PWR_A7_WK_ACK_MK_WE;
	reg |= 0 << AP_PWR_L2C_A7PD_MK | 
		1 << AP_PWR_L2C_A7PD_MK_WE;
	reg |= 0 << AP_PWR_A7_PU_CDBGPWRUPREQ_MK |
		1 << AP_PWR_A7_PU_CDBGPWRUPREQ_MK_WE;
	reg |= 1 << AP_PWR_A7_INTR_MK |
		1 << AP_PWR_A7_INTR_MK_WE;
	__raw_writel(reg, io_p2v(AP_PWR_A7_PD_CTL));
	...
	reg = 0 << 4 | 1 << (4 + BIT_WE_OFF); /* A7PD2DBG_MK = 0 */
	__raw_writel(reg, io_p2v(AP_PWR_A7DBG_PD_CTL));

	reg = 1 << 8 | 1 << (8 + BIT_WE_OFF); /* PU_WITH_A7 = 1 */
	reg |= 0 << 3 | 1 << (3 + BIT_WE_OFF); /* PD_MK = 0 */
	__raw_writel(reg, io_p2v(AP_PWR_A7C1_PD_CTL));
	__raw_writel(reg, io_p2v(AP_PWR_A7C2_PD_CTL));
	__raw_writel(reg, io_p2v(AP_PWR_A7C3_PD_CTL));

	reg = __raw_readl(io_p2v(AP_PWR_LP_CTL));
	reg |= 1 << AP_PWR_L2C_MEM_LP_EN;  /*L2C_LP_MODE = 1 */
	__raw_writel(reg, io_p2v(AP_PWR_LP_CTL));

	reg = 0 << 0 | 0x0f << (0 + BIT_WE_OFF); /* L1RSTDISABLE = 0 */
	reg |= 0 << 4 | 1 << (1 + BIT_WE_OFF);  /* L2RSTDISABLE = 0 */
	__raw_writel(reg, io_p2v(AP_PWR_A7_RSTCTL1));
	dsb();
}
