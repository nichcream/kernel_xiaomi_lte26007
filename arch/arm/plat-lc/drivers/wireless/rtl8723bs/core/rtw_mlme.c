...
#ifdef CONFIG_P2P
	pmlmepriv->linked_p2p_lock_flag = 0;
#endif
#endif
...
#ifdef CONFIG_CONCURRENT_MODE
#ifdef CONFIG_P2P
	if (pmlmepriv->linked_p2p_lock_flag){
		pmlmepriv->linked_p2p_lock_flag = 0;
		rtw_unlock_suspend_p2p();
	}
#endif
#endif
#endif
...
static void rtw_reset_linkedinfo(_adapter *padapter)
{
	struct mlme_priv *pmlmepriv= &padapter->mlmepriv;

	padapter->xmitpriv.tx_pkts = 0;
	padapter->recvpriv.rx_pkts = 0;
	padapter->recvpriv.rx_bytes = 0;

	pmlmepriv->android_btcex_bdhcp = _FALSE;

	pmlmepriv->LinkDetectInfo.MaxRxBytesInPeriod = 0;
	pmlmepriv->LinkDetectInfo.LastRxBytesInPeriod = 0;
	...
	pmlmepriv->LinkDetectInfo.bTxBusyTraffic = _FALSE;
	pmlmepriv->LinkDetectInfo.bRxBusyTraffic = _FALSE;
	pmlmepriv->LinkDetectInfo.bHigherBusyTraffic = _FALSE;
	pmlmepriv->LinkDetectInfo.bHigherBusyRxTraffic = _FALSE;
	pmlmepriv->LinkDetectInfo.bHigherBusyTxTraffic = _FALSE;
}
...
_func_enter_;
...
#ifdef CONFIG_LPS
#if (!defined(CONFIG_SUSPEND_NEW_FLOW)) && defined(CONFIG_WOWLAN)
	if(adapter_to_pwrctl(padapter)->wowlan_mode==_FALSE)
#endif //CONFIG_WOWLAN
	...
	rtw_reset_linkedinfo(padapter);
_func_exit_;
}

inline void rtw_indicate_scan_done( _adapter *padapter, bool aborted)
{
	/* if suspend under scan, this function will called, we must clear _FW_UNDER_SURVEY */
	/* or _FW_UNDER_SURVEY will never cleared, because all thread and timer have stoped */
	_clr_fwstate_(&(padapter->mlmepriv), _FW_UNDER_SURVEY);
...
/* ap/p2p have locked in wifi open/close, here just care station mode */
/* lock when BT busy & wifi linked */
void rtw_dynamic_check_suspend_lock(_adapter *adapter)
{
	...
	if (adapter->pbuddy_adapter) {
		pbuddy_adapter = adapter->pbuddy_adapter;
#ifdef CONFIG_P2P
	...
	if(rtw_p2p_chk_role(pwdinfo, P2P_ROLE_CLIENT) ||rtw_p2p_chk_role(pwdinfo, P2P_ROLE_GO)
	 ||rtw_p2p_chk_role(pbuddy_wdinfo, P2P_ROLE_CLIENT) ||rtw_p2p_chk_role(pbuddy_wdinfo, P2P_ROLE_GO))
	{
		if(!adapter->mlmepriv.linked_p2p_lock_flag)
		{
			DBG_871X("%s lock p2p suspend\n", __func__);
			adapter->mlmepriv.linked_p2p_lock_flag = 1;
			rtw_lock_suspend_p2p();
		}
	}
	else
	{
		if(adapter->mlmepriv.linked_p2p_lock_flag)
		{
			DBG_871X("%s unlock p2p suspend\n", __func__);
			adapter->mlmepriv.linked_p2p_lock_flag = 0;
			rtw_unlock_suspend_p2p();
		}
	}
#endif
	...
	if (check_fwstate(&adapter->mlmepriv, _FW_LINKED) && should_lock) {
...
void rtw_dynamic_check_timer_handlder(_adapter *adapter)
{
	...
	rtw_dynamic_check_suspend_lock(adapter);
	...
	, struct wlan_network **candidate, struct wlan_network *competitor)
{
	...
	if(!pmlmepriv->to_roaming &&
		_rtw_memcmp(zero_addr, pmlmepriv->assoc_bssid, ETH_ALEN) == _FALSE) {
		if(_rtw_memcmp(competitor->network.MacAddress, pmlmepriv->assoc_bssid,
					ETH_ALEN) == _FALSE) {
			if (_rtw_memcmp(competitor->network.Ssid.Ssid,
					pmlmepriv->assoc_ssid.Ssid,
					pmlmepriv->assoc_ssid.SsidLength))
			DBG_871X("%s ssid:%s UI have set bssid \n", __func__,
...
_func_enter_;
	...
	if (check_fwstate(pmlmepriv, _FW_LINKED) == _TRUE)
	{
		...
		#endif
		{
			rtw_disassoc_cmd(adapter,_TRUE);
			rtw_indicate_disconnect(adapter, 1);
...
_func_enter_;
	...
	if( _TRUE  == pwrctrlpriv->bInternalAutoSuspend)
	{
		pwrctrlpriv->wepkeymask = pmlmepriv->key_mask;
		DBG_871X("....AutoSuspend pwrctrlpriv->wepkeymask(%x)\n",pwrctrlpriv->wepkeymask);
	}
#endif
