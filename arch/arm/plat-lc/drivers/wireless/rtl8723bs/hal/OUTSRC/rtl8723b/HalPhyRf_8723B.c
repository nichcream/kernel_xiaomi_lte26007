	...
	)
{
   ...
   //new element A = element D x X
	...
	if(IqkResult_X != 0)  //pBandType is not initialized for 8723B
	{
	...
	)
{
	if (Method == TXAGC)
	{
		...
		ODM_RT_TRACE(pDM_Odm, ODM_COMP_TX_PWR_TRACK, ODM_DBG_LOUD, ("ODM_TxPwrTrackSetPwr_8723B CH=%d\n", *(pDM_Odm->pChannel)));
		...
		if (Adapter->registrypriv.mp_mode == 1)
		{
			...
			ODM_RT_TRACE(pDM_Odm, ODM_COMP_TX_PWR_TRACK, ODM_DBG_LOUD, ("ODM_TxPwrTrackSetPwr_8723B: CCK Tx-rf(A) Power = 0x%x\n", TxAGC));
			...
			ODM_RT_TRACE(pDM_Odm, ODM_COMP_TX_PWR_TRACK, ODM_DBG_LOUD, ("ODM_TxPwrTrackSetPwr_8723B: OFDM Tx-rf(A) Power = 0x%x\n", TxAGC));
		}
		else
#endif
		{
	...
	{
		if (pDM_Odm->BbSwingIdxCck >= CCK_TABLE_SIZE)
			pDM_Odm->BbSwingIdxCck = CCK_TABLE_SIZE-1;
		else if (pDM_Odm->BbSwingIdxCck < 0)
			pDM_Odm->BbSwingIdxCck = 0;
		...
		//DBG_871X("%s(): RFPath=%d ChannelMappedIndex=%d\n", __func__, RFPath, ChannelMappedIndex);
		// Adjust BB swing by OFDM IQ matrix
		if (RFPath == ODM_RF_PATH_A)
		{
			setIqkMatrix_8723B(pDM_Odm, pDM_Odm->BbSwingIdxOfdm[ODM_RF_PATH_A], ODM_RF_PATH_A,
						 pDM_Odm->RFCalibrateInfo.IQKMatrixRegSetting[ChannelMappedIndex].Value[0][0],
						 pDM_Odm->RFCalibrateInfo.IQKMatrixRegSetting[ChannelMappedIndex].Value[0][1]);
		}
		else if (RFPath == ODM_RF_PATH_B)
		{
			setIqkMatrix_8723B(pDM_Odm, pDM_Odm->BbSwingIdxOfdm[ODM_RF_PATH_B], ODM_RF_PATH_B,
	...
	)
{
	...
	pConfig->PHY_LCCalibrate = _PHY_LCCalibrate_8723B;
	...
	)
{
	...
	//Workaround for chnl 10 frequency calibrating issue
	ODM_SetRFReg(pDM_Odm, ODM_RF_PATH_A, RF_CHNLBW, bMask12Bits, 1|0x08000);
	...
	)
{
	...
	#endif
#endif
	_PHY_LCCalibrate_8723B(pDM_Odm);
}
