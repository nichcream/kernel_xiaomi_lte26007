...
static int rtw_sdio_suspend(struct device *dev)
{
	...
	/* wait enqueued thread for timer complete */
	rtw_msleep_os(10);
	...
	if(check_fwstate(pmlmepriv, _FW_UNDER_SURVEY)){
		...
		//scan abot need scan timer & thread run
...
static int rtw_sdio_resume(struct device *dev)
{
	...
	{
#if defined (CONFIG_RESUME_IN_WORKQUEUE) &&!(defined(CONFIG_HAS_EARLYSUSPEND) ||defined(CONFIG_ANDROID_POWER))
...
#if (!defined CONFIG_PLATFORM_HUAWEI_K3V2) && (!defined CONFIG_PLATFORM_LEADCORE)
	rtw_android_wifictrl_func_add();
#endif
