...
static void comip_clk_pll_init(struct clk *clk)
{
	...
	if (!strcmp(clk->name, "pll0_out"))
		clk->rate = 988000000;
	else if(!strcmp(clk->name, "pll1_out"))
		clk->rate = 1248000000;
	else if(!strcmp(clk->name, "pll2_out"))
		clk->rate = 1020000000;
	printk("%s rate:%d\n", clk->name, clk->rate);
}
#else
static void comip_clk_pll_init(struct clk *clk)
{
	if (cpu_is_lc1813()) {
	...
	} else if (cpu_is_lc1813s()) {
		unsigned int pllctl0_val;
		unsigned int pllctl1_val;
		unsigned int dsmpd;
		unsigned int refdiv;
		unsigned int fbdiv;
		...
		parent_rate = (u64)(clk->parent->rate) & 0xffffffff;
		pllctl0_val = readl(clk->grclk_reg);
		pllctl1_val = readl(clk->grclk_reg + 0x4);
		dsmpd = (pllctl0_val >> 28) & 0x1;
		refdiv = pllctl0_val & 0x3f;
		fbdiv = (pllctl0_val >> 8) & 0xfff;
		postdiv1 = (pllctl0_val >> 20) & 0x7;
		postdiv2 = (pllctl0_val >> 24) & 0x7;
		frac = pllctl1_val & 0xffffff;

		if (dsmpd) {
			rate = parent_rate * fbdiv;
			do_div(rate, refdiv * postdiv1 * postdiv2);
			clk->rate = (unsigned long)rate;
		} else {
			rate = parent_rate * (fbdiv * (1 << 24) + frac);
			do_div(rate, refdiv * postdiv1 * postdiv2 * (1 << 24));
			clk->rate = (unsigned long)rate;
		}
	}
...
static void comip_clk_a7_init(struct clk *clk)
{
	...
	if (!gr)
		gr = 16;
	clk->divclk_val = !!(readl(clk->divclk_reg) & (1 << clk->divclk_bit));
	clk->grclk_val = gr;
	clk->rate = clk->parent->rate / (1 << clk->divclk_val) / 16 * gr;
	printk("A7 rate: %ld, div2: %d, gr:%d\n", clk->rate, clk->divclk_val, gr);
}
...
#if defined(CONFIG_CPU_FREQ_DIV2)
	div2 = ((clk->parent->rate / rate) >= 2) ? 1 : 0;
#endif

	gr = rate / (clk->parent->rate / (1 << div2) / 16);
	if (gr > 16)
		return -EINVAL;

	if (div2 > clk->divclk_val) {
		val = readl(clk->divclk_reg) | (1 << clk->divclk_bit);
		writel(val, clk->divclk_reg);
	}
...
static int comip_clk_lcd_set_rate(struct clk *clk, unsigned long rate)
{
	...
	if (find) {
		val = ((tbl->div - 1) << clk->divclk_bit) | (1 << clk->divclk_we_bit);
		writel(val, clk->divclk_reg);
		val = (tbl->gr << clk->grclk_bit) | (1 << clk->grclk_we_bit);
		writel(val, clk->divclk_reg);
		clk->rate = parent_rate * tbl->gr / (8 * tbl->div);
		clk->divclk_val = tbl->div - 1;
		clk->grclk_val = tbl->gr;
	} else {
