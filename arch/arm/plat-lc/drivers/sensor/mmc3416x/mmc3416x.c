...
#define MMC3416X_DEV_NAME	"mag_mmc3416x"
...
static struct platform_data_mmc3416x *mmc3416x_pdata = NULL;
...
#else
		rawdata[0] = ((mmc3416x_pdata->negate_x) ? (65536-vec[mmc3416x_pdata->axis_map_x])
		              : (vec[mmc3416x_pdata->axis_map_x]));
		rawdata[1] = ((mmc3416x_pdata->negate_y) ? (65536-vec[mmc3416x_pdata->axis_map_y])
		              : (vec[mmc3416x_pdata->axis_map_y]));
		rawdata[2] = ((mmc3416x_pdata->negate_z) ? (65536-vec[mmc3416x_pdata->axis_map_z])
		              : (vec[mmc3416x_pdata->axis_map_z]));
...
static DEVICE_ATTR(mag_mmc3416x, S_IRUGO, mmc3416x_show, NULL);
...
static int mmc3416x_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	unsigned char data[16] = {0};
	int res = 0;
	mmc3416x_pdata = client->dev.platform_data;
	...
	res = device_create_file(&client->dev, &dev_attr_mag_mmc3416x);
	if (res) {
...
{
	device_remove_file(&client->dev, &dev_attr_mag_mmc3416x);
	misc_deregister(&mmc3416x_device);
