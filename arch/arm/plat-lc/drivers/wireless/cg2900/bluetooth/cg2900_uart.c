...
#define HIGH_BAUD_RATE		CG29XX_HOST_UART_HIGH_BAUD_RATE
...
static irqreturn_t cts_interrupt(int irq, void *dev_id)
{
	...
	dev_dbg(MAIN_DEV,"++ %s, irq: %d\n",__func__,irq);
	
	while(try_thrice--) {	
		mdelay(5);
		cts_val = gpio_get_value(uart_info->cts_gpio);
		    if (cts_val) {
			dev_info(MAIN_DEV, "CTS is HIGH, return from ISR\n");
			return IRQ_HANDLED;
			}
	}
...
static void wake_up_chip(struct uart_info *uart_info)
{
	...
	if (CHIP_ASLEEP == uart_info->sleep_state ||
		...
		dev_dbg(MAIN_DEV, "enable flow control\n");
		/* Set FLOW on. */
...
static void set_chip_sleep_mode(struct work_struct *work)
{
	...
	struct uart_info *uart_info = (struct uart_info *)current_work->data;
	//test only +++
	struct uart_state *state = uart_info->hu->tty->driver_data;
	struct uart_port *port = state->uart_port;
	struct uart_comip_port *up = (struct uart_comip_port *)port;
	u32 reg_val = 0;
	static int error_test1 = 0;
	static int error_test2 = 0;
	static int error_test3 = 0;
	//test only ---
	unsigned long timeout_jiffies = get_sleep_timeout(uart_info);
	int err = 0;
	int chars_in_buffer;
	...
	case CHIP_FALLING_ASLEEP:
		...
		if (chars_in_buffer) {
			dev_dbg(MAIN_DEV,"state is CHIP_FALLING_ASLEEP\n");
			dev_err(MAIN_DEV,"chip_falling_asleep:sleep_timer_expired: "
					"tx not finished, stay awake and "
					"restart the sleep timer\n");
			//test only++++
			if(error_test1++ < 10){
	 			comip_dmas_dump(2);
	 			reg_val = readl(io_p2v(0xA0148000 + 0x14));
	 			printk("UART1_TSR = 0x%x \n", reg_val);
	 			printk("tasklet state=0x%lx\n",up->tklet.state);
	 			printk("tasklet count=%d\n",up->tklet.count.counter);
	 			printk("up->tx_stop=%d\n",up->tx_stop);
	 			printk("tty->stopped=%d,tty->hw_stopped=%d\n",uart_info->hu->tty->stopped,uart_info->hu->tty->hw_stopped);
				reg_val = readl(io_p2v(0xA0148000 + 0x18));
				printk("UART1_MSR = 0x%x \n", reg_val);
			}
			//test only ----
			...
			uart_info->sleep_state = CHIP_AWAKE;
			timeout_jiffies = get_sleep_timeout(uart_info);
			goto schedule_sleep_work;
		}


		if (!is_chip_flow_off(uart_info)) {
			dev_err(MAIN_DEV, "cts is lo Chip flow is on, it's not ready to"
				"sleep yet\n");
			if(error_test2++ < 10){
				printk("IN CHIP_FALLING_ASLEEP \n");
				comip_dmas_dump(2);
				reg_val = readl(io_p2v(0xA0148000 + 0x14));
				printk("UART1_TSR = 0x%x \n", reg_val);
				reg_val = readl(io_p2v(0xA0148000 + 0x18));
		...
		dev_dbg(MAIN_DEV,"CTS= %d, \n", gpio_get_value(uart_info->cts_gpio));
		/*
		 * Set baud zero.
		...
		if (err < 0) {
			...
			if (err == -ECANCELED)
				...
				dev_dbg(MAIN_DEV,"Can not set interrupt on "
						"CTS, err:%d\n", err);
				goto error;
			}
		}
	...
	case CHIP_AWAKE:
		chars_in_buffer = hci_uart_chars_in_buffer(uart_info->hu);
		if (chars_in_buffer) {
			dev_dbg(MAIN_DEV,"state is CHIP_AWAKE\n");
			dev_err(MAIN_DEV, "sleep_timer_expired: "
					"tx not finished, stay awake and "
					"restart the sleep timer\n");
			//test only ++++
			if(error_test3++ < 10){
				printk("IN CHIP_AWAKE \n");
				comip_dmas_dump(2);
				reg_val = readl(io_p2v(0xA0148000 + 0x14));
				printk("UART1_TSR = 0x%x \n", reg_val);
				printk("tasklet state=0x%lx\n",up->tklet.state);
		...
		if(comip_dmas_state(up->txdma) > 0){
				printk("%s:DMA is busy\n",__func__);
				comip_dmas_dump(up->txdma);
				reg_val = readl(io_p2v(0xA0148000 + 0x18));
				printk("UART1_MSR = 0x%x \n", reg_val);
				
			}
		if(readl(io_p2v(0xA0148000 + (31<<2)))){
			printk("UART is busy\n");
	 			comip_dmas_dump(up->txdma);
	 			reg_val = readl(io_p2v(0xA0148000 + 0x14));
	 			printk("UART1_TSR = 0x%x \n", reg_val);
...
static void finish_setting_baud_rate(struct hci_uart *hu)
{
	 ...
	 * the old termios.
	 */
	if (hci_uart_set_baudrate(hu, uart_info->baud_rate,0) < 0) {
...
static void uart_set_chip_power(struct cg2900_chip_dev *dev, bool chip_on)
{
	...
	if (chip_on) {
		...
		(void)hci_uart_set_baudrate(uart_info->hu, uart_baudrate,1);
		...
		if (uart_info->irq_enabled) {
#ifdef CONFIG_PM
			disable_irq_wake(uart_info->cts_irq);
#endif
			disable_irq_nosync(uart_info->cts_irq);
			uart_info->irq_enabled = false;
		}
					...
					void *data, int count)
{
	...
	u8 *tmp;
	struct skb_list *skb_head = NULL;
	struct skb_list *skb_tmp = NULL;
	//fix L1813_Bug00002266 by Chengpenghui,BEGIN,fix hci command credit issue
	int last_cc;
	//fix L1813_Bug00002266 by Chengpenghui,END,fix hci command credit issue
	...
	while (count) {
		...
		case W4_DATA:
			 ...
			 */
				 // Commmand Status or Complete
				if (uart_info->rx_skb->data[0] == HCI_BT_EVT_H4_CHANNEL	&& 
					(uart_info->rx_skb->data[1] == 0x0f || uart_info->rx_skb->data[1] ==0x0e)) {
						if (!skb_head) {
							skb_head = kmalloc(sizeof(*skb_head), GFP_ATOMIC);
							skb_tmp = skb_head;
							skb_tmp->skb_buf = uart_info->rx_skb;
						} else {
							skb_tmp->next = kmalloc(sizeof(*skb_head), GFP_ATOMIC);
							skb_tmp = skb_tmp->next;
							skb_tmp->skb_buf = uart_info->rx_skb;
						}
						skb_tmp->next = NULL;
						//fix L1813_Bug00002266 by Chengpenghui,BEGIN,fix hci command credit issue
						/* Either it is command status or complete event */
						if (uart_info->rx_skb->data[1] == 0x0f)
							last_cc = uart_info->rx_skb->data[4];
						else
							last_cc = uart_info->rx_skb->data[3];
						//fix L1813_Bug00002266 by Chengpenghui,END,fix hci command credit issue
						uart_info->rx_state = W4_PACKET_TYPE;
						uart_info->rx_skb = NULL;
						continue;
...
check_h4_header:
		...
		if (*r_ptr == HCI_BT_EVT_H4_CHANNEL) {
			...
			dev_err(MAIN_DEV, "Unknown HCI packet type 0x%X, dumping all the bytes received\n",
				(u8)*r_ptr);
			uart_info->rx_count = 0;
			count = 0;
			goto sleep_exit;
	...
	/* Now transmit all the BT command status and complete events
	   And make sure all the events have the latest command credit 
	   from the controller.
	*/

	//fix L1813_Bug00002185 by Chengpenghui,BEGIN,fix skb_tmp null pointer issue.
	while (skb_head) {
	//fix L1813_Bug00002266 by Chengpenghui,BEGIN,fix hci command credit issue
		if (skb_head->skb_buf->data[1] == 0x0f) {
			if (last_cc < skb_head->skb_buf->data[4])
			        skb_head->skb_buf->data[4] = last_cc;
		} else {
			if (last_cc < skb_head->skb_buf->data[3])
			        skb_head->skb_buf->data[3] = last_cc;
		}
	//fix L1813_Bug00002266 by Chengpenghui,END,fix  command credit issue
                send_skb_to_core(uart_info, skb_head->skb_buf);
                skb_tmp = skb_head->next;
                kfree(skb_head);
                skb_head = skb_tmp;
	}
	 //fix L1813_Bug00002185 by Chengpenghui,END,fix skb_tmp null pointer issue.
...
/**
 * cg2900_hu_write() - Callback from UART for sending more data.
 * @hu: Pointer to associated hci_uart structure.
 *
 * Returns:
 *   0 if there is no error.
 *   Errors from create_work_item.
 */
static int cg2900_hu_write_callback(struct hci_uart *hu)
{
	int err;
	struct uart_info *uart_info = dev_get_drvdata(hu->proto->dev);
	...
	if (err)
		dev_err(MAIN_DEV,
			"Failed to create work item (%d) "
			"cg2900_hu_write_callback\n",
			err);
...
static struct sk_buff *cg2900_hu_dequeue(struct hci_uart *hu)
{
	...
	if (skb && skb->data[0] == HCI_BT_CMD_H4_CHANNEL) {
			uart_info->chip_dev.c_cb.decrement_flow_cnt(&uart_info->chip_dev);
	}
	return skb;
}
...
static int /*__devinit*/ cg2900_uart_probe(struct platform_device *pdev)
{
	...
	struct cg2900_platform_data *pf_data = dev_get_platdata(&pdev->dev);
	...
	if (pf_data->gpios.cts_gpio == -1) {
		dev_err(&pdev->dev, "CTS GPIO does not exist\n");
		err = -EINVAL;
		goto error_handling_free;
	}
	uart_info->cts_gpio = pf_data->gpios.cts_gpio;
	...
	p->send_callback = &cg2900_hu_write_callback;
...
module_platform_driver(cg2900_uart_driver);
