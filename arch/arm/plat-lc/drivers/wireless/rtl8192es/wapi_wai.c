...
static void wapiInstallMCastKey(struct rtl8192cd_priv	*priv)
{
	...
	if (GET_ROOT(priv)->pmib->miscEntry.vap_enable)
	{
		...
		printk("%s,vap_enable=1\n",__FUNCTION__);
		set_gkey_to_cam = 0;
	}
#endif
	#if defined(CONFIG_RTL_HW_WAPI_SUPPORT)
	if (!SWCRYPTO && set_gkey_to_cam)
	{

		/*Delete Key before add*/
		retVal = CamDeleteOneEntry(priv, CAM_CONST_BCAST, priv->wapiMCastKeyId<<1, 0);
		if (retVal) {
			priv->pshare->CamEntryOccupied--;
			
			retVal = CamDeleteOneEntry(priv, CAM_CONST_BCAST, (priv->wapiMCastKeyId<<1)+1, 0);
			if (retVal) {
				priv->pshare->CamEntryOccupied--;
			}
		}
...
static int WapiRecvMulticastKeyNotification(wapiWaiHeader *waihdr, struct stat_info *pstat, int *status)
{
	...
	for (i = 0; i < WAPI_PN_LEN; i++) {
		pn_le[i] = waiMCastNoti->keyPN[WAPI_PN_LEN - 1 - i];
	}

	if (WapiComparePN(pn_le, priv->keyNotify)==WAPI_RETURN_FAILED) {
		WAI_DBGPRINT("[%s] Discard packet - Invalid keyNotifyID\n", __FUNCTION__);
		return WAPI_RETURN_FAILED;
	}
	memcpy(priv->keyNotify, pn_le, WAPI_PN_LEN);

	WapiSMS4ForMNKEncrypt(wapiInfo->wapiWaiKey.dataKey, 
		//priv->keyNotify, 
		waiMCastNoti->keyPN,
		&waiMCastNoti->keyData[1], waiMCastNoti->keyData[0], 
...
int SecIsWAIPacket(struct sk_buff *pskb, int *waiOffset)
{
	int		Offset_TypeWAI;
...
static int	WapiSendMulticastKeyResponse(struct rtl8192cd_priv *priv, struct stat_info *pstat)
{
	...
	*((uint16*)&pskb->data[ETH_ALEN<<1]) = __constant_htons(ETH_P_WAPI);
	...
	for (i = 0; i < WAPI_PN_LEN; i++) {
		pn_be[i] = priv->keyNotify[WAPI_PN_LEN - 1 - i];
	}
	memcpy(wai_mcast_rsp->keyPN, pn_be, WAPI_PN_LEN);
...
int	wapiSendMulticastKeyNotification(struct rtl8192cd_priv *priv, struct stat_info *pstat)
{
	...
	*((uint16*)&pskb->data[ETH_ALEN<<1]) = __constant_htons(ETH_P_WAPI);
	...
	for (i = 0; i < WAPI_PN_LEN; i++) {
		pn_be[i] = priv->txMCast[WAPI_PN_LEN - 1 - i];
	}
	memcpy(wai_mcast_noti->dataPN, pn_be, WAPI_PN_LEN);

	for (i = 0; i < WAPI_PN_LEN; i++) {
		pn_be[i] = priv->keyNotify[WAPI_PN_LEN - 1 - i];
	}
	memcpy(wai_mcast_noti->keyPN, pn_be, WAPI_PN_LEN);
