...
#ifdef CONFIG_USB_GADGET_WAKELOCK
static struct wake_lock usb_gadget_wakelock;
static struct wake_lock usb_gadget_timeout_wakelock;
static int usb_gadget_timeout_flag = 0;
#endif
...
#ifdef CONFIG_USB_GADGET_WAKELOCK
	if (wake_lock_active(&usb_gadget_wakelock)) {
		if(usb_gadget_timeout_flag)
		{
			usb_gadget_timeout_flag = 0;
			wake_lock_timeout(&usb_gadget_timeout_wakelock,5*HZ);
		}
		wake_unlock(&usb_gadget_wakelock);
		printk(KERN_INFO "usb wake unlock\n");
	}
	if (cdev->config) {
		wake_lock(&usb_gadget_wakelock);
		printk(KERN_INFO "usb wake lock\n");
	}
#endif
...
static DEVICE_ATTR(serial_port_num, S_IRUGO | S_IWUSR, serial_instances_show, serial_instances_store);
static struct device_attribute *serial_function_attributes[] = { &dev_attr_serial_port_num, NULL };
					...
					struct usb_composite_dev *cdev)
{
	...
	config->fsg.nluns = CONFIG_USB_GADGET_STORAGE_LUN_NUM;
...
bool android_have_functions(void)
{
	struct android_dev *dev = _android_dev;
	if(dev)
		return !(list_empty(&dev->enabled_functions));
	else
		return false;
}
...
static ssize_t
supported_show(struct device *pdev, struct device_attribute *attr, char *buf)
{
	struct android_usb_function **f = supported_functions;
	struct android_usb_function *func;
	char *buff = buf;

	while((func = *f++)){
		buff += sprintf(buff, "%s,", func->name);
	}
...
static DEVICE_ATTR(supported, S_IRUGO, supported_show, NULL);
...
#ifdef CONFIG_USB_GADGET_WAKELOCK
	wake_lock_init(&usb_gadget_wakelock, WAKE_LOCK_SUSPEND, "usb_gadget_wakelock");
	wake_lock_init(&usb_gadget_timeout_wakelock, WAKE_LOCK_SUSPEND, "usb_gadget_timeout_wakelock");
#endif
...
static void __exit cleanup(void)
{
	...
	_android_dev = NULL;
#ifdef CONFIG_USB_GADGET_WAKELOCK
	wake_lock_destroy(&usb_gadget_timeout_wakelock);
#endif
}
module_exit(cleanup);
