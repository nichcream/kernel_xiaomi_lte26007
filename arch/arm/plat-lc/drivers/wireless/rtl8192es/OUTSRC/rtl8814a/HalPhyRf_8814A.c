	...
	)
{
	...
	if(Method == MIX_MODE)	//normal Tx power tracking
	{
		...
		Final_RF_Index = pDM_Odm->Absolute_OFDMSwingIdx[RFPath] >> 1;		//TxBB 1 step equal 1dB, BB swing 1step equal 0.5dB
	...
	if(Final_RF_Index > UpperBound)		//Upper bound = 10dB, if more htan upper bound, then move to bb swing max = +2dB
	{
		...
		Final_OFDM_Swing_Index = pDM_Odm->DefaultOfdmIndex + (pDM_Odm->Absolute_OFDMSwingIdx[RFPath] - (UpperBound << 1));
	...
	else if(Final_RF_Index < LowerBound)	// lower bound = -5dB
	{
		...
		Final_OFDM_Swing_Index = pDM_Odm->DefaultOfdmIndex - ((LowerBound<<1) - pDM_Odm->Absolute_OFDMSwingIdx[RFPath]);
		...
		)
{
	...
	{
		phy_LCCalibrate_8814A(pAdapter, TRUE);
	}
	else
#endif		
	{
		// For 88C 1T1R
#if !(DM_ODM_SUPPORT_TYPE & ODM_AP)
		phy_LCCalibrate_8814A(pAdapter, FALSE);
#else
		phy_LCCalibrate_8814A(pDM_Odm, FALSE);
#endif		
	}
