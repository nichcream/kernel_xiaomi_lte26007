...
static void add_RATid(_adapter *padapter, struct sta_info *psta)
{
...
#endif
	psta->tx_ra_bitmap = tx_ra_bitmap;
	...
	raid = rtw_hal_get_arfrid(padapter, raid);
	...
	if (psta->aid < NUM_STA)
	{
		...
		if (shortGIrate==_TRUE)
			arg |= BIT(5);
#ifdef CONFIG_SOFTAP_11N_40MHZ_SUPPORT
		if(psta->htpriv.bwmode == 0)
			arg |= BIT(6);
#endif
...
int rtw_check_beacon_data(_adapter *padapter, u8 *pbuf,  int len)
{
	...
	if(p && ie_len>0)
	{
		...
		p[2] =0x2C;
#ifdef CONFIG_SOFTAP_11N_40MHZ_SUPPORT
		p[2] = 0x6E;   //Short-GI for 20 and 40 MHz
#endif
...
u8 ap_free_sta(_adapter *padapter, struct sta_info *psta, u8 send_deauth_flag)
{
	_irqL irqL;
	u8 beacon_updated = _FALSE;
	struct mlme_priv *pmlmepriv = &(padapter->mlmepriv);
	...
	/*if caller is OnDeAuth or OnDisassoc, not send deauth, otherwise STA's connection will fail*/
	if(_TRUE == send_deauth_flag)
		issue_deauth(padapter, psta->hwaddr, WLAN_REASON_DEAUTH_LEAVING);
...
int rtw_sta_flush(_adapter *padapter)
{
	...
	while ((rtw_end_of_queue_search(phead, plist)) == _FALSE)
	{
		...
		//_exit_critical_bh(&pstapriv->asoc_list_lock, &irqL);
		ap_free_sta(padapter, psta, _TRUE);
		//_enter_critical_bh(&pstapriv->asoc_list_lock, &irqL);
	}
	_exit_critical_bh(&pstapriv->asoc_list_lock, &irqL);
	...
	else if(check_buddy_fwstate(padapter, _FW_LINKED)==_TRUE)//only second adapter can enter AP Mode
	{
		...
		if(cur_bwmode == HT_CHANNEL_WIDTH_40)
		{
			...
			else if(pbuddy_mlmeext->cur_bwmode == HT_CHANNEL_WIDTH_20)
			{
				...
				if(cur_channel>=1 && cur_channel<=4)
				{
					if(pht_info)
						pht_info->infos[0] |= HT_INFO_HT_PARAM_SECONDARY_CHNL_ABOVE;
				...
				else if(cur_channel>=5 && cur_channel<=14)
				{
					if(pht_info)
						pht_info->infos[0] |= HT_INFO_HT_PARAM_SECONDARY_CHNL_BELOW;
