...
#include <plat/bq24296_charger.h>
#include <plat/comip-battery.h>

#define BQ24296_POLL_CHARGING
//#define BQ24296_DEBUG


struct bq24296_device_info {
	struct device        *dev;
	struct i2c_client    *client;
	struct delayed_work   bq24296_charger_work;
...
static struct bq24296_device_info  *bq;

static DEFINE_MUTEX(bq24296_i2c_mutex);


static int bq24296_write_block(struct bq24296_device_info *di, u8 *value,
...
static int bq24296_read_block(struct bq24296_device_info *di, u8 *value,
                              u8 reg, unsigned num_bytes)
{
...
static int bq24296_write_byte(struct bq24296_device_info *di, u8 value, u8 reg)
{
	...
	temp_buffer[1] = value;
	return bq24296_write_block(di, temp_buffer, reg, 1);
}

static int bq24296_read_byte(struct bq24296_device_info *di, u8 *value, u8 reg)
{
	return bq24296_read_block(di, value, reg, 1);
}

static void bq24296_config_input_source_reg(struct bq24296_device_info *di)
{
	...
	di->input_source_reg00 = (di->hz_mode << BQ24296_EN_HIZ_SHIFT)
	                         | (Vdpm << BQ24296_VINDPM_SHIFT) |Iin_limit;

	bq24296_write_byte(di, di->input_source_reg00, INPUT_SOURCE_REG00);
	return;
}

static void bq24296_config_power_on_reg(struct bq24296_device_info *di)
{
	...
	di->power_on_config_reg01 = WATCHDOG_TIMER_RST
	                            | (di->chrg_config << BQ24296_EN_CHARGER_SHIFT)
	                            | (Sysmin << BQ24296_SYS_MIN_SHIFT) | di->boost_lim;

	bq24296_write_byte(di, di->power_on_config_reg01, POWER_ON_CONFIG_REG01);
	return;
}

static void bq24296_config_current_reg(struct bq24296_device_info *di)
{
	...
	di->charge_current_reg02 = (Vichrg << BQ24296_ICHG_SHIFT) | di->enable_low_chg;

	bq24296_write_byte(di, di->charge_current_reg02, CHARGE_CURRENT_REG02);
	return;
}

static void bq24296_config_prechg_term_current_reg(struct bq24296_device_info *di)
{
	...
	di->prechrg_term_current_reg03 = (Prechg <<  BQ24296_IPRECHRG_SHIFT| Viterm);
	bq24296_write_byte(di, di->prechrg_term_current_reg03, PRECHARGE_TERM_CURRENT_REG03);
	return;
}

static void bq24296_config_voltage_reg(struct bq24296_device_info *di)
{
	...
	di->charge_voltage_reg04 = (Voreg << BQ24296_VCHARGE_SHIFT) | BATLOWV_3000 |VRECHG_100;
	bq24296_write_byte(di, di->charge_voltage_reg04, CHARGE_VOLTAGE_REG04);
	return;
}

static void bq24296_config_term_timer_reg(struct bq24296_device_info *di)
{
	di->term_timer_reg05 = (di->enable_iterm << BQ24296_EN_TERM_SHIFT)
	                       | di->watchdog_timer | (di->enable_timer << BQ24296_EN_TIMER_SHIFT)
	                       | di->chrg_timer;// | JEITA_ISET;

	bq24296_write_byte(di, di->term_timer_reg05, CHARGE_TERM_TIMER_REG05);
	return;
}

static void bq24296_config_thernal_regulation_reg(struct bq24296_device_info *di)
{
	//di->thermal_regulation_reg06 =0x7F;
	//bq24296_write_byte(di, di->thermal_regulation_reg06, THERMAL_REGUALTION_REG06);

	return;

}

static void bq24296_config_misc_operation_reg(struct bq24296_device_info *di)
{
	di->misc_operation_reg07 = (di->enable_dpdm << BQ24296_DPDM_EN_SHIFT)
	                           | TMR2X_EN |(di->enable_batfet<< BQ24296_BATFET_EN_SHIFT)
	                           | CHRG_FAULT_INT_EN |BAT_FAULT_INT_EN;

	bq24296_write_byte(di, di->misc_operation_reg07, MISC_OPERATION_REG07);
	return;
}

static void bq24296_config_reg_param(struct bq24296_device_info *di)
{
	...
	bq24296_config_power_on_reg(di);
	msleep(500);
	bq24296_config_input_source_reg(di);
	bq24296_config_current_reg(di);
	bq24296_config_prechg_term_current_reg(di);
	bq24296_config_voltage_reg(di);
	bq24296_config_term_timer_reg(di);
	bq24296_config_thernal_regulation_reg(di);
	bq24296_config_misc_operation_reg(di);
	return;
}

static int external_charger_ce(int onoff)
{
	int gpio_external_charger_en =165;
	gpio_request(gpio_external_charger_en, "external_charger");

	if (onoff) {
		gpio_direction_output(gpio_external_charger_en, 1);
	} else {
		gpio_direction_output(gpio_external_charger_en, 0);
	}
	gpio_free(gpio_external_charger_en);
	return 0;
}

static int external_charger_otg(int onoff)
{
	int gpio_external_charger_otg = 148;
	gpio_request(gpio_external_charger_otg, "external_charger");

	if (onoff) {
		gpio_direction_output(gpio_external_charger_otg, 1);
	} else {
		gpio_direction_output(gpio_external_charger_otg, 0);
	}
	gpio_free(gpio_external_charger_otg);
	return 0;
}

static int external_charger_psel(int onoff)
{
	int gpio_external_charger_psel =145;
	gpio_request(gpio_external_charger_psel, "external_charger");

	if (onoff) {
		gpio_direction_output(gpio_external_charger_psel, 1);
	} else {
		gpio_direction_output(gpio_external_charger_psel, 0);
	}
	gpio_free(gpio_external_charger_psel);
	return 0;
}


static void bq24296_start_usb_charging(struct bq24296_device_info *di)
{
...
#ifdef BQ24296_POLL_CHARGING
	schedule_delayed_work(&di->bq24296_charger_work, msecs_to_jiffies(0));
#endif
...
static void bq24296_start_ac_charging(struct bq24296_device_info *di)
{
...
static void bq24296_stop_charging(struct bq24296_device_info *di)
{
	...
	schedule_delayed_work(&di->bq24296_charger_work, msecs_to_jiffies(0));
#endif
	dev_info(di->dev,"bq24296_stop_charging!\n");
	//wake_unlock(&di->chrg_lock);
	return;
}

static void bq24296_start_otg(struct bq24296_device_info *di)
{
	di->charger_source = POWER_SUPPLY_TYPE_BATTERY;
	di->charge_status  = POWER_SUPPLY_STATUS_DISCHARGING;
	di->charger_health = POWER_SUPPLY_HEALTH_UNKNOWN;
...
#endif
	//wake_lock(&di->chrg_lock);
	dev_info(di->dev,"bq24296_start_otg!\n");
	return;
}

static void bq24296_config_status_reg(struct bq24296_device_info *di)
{
...
static void bq24296_charger_update_status(struct bq24296_device_info *di)
{
	int i = 0;
	u8 read_reg[11] = {0};
	...
	di->timer_fault = 0;
	for(i = 0; i< 11; i++)
		bq24296_read_byte(di, &read_reg[i], i);

	if(di->boost_mode == CHARGE_MODE) { /*charge process mode*/
		if ((read_reg[8] & BQ24296_CHGR_STAT_CHAEGE_DONE) == BQ24296_CHGR_STAT_CHAEGE_DONE) {
			dev_info(di->dev, "CHARGE DONE\n");
		}

		if ((read_reg[8] & BQ24296_PG_STAT) == BQ24296_NOT_PG_STAT) {
			di->cfg_params = 1;
			dev_info(di->dev, "not power good\n");
		}

		if ((read_reg[9] & BQ24296_POWER_SUPPLY_OVP) == BQ24296_POWER_SUPPLY_OVP) {
			dev_err(di->dev, "POWER_SUPPLY_OVERVOLTAGE = %x\n", read_reg[9]);
		}

		if ((read_reg[9] & BQ24296_WATCHDOG_FAULT) == BQ24296_WATCHDOG_FAULT)
			di->timer_fault = 1;
		...
		if ((read_reg[9] & BQ24296_BAT_FAULT_OVP) == BQ24296_BAT_FAULT_OVP) {
			di->hz_mode = EN_HIZ;
			bq24296_config_input_source_reg(di);
			bq24296_write_byte(di, di->charge_voltage_reg04, CHARGE_VOLTAGE_REG04);
		...
		if(read_reg[9] & BQ24296_OTG_FAULT) {
			dev_err(di->dev, "VBUS overloaded in OTG, or VBUS OVP = %x\n", read_reg[9]);
		}
		bq24296_config_power_on_reg(di);
	}
	/* reset 30 second timer */
	bq24296_config_status_reg(di);
	return;
}
/*===========================================================================
  Function:       bq24296_charger_work
  ...
  Others:         none
===========================================================================*/
static void bq24296_charger_work(struct work_struct *work)
{
	struct bq24296_device_info *di = container_of(work,
	                                 struct bq24296_device_info, bq24296_charger_work.work);

	bq24296_charger_update_status(di);

#ifdef BQ24296_POLL_CHARGING
	schedule_delayed_work(&di->bq24296_charger_work, msecs_to_jiffies(BQ24296_WATCHDOG_TIMEOUT));
#endif
	return;
}

static irqreturn_t bq24296_charger_irq(int irq, void *_di)
{
	struct bq24296_device_info *di = _di;

	schedule_delayed_work(&di->bq24296_charger_work, 0);

	return IRQ_HANDLED;
}

static int bq24296_init_irq(struct bq24296_device_info *di)
{
	int ret = 0;
	...
	if (gpio_request(di->client->irq, "bq24296_irq") < 0) {
		ret = -ENOMEM;
		goto irq_fail;
	}
	...
	ret = request_irq(gpio_to_irq(di->client->irq), bq24296_charger_irq,
	                  IRQF_TRIGGER_FALLING |IRQF_TRIGGER_RISING| IRQF_SHARED,"bq24296_irq",di);
	if(ret) {
...
*/
static ssize_t bq24296_set_enable_hz_mode(struct device *dev,
                struct device_attribute *attr,
                const char *buf, size_t count)
{
	long val;
	int status = count;
	struct bq24296_device_info *di = dev_get_drvdata(dev);
...
static ssize_t bq24296_show_enable_hz_mode(struct device *dev,
                struct device_attribute *attr,
                char *buf)
{
	unsigned long val;
...
static ssize_t bq24296_set_dppm_voltage(struct device *dev,
                                        struct device_attribute *attr,
                                        const char *buf, size_t count)
{
	long val;
...
static ssize_t bq24296_show_dppm_voltage(struct device *dev,
                struct device_attribute *attr,
                char *buf)
{
	unsigned long val;
...
static ssize_t bq24296_set_cin_limit(struct device *dev,
                                     struct device_attribute *attr,
                                     const char *buf, size_t count)
{
	long val;
...
static ssize_t bq24296_show_cin_limit(struct device *dev,
                                      struct device_attribute *attr,
                                      char *buf)
{
	unsigned long val;
...
static ssize_t bq24296_set_regulation_voltage(struct device *dev,
                struct device_attribute *attr,
                const char *buf, size_t count)
{
	long val;
...
static ssize_t bq24296_show_regulation_voltage(struct device *dev,
                struct device_attribute *attr,
                char *buf)
{
	unsigned long val;
...
static ssize_t bq24296_set_charge_current(struct device *dev,
                struct device_attribute *attr,
                const char *buf, size_t count)
{
	long val;
...
static ssize_t bq24296_show_charge_current(struct device *dev,
                struct device_attribute *attr,
                char *buf)
{
	unsigned long val;
...
static ssize_t bq24296_set_precharge_current(struct device *dev,
                struct device_attribute *attr,
                const char *buf, size_t count)
{
	long val;
...
static ssize_t bq24296_show_precharge_current(struct device *dev,
                struct device_attribute *attr,
                char *buf)
{
	unsigned long val;
...
static ssize_t bq24296_set_termination_current(struct device *dev,
                struct device_attribute *attr,
                const char *buf, size_t count)
{
	long val;
...
static ssize_t bq24296_show_termination_current(struct device *dev,
                struct device_attribute *attr,
                char *buf)
{
	unsigned long val;
...
static ssize_t bq24296_set_enable_itermination(struct device *dev,
                struct device_attribute *attr,
                const char *buf, size_t count)
{
	long val;
...
static ssize_t bq24296_show_enable_itermination(struct device *dev,
                struct device_attribute *attr,
                char *buf)
{
	unsigned long val;
...
static ssize_t bq24296_set_enable_charger(struct device *dev,
                struct device_attribute *attr,
                const char *buf, size_t count)
{
	long val;
...
static ssize_t bq24296_show_enable_charger(struct device *dev,
                struct device_attribute *attr,
                char *buf)
{
	unsigned long val;
...
static ssize_t bq24296_set_enable_batfet(struct device *dev,
                struct device_attribute *attr,
                const char *buf, size_t count)
{
	long val;
...
static ssize_t bq24296_show_enable_batfet(struct device *dev,
                struct device_attribute *attr,
                char *buf)
{
	unsigned long val;
...
*
*/
static ssize_t bq24296_set_enable_cd(struct device *dev,
                                     struct device_attribute *attr,
                                     const char *buf, size_t count)
{
	long val;
...
static ssize_t bq24296_show_enable_cd(struct device *dev,
                                      struct device_attribute *attr,
                                      char *buf)
{
	unsigned long val;
...
static ssize_t bq24296_set_charging(struct device *dev,
                                    struct device_attribute *attr,
                                    const char *buf, size_t count)
{
	int status = count;
...
static ssize_t bq24296_show_chargelog(struct device *dev,
                                      struct device_attribute *attr,
                                      char *buf)
{
	int i = 0;
...
static DEVICE_ATTR(enable_hz_mode, S_IWUSR | S_IRUGO,
                   bq24296_show_enable_hz_mode,
                   bq24296_set_enable_hz_mode);
static DEVICE_ATTR(dppm_voltage, S_IWUSR | S_IRUGO,
                   bq24296_show_dppm_voltage,
                   bq24296_set_dppm_voltage);
static DEVICE_ATTR(cin_limit, S_IWUSR | S_IRUGO,
                   bq24296_show_cin_limit,
                   bq24296_set_cin_limit);
static DEVICE_ATTR(regulation_voltage, S_IWUSR | S_IRUGO,
                   bq24296_show_regulation_voltage,
                   bq24296_set_regulation_voltage);
static DEVICE_ATTR(charge_current, S_IWUSR | S_IRUGO,
                   bq24296_show_charge_current,
                   bq24296_set_charge_current);
static DEVICE_ATTR(termination_current, S_IWUSR | S_IRUGO,
                   bq24296_show_termination_current,
                   bq24296_set_termination_current);
static DEVICE_ATTR(precharge_current, S_IWUSR | S_IRUGO,
                   bq24296_show_precharge_current,
                   bq24296_set_precharge_current);
static DEVICE_ATTR(enable_itermination, S_IWUSR | S_IRUGO,
                   bq24296_show_enable_itermination,
                   bq24296_set_enable_itermination);

static DEVICE_ATTR(enable_charger, S_IWUSR | S_IRUGO,
                   bq24296_show_enable_charger,
                   bq24296_set_enable_charger);
static DEVICE_ATTR(enable_batfet, S_IWUSR | S_IRUGO,
                   bq24296_show_enable_batfet,
                   bq24296_set_enable_batfet);
static DEVICE_ATTR(enable_cd, S_IWUSR | S_IRUGO,
                   bq24296_show_enable_cd,
                   bq24296_set_enable_cd);
static DEVICE_ATTR(charging, S_IWUSR | S_IRUGO,
                   NULL,
                   bq24296_set_charging);
static DEVICE_ATTR(chargelog, S_IWUSR | S_IRUGO,
                   bq24296_show_chargelog,
                   NULL);

static struct attribute *bq24296_attributes[] = {
...
static const struct attribute_group bq24296_attr_group = {
	.attrs = bq24296_attributes,
};


static int bq24296_set_charging_enable(bool enable)
{
	struct bq24296_device_info *di = bq;

	if(!di) {
		pr_err("%s: bq24296 not init\n", __func__);
		return -EINVAL;
	}

#if defined(BQ24296_DEBUG)
	return 0;
#endif
...
static int bq24296_set_battery_exist(bool battery_exist)
{
	struct bq24296_device_info *di = bq;
...
static int bq24296_set_charging_source(int charging_source)
{
	struct bq24296_device_info *di = bq;
...
static int bq24296_set_charging_temp_stage(int temp_status,int volt,bool enable)
{
	struct bq24296_device_info *di = bq;
	u8 read_reg[11] = {0};
	...
	bq24296_read_byte(di, &read_reg[8], 8);
	bq24296_read_byte(di, &read_reg[9], 9);
	if((!di->chrg_config)||(!di->bat_present)) {
		di->charge_status = POWER_SUPPLY_STATUS_NOT_CHARGING;
	} else {
		di->charge_status = POWER_SUPPLY_STATUS_CHARGING;
	}
...
static int bq24296_get_charging_status(void)
{
	struct bq24296_device_info *di = bq;
...
static int bq24296_get_charger_health(void)
{
	struct bq24296_device_info *di = bq;
...
struct extern_charger_chg_config extern_charger_chg = {
	.name = "bq24296",
	.set_charging_source = bq24296_set_charging_source,
	.set_charging_enable = bq24296_set_charging_enable,
	//.set_battery_exist = bq24296_set_battery_exist,
	.set_charging_temp_stage = bq24296_set_charging_temp_stage,
	.get_charging_status = bq24296_get_charging_status,
	.get_charger_health = bq24296_get_charger_health,
};
                                 ...
                                 const struct i2c_device_id *id)
{
	...
	ret = bq24296_read_byte(di, &read_reg, PART_REVISION_REG0A);
	...
	if (((read_reg & 0x38) == BQ24296 || (read_reg & 0x38) == BQ24192)
	    && (client->addr == 0x6B))
	...
	external_charger_ce(0);
	external_charger_psel(1);
	external_charger_otg(1);
	...
	INIT_DELAYED_WORK(&di->bq24296_charger_work,bq24296_charger_work);

	ret = bq24296_init_irq(di);
	if(ret) {
		dev_err(&client->dev, "irq init failed\n");
		goto irq_fail;
	}

	ret = sysfs_create_group(&client->dev.kobj, &bq24296_attr_group);
	if (ret) {
		dev_err(&client->dev, "could not create sysfs files\n");
	}
	dev_info(&client->dev, "bq24296 Charger Driver Probe\n");
...
static int bq24296_charger_remove(struct i2c_client *client)
{
	...
	wake_lock_destroy(&di->chrg_lock);
	sysfs_remove_group(&client->dev.kobj, &bq24296_attr_group);
	cancel_delayed_work_sync(&di->bq24296_charger_work);
...
#ifdef CONFIG_PM
static int bq24296_charger_suspend(struct device *dev)
{
	struct platform_device *pdev = to_platform_device(dev);
	struct bq24296_device_info *di = platform_get_drvdata(pdev);
...
#else
#define bq24296_charger_suspend       NULL
#define bq24296_charger_resume        NULL
#endif /* CONFIG_PM */
