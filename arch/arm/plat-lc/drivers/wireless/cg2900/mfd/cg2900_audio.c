...
#include "../include/cg2900_audio.h"
				   ...
				   unsigned int *stream_handle)
{
	...
	printk("[BINGGE]conn_start_i2s_to_fm_rx\n");
...
//jogal 20121108
 /**
 * config_fm_rx_audio_dac() - Configures the Audio DAC for FM Rx.
 * @audio_user:		Audio user to check for.
 *
 * This function Configures the Audio DAC for FM Rx.
 *
 * Returns:
 *   0 if there is no error.
 *   -ECOMM if no response was received.
 ...
 *   -EIO for other errors.
 */
static int config_fm_rx_audio_dac(struct audio_user *audio_user)
{
	int err = 0;
	struct audio_info *info = audio_user->info;
	...
	/*
	 * Enable the Audio DAC by sending HCI_Write command with
	 * AUP_SetAudioDAC. Since their is no involvement of CG29XX
	 * Multimedia controller for this end-point, stream control(start/stop)
	 * is not required for this end-point.
	 */
	dev_dbg(FM_DEV, "FM: AUP_SetAudioDAC\n");
	err = send_fm_write_1_param(audio_user,
		CG2900_FM_CMD_ID_AUP_SET_AUDIO_DAC,
		CG2900_FM_CMD_AUP_SET_AUDIO_DAC_ON);
	if (err)
		dev_err(FM_DEV, "failed to enable Audio DAC\n");
...
//jogal 20121108
	case ENDPOINT_ANALOG_OUT:
		/* Analog FM Audio is suported only on CG2900 */
		if (info->revision == CG2900_CHIP_REV_PG1 ||
			info->revision == CG2900_CHIP_REV_PG2)
			return config_fm_rx_audio_dac(audio_user);
		else {
			dev_err(BT_DEV, "Feature not suported");
			return -EACCES;
		}
...
//	case ENDPOINT_ANALOG_OUT://jogal 20121108 its giving error if commented out
			      ...
			      unsigned int *stream_handle)
{
	...
	pr_debug("cg2900_audio_start_stream session %d ep_1 %d ep_2 %d",
		 session, ep_1, ep_2);
	printk("[BINGGE]cg2900_audio_start_stream session %d ep_1 %d ep_2 %d",
		 session, ep_1, ep_2);
			       ...
			       size_t count, loff_t *f_pos)
{
	...
	printk("[BINGGE]audio_dev_write count %d", count);
	...
	case CG2900_OPCODE_START_STREAM:
		...
		dev_dbg(BT_DEV, "CG2900_OPCODE_START_STREAM ep_1 %d ep_2 %d\n",
			ep_1, ep_2);
		printk( "[BINGGE]CG2900_OPCODE_START_STREAM ep_1 %d ep_2 %d\n",
			ep_1, ep_2);
