...
static void Init_ODM_ComInfo_8723b(PADAPTER	Adapter)
{
	...
	if(IS_8723B_A_CUT(pHalData->VersionID))
	{
		fab_ver = ODM_TSMC;
		cut_ver = ODM_CUT_A;
	}
...
#if 0
#ifdef CONFIG_WOWLAN
	if(Adapter->pwrctrlpriv.wowlan_mode != _TRUE)
#endif
		BT_CoexistMechanism(Adapter); //Concider more later.
#endif
	//ODM
	if (Adapter->hw_init_completed == _TRUE)
	{
		...
		ODM_CmnInfoUpdate(&pHalData->odmpriv ,ODM_CMNINFO_BT_LIMITED_DIG, BT_IsLimitedDig(Adapter));
