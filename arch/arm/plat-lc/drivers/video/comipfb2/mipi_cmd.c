...
static int mipi_dsih_hal_gen_packet_payload(struct comipfb_info *fbi, uint32_t payload)
{
	if (mipi_dsih_hal_gen_write_fifo_full(fbi)) {
		dev_err(fbi->dev, "Generic write fifo is full\n");
		return -EINVAL;
	}
...
static unsigned short mipi_dsih_gen_rd_packet(struct comipfb_info *fbi, uint8_t vc, uint8_t data_type, uint8_t msb_byte, uint8_t lsb_byte, uint8_t bytes_to_read, uint8_t* read_buffer)
{
	...
	if (!(timeout < DSIH_FIFO_ACTIVE_WAIT)) {
		printk(KERN_WARNING "mipi read, tx timeout");
	...
	for (timeout = 0; timeout < DSIH_FIFO_ACTIVE_WAIT; timeout++) {
		...
		if (!mipi_dsih_hal_gen_rd_cmd_busy(fbi)) {
			if (!mipi_dsih_hal_gen_read_fifo_empty(fbi)) {
				for (counter = 0; (!mipi_dsih_hal_gen_read_fifo_empty(fbi)) && (counter < MAX_RD_COUNT); counter += 4) {
	...
	printk(KERN_WARNING "rx command timed out\n");
	if ((!mipi->commandmode_info.ack_rqst_en) && (!mipi->commandmode_info.ack_rqst_en) 
			&& (!mipi->videomode_info.frame_bta_ack))
		mipi_dsih_hal_bta_en(fbi, 0);
...
int mipi_dsih_gen_wr_packet(struct comipfb_info *fbi, uint8_t vc, uint8_t data_type, uint8_t* params, uint16_t param_length)
{
	...
	if ((params == 0) && (param_length != 0)) {
		dev_err(fbi->dev, "params NULL pointer\n");
		 /* pointer NULL */
		return -EINVAL;
	}
	if (param_length > 2) {	
		...
		if ((param_length - 2) < word_count) {
			...
			dev_err(fbi->dev,"sent > input payload. complemented with zeroes\n");
		}else if ((param_length - 2) > word_count)
			dev_err(fbi->dev,"Overflow - input > sent. payload truncated\n");
		...
		for (i = 0; i < (param_length - 2); i += j) {
			...
			if (!(timeout < DSIH_FIFO_ACTIVE_WAIT)) {
				dev_err(fbi->dev, "tx fifo full timeout\n");
				return -EINVAL;
			}
		}
		...
		for (i = 0; i < compliment_counter; i++) {
			...
			if (!(timeout < DSIH_FIFO_ACTIVE_WAIT)) {
				dev_err(fbi->dev, "compensation zero tx fifo full timeout\n");
				return -EINVAL;
			}
		}
	}
	/*short packet & long packet header*/
	for (timeout = 0; timeout < DSIH_FIFO_ACTIVE_WAIT; timeout++) {
		/* check if payload Tx fifo is not full */
		if (!mipi_dsih_hal_gen_cmd_fifo_full(fbi)) {
			if (param_length == 0) {
	...
	if (!(timeout < DSIH_FIFO_ACTIVE_WAIT)) {
		dev_err(fbi->dev, "header tx fifo full timeout\n");
		err_code = -EINVAL;
	}
...
int mipi_dsih_gen_wr_cmd(struct comipfb_info *fbi, uint8_t vc, uint8_t* params, uint16_t param_length)
{
	...
	switch(params[0]) {
		...
		break;
		default:
			dev_err(fbi->dev, "*NOT GEN COMMAND*\n");
		return -EINVAL;
	}
...
int mipi_dsih_dcs_wr_cmd(struct comipfb_info *fbi, uint8_t vc, uint8_t* params, uint16_t param_length)
{
	...
	switch (params[0]) {
		...
		case SHUTDOWN_SW_PACK:
			packet_type = 0x22;
		default:
			dev_err(fbi->dev, "NOT DCS COMMAND\n");
		return -EINVAL;
	}
...
EXPORT_SYMBOL(mipi_dsih_gen_wr_packet);
