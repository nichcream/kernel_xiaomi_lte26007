...
static struct part_device *parse_device(char *s, int alloc_size, void **alloc) {
	char *p;
	struct part *part;
	struct part_device *device = NULL;
...
int cmdline_get_partition_byname(char *p_name, unsigned long *from,  unsigned long *to)
{
	struct part *part = cmdline_device->first_part;
	...
	while (part) {
		if (strncmp(p_name, part->name, strlen(p_name))) {
			part = part->next_part;
			continue;
		}

		*from =  part->from * part->sector_size;
		*to =  *from + part->size * part->sector_size;;
		break;
	}
...
__setup("lcpart=", cmdline_partition_setup);
