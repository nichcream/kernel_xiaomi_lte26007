	...
	if(bUseExtSwitch)
	{
		...
		switch(antPosType)
		{
			...
			case BTC_ANT_PATH_PTA:
				pBtCoexist->fBtcWrite1Byte(pBtCoexist, 0x930, 0x66);
				if(pBoardInfo->btdmAntPos == BTC_ANTENNA_AT_MAIN_PORT)
					pBtCoexist->fBtcWrite1ByteBitMask(pBtCoexist, 0x92c, 0x3, 0x1);
				else
					pBtCoexist->fBtcWrite1ByteBitMask(pBtCoexist, 0x92c, 0x3, 0x2);
	...
	)
{
	...
	u1Byte			drv_in_test=0;

	pBtCoexist->fBtcGet(pBtCoexist, BTC_GET_DRIVER_TEST_CFG, &drv_in_test);
	BTC_PRINT(BTC_MSG_ALGORITHM, ALGO_TRACE_FW_DETAIL, ("[BTCoex], drv_in_test:%d\n", drv_in_test));
	...
	if(bTurnOn)
	{
		switch(type)
		{
			...
			case 2:
				...
				break;
			case 3:
				if (drv_in_test == CMCC_TP_TEST)
					halbtc8723b1ant_SetFwPstdma(pBtCoexist, 0xd3, 0x30, 0x03, 0x10, 0x58);
				else
			...
			case 40: // SoftAP only with no sta associated,BT disable ,TDMA mode for power saving
				...
				break;
			case 200:
				halbtc8723b1ant_SetFwPstdma(pBtCoexist, 0x51, 0x40, 0x03, 0x10, 0x50);
				break;
		}
	}
	else
	...
	)
{
	...
	/* becaue 770 & 774 can not work under LPS 32k */
	/* we can not kown if BT is OFF, so we should */
	/* always set BT ON*/
	bBtActive = TRUE;

	pBtCoexist->fBtcGet(pBtCoexist, BTC_GET_BL_WIFI_AP_MODE_ENABLE, &bApEnable);
	pBtCoexist->fBtcGet(pBtCoexist, BTC_GET_DRIVER_TEST_CFG, &drv_in_test);
	if (drv_in_test == CMCC_RF_TEST)
		bBtActive = FALSE;
	...
	else
	{
		...
		if(btDisableCnt >= 2)
		{
			...
			if(bApEnable)
				halbtc8723b1ant_PowerSaveforSoftAP(pBtCoexist);
			else
...
{
	BOOLEAN 	bWifiBusy=FALSE, bWifiBusyTdma=FALSE;
	BOOLEAN		bLink=FALSE, bRoam=FALSE;
	BOOLEAN		bApEnable=FALSE;
	u4Byte		wifiBw;
	...
	pBtCoexist->fBtcGet(pBtCoexist, BTC_GET_BL_WIFI_AP_MODE_ENABLE, &bApEnable);
	pBtCoexist->fBtcGet(pBtCoexist, BTC_GET_BL_WIFI_BUSY, &bWifiBusy);
	pBtCoexist->fBtcGet(pBtCoexist, BTC_GET_BL_WIFI_BUSY_TDMA, &bWifiBusyTdma);
	// power save state
	if((!bApEnable && !pBtCoexist->btLinkInfo.bHidOnly) &&
		(BT_8723B_1ANT_BT_STATUS_ACL_BUSY == pCoexDm->btStatus || bWifiBusyTdma))
	{
	...
	else
	{
		...
		{
			if (bWifiBusyTdma) {
				halbtc8723b1ant_PsTdma(pBtCoexist, NORMAL_EXEC, TRUE, 200);
				halbtc8723b1ant_CoexTableWithType(pBtCoexist, NORMAL_EXEC, 1);
			} else {
				halbtc8723b1ant_PsTdma(pBtCoexist, NORMAL_EXEC, FALSE, 8);
				halbtc8723b1ant_CoexTableWithType(pBtCoexist, NORMAL_EXEC, 2);
	...
	)
{
	...
	u1Byte	aggBufSize=5;
	u1Byte	wifiRssiState=BTC_RSSI_STATE_HIGH;
	BOOLEAN	 bApEnable=FALSE, bScan=FALSE, bWifiBusy=FALSE;
	BOOLEAN bUnder4way=FALSE, bUnderDHCP=FALSE;
	u4Byte	wifiLinkStatus=0;
	u4Byte	numOfWifiLink=0;
	BOOLEAN bWifiBusyTdma=0;

	BTC_PRINT(BTC_MSG_ALGORITHM, ALGO_TRACE, ("[BTCoex], RunCoexistMechanism()===>\n"));

	pBtCoexist->fBtcGet(pBtCoexist, BTC_GET_BL_WIFI_4_WAY_PROGRESS, &bUnder4way);
	pBtCoexist->fBtcGet(pBtCoexist, BTC_GET_BL_WIFI_DHCP, &bUnderDHCP);
	pBtCoexist->fBtcGet(pBtCoexist, BTC_GET_BL_WIFI_AP_MODE_ENABLE, &bApEnable);
	...
	if (bScan || bUnder4way || bUnderDHCP) {
		BTC_PRINT(BTC_MSG_ALGORITHM, ALGO_TRACE, ("[BTCoex], RunCoexistMechanism(), return for bScan || bUnder4way || bUnderDHCP <===\n"));
		return;
	}
	...
	if(pBtLinkInfo->bScoExist || bWifiBusyTdma)
	{
	...
	pBtCoexist->fBtcGet(pBtCoexist, BTC_GET_BL_WIFI_AP_MODE_ENABLE, &bApEnable);
	if(bApEnable) {
		if(!GLBtCoexist.btInfo.bBtBusy){
			halbtc8723b1ant_PowerSaveforSoftAP(pBtCoexist);
			if(pCoexDm->curPsTdma == 40)
				return;
		}
	}
	...
	)
{
	...
	if(pBtCoexist->bManualControl ||
		...
		pBtCoexist->btInfo.bBtDisabled )
		return;
	if(pCoexDm->btStatus < BT_8723B_1ANT_BT_STATUS_INQ_PAGE)
		return;
	...
	)
{
	...
	BTC_PRINT(BTC_MSG_ALGORITHM, ALGO_TRACE, ("[BTCoex], bC2hBtInquiryPage:%x bBtLinkExist:%x "
				"bPanExist:%x bA2dpExist:%x bHidExist:%x bScoExist:%x\n",
				pCoexSta->bC2hBtInquiryPage, pCoexSta->bBtLinkExist, pCoexSta->bPanExist,
				pCoexSta->bA2dpExist, pCoexSta->bHidExist, pCoexSta->bScoExist));
...
VOID
EXhalbtc8723b1ant_DownloadFwNotify(
	IN	PBTC_COEXIST			pBtCoexist,
	IN	u1Byte				DLFwState
	)
{
	...
	if(BTC_WIFI_DLFW_BEGAIN == DLFwState)
	{
		if (!bUseExtSwitch) {
			// FWDL switch antenna control to BT
			// reg 0x764 BIT[12:11]=2'b11
			//== reg 0x765 BIT[4:3]=2'b11
			//WriteMACRegByte(REG_BT_COEX+1,ReadMACRegByte(REG_BT_COEX+1)|BIT3|BIT4);
			temp = pBtCoexist->fBtcRead1Byte(pBtCoexist, 0x765) |BIT3|BIT4;
			pBtCoexist->fBtcWrite1Byte(pBtCoexist, 0x765, temp);
		} else {
			//0x4c[23]=1
			//0x64[0]=1
			//WriteMACRegByte(REG_LEDCFG+2,ReadMACRegByte(REG_LEDCFG+2)|BIT7);
			//WriteMACRegByte(REG_PAD_CTRL1,ReadMACRegByte(REG_PAD_CTRL1)|BIT0);
			temp = pBtCoexist->fBtcRead1Byte(pBtCoexist, 0x4e) |BIT7;
			pBtCoexist->fBtcWrite1Byte(pBtCoexist, 0x4e, temp);//BIT7
			temp = pBtCoexist->fBtcRead1Byte(pBtCoexist, 0x64) |BIT0;
			pBtCoexist->fBtcWrite1Byte(pBtCoexist, 0x64, temp);//BIT0
		}
		BTC_PRINT(BTC_MSG_INTERFACE, INTF_NOTIFY, ("[BTCoex], DownloadFw notify to Begain 0x765:%x 0x4e:%x 0x64:%x\n",
			pBtCoexist->fBtcRead1Byte(pBtCoexist, 0x765), pBtCoexist->fBtcRead1Byte(pBtCoexist, 0x4e),
			pBtCoexist->fBtcRead1Byte(pBtCoexist, 0x64)));
	}
	else if(BTC_WIFI_DLFW_END == DLFwState)
	{
		if (!bUseExtSwitch) {
			// After FWDL switch antenna control to WiFi
			// reg 0x764 BIT[12:11]=2'b00
			//== reg 0x765 BIT[4:3]=2'b00
			//WriteMACRegByte(REG_BT_COEX+1,ReadMACRegByte(REG_BT_COEX+1)&(~BIT3)&(~BIT4));
			temp = pBtCoexist->fBtcRead1Byte(pBtCoexist, 0x765) &(~BIT3)&(~BIT4);
			pBtCoexist->fBtcWrite1Byte(pBtCoexist, 0x765, temp);//BIT3|BIT4
		}
		else{
			//0x4c[23]=0
			//WriteMACRegByte(REG_LEDCFG+2,ReadMACRegByte(REG_LEDCFG+2)&~BIT7);
			temp = pBtCoexist->fBtcRead1Byte(pBtCoexist, 0x4e) & ~BIT7;
			pBtCoexist->fBtcWrite1Byte(pBtCoexist, 0x4e, temp);
		}
		BTC_PRINT(BTC_MSG_INTERFACE, INTF_NOTIFY, ("[BTCoex], DownloadFw notify to End 0x765:%x 0x4e:%x 0x64:%x\n",
			pBtCoexist->fBtcRead1Byte(pBtCoexist, 0x765), pBtCoexist->fBtcRead1Byte(pBtCoexist, 0x4e),
			pBtCoexist->fBtcRead1Byte(pBtCoexist, 0x64)));
	}
}
	...
	)
{
	...
	if( halbtc8723b1ant_IsWifiStatusChanged(pBtCoexist) ||
		...
		pCoexSta->specialPktPeriodCnt > 2)
	{
...
VOID halbtc8723b1ant_PowerSaveforSoftAP(
	IN	PBTC_COEXIST		pBtCoexist
	)
{
	u1Byte	bSoftAPNoConnect=TRUE;
	u1Byte	blinking = 0,bInEarlySp=0;

	pBtCoexist->fBtcGet(pBtCoexist, BTC_GET_BL_WIFI_SOFTAP_IDLE, &bSoftAPNoConnect);
	pBtCoexist->fBtcGet(pBtCoexist, BTC_GET_BL_WIFI_SOFTAP_LINKING, &blinking);
	pBtCoexist->fBtcGet(pBtCoexist, BTC_GET_BL_WIFI_IN_EARLY_SUSPEND, &bInEarlySp);

	BTC_PRINT(BTC_MSG_ALGORITHM, ALGO_TRACE, ("[BTCoex], BTDM_SoftAP_PowerSave  in_early_suspend:%d linking:%d,bSoftAPNoConnect:%d\n",
				bInEarlySp, blinking,bSoftAPNoConnect));

	/* broadcom always power save under softap, but we just power save under early_suspend */
	/* if we don't care linked or no linked, use following */
	//if (bInEarlySp && !blinking) {
	/* if we dont want power save under linked use following */
	if (bInEarlySp && bSoftAPNoConnect && !blinking) {
		if (pCoexDm->curPsTdma != 40) {
			BTC_PRINT(BTC_MSG_ALGORITHM, ALGO_TRACE, ("[BTCoex], BTDM_SoftAP_PowerSave: no sta case\n"));
			halbtc8723b1ant_PsTdma(pBtCoexist, NORMAL_EXEC, TRUE, 40);
			halbtc8723b1ant_CoexTableWithType(pBtCoexist, NORMAL_EXEC, 0);
		}
	} else if (pCoexDm->curPsTdma == 40) {
		/* close TDMA when RF is ON */
		BTC_PRINT(BTC_MSG_ALGORITHM, ALGO_TRACE, ("[BTCoex], BTDM_SoftAP_PowerSave Close SoftAP PowerSave\n"));
		halbtc8723b1ant_PsTdma(pBtCoexist, NORMAL_EXEC, FALSE, 9);
		halbtc8723b1ant_CoexTableWithType(pBtCoexist, NORMAL_EXEC, 0);
