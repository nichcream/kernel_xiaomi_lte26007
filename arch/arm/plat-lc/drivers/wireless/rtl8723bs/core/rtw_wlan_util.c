	...
	if (rtw_get_capability((WLAN_BSSID_EX *)cur_network) & WLAN_CAPABILITY_PRIVACY)
	{
		for (i = sizeof(NDIS_802_11_FIXED_IEs); i < pmlmeinfo->network.IELength;)
		{
			...
			switch (pIE->ElementID)
			{
				...
				case _RSN_IE_2_:
				case _HT_ADD_INFO_IE_:	//11n ap  not support wep
					return _FALSE;
...
int support_short_GI(_adapter *padapter, struct HT_caps_element *pHT_caps)
{
	...
	if ((pmlmeinfo->assoc_AP_vendor == HT_IOT_PEER_RALINK) ||(pmlmeinfo->assoc_AP_vendor == HT_IOT_PEER_TENDA))
		return _FAIL;
...
*/
	...
	if (pmlmeext->cur_wireless_mode == WIRELESS_11BG ||
		pmlmeext->cur_wireless_mode == WIRELESS_11G) {
		u8 rx_agg_to = 0xf0;
		padapter->HalFunc.SetHwRegHandler( padapter, HW_VAR_RXDMA_AGG_TO,  &rx_agg_to);
	} else {
		u8 rx_agg_to = 0x0f;
		padapter->HalFunc.SetHwRegHandler( padapter, HW_VAR_RXDMA_AGG_TO,  &rx_agg_to);
	}
