...
static void sta_set_sinfo(struct sta_info *sta, struct station_info *sinfo)
{
	...
	if ((sta->local->hw.flags & IEEE80211_HW_SIGNAL_DBM) ||
		...
		WARN_ON(drv_get_signal(sdata->local, &sta->last_signal));
		ewma_add(&sta->avg_signal, -sta->last_signal);
		sinfo->signal = (s8)sta->last_signal;
		sinfo->signal_avg = (s8) -ewma_read(&sta->avg_signal);
		printk(KERN_DEBUG "sinfo->signal_avg = %d\n", sinfo->signal_avg);
	}
...
static int ieee80211_set_hw_scan_disable(struct wiphy *wiphy,
					 struct net_device *dev,
					 bool state)
{
	struct ieee80211_local *local = wdev_priv(dev->ieee80211_ptr);

	local->hw_scan_disable = state;
	printk("%s::%d!\n",__func__,state);
	return 0;
}
...
struct cfg80211_ops mac80211_config_ops = {
	...
	.set_hw_scan_disable = ieee80211_set_hw_scan_disable,
};
