...
)
{
	...
	//-----Match MAC ADDR-----
	//ODM_SetMACReg(pDM_Odm, 0x7b4, 0xFFFF, 0);           //  Reg 7b4[15:0]=0
	//ODM_SetMACReg(pDM_Odm, 0x7b0, bMaskDWord, 0);//  Reg 7b0[31:0]=0
	//ODM_SetMACReg(pDM_Odm, 0x7b4, BIT16|BIT17, 3); //  16:Enable antenna training // 17: Enable match source address
	
	//ODM_SetBBReg(pDM_Odm, 0xB38 , BIT2|BIT1|BIT0, 4 );	   //path-A  // ant combination=regB38[2:0]+1
	//ODM_SetMACReg(pDM_Odm, 0x608, BIT8, 1);
...
)
{
	...
	if((pDM_Odm->SupportICType & ODM_EVM_ENHANCE_ANTDIV_SUPPORT_IC))
	{
		...
		{
			//ODM_AntDiv_Printk(("[One Client only] \n"));
			...
			if (pDM_Odm->antdiv_rssi)
			{
			        //panic_printk("***  Only one Client[ %d ] : Diff_RSSI   = (( %d ))  , EVM_method_enable = ((  %d )) \n" , i,Diff_RSSI , pDM_FatTable->EVM_method_enable);
				panic_printk(" Main_Cnt = (( %d ))  , Main_RSSI= ((  %d )) \n", pDM_FatTable->MainAnt_Cnt[i], Main_RSSI);
				panic_printk(" Aux_Cnt   = (( %d ))  , Aux_RSSI = ((  %d )) \n" , pDM_FatTable->AuxAnt_Cnt[i] , Aux_RSSI);				
			}
			...
			{
		        	ODM_AntDiv_Printk(("[> TH_H || EVM_method_enable==1]  && "));
				if(((Main_RSSI>=Evm_RSSI_TH_Low)||(Aux_RSSI>=Evm_RSSI_TH_Low) ))
				{
					ODM_AntDiv_Printk(("[> TH_L ] \n"));
					...
					if(pDM_FatTable->FAT_State == NORMAL_STATE_MIAN)
					{
						...
						ODM_AntDiv_Printk(("[ start training: MIAN] \n"));
					...
					else if(pDM_FatTable->FAT_State == NORMAL_STATE_AUX)
					{
						...
						ODM_AntDiv_Printk(("[ start training: AUX] \n"));
					...
					{
						//ODM_AntDiv_Printk(("[Training state ] \n"));
						pDM_FatTable->FAT_State = NORMAL_STATE_MIAN;
				...
				if(pDM_Odm->antdiv_rssi)
				{
					panic_printk("*** Client[ %d ] : MainEVM_Cnt = (( %d ))  , Main_EVM= ((  %d )) \n",i, pDM_FatTable->MainAntEVM_Cnt[i], Main_EVM);
					panic_printk("*** Client[ %d ] : AuxEVM_Cnt   = (( %d ))  , Aux_EVM = ((  %d )) \n" ,i, pDM_FatTable->AuxAntEVM_Cnt[i] , Aux_EVM);
					panic_printk("*** TargetAnt_EVM = (( %s ))\n", ( pDM_FatTable->TargetAnt_EVM  ==MAIN_ANT)?"MAIN_ANT":"AUX_ANT");
					
					//panic_printk("*** Client[ %d ] : Mian_MPDU   = (( %d ))  , Aux_MPDU = ((  %d )) \n" ,i, pDM_FatTable->Main_MPDU_OK_cnt, pDM_FatTable->Aux_MPDU_OK_cnt);
					panic_printk("M_CRC_Ok = (( %d ))  , M_CRC_Fail = ((  %d )), Main_CRC_utility = (( %d )) \n" , pDM_FatTable->MainCRC32_Ok_Cnt, pDM_FatTable->MainCRC32_Fail_Cnt,Main_CRC_utility);
					panic_printk("A_CRC_Ok  = (( %d ))  , A_CRC_Fail = ((  %d )), Aux_CRC_utility   = ((  %d )) \n" , pDM_FatTable->AuxCRC32_Ok_Cnt, pDM_FatTable->AuxCRC32_Fail_Cnt,Aux_CRC_utility);
					panic_printk("*** TargetAnt_CRC32 = (( %s ))\n", ( pDM_FatTable->TargetAnt_CRC32 ==MAIN_ANT)?"MAIN_ANT":"AUX_ANT");
					panic_printk("****** TargetAnt_enhance = (( %s ))******\n", ( pDM_FatTable->TargetAnt_enhance ==MAIN_ANT)?"MAIN_ANT":"AUX_ANT");
				}						
					}
				}
				else // RSSI< = Evm_RSSI_TH_Low
				{ 
					ODM_AntDiv_Printk(("[ <TH_L: escape from > TH_L ] \n"));
					odm_EVM_FastAnt_Reset(pDM_Odm);
				}
			}
			else 
			{ 
				ODM_AntDiv_Printk(("[escape from> TH_H || EVM_method_enable==1] \n"));
				odm_EVM_FastAnt_Reset(pDM_Odm);
			}
		}
		else
		{
			ODM_AntDiv_Printk(("[multi-Client] \n"));
			odm_EVM_FastAnt_Reset(pDM_Odm);
		}			
	}
}
...
{
	ODM_AntDiv_Printk(("******odm_EVM_FastAntTrainingCallback****** \n"));
