...
FirmwareDownloadBT(IN PADAPTER Adapter, PRT_FIRMWARE_8723B pFirmware)
{
	...
	rtStatus = _CheckWLANFwPatchBTFwReady(Adapter);
...
s32 rtl8723b_FirmwareDownload(PADAPTER padapter, BOOLEAN  bUsedWoWLANFw)
{
	...
	switch(pFirmware->eFWSource)
	{
		...
		case FW_SOURCE_HEADER_FILE:
			...
			{
				DBG_871X("For read fw\n");
				ODM_ConfigFWWithHeaderFile(&pHalData->odmpriv, CONFIG_FW_MP, (u8 *)&(pFirmware->szFwBuffer), &(pFirmware->ulFwLength));
				DBG_871X(" %s fw:%s, size: %d\n", "Firmware for MP",__FUNCTION__, pFirmware->ulFwLength);
			}
#ifdef CONFIG_WOWLAN
	...
	if (padapter->registrypriv.mp_mode == 1)
	{
		...
		rtStatus = FirmwareDownloadBT(padapter, pBTFirmware);
	}
#endif
...
u8 MRateIdxToARFRId8723B(PADAPTER padapter, u8 rate_idx)
{
	u8 ret = 0;
...
static u32 rateid_table[]={ 0xf8ff015,	//BGN 40M 2ss
						0xff015,	//BGN 40M 1ss
						0xf8ff005,	//BGN 20M 2ss and BN
						0xff005,	//BGN 20M 1ss and BN
						0xf8ff010,	//GN and N 2ss
						0xff010,	//GN and N 1ss
						0xff5,		//BG
						0xff0,		//G mode
						0xd			//B mode
						};
static u32 ratefilter= RATE_1M | RATE_2M | RATE_5_5M;
void UpdateHalRAMask8723B(PADAPTER padapter, u32 mac_id, u8 rssi_level)
{
	...
	DBG_871X("YJ: %s(): mac_id=%d rssi_level=%d\n", __func__, mac_id, rssi_level);
...
#ifdef CONFIG_CONCURRENT_MODE
	if ((padapter->isprimary == _TRUE) && padapter->pbuddy_adapter){
		pbuddy_adapter=padapter->pbuddy_adapter;
		pHalData = GET_HAL_DATA(pbuddy_adapter);
		pdmpriv = &pHalData->dmpriv;
		pmlmepriv = &pbuddy_adapter->mlmepriv;
		pmlmeext = &pbuddy_adapter->mlmeextpriv;
		...
		DBG_871X("%s():pbuddy_adapter  psta find--------------- \n", __func__);
...
DBG_871X("%s(): Unicast Data, raid=%d mask=%.8x shortGIrate=%d\n", __func__, raid, mask, shortGIrate);
			break;
		...
		default: //for each sta in IBSS
			...
			raid = psta->raid;
			mask = psta->tx_ra_bitmap;
			shortGIrate = psta->htpriv.sgi;
...
#ifdef CONFIG_BT_COEXIST
	rate_bitmap = BT_GetRaMask(padapter);
	mask &= ~rate_bitmap;
#endif // CONFIG_BT_COEXIST

	init_rate = get_highest_rate_idx(mask)&0x3f;
	if(psta->mac_id < 3)
		raid = MRateIdxToARFRId8723B(padapter, raid);
	//set sta ra_id before changed
	psta->raid = raid;
...
#ifdef CONFIG_BT_COEXIST
	if(pHalData->bBtScoBusy && (raid&0xF) < 9 && mac_id == 0){
		//u32 fallback_mask = rateid_table[raid&0xF] & (~ratefilter);
		u32 fallback_mask = mask & (~0xf0000000); //remove normal raid;
		if(rtw_read32(padapter, 0x444) != fallback_mask) //REG_ARFR0_8723B
			rtw_write32(padapter, 0x444, fallback_mask);

		mask &= (~ratefilter); //disable 1M/5.5M when sco
	}
#endif
	...
	{
		if(psta->mac_id < 3)
			rtl8723b_set_FwMacIdConfig_cmd(padapter, mac_id, raid, pmlmeext->cur_bwmode, mask);
		else
			rtl8723b_set_FwMacIdConfig_cmd(padapter, mac_id, raid, psta->htpriv.bwmode, mask);
	}else {
		if (padapter->drv_in_test == CMCC_RF_TEST) {
			u32 fallback_mask = mask & (~0xf0000000); //remove normal raid;

			/* RateFallBackMask for RetryRate */
			if(rtw_read32(padapter, REG_ARFR0) != fallback_mask)
				rtw_write32(padapter, REG_ARFR0, fallback_mask);

			DBG_871X("update CMCC_RF_TEST raid entry, fallback_mask:%x\n",
					fallback_mask);
		}
	}
...
void rtl8723b_set_hal_ops(struct hal_ops *pHalFunc)
{
	...
	pHalFunc->MRateIdxToARFRId = &MRateIdxToARFRId8723B;
	...
	IN	u8*			hwinfo)
{
	...
	//0x1FB bit[0:2]
	res = rtw_efuse_access(padapter, _FALSE, EEPROM_PACKAGE_CHIP_TYPE, 1, &buff);
	...
	IN	u8*			hwinfo)
{
	...
	if (EEPROMId == RTL_EEPROM_ID && res != _FAIL) {
		/* 40M Disable */
		/* RTL8703AS: 0x1FB[4:5] 2b'10 */
		/* RTL8723BS: 0x1FB[4:5] 2b'11 */
		buff &= BIT(4) | BIT(5);
		if (buff == EEPROM_CHIP_RTL8703AS)
			padapter->registrypriv.cbw40_enable = 0;
	}

	DBG_871X("Hal_EfuseParse40MInfo_8723B(), EEPROM_40M_SUPPORT_8723BS:%x cbw40_enable:%d\n",
		buff, padapter->registrypriv.cbw40_enable);
}
	...
	u8 *pbuf)
{
	...
	PTXDESC ptxdesc;
	s32 bmcst;
	u8 *pframe = (u8 *)(pxmitframe->buf_addr) + TXDESC_OFFSET;
...
//Add by sherry to determine port 0 and port 1 send beacon
//20130926
#ifdef CONFIG_CONCURRENT_MODE
		if(GetFrameSubType(pframe)==WIFI_BEACON)
		{
			if(padapter->iface_type == IFACE_PORT0)
				ptxdesc->use_beacon_selection = 0;
			else
				ptxdesc->use_beacon_selection = 1;
		}
#endif

//Added for p2p scan on port 1
//20130927
#ifdef CONFIG_CONCURRENT_MODE
		if((GetFrameSubType(pframe)==WIFI_PROBEREQ))
		{
			if((padapter->adapter_type == SECONDARY_ADAPTER) && (pmlmeext->tx_rate == IEEE80211_CCK_RATE_1MB))
			{
...
#define GET_8723B_C2H_TX_RPT_RETRY_OVER(_Header)	LE_BITS_TO_1BYTE((_Header + 0), 7, 1)
	...
	if (GET_8723B_C2H_TX_RPT_RETRY_OVER(pdata) | GET_8723B_C2H_TX_RPT_LIFE_TIME_OVER(pdata)) {
		DBG_871X("Tx Fail 1111:tx_seq_no=%d, rpt_seq_no=%d\n", padapter->xmitpriv.seq_no, seq_no);
		rtw_ack_tx_done(&padapter->xmitpriv, RTW_SCTX_DONE_CCX_PKT_FAIL);
	}
	else if(seq_no != padapter->xmitpriv.seq_no) {
		DBG_871X("Tx Fail:tx_seq_no=%d, rpt_seq_no=%d\n", padapter->xmitpriv.seq_no, seq_no);
		rtw_ack_tx_done(&padapter->xmitpriv, RTW_SCTX_DONE_CCX_PKT_FAIL);
	} else {
		DBG_871X("Tx OK:tx_seq_no=%d, rpt_seq_no=%d\n", padapter->xmitpriv.seq_no, seq_no);
...
//					BT_WifiMediaStatusNotify(padapter, RT_MEDIA_CONNECT);
		...
		case HW_VAR_RXDMA_AGG_TO:
			rtw_write8(padapter, REG_RXDMA_AGG_PG_TH + 1, *val);
			break;
		...
		case HW_VAR_H2C_MEDIA_STATUS_RPT:
			{
				u8 opmode,macid;
				u16 mst_rpt = cpu_to_le16 ((*(u16 *)val));
				opmode = (u8) mst_rpt;
				macid = (u8)(mst_rpt >> 8);
				rtl8723b_set_FwMediaStatusRpt_cmd(padapter, opmode, macid);
			}
			break;
#ifdef CONFIG_P2P
		...
		case HW_VAR_CHECK_TXBUF:
#ifdef CONFIG_CONCURRENT_MODE
			{
				...
				for (i=0; i<100; i++)
				{
					...
					{
						DBG_871X_LEVEL(_drv_always_,"packet in tx packet buffer - 0x204=%x, 0x200=%x (%d)\n", rtw_read32(padapter, 0x204), rtw_read32(padapter, 0x200), i);
						rtw_msleep_os(10);
					}
					else
					{
...
void rtl8723b_start_thread(_adapter *padapter)
{
	...
	pxmitpriv->SdioXmitThread = kthread_run(rtl8723bs_xmit_thread, padapter, "RTW_RTL8723BS_THREAD");
	if (IS_ERR(pxmitpriv->SdioXmitThread)) {
...
void Hal_BBRF_Gain_Offset(_adapter *padapter)
{
