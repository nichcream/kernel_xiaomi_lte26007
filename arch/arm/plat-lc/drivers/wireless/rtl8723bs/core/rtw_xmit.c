...
static s32 update_attrib(_adapter *padapter, _pkt *pkt, struct pkt_attrib *pattrib)
{
 ...
 _func_enter_;
	...
	if (psta->ieee8021x_blocked == _TRUE)
	{
		...
		if((pattrib->ether_type != 0x888e) && (pattrib->ether_type != 0x88B4) &&
				(check_fwstate(pmlmepriv, WIFI_MP_STATE) == _FALSE))
		{
			RT_TRACE(_module_rtl871x_xmit_c_,_drv_err_,("\npsta->ieee8021x_blocked == _TRUE,  pattrib->ether_type(%.4x) != 0x888e\n",pattrib->ether_type));
			DBG_871X_LEVEL(_drv_err_, "psta->ieee8021x_blocked == _TRUE,  pattrib->ether_type(%.4x) != 0x888e |n0x88B4\n",pattrib->ether_type);
#ifdef DBG_TX_DROP_FRAME
	...
	if (check_fwstate(pmlmepriv, WIFI_STATION_STATE)) {
		...
		else
		{
			...
			if (adapter_to_pwrctl(padapter)->pwr_mode != PS_MODE_ACTIVE && !bmcast) {
	...
	if (psta->ieee8021x_blocked == _FALSE) {
...
_func_enter_;
	...
	if(pattrib->bswenc)
	{
		...
		RT_TRACE(_module_rtl871x_xmit_c_,_drv_info_,("### xmitframe_hwencrypt\n"));
	}
...
_func_enter_;
	...
	while (rtw_end_of_queue_search(phead, plist) == _FALSE)
	{
		...
		if(pxmitframe->attrib.psta) {
			ptxservq = rtw_get_sta_pending(padapter, pxmitframe->attrib.psta, pxmitframe->attrib.priority, &ac_index);
			ptxservq->qcnt--;
...
int rtw_sctx_wait2(struct xmit_priv *pxmitpriv)
{
#ifdef CONFIG_XMIT_ACK
	int ret = _FAIL;
	unsigned long expire;
	int status = 0;
	struct submit_ctx *sctx = &pxmitpriv->ack_tx_ops;
	DBG_871X("++rtw_sctx_wait2:(&sctx->done)->done:%d\n",(&sctx->done)->done);
//	DBG_871X("===>rtw_sctx_wait2 sctx->timeout_ms:%d pxmitpriv:%p &pxmitpriv->ack_tx_ops:%p\n",(sctx)->timeout_ms,pxmitpriv,&pxmitpriv->ack_tx_ops);
#ifdef PLATFORM_LINUX
	expire= (sctx)->timeout_ms ? msecs_to_jiffies((sctx)->timeout_ms) : MAX_SCHEDULE_TIMEOUT;
	if (!wait_for_completion_timeout(&sctx->done, expire)) {
		/* timeout, do something?? */
		//status = sctx->status;
		DBG_871X("rtw_sctx_wait2  timeout!!!!!!!!!!!!!\n");
		status = (-1);
	} else {
			status = (sctx)->status;
	//	DBG_871X("===rtw_sctx_wait2 sctx->status:%d &pxmitpriv->ack_tx_ops->status:%d\n",(sctx)->status,(&pxmitpriv->ack_tx_ops)->status);
	}
#endif
	...
	sctx = &pxmitpriv->ack_tx_ops;
	DBG_871X("--rtw_sctx_wait2:(&sctx->done)->done:%d\n",(&sctx->done)->done);
	//DBG_871X("<===rtw_sctx_wait2 ret:%d (sctx)->status:%d pxmitpriv->test:%d &(pxmitpriv->test):%p *(&(pxmitpriv->test)):%d\n",ret,(sctx)->status,pxmitpriv->test,&(pxmitpriv->test),(int)*(&(pxmitpriv->test)));
	return ret;
#else
	return _TRUE;
#endif //CONFIG_XMIT_ACK
...
void rtw_sctx_done_err2(struct xmit_priv *pxmitpriv, int status)
{
#ifdef CONFIG_XMIT_ACK
	struct submit_ctx *sctx = &pxmitpriv->ack_tx_ops;
	(sctx)->status = status;
	//DBG_871X("++rtw_sctx_done_err2:(&sctx->done)->done:%d\n",(&sctx->done)->done);
	//DBG_871X("rtw_sctx_done_err2 (*sctx)->status:%d &pxmitpriv->ack_tx_ops->status:%d pxmitpriv:%p ack_tx_ops:%p\n",(sctx)->status,(&pxmitpriv->ack_tx_ops)->status,pxmitpriv,&pxmitpriv->ack_tx_ops);
#ifdef PLATFORM_LINUX
	complete(&sctx->done);
#endif
	//DBG_871X("--rtw_sctx_done_err2:(&sctx->done)->done:%d\n",(&sctx->done)->done);
	//DBG_871X("rtw_sctx_done_err2-2 (*sctx)->status:%d pxmitpriv->test :%d &(pxmitpriv->test):%p  *(&(pxmitpriv->test)):%d\n",(sctx)->status,pxmitpriv->test,&(pxmitpriv->test),(int)*(&(pxmitpriv->test)));
#endif
}
...
int rtw_ack_tx_wait(struct xmit_priv *pxmitpriv, u32 timeout_ms)
{
	...
	return rtw_sctx_wait2(pxmitpriv);
}

void rtw_ack_tx_done(struct xmit_priv *pxmitpriv, int status)
{
	struct submit_ctx *pack_tx_ops = &pxmitpriv->ack_tx_ops;
	//DBG_871X("===>%s pxmitpriv->ack_tx:%d status:%d\n",__FUNCTION__,pxmitpriv->ack_tx,status);
	if (pxmitpriv->ack_tx) {
		rtw_sctx_done_err2(pxmitpriv, status);
	} else {
