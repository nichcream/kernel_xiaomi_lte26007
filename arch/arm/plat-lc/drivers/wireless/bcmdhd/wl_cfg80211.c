...
int wl_rssi_average(s32 rssi)
{
	...
	if(old_rssi){
		...
		if((delt_val >= RSSI_TREMBLE_LIMIT) && (delt_val < 5 * RSSI_TREMBLE_LIMIT)){
...
static s32 g_macdisable_flag = FALSE;
...
#endif /* WL_CFG80211_P2P_DEV_IF */
{
	...
	{
		printk(KERN_DEBUG "%s::scan disabled!\n",__func__);
		return -EBUSY;
	}
	...
	struct cfg80211_connect_params *sme)
{
	...
	*/
	...
	printk(KERN_DEBUG "Connectting with" MACDBG " channel (%d) ssid \"%s\", len (%d)\n\n",
	...
	enum nl80211_channel_type channel_type)
{
	...
	printk(KERN_DEBUG "netdev_ifidx(%d), chan_type(%d) target channel(%d) \n",
	...
	u32 dev_role, s32 bssidx)
{
	...
	if (info->beacon_interval) {
		s32 bi =300;
		printk(KERN_DEBUG "%s::bi=%d",__func__,info->beacon_interval);
		if ((err = wldev_ioctl(dev, WLC_SET_BCNPRD,
			&bi, sizeof(s32), true)) < 0) {
	...
	u8* mac_addr)
{
	...
	err = wldev_ioctl(dev, WLC_SCB_DEAUTHENTICATE_FOR_REASON, &scb_val,
		sizeof(scb_val_t), true);
	if (err < 0)
		WL_DBG(("WLC_SCB_DEAUTHENTICATE_FOR_REASON err %d\n", err));
	printk(KERN_DEBUG "Disconnect STA : %s scb_val.val %d\n",
	...
	struct cfg80211_ap_settings *info)
{
	...
	{
		uint32 interference = 2;
		wldev_ioctl(dev, WLC_SET_INTERFERENCE_OVERRIDE_MODE, (char *)&interference, sizeof(interference), TRUE);
		wldev_ioctl(dev, WLC_SET_INTERFERENCE_MODE, (char *)&interference, sizeof(interference), TRUE);
	}
fail:
	if (err) {
...
static bool wl_is_linkdown(struct bcm_cfg80211 *cfg, const wl_event_msg_t *e)
{
	...
	event == WLC_E_DEAUTH) {
#if (WL_DBG_LEVEL > 0)
	printk(KERN_DEBUG "Link down Reason : WLC_E_%s\n", wl_dbg_estr[event]);
	...
	if (wl_get_mode_by_netdev(cfg, ndev) == WL_MODE_AP) {
		...
		if (wl_is_linkup(cfg, e, ndev)) {
			...
			if (!wl_get_drv_status(cfg, DISCONNECTING, ndev)) {
					printk(KERN_DEBUG "wl_bss_connect_done succeeded with " MACDBG "\n",
						MAC2STRDBG((u8*)(&e->addr)));
			...
			/* brcm, cg. If no p2p connection, when sta link down, reset MAC.	*/
			if ((event == WLC_E_LINK) &&
				!(DHD_FLAG_P2P_GC_MODE & dhd_get_opmode(ndev)) &&
				!(DHD_FLAG_P2P_GO_MODE & dhd_get_opmode(ndev))) {
				if ((!(flags & WLC_EVENT_MSG_LINK)) && (2 == ntoh32(e->reason))) {
					g_macdisable_flag = TRUE;
					val = 0;
					err = wldev_ioctl(ndev, WLC_DOWN, &val, sizeof(s32), true);
					printf(KERN_DEBUG"WAR:WLC_DOWN command rtn:[%d]\n", err);
					//mdelay(200);
					val = 0;
					err = wldev_ioctl(ndev, WLC_UP, &val, sizeof(s32), true);
					printf(KERN_DEBUG"WAR:WLC_UP command rtn:[%d]\n", err);
					//mdelay(200);
					g_macdisable_flag = FALSE;
				}
			}
			...
			if (wl_get_drv_status(cfg, CONNECTED, ndev)) {
				...
				printk(KERN_DEBUG "link down if %s may call cfg80211_disconnected. "
				...
				printk(KERN_DEBUG "link down, during connecting\n");
			...
			printk(KERN_DEBUG "connect failed event=%d e->status %d e->reason %d \n",
...
#endif /* WLFBT */
	printk(KERN_DEBUG "wl_bss_roaming_done succeeded to " MACDBG "\n",
		MAC2STRDBG((u8*)(&e->addr)));
...
s32 wl_cfg80211_up(void *para)
{
	...
	if (!(dhd->op_mode & DHD_FLAG_HOSTAP_MODE)) {
		...
		if (unlikely(err)) {
			WL_ERR(("wl_cfg80211_attach_post failed\n"));
			mutex_unlock(&cfg->usr_sync);
			return err;
		}
	}
