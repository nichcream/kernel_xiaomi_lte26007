...
static int cw1200_upload_null(struct cw1200_common *priv)
{
	...
	printk(KERN_DEBUG "Manoj: cw1200_upload_null: frame.rate = %d\n", frame.rate);
	if (priv->hw->single_rate_ap) {
		int i;
		printk(KERN_DEBUG "Manoj: priv->hw->single_rate_ap_rate = %d\n", priv->hw->single_rate_ap_rate);

		for (i = 0; i < 32; ++i) {
			if (priv->hw->single_rate_ap_rate & BIT(i)) {
				frame.rate = priv->rates[i].hw_value;
				break;
			}
		}
	}
...
static int cw1200_upload_qosnull(struct cw1200_common *priv)
{
	...
	//printk(KERN_DEBUG "Manoj: cw1200_upload_qosnull: frame.rate = %d\n", frame.rate);
	if (priv->hw->single_rate_ap) {
		int i;
		printk(KERN_DEBUG "Manoj: priv->hw->single_rate_ap_rate = %d\n", priv->hw->single_rate_ap_rate);
		for (i = 0; i < 32; ++i) {
