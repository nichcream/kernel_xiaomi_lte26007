	...
	)
{
	...
	//
	pDM_Odm->BbSwingIdxOfdm[0] 		= 12; // Set defalut value as index 12.
	...
	)
{
	...
	if((pDM_Odm->SupportICType & (ODM_RTL8723A)) || (pDM_Odm->SupportICType & (ODM_RTL8723B) ))
		pDM_Odm->AntDivType = CGCS_RX_SW_ANTDIV;
	...
	if(pDM_DigTable->CurIGValue != CurrentIGI)//if(pDM_DigTable->PreIGValue != CurrentIGI)
	{
		if(pDM_Odm->SupportPlatform & (ODM_CE|ODM_MP))
		{
			...
			if(pDM_Odm->SupportICType != ODM_RTL8188E &&
					pDM_Odm->SupportICType != ODM_RTL8723A &&
					pDM_Odm->SupportICType != ODM_RTL8723B)
...
#ifdef CONFIG_LPS
	bFwCurrentInPSMode = pwrctrlpriv->bFwCurrentInPSMode;
#endif
...
// This should be moved out of OUTSRC
	...
	//1 Boundary Decision
	if(pDM_Odm->SupportICType & (ODM_RTL8192C|ODM_RTL8723A) && (pDM_Odm->BoardType & (ODM_BOARD_EXT_LNA | ODM_BOARD_EXT_PA)))
	{
		if(pDM_Odm->SupportPlatform & (ODM_AP|ODM_ADSL))
		{
	...
	if(pDM_Odm->bLinked)
	{
		...
		if(pDM_Odm->SupportICType==ODM_RTL8723A)
		{
			...
			if((pDM_Odm->SupportICType & (ODM_RTL8192E|ODM_RTL8723B|ODM_RTL8812|ODM_RTL8821)) && (pDM_Odm->bBtLimitedDig==1)){
				/* Modify DIG upper bound for 92E, 8723B, 8821 & 8812 BT */
	...
	)
{
	...
	#if((RTL8188E_SUPPORT==1) ||(RTL8723B_SUPPORT==1))
	...
	{
		if(IS_HARDWARE_TYPE_8192D(Adapter))
			PHY_SetRFReg(Adapter, ODM_RF_PATH_A, RF_T_METER, BIT17 | BIT16, 0x03);
		else if(IS_HARDWARE_TYPE_8188E(Adapter) || IS_HARDWARE_TYPE_8723B(Adapter))
	...
	{
		//DBG_871X("===> odm_TXPowerTrackingCheckCE()\n");
#if (RTL8188E_SUPPORT == 1)
		if(IS_HARDWARE_TYPE_8188E(Adapter))
			odm_TXPowerTrackingCallback_ThermalMeter_8188E(Adapter);
#endif
#if (RTL8723B_SUPPORT == 1)
		//if(IS_HARDWARE_TYPE_8723B(Adapter))   //YJ,TODO
	...
	IN	PDM_ODM_T		pDM_Odm)
{
	...
	btAntNum= BT_GetPgAntNum(Adapter);
	...
	)
{
	...
	if (BT_IsDisableEdcaTurbo(Adapter))
	{
		goto dm_CheckEdcaTurbo_EXIT;
	}
#endif
