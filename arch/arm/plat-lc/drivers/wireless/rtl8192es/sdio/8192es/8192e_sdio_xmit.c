...
const u8 Queue2Oqt[MAX_HW_TX_QUEUE] = {
...
static u8 rtw_sdio_wait_enough_TxOQT_space(struct rtl8192cd_priv *priv, u8 q_num, u8 agg_num)
{
	u32 n = 0;
	u32 oqt_idx = Queue2Oqt[q_num];
	HAL_INTF_DATA_TYPE *pHalData = GET_HAL_INTF_DATA(priv);
	...
	while (pHalData->SdioTxOQTFreeSpace[oqt_idx] < agg_num) {
		...
		if ((++n % 60) == 0) {
			printk("%s(%d): QOT %d does not have enough space(%d), it needs: %d\n",
