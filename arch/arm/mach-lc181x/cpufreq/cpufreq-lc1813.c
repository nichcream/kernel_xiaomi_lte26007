/*
 * arch/arm/mach-lc181x/cpufreq/cpufreq-lc1813.c
 *
...
#define AP_PWR_DV_CTL AP_PWR_A7DV_CTL
...
#ifdef CONFIG_THERMAL_COMIP
static struct cpufreq_thermal_map thermal_freq_table[] = {
	{THERMAL_NORMAL,	0},
	{THERMAL_WARM,		2},
	{THERMAL_HOT,		3},
	{THERMAL_TORRID, 	4},
...
static unsigned int cpu_thermal_freq = FREQ_LEVEL_HIGH;
#endif
...
#ifdef CONFIG_THERMAL_COMIP
	rate = max(rate, cpu_thermal_freq);
#endif
...
#ifdef CONFIG_THERMAL_COMIP
static unsigned int cpufreq_get_by_notifier(unsigned long event, void *ptr)
{
	struct thermal_data *data = (struct thermal_data*)ptr;
	unsigned int cpu_freq = FREQ_LEVEL_MAX;
	static unsigned int history_freq = 0;
	static int history_temperature = -100;

	if(data->dfb_cpu.enable){
		if(-100 == history_temperature)
			history_temperature = data->temperature;

		if(data->temperature > data->dfb_cpu.target_temperature){
			if(data->temperature > history_temperature){
				if(10000*(FREQ_LEVEL_MAX - history_freq)/FREQ_LEVEL_MAX >= data->dfb_cpu.limit_percent)
					history_freq++;
			}
		...
		cpu_freq = history_freq;
		history_temperature = data->temperature;

	}else{
		int i;
		for (i = 0; i < ARRAY_SIZE(thermal_freq_table); i++) {
			if (thermal_freq_table[i].level == event) {
				cpu_freq = thermal_freq_table[i].index;
				CPUFREQ_PR("%s: thermal cpu freq is %d\n", __func__, cpu_freq);
				break;
			}
		}
	}
...
static int cpu_thermal_event(struct notifier_block *this, unsigned long event,
			     void *ptr)
{
	int ret;
	unsigned int cpu_freq = FREQ_LEVEL_MAX;

	cpu_freq = cpufreq_get_by_notifier(event, ptr);

	if(FREQ_LEVEL_MAX == cpu_freq)
		goto exit;

	mutex_lock(&comip_cpufreq_lock);
	cpu_thermal_freq = cpu_freq;

	if (!cpufreq_is_suspended) {
		unsigned int rate = FREQ_LEVEL_MAX;
		rate = cpus_highest_freq();

		if(rate == cpu_freq){
			ret = cpufreq_update_speed(cpufreq_cpu_get(0), cpu_freq, FREQ_IN_RUN);	
			if(ret)
				printk("cpu thermal error!\n");		
		}
	}
...
static struct notifier_block cpu_thermal =
{
	.notifier_call = cpu_thermal_event,
};
#endif
...
#ifdef CONFIG_THERMAL_COMIP
	thermal_notifier_register(&cpu_thermal);
#endif
