...
static int cw1200_status_show(struct seq_file *seq, void *v)
{
	...
	seq_printf(seq, "WSM dumps:  %s\n",
		priv->wsm_enable_wsm_dumps ? "enabled" : "disabled");
#endif /* CONFIG_CW1200_WSM_DUMPS */

#if defined(CONFIG_CW1200_WSM_DUMPS_SHORT)
	seq_printf(seq, "WSM short dumps size:  %u\n", priv->wsm_dump_max_size);
#endif /* CONFIG_CW1200_WSM_DUMPS_SHORT */

#if defined(CONFIG_CW1200_WSM_DEBUG)
	seq_printf(seq, "WSM debug:  %s\n",
		priv->wsm_enable_wsm_debug ? "enabled" : "disabled");
#endif /* CONFIG_CW1200_WSM_DEBUG */

#if defined(CONFIG_CW1200_BH_DEBUG)
	seq_printf(seq, "BH debug:   %s\n",
		priv->bh_enable_bh_debug ? "enabled" : "disabled");
#endif /* CONFIG_CW1200_BH_DEBUG */
	...
	const char __user *user_buf, size_t count, loff_t *ppos)
{
	...
	if (buf[0] == '1') {
		printk(KERN_DEBUG "[WSM] DUMPS enabled.\n");
		priv->wsm_enable_wsm_dumps = 1;
	} else if (buf[0] == '0') {
		printk(KERN_DEBUG "[WSM] DUMPS disabled.\n");
...
#ifdef CONFIG_CW1200_WSM_DEBUG
static ssize_t cw1200_wsm_debug(struct file *file,
	const char __user *user_buf, size_t count, loff_t *ppos)
{
	struct cw1200_common *priv = file->private_data;
	char buf[1];
	...
	if (buf[0] == '1') {
		printk(KERN_DEBUG "[WSM] Debug enabled.\n");
		priv->wsm_enable_wsm_debug = 1;
	} else if (buf[0] == '0') {
		printk(KERN_DEBUG "[WSM] Debug disabled.\n");
		priv->wsm_enable_wsm_debug = 0;
	} else {
		return -EINVAL;
	}
...
static const struct file_operations fops_wsm_debug = {
	.open = cw1200_generic_open,
	.write = cw1200_wsm_debug,
	.llseek = default_llseek,
};
#endif /* CONFIG_CW1200_WSM_DEBUG */

#ifdef CONFIG_CW1200_BH_DEBUG
static ssize_t cw1200_bh_debug(struct file *file,
	const char __user *user_buf, size_t count, loff_t *ppos)
{
	struct cw1200_common *priv = file->private_data;
	char buf[1];
	...
	if (buf[0] == '1') {
		printk(KERN_DEBUG "[BH] Debug enabled.\n");
		priv->bh_enable_bh_debug = 1;
	} else if (buf[0] == '0') {
		printk(KERN_DEBUG "[BH] Debug disabled.\n");
		priv->bh_enable_bh_debug = 0;
	} else {
		return -EINVAL;
	}
...
static const struct file_operations fops_bh_debug = {
	.open = cw1200_generic_open,
	.write = cw1200_bh_debug,
	.llseek = default_llseek,
};
#endif /* CONFIG_CW1200_BH_DEBUG */
...
#if defined(CONFIG_CW1200_WSM_DEBUG)
	if (!debugfs_create_file("wsm_debug", S_IWUSR, d->debugfs_phy,
			priv, &fops_wsm_debug))
		goto err;
#endif /* CONFIG_CW1200_WSM_DEBUG */

#if defined(CONFIG_CW1200_BH_DEBUG)
	if (!debugfs_create_file("bh_debug", S_IWUSR, d->debugfs_phy,
			priv, &fops_bh_debug))
		goto err;
#endif /* CONFIG_CW1200_BH_DEBUG */

#if defined(CONFIG_CW1200_DEBUG_ON_BOOT)
	priv->wsm_enable_wsm_dumps = 1;
	priv->wsm_enable_wsm_debug = 1;
	priv->bh_enable_bh_debug = 1;
#endif /* CONFIG_CW1200_DEBUG_ON_BOOT */
