				 ...
				 struct ieee80211_work *wk)
{
	...
	if (wk->assoc.supp_rates_len) {
		...
		rates_len = ieee80211_compatible_rates(wk->assoc.supp_rates,
						       ...
						       sband, &rates);
		local->rates = rates;
		local->rates_len = rates_len;
		printk(KERN_DEBUG "Manoj: local->rates = %d, local->rates_len = %d\n", local->rates, local->rates_len);
	} else {
		/*
	...
	*pos++ = supp_rates_len;
	...
	//printk(KERN_DEBUG "Manoj: ieee80211_send_assoc rates = %d\n", rates);
	count = 0;
	...
	//printk(KERN_DEBUG "Manoj: ieee80211_send_assoc; count = %d\n", count);
	if (count == 1) {
		u8 single_rate = *(pos - 1);
		//printk(KERN_DEBUG "Manoj: rate = %x", single_rate);
		single_rate = (single_rate | (u8)0x80);
		//printk(KERN_DEBUG "Manoj: rate = %x", single_rate);
		*(pos - 1) = single_rate;
	}
