...
static void resume_work_fn(struct work_struct *work)
{
	struct inv_mpu_iio_s *st
		= container_of((struct delayed_work *)work, struct inv_mpu_iio_s, resume_work);
	...
	result = inv_switch_engine(st, st->chip_config.gyro_enable,
				BIT_PWR_GYRO_STBY);
	if (result)
		printk("inv_switch_engine gyro_enable fail\n");

	result = inv_switch_engine(st, st->chip_config.accl_enable,
				BIT_PWR_ACCL_STBY);
	if (result)
		printk("inv_switch_engine accl_enable fail\n");

	result = inv_lpa_freq(st, st->chip_config.lpa_freq);
	if (result)
		printk("inv_switch_engine lpa_freq fail\n");
...
static int set_power_itg_resume(struct inv_mpu_iio_s *st, bool power_on)
{
	struct inv_reg_map_s *reg;
	u8 data;
	int result;
	...
	if (power_on) {
		...
		schedule_delayed_work(&st->resume_work, 10);
	}
...
/**
 *  inv_read_accel_show() - Register dump for testing.
 */
static ssize_t inv_read_accel_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	char data[6];
	s8 *m;
	...
	inv_i2c_read(st, REG_RAW_ACCEL, 6, data);
	...
	accel_x = x * m[0] + y * m[1] + z * m[2];
	accel_y = x * m[3] + y * m[4] + z * m[5];
	accel_z = x * m[6] + y * m[7] + z * m[8];
...
/**
 *  inv_read_gyro_show() - Register dump for testing.
 */
static ssize_t inv_read_gyro_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	char data[6];
	s8 *m;
	...
	inv_i2c_read(st, REG_RAW_GYRO, 6, data);
	...
	gyro_x = x * m[0] + y * m[1] + z * m[2];
	gyro_y = x * m[3] + y * m[4] + z * m[5];
	gyro_z = x * m[6] + y * m[7] + z * m[8];
...
/**
 *  inv_bias_accel_show() - Register dump for testing.
 */
static ssize_t inv_bias_accel_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
//	u8 data[2];
	s8 *m;
	...
	inv_i2c_single_write(st, REG_6500_ACCEL_CONFIG2, 3);
#if 0
	inv_i2c_read(st, 0x77, 2, data);
	st->bias_accel[0] = data[0] << 8 | data[1];

	inv_i2c_read(st, 0x7A, 2, data);
	st->bias_accel[1] = data[0] << 8 | data[1];

	inv_i2c_read(st, 0x7D, 2, data);
	st->bias_accel[2] = data[0] << 8 | data[1];
#endif
	x = st->bias_accel[0] * m[0] + st->bias_accel[1] * m[1] + st->bias_accel[2] * m[2];
	y = st->bias_accel[0] * m[3] + st->bias_accel[1] * m[4] + st->bias_accel[2] * m[5];
	z = st->bias_accel[0] * m[6] + st->bias_accel[1] * m[7] + st->bias_accel[2] * m[8];
...
/**
 *  inv_bias_accel_store() - Register dump for testing.
 */
static ssize_t inv_bias_accel_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	s8 *m;
	int x, y, z;
	...
	if(sscanf(buf, "%d,%d,%d",&x,&y,&z) < 0)
		return -1;
	st->bias_accel[0] = x * m[0] + y * m[3] + z * m[6];
	st->bias_accel[1] = x * m[1] + y * m[4] + z * m[7];
	st->bias_accel[2] = x * m[2] + y * m[5] + z * m[8];
#if 0
	inv_i2c_single_write(st, 0x77, (u8)(st->bias_accel[0] >> 8));
	inv_i2c_single_write(st, 0x78, (u8)(st->bias_accel[0]));

	inv_i2c_single_write(st, 0x7a, (u8)(st->bias_accel[1] >> 8));
	inv_i2c_single_write(st, 0x7b, (u8)(st->bias_accel[1]));
	
	inv_i2c_single_write(st, 0x7d, (u8)(st->bias_accel[2] >> 8));
	inv_i2c_single_write(st, 0x7e, (u8)(st->bias_accel[2]));
#endif
	return count;
}

/**
 *  inv_bias_gyro_show() - Register dump for testing.
 */
static ssize_t inv_bias_gyro_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	s8 *m;
	short x, y, z;
	...
	x = st->bias_gyro[0] * m[0] + st->bias_gyro[1] * m[1] + st->bias_gyro[2] * m[2];
	y = st->bias_gyro[0] * m[3] + st->bias_gyro[1] * m[4] + st->bias_gyro[2] * m[5];
	z = st->bias_gyro[0] * m[6] + st->bias_gyro[1] * m[7] + st->bias_gyro[2] * m[8];
...
/**
 *  inv_bias_gyro_store() - Register dump for testing.
 */
static ssize_t inv_bias_gyro_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	s8 *m;
	int x, y, z;
	...
	st->bias_gyro[0] = x * m[0] + y * m[3] + z * m[6];
	st->bias_gyro[1] = x * m[1] + y * m[4] + z * m[7];
	st->bias_gyro[2] = x * m[2] + y * m[5] + z * m[8];

	inv_i2c_single_write(st, 0x13, (u8)(st->bias_gyro[0] >> 8));
	inv_i2c_single_write(st, 0x14, (u8)(st->bias_gyro[0]));
	inv_i2c_single_write(st, 0x15, (u8)(st->bias_gyro[1] >> 8));
	inv_i2c_single_write(st, 0x16, (u8)(st->bias_gyro[1]));
	inv_i2c_single_write(st, 0x17, (u8)(st->bias_gyro[2] >> 8));
	inv_i2c_single_write(st, 0x18, (u8)(st->bias_gyro[2]));
...
#endif
static DEVICE_ATTR(read_accel, S_IRUGO, inv_read_accel_show, NULL);
static DEVICE_ATTR(read_gyro, S_IRUGO, inv_read_gyro_show, NULL);
static DEVICE_ATTR(bias_accel, S_IRUGO | S_IWUSR, inv_bias_accel_show, inv_bias_accel_store);
static DEVICE_ATTR(bias_gyro, S_IRUGO | S_IWUSR, inv_bias_gyro_show, inv_bias_gyro_store);
...
static const struct attribute *inv_gyro_attributes[] = {
	...
	&dev_attr_read_accel.attr,
	&dev_attr_read_gyro.attr,
	&dev_attr_bias_accel.attr,
	&dev_attr_bias_gyro.attr,
};
	...
	const struct i2c_device_id *id)
{
	...
	if (st->plat_data.irq_init)
		st->plat_data.irq_init();
	...
	INIT_DELAYED_WORK(&st->resume_work, resume_work_fn);
...
static int inv_mpu_resume(struct device *dev)
{
	...
	if (st->chip_type == INV_MPU3050)
		return set_power_mpu3050(st, true);
	else
		return set_power_itg_resume(st, true);
}
