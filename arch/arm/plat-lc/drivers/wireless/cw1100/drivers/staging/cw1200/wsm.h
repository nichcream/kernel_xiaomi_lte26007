...
int wsm_set_bss_lost_count(struct cw1200_common *priv,
			const struct wsm_set_bss_params *arg);
...
/* 4.18 Statistics Table */
struct wsm_signal_info {
	__le16 LatestSnr;
	u8 LatestRcpi;
	s8 LatestRssi;
} __packed;

static inline int wsm_get_signal(struct cw1200_common *priv,
					  int *signal)
{
	int ret;
	struct wsm_signal_info signal_info;
	ret = wsm_read_mib(priv, WSM_MIB_ID_STATISTICS_TABLE,
			    &signal_info, sizeof(signal_info));
	if (!ret)
		*signal = (int)signal_info.LatestRssi;
	printk(KERN_DEBUG "signal_info.LatestRssi = %d\n", signal_info.LatestRssi);
	printk(KERN_DEBUG "signal_info.LatestRcpi = %d\n", signal_info.LatestRcpi);
	return ret;
}
