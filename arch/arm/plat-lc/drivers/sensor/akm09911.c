...
#define AKM_INPUT_DEVICE_NAME	"mag_akm"
...
int akm_compass_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (pdata->addr_amend){
		if (0 == pdata->addr_amend()){
			printk(KERN_ERR " get addr_amend fail\n");
			goto exit0;
		} else {
			client->addr = pdata->addr_amend();
		}
	}
	...
	if (s_akm->gpio_rstn) {
		gpio_request(s_akm->gpio_rstn, "akm_reset");
