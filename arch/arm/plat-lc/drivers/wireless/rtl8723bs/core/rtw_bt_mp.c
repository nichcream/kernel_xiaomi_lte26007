...
//	{
		...
		{
			h2cStatus = BT_STATUS_H2C_SUCCESS;
			...
			if(WaitC2Hevent(Adapter, &pMptCtx->MptH2cRspEvent, 600))
			{
				DBG_8192C("[MPT], Received WiFi MptH2cRspEvent!!!\n");
				if(WaitC2Hevent(Adapter, &pMptCtx->MptBtC2hEvent, 2000))
				{
...
#else
		AntForWifi = _FALSE;
		rtw_hal_set_hwreg(Adapter, HW_VAR_SWANT_OUTOFCOEX, &AntForWifi);
#endif
	} else {
		////1>. Open WiFi RF
	...
	)
{
	...
	if (Adapter->registrypriv.mp_mode != 1) {
		//DBG_871X_LEVEL(_drv_err_, "%s recv BtMpC2H under normal mode\n", __func__);
		return;
	}
