...
#define wsm_printk(...) do{\
	if(priv->wsm_enable_wsm_debug)\
		printk(__VA_ARGS__);\
}while(0)
//#else
//#define wsm_printk(...)
			       ...
			       struct cw1200_queue *queue)
{
	...
	case doTx:
	{
		...
		if (ieee80211_is_deauth(fctl))
			wsm_printk(KERN_ERR "[SCANABORT] deauth frame\n");
		else if(ieee80211_is_disassoc(fctl))
			wsm_printk(KERN_ERR "[SCANABORT] disassoc frame\n");

		if ((ieee80211_is_deauth(fctl) || ieee80211_is_disassoc(fctl)) &&
				priv->mode != NL80211_IFTYPE_AP) {
				wsm_printk(KERN_ERR "[SCANABORT] issuing unjoin command\n");
