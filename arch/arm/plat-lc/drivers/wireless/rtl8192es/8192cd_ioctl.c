...
/*modified for revoview because 0x8b30 is used by others. 20140703 */
//#define SIOCGIWRTLSTAINFO		0x8B30
#define SIOCGIWRTLSTAINFO		0x8B40//0x8B30
#define SIOCGIWRTLSTANUM		0x8B51//0x8B31
#define SIOCGIWRTLDRVVERSION	0x8B52//0x8B32
#define SIOCGIWRTLSCANREQ		0x8B53//0x8B33
#define SIOCGIWRTLGETBSSDB		0x8B54//0x8B34
#define SIOCGIWRTLJOINREQ		0x8B55//0x8B35
#define SIOCGIWRTLJOINREQSTATUS	0x8B56//0x8B36
...
#if defined (CONFIG_RTL_COMAPI_WLTOOLS) || defined(SUPPORT_MONITOR) || defined (WIFI_WPAS)
...
static struct iwpriv_arg mib_table[] = {
	...
	{"sc_get_sync_time",		INT_T,		_OFFSET(dot11StationConfigEntry.sc_get_sync_time), _SIZE(dot11StationConfigEntry.sc_get_sync_time), 10},
	{"sc_get_profile_time",		INT_T,		_OFFSET(dot11StationConfigEntry.sc_get_profile_time), _SIZE(dot11StationConfigEntry.sc_get_profile_time), 20},
	...
	{"sc_sync_vxd_to_root",		INT_T,		_OFFSET(dot11StationConfigEntry.sc_sync_vxd_to_root), _SIZE(dot11StationConfigEntry.sc_sync_vxd_to_root), 1},
...
#ifdef HS2_SUPPORT
/* Release 1 */
	{"cubeaconintval",	INT_T,  _OFFSET(hs2Entry.channel_utili_beaconIntval), _SIZE(hs2Entry.channel_utili_beaconIntval), 10},
...
#if defined(CONFIG_WLAN_HAL_8814AE) || defined(CONFIG_SDIO_HCI)
...
#if 0//def FOR_VHT5G_PF for 11ac logo ??
	{"swqaggnum",		RFFT_T, _OFFSET_RFFT(swq_aggnum), _SIZE_RFFT(swq_aggnum), 12},
#else
...
unsigned long rtl8192cd_debug_info=2;
	...
	while (((plist = asoc_list_get_next(priv, plist)) != phead) && (size > 0))
	{
		...
		if ((pstat->state & WIFI_ASOC_STATE) &&
			...
			if (priv->pmib->dot11BssType.net_work_type & WIRELESS_11N) {
				if (pstat->ht_cap_len) {
#ifdef WIFI_11N_2040_COEXIST
				unsigned char true_tx_bw = (pstat->tx_bw == HT_CHANNEL_WIDTH_20_40)
...
#ifdef WIFI_11N_2040_COEXIST
						pInfo->ht_info |= (TX_USE_40M_MODE&true_tx_bw);
#else
...
static int	rtl8192cd_join(struct rtl8192cd_priv *priv, unsigned char *data)
{
...
#ifdef WIFI_WPAS
	struct iw_scan_req* req = (struct iw_scan_req*)data;
#endif
	...
	{
#ifdef WIFI_WPAS
		if ((req->essid_len) && (req->essid_len <= 32))
		{
		    memcpy(priv->pmib->dot11Bss.ssid, req->essid, req->essid_len);
		    priv->pmib->dot11Bss.ssid[req->essid_len] = 0;
		    priv->pmib->dot11Bss.ssidlen = req->essid_len;
	    	}
		else
		    return -1;	
#else		
