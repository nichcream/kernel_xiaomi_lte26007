...
//	   SAVE_INT_AND_CLI(x);
	...
	if (GET_CHIP_VER(priv) == VERSION_8192E) {
		if (priv->pmib->dot11RFEntry.trswitch
			...
			|| priv->pshare->rf_ft_var.use_ext_pa 
#elif defined (HIGH_POWER_EXT_LNA)
			|| priv->pshare->rf_ft_var.use_ext_lna
#endif
			) {
...
#ifdef CONFIG_92ES_1T1R
printk("rtl8192cd_init_hw_PCI before 0x800=%x ,0xc04=%x,0x879=%x, 0x826=%x, 0x878=%x\n",RTL_R8(0x800),RTL_R8(0xc04),RTL_R8(0x879),RTL_R8(0x826),RTL_R8(0x878));
printk("rtl8192cd_init_hw_PCI for 1T1R\n");
RTL_W8(0x800,RTL_R8(0x800)|BIT1);
RTL_W8(0xc04,0x13);
RTL_W8(0x879,RTL_R8(0x879)&~BIT4);
RTL_W8(0x826,(RTL_R8(0x826)&~0x70)|0x30);
RTL_W8(0x878,(RTL_R8(0x878)&~0x78)|0x08);
printk("rtl8192cd_init_hw_PCI 0x800=%x ,0xc04=%x,0x879=%x, 0x826=%x, 0x878=%x\n",RTL_R8(0x800),RTL_R8(0xc04),RTL_R8(0x879),RTL_R8(0x826),RTL_R8(0x878));
#endif
//	RESTORE_INT(x);
	DBFEXIT;
...
#if defined(CONFIG_RTL_88E_SUPPORT) || defined(CONFIG_WLAN_HAL_8192EE) //CONFIG_WLAN_HAL_8192EE
	if (GET_CHIP_VER(priv) == VERSION_8188E || GET_CHIP_VER(priv) == VERSION_8192E) {
		if(GET_CHIP_VER(priv) == VERSION_8188E)
			set_dk_margin = 3;
...
u1Byte efuse_OneByteWrite(struct rtl8192cd_priv *priv, u2Byte addr, u1Byte *data, bool bPseudoTest)
{
	u1Byte	tmpidx = 0;
	u1Byte	bResult;
	...
	// -----------------e-fuse reg ctrl ---------------------------------
	//address
	RTL_W8(EFUSE_CTRL+1, (u1Byte)(addr&0xff));
	RTL_W8(EFUSE_CTRL+2,
	(RTL_R8(EFUSE_CTRL+2)&0xFC )|(u1Byte)((addr>>8)&0x03) );
...
static u1Byte efuse_write8(struct rtl8192cd_priv *priv, u2Byte address, u1Byte *value)
{
	return efuse_OneByteWrite(priv, address, value, _FALSE);
}

/*  11/16/2008 MH Read one byte from real Efuse. */
u1Byte efuse_OneByteRead(struct rtl8192cd_priv *priv, u2Byte addr, u1Byte *data, bool bPseudoTest)
{
	u1Byte	tmpidx = 0;
	u1Byte	bResult;
	...
	//address
	RTL_W8(EFUSE_CTRL+1, (u1Byte)(addr&0xff));
	RTL_W8(EFUSE_CTRL+2, ((u1Byte)((addr>>8) &0x03) ) |
	(RTL_R8(EFUSE_CTRL+2)&0xFC ));
...
static u1Byte efuse_read8(struct rtl8192cd_priv *priv, u2Byte address, u1Byte *value)
{
	return efuse_OneByteRead(priv, address, value, _FALSE);
}

/*
 * read/wirte raw efuse data
 */
u1Byte rtw_efuse_access(struct rtl8192cd_priv *priv, u1Byte bWrite, u2Byte start_addr, u2Byte cnts, u1Byte *data)
{
	int i = 0;
	u2Byte	real_content_len = 0, max_available_size = 0;
	u1Byte res = _FALSE;
	u1Byte  (*rw8)(struct rtl8192cd_priv *, u2Byte, u1Byte*);

	real_content_len = 512;
	max_available_size = 512 - 18;
	...
	efuse_PowerSwitch(priv, bWrite, _TRUE);
	...
	for (i = 0; i < cnts; i++) {
		...
		res = rw8(priv, start_addr++, data++);
		if (FAILED == res) break;
	}

	efuse_PowerSwitch(priv, bWrite, _FALSE);
...
#ifdef CONFIG_EFUSE_CONFIG_FILE
static u32 Hal_readPGDataFromConfigFile(	struct rtl8192cd_priv *priv,  u8 *PROMContent)
{
	u32 i;
	struct file *fp;
	mm_segment_t fs;
	...
	fp = filp_open("/system/etc/wifi/rtk_efuse_8192es.map", O_RDONLY,  0);
	if (IS_ERR(fp)) {
		DEBUG_INFO("Error, Efuse configure file doesn't exist.\n");
		return FALSE;
	}
	...
	DEBUG_INFO("Efuse configure file: /system/etc/wifi/rtk_efuse_8192es.map\n");
	for (i=0; i<priv->EfuseMapLen; i++) {
...
#ifdef CONFIG_EFUSE_CONFIG_FILE
	Hal_readPGDataFromConfigFile(priv,Efuse);
#endif		
}
...
int efuse_get(struct rtl8192cd_priv *priv, unsigned char *data)
{
	...
	ReadEFuse(priv, 0, 512, data);
	printk("\n");
	for (i=0; i < 512 ; i+=16) {
		printk("0x%03x  ", i);
		for (j=i; j < i+16 ; j++) {
			//rtw_efuse_access(priv, 0,j, 1, &read_byte);
			//ReadEFuse(priv, );
			read_byte = data[j];
			printk("%02x", read_byte);
			if ((j - i + 1)  == 8) printk("       ");
		}
		printk("\n");
	}
...
static u1Byte key_char2num(u1Byte ch)
{
    if((ch>='0')&&(ch<='9'))
        return ch - '0';
    else if ((ch>='a')&&(ch<='f'))
...
u1Byte key_2char2num(u1Byte hch, u1Byte lch)
{
    return ((key_char2num(hch) << 4) | key_char2num(lch));
}

//------------------------------------------------------------------------------
u1Byte rtw_efuse_map_read(struct rtl8192cd_priv *priv, u2Byte addr, u2Byte cnts, u1Byte *data)
{
	u2Byte	mapLen=0;

	mapLen = EFUSE_MAP_LEN_8192ES;
...
//------------------------------------------------------------------------------
u1Byte rtw_efuse_map_write(struct rtl8192cd_priv *priv, u2Byte addr, u2Byte cnts, u1Byte *data)
{
	u1Byte	offset, word_en;
	u1Byte	*map;
	u1Byte	newdata[PGPKT_DATA_SIZE];
	u4Byte	i, j, idx;
	u1Byte	ret = SUCCESS;
	u2Byte	mapLen=0;
	...
	ret = rtw_efuse_map_read(priv, 0, mapLen, map);
	if (ret == FAIL) goto exit;
	...
	do {
		...
		if (word_en != 0xF) {
			ret = efuse_PgPacketWrite(priv, offset, word_en, newdata);
			printk("offset=%x \n",offset);
			printk("word_en=%x \n",word_en);
...
int efuse_set(struct rtl8192cd_priv *priv, unsigned char *data)
{
	char *val;
	int j;
	u2Byte addr=0, cnts=0, max_available_size=0;
	unsigned char *pch, *ptmp, *token, *tmp[3]={0x00,0x00,0x00};
	u4Byte i, jj, kk;
	u8 *setdata = NULL;
#if 0
#ifdef CONFIG_RTL_92C_SUPPORT
	...
	if (strcmp(tmp[0], "wmap") == 0)
	{
		...
		max_available_size = EFUSE_MAP_LEN_8192ES;
			
		if ((addr+cnts) > max_available_size)
		{
			printk("%s: addr(0x%X)+cnts(%d) parameter error!\n", __FUNCTION__, addr, cnts);
			return 0;
		}

		if (rtw_efuse_map_write(priv, addr, cnts, setdata) == FAIL)
		{
			printk("%s: rtw_efuse_map_write error!!\n", __FUNCTION__);
			return 0;
		}
	}
...
#ifdef CONFIG_WLAN_HAL_8192EE
	if (GET_CHIP_VER(priv) == VERSION_8192E) {
		efuse_MAC = EEPROM_MAC_ADDR_8192EE;	
	}
#endif
...
void start_bbp_ch_load(struct rtl8192cd_priv *priv)
{
	if(GET_CHIP_VER(priv)==VERSION_8192E)
	{
	RTL_W16(0x894, 0xc350); //set ch load period time 200ms
	...
	{
		RTL_W16(0x990, 0xc350); //set ch load period time 200ms
