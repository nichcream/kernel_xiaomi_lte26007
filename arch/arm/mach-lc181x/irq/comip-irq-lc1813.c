...
static void comip_cp_irq_mask(struct irq_data *d)
{
	...
	val = readl(io_p2v(CTL_ARM2A7_INTR_EN));
	val &= ~(1 << cp_irq);
	writel(val, io_p2v(CTL_ARM2A7_INTR_EN));
}
...
static void comip_cp_irq_handler(unsigned int irq, struct irq_desc *desc)
{
	...
	status = readl(io_p2v(CTL_ARM2A7_INTR_STA));
	for_each_set_bit(bit, &status, NR_CP_IRQS) {
		/* Clear interrupt. */
		writel(1 << bit, io_p2v(CTL_ARM2A7_INTR_STA));

		/* Check interrupt status. */
		while(readl(io_p2v(CTL_ARM2A7_INTR_STA)) & (1 << bit)) {
			writel(1 << bit, io_p2v(CTL_ARM2A7_INTR_STA));
...
static void comip_cp_irq_init(void)
{
	...
	writel((1 << 0), io_p2v(CTL_ARM2A7_INTR_ALL_EN));
...
{
	irq = IRQ_TO_CP(irq);
	writel(irq, io_p2v(CTL_AP2ARM926_INTR_SET));
}
...
static int comip_gic_set_wake(struct irq_data *d, unsigned int on)
{
	...
	if (irq < 32)
		addr = io_p2v(AP_PWR_A7INTIN_MK0);
	else
		addr = io_p2v(AP_PWR_A7INTIN_MK1);
...
{
	writel(0xffffffff, io_p2v(AP_PWR_A7INTIN_MK0));
	writel(0xffffffff, io_p2v(AP_PWR_A7INTIN_MK1));
...
void __init comip_irq_init(void)
{
	gic_init(0, 29, __io_address(GIC_DIST_BASE),
		__io_address(CPU_CORE_BASE + 0x2000));
