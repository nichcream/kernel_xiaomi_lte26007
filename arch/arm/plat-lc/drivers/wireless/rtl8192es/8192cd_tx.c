...
//			get_txsc_AC(priv, priv->pmib->dot11RFEntry.dot11channel);
	...
	if(CheckCts2SelfEnable(RtsRate))
	{
		bRtsEnable = FALSE;
		bCts2SelfEnable = TRUE;
	}
...
int rtl88XX_signin_txdesc(struct rtl8192cd_priv *priv, struct tx_insn* txcfg, unsigned char convHdr)
{
	...
	for (i=0; i < txcfg->frg_num; i++)
	{
        ...
        if ((txcfg->privacy == _CCMP_PRIVACY_) && 
            ((UseSwCrypto(priv, txcfg->pstat, (txcfg->pstat ? FALSE : TRUE))) == FALSE)) {
            desc_data.secType = txcfg->privacy;
            desc_data.swCrypt = FALSE;
            desc_data.icv     = 0;
            desc_data.mic     = 0;
...
static void send_br_packet(struct rtl8192cd_priv *priv, struct sk_buff *skb, struct stat_info	*pstat)
{
	...
	txcfg->next_txpath = TXPATH_SLOW_PATH;
...
#endif		
		{
			if(memcmp(skb->data+ETH_ALEN, GET_MY_HWADDR, ETH_ALEN)) {	// ignore tx from root br to vap(client mode) if vap is not connected to remote ap successfully
...
#if defined(__KERNEL__) && (defined(CONFIG_USB_HCI) || defined(CONFIG_SDIO_HCI)) && !defined(SUPPORT_TX_AMSDU)
...
//	int priority;
...
#endif        
    {
        ...
        if ((priv->swq_en == 0) && (priv->assoc_num > 1) && (AMPDU_ENABLE))
        {
            int thd_value;
            ...
            if (priv->swq_txmac_chg >= thd_value);
            {
...
//#endif
			...
			if (txcfg->privacy == _TKIP_PRIVACY_) {
				#if 1
				if ((skb->tail + 8) > (skb->end)){
					struct sk_buff *pnewskb;
					pnewskb = skb_copy_expand(skb, skb_headroom(skb), 8, GFP_ATOMIC);
					if (NULL == pnewskb) {
						priv->ext_stats.tx_drops++;
						goto stop_proc;
					}		
...
#endif
		{
			...
			if (IS_HAL_CHIP(priv)) {
				...
				if((!priv->pmib->dot11OperationEntry.disable_txsc) &&
					...
					(txcfg->pstat) &&
					(txcfg->frg_num == 1) &&
					((txcfg->privacy == 0)	|| (!UseSwCrypto(priv, txcfg->pstat, FALSE))) &&
