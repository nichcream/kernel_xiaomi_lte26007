...
{
	PHY_IQCalibrate_8723B(padapter, bReCovery, _FALSE);
}
	...
	)
{
	...
	{
		rtw_write32(pAdapter, 0x948, 0);
	}
#endif
	DBG_871X("8723bs,Set Ant!\n");
	...
	DBG_871X("create random buffer content for MP test\n");

	pAdapter->TXradomBuffer = rtw_zmalloc(4096);
	if(pAdapter->TXradomBuffer == NULL)
	{
	...
	else
	{
		...
		pAdapter->TXradomBuffer[i] = prandom_u32() %0xFF;
#else
	...
	)
{
	...
	#endif
	if(pAdapter->TXradomBuffer != NULL) rtw_mfree(pAdapter->TXradomBuffer, 4096);
...
void SetPacketTx(PADAPTER padapter)
{
	...
	u32 pkt_size,offset,startPlace;
	struct tx_desc *desc;
	PTXDESC ptxdesc;
...
//	desc->txdw3 |= cpu_to_le32((pattrib->seqnum & 0xFFF) << SEQ_SHT);
	...
	_rtw_memcpy(hdr->addr3, pmp_priv->mp_network.network.MacAddress, ETH_ALEN);
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3,8,0))
	startPlace = (u32)(prandom_u32() % 3450);
#else
...
void Hal_ProSetCrystalCap(PADAPTER pAdapter, u32 CrystalCap)
{
	...
	PHY_SetBBReg(pAdapter , REG_AFE_XTAL_CTRL ,
		0x7FF800 , (CrystalCap | (CrystalCap << 6)));
}
