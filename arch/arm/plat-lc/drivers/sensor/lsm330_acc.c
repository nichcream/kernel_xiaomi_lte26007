...
static struct lsm330_acc_data *acc;
				...
				u8 * buf, int len)
{
	...
	if (err != 2) {
		printk(KERN_DEBUG "lsm330_acc read transfer error\n");
		err = -EIO;
	} else {
		err = 0;
	}
...
static int lsm330_acc_hw_init(struct lsm330_acc_data *acc)
{
	...
	if (err < 0) {
	printk(KERN_DEBUG "Error reading WHO_AM_I: is device available/working?\n");
		goto err_firstread;
	} else
		acc->hw_working = 1;
		...
		u8 reg_address, u8 mask, u8 new_bit_values, int resume_index)
{
	...
	return err;
	error:
		printk(KERN_DEBUG "lsm330_acc register %x update failed at step %d, error: %d\n",
			config[0], step, err);
	return err;
}
								...
								u8 new_fs_range)
{
	...
	if (atomic_read(&acc->enabled)) {
		...
		* which contains g range setting */
		...
		if(err < 0) {
			printk(KERN_DEBUG "lsm330_acc update g range failed\n");
			return err;
		}
		else
...
#endif
static struct device_attribute pollrate_ms_attribute = __ATTR(pollrate_ms, S_IRUGO|S_IWUSR, attr_get_polling_rate,attr_set_polling_rate);
static struct device_attribute range_attribute = __ATTR(range, S_IRUGO|S_IWUSR, attr_get_range, attr_set_range);
static struct device_attribute enable_device_attribute = __ATTR(enable_device, S_IRUGO|S_IWUSR, attr_get_enable, attr_set_enable);
static struct device_attribute enable_interrupt_output_attribute = __ATTR(enable_interrupt_output, S_IRUGO|S_IWUSR, attr_get_interr_enable,attr_set_interr_enable);
#ifdef DEBUG							
...
#endif
static struct attribute *lsm330_acc_attrs[] =
{
    &pollrate_ms_attribute.attr,
    &range_attribute.attr,
    &enable_device_attribute.attr,
    &enable_interrupt_output_attribute.attr,
    #ifdef DEBUG	
...
static struct attribute_group lsm330_acc_attribute_group = {
	.attrs = lsm330_acc_attrs,
};
...
//input_set_abs_params(acc->input_dev, ABS_WHEEL, INT_MIN, INT_MAX, 0, 0);
	...
	if (err) {
      ...
      err = sysfs_create_group(&acc->input_dev->dev.kobj, &lsm330_acc_attribute_group);
      if (err < 0) {
		printk(KERN_ERR "%s:could not create sysfs group for lsm330_acc\n", __func__);
		goto err2;
       }
	...
	return 0;
err2:
	sysfs_remove_group(&acc->input_dev->dev.kobj, &lsm330_acc_attribute_group);	
err1:
		...
		const struct i2c_device_id *id)
{
	...
	if (err < 0) {
		printk(KERN_DEBUG "lsm330_acc update_fs_range failed\n");
		goto  err_pdata_init;
	}
