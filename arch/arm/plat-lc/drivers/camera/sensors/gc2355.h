#ifndef __GC2355_H__
#define __GC2355_H__

#include "../comip-isp-dev.h"

#if defined(CONFIG_CPU_LC1860)
static struct v4l2_isp_parm gc2355_isp_parm = {
	.min_exposure = 0x01,
	.default_snapshot_exposure_ratio = 0x0100,
	.default_preview_exposure_ratio = 0x0100,
	.sensor_vendor = SENSOR_VENDOR_GALAXYCORE,
	.sensor_main_type = GC_2355,
	.sensor_sub_type = 1,
};

struct isp_regb_vals {
	unsigned int reg;
	unsigned char value;
};
#if CONFIG_VIDEO_COMIP_ISP

static struct isp_regb_vals focus_mode_infinity[] = {
	{0x1cca4, 0x00},    //OffsetInit
	{0x1cca5, 0xa0},
	{0x1cca6, 0x02},    //FullRange
	{0x1cca7, 0x00},

};

static struct isp_regb_vals focus_mode_normal[] = {
	{0x1cca4, 0x00},    //OffsetInit
	{0x1cca5, 0xa0},
	{0x1cca6, 0x02},    //FullRange
	{0x1cca7, 0x00},

};

static struct isp_regb_vals focus_mode_macro[] = {
	{0x1cca4, 0x01},    //OffsetInit
	{0x1cca5, 0x20},
	{0x1cca6, 0x02},    //FullRange
	{0x1cca7, 0xf0},
};

static struct isp_regb_vals isp_exposure_l2[] = {
	{0x1c146, 0x14},
	{0x1c5a0, 0x14},
};

static struct isp_regb_vals isp_exposure_l1[] = {
	{0x1c146, 0x1d},
	{0x1c5a0, 0x1d},
};

static struct isp_regb_vals isp_exposure_h0[] = {
	{0x1c146, 0x30},
	{0x1c5a0, 0x50},
};

static struct isp_regb_vals isp_exposure_h1[] = {
	{0x1c146, 0x68},
	{0x1c5a0, 0x68},
};

static struct isp_regb_vals isp_exposure_h2[] = {
	{0x1c146, 0x78},
	{0x1c5a0, 0x78},
};

static struct isp_regb_vals isp_iso_100[] = {
	{0x1c150, 0x00}, //max gain
	{0x1c151, 0x14}, //max gain
	{0x1c154, 0x00}, //min gain
	{0x1c155, 0x10}, //min gain
};

static struct isp_regb_vals isp_iso_200[] = {
	{0x1c150, 0x00},
	{0x1c151, 0x23},
	{0x1c154, 0x00},
	{0x1c155, 0x1d},
};

static struct isp_regb_vals isp_iso_400[] = {
	{0x1c150, 0x00},
	{0x1c151, 0x44},
	{0x1c154, 0x00},
	{0x1c155, 0x3c},
};

static struct isp_regb_vals isp_iso_800[] = {
	{0x1c150, 0x00},
	{0x1c151, 0x80},
	{0x1c154, 0x00},
	{0x1c155, 0x70},
};

static struct isp_regb_vals isp_contrast_l4[] = {
//Low gain Y-Curve
	{0x1c4c0, 0x10},
	{0x1c4c1, 0x20},
	{0x1c4c2, 0x30},
	{0x1c4c3, 0x40},
	{0x1c4c4, 0x50},
	{0x1c4c5, 0x60},
	{0x1c4c6, 0x70},
	{0x1c4c7, 0x80},
	{0x1c4c8, 0x90},
	{0x1c4c9, 0xa0},
	{0x1c4ca, 0xb0},
	{0x1c4cb, 0xc0},
	{0x1c4cc, 0xd0},
	{0x1c4cd, 0xe0},
	{0x1c4ce, 0xf0},
//High gain Y-Curve
	{0x1d963, 0x10},
	{0x1d964, 0x20},
	{0x1d965, 0x30},
	{0x1d966, 0x40},
	{0x1d967, 0x50},
	{0x1d968, 0x60},
	{0x1d969, 0x70},
	{0x1d96a, 0x80},
	{0x1d96b, 0x90},
	{0x1d96c, 0xa0},
	{0x1d96d, 0xb0},
	{0x1d96e, 0xc0},
	{0x1d96f, 0xd0},
	{0x1d970, 0xe0},
	{0x1d971, 0xf0},
//low gain UV curve 1/2
	{0x1d904, 0x7a},//0x93
	{0x1d905, 0x90},//0x9d
	{0x1d906, 0x9f},//0xa0
	{0x1d907, 0xa0},
	{0x1d908, 0xa0},
	{0x1d909, 0xa0},
	{0x1d90a, 0xa0},
	{0x1d90b, 0xa0},
	{0x1d90c, 0xa0},
	{0x1d90d, 0xa0},
	{0x1d90e, 0xa0},
	{0x1d90f, 0x9f},
	{0x1d910, 0x98},
	{0x1d911, 0x85},//0x89
	{0x1d912, 0x6d},//0x73
	{0x1d913, 0x50},//0x55

//high gain UV curve 1/2
	{0x1d914, 0x10},
	{0x1d915, 0x2c},
	{0x1d916, 0x3d},
	{0x1d917, 0x5c},
	{0x1d918, 0x7d},
	{0x1d919, 0x9f},
	{0x1d91a, 0xa0},
	{0x1d91b, 0xa0},
	{0x1d91c, 0xa0},
	{0x1d91d, 0xa0},
	{0x1d91e, 0xa0},
	{0x1d91f, 0x9f},
	{0x1d920, 0x98},
	{0x1d921, 0x85},
	{0x1d922, 0x6d},
	{0x1d923, 0x50},
};

static struct isp_regb_vals isp_contrast_l3[] = {
//Low gain Y-Curve
	{0x1c4c0, 0x10},
	{0x1c4c1, 0x20},
	{0x1c4c2, 0x30},
	{0x1c4c3, 0x40},
	{0x1c4c4, 0x50},
	{0x1c4c5, 0x60},
	{0x1c4c6, 0x70},
	{0x1c4c7, 0x80},
	{0x1c4c8, 0x90},
	{0x1c4c9, 0xa0},
	{0x1c4ca, 0xb0},
	{0x1c4cb, 0xc0},
	{0x1c4cc, 0xd0},
	{0x1c4cd, 0xe0},
	{0x1c4ce, 0xf0},
//High gain Y-Curve
	{0x1d963, 0x10},
	{0x1d964, 0x20},
	{0x1d965, 0x30},
	{0x1d966, 0x40},
	{0x1d967, 0x50},
	{0x1d968, 0x60},
	{0x1d969, 0x70},
	{0x1d96a, 0x80},
	{0x1d96b, 0x90},
	{0x1d96c, 0xa0},
	{0x1d96d, 0xb0},
	{0x1d96e, 0xc0},
	{0x1d96f, 0xd0},
	{0x1d970, 0xe0},
	{0x1d971, 0xf0},
//low gain UV curve 1/2
	{0x1d904, 0x7a},//0x93
	{0x1d905, 0x90},//0x9d
	{0x1d906, 0x9f},//0xa0
	{0x1d907, 0xa0},
	{0x1d908, 0xa0},
	{0x1d909, 0xa0},
	{0x1d90a, 0xa0},
	{0x1d90b, 0xa0},
	{0x1d90c, 0xa0},
	{0x1d90d, 0xa0},
	{0x1d90e, 0xa0},
	{0x1d90f, 0x9f},
	{0x1d910, 0x98},
	{0x1d911, 0x85},//0x89
	{0x1d912, 0x6d},//0x73
	{0x1d913, 0x50},//0x55

//high gain UV curve 1/2
	{0x1d914, 0x10},
	{0x1d915, 0x2c},
	{0x1d916, 0x3d},
	{0x1d917, 0x5c},
	{0x1d918, 0x7d},
	{0x1d919, 0x9f},
	{0x1d91a, 0xa0},
	{0x1d91b, 0xa0},
	{0x1d91c, 0xa0},
	{0x1d91d, 0xa0},
	{0x1d91e, 0xa0},
	{0x1d91f, 0x9f},
	{0x1d920, 0x98},
	{0x1d921, 0x85},
	{0x1d922, 0x6d},
	{0x1d923, 0x50},
};

static struct isp_regb_vals isp_contrast_l2[] = {
//Low gain Y-Curve
	{0x1c4c0, 0x0c},
	{0x1c4c1, 0x19},
	{0x1c4c2, 0x26},
	{0x1c4c3, 0x35},
	{0x1c4c4, 0x46},
	{0x1c4c5, 0x58},
	{0x1c4c6, 0x6a},
	{0x1c4c7, 0x7d},
	{0x1c4c8, 0x92},
	{0x1c4c9, 0xa5},
	{0x1c4ca, 0xb7},
	{0x1c4cb, 0xc9},
	{0x1c4cc, 0xda},
	{0x1c4cd, 0xe6},
	{0x1c4ce, 0xf3},
//High gain Y-Curve
	{0x1d963, 0x0c},
	{0x1d964, 0x19},
	{0x1d965, 0x26},
	{0x1d966, 0x35},
	{0x1d967, 0x46},
	{0x1d968, 0x58},
	{0x1d969, 0x6a},
	{0x1d96a, 0x7d},
	{0x1d96b, 0x92},
	{0x1d96c, 0xa5},
	{0x1d96d, 0xb7},
	{0x1d96e, 0xc9},
	{0x1d96f, 0xda},
	{0x1d970, 0xe6},
	{0x1d971, 0xf3},
//low gain UV curve 1/2
	{0x1d904, 0x7a},//0x93
	{0x1d905, 0x90},//0x9d
	{0x1d906, 0x9f},//0xa0
	{0x1d907, 0xa0},
	{0x1d908, 0xa0},
	{0x1d909, 0xa0},
	{0x1d90a, 0xa0},
	{0x1d90b, 0xa0},
	{0x1d90c, 0xa0},
	{0x1d90d, 0xa0},
	{0x1d90e, 0xa0},
	{0x1d90f, 0x9f},
	{0x1d910, 0x98},
	{0x1d911, 0x85},//0x89
	{0x1d912, 0x6d},//0x73
	{0x1d913, 0x50},//0x55

//high gain UV curve 1/2
	{0x1d914, 0x10},
	{0x1d915, 0x2c},
	{0x1d916, 0x3d},
	{0x1d917, 0x5c},
	{0x1d918, 0x7d},
	{0x1d919, 0x9f},
	{0x1d91a, 0xa0},
	{0x1d91b, 0xa0},
	{0x1d91c, 0xa0},
	{0x1d91d, 0xa0},
	{0x1d91e, 0xa0},
	{0x1d91f, 0x9f},
	{0x1d920, 0x98},
	{0x1d921, 0x85},
	{0x1d922, 0x6d},
	{0x1d923, 0x50},

};

static struct isp_regb_vals isp_contrast_l1[] = {
//Low gain Y-Curve
   	{0x1c4c0, 0x0a},
   	{0x1c4c1, 0x16},
   	{0x1c4c2, 0x22},
   	{0x1c4c3, 0x30},
   	{0x1c4c4, 0x42},
   	{0x1c4c5, 0x54},
  	{0x1c4c6, 0x67},
  	{0x1c4c7, 0x7b},
  	{0x1c4c8, 0x90},
  	{0x1c4c9, 0xa7},
  	{0x1c4ca, 0xbb},
  	{0x1c4cb, 0xce},
  	{0x1c4cc, 0xdd},
  	{0x1c4cd, 0xea},
  	{0x1c4ce, 0xf4},
//High gain Y-Curve
	{0x1d963, 0x0a},
	{0x1d964, 0x16},
	{0x1d965, 0x22},
	{0x1d966, 0x30},
	{0x1d967, 0x42},
	{0x1d968, 0x54},
	{0x1d969, 0x67},
	{0x1d96a, 0x7b},
	{0x1d96b, 0x90},
	{0x1d96c, 0xa7},
	{0x1d96d, 0xbb},
	{0x1d96e, 0xce},
	{0x1d96f, 0xdd},
	{0x1d970, 0xea},
	{0x1d971, 0xf4},
//low gain UV c1/2
	{0x1d904, 0x7a},//0x93
	{0x1d905, 0x90},//0x9d
	{0x1d906, 0x9f},//0xa0
	{0x1d907, 0xa0},
	{0x1d908, 0xa0},
	{0x1d909, 0xa0},
	{0x1d90a, 0xa0},
	{0x1d90b, 0xa0},
	{0x1d90c, 0xa0},
	{0x1d90d, 0xa0},
	{0x1d90e, 0xa0},
	{0x1d90f, 0x9f},
	{0x1d910, 0x98},
	{0x1d911, 0x85},//0x89
	{0x1d912, 0x6d},//0x73
	{0x1d913, 0x50},//0x55

//high gain UV curve 1/2
	{0x1d914, 0x10},
	{0x1d915, 0x2c},
	{0x1d916, 0x3d},
	{0x1d917, 0x5c},
	{0x1d918, 0x7d},
	{0x1d919, 0x9f},
	{0x1d91a, 0xa0},
	{0x1d91b, 0xa0},
	{0x1d91c, 0xa0},
	{0x1d91d, 0xa0},
	{0x1d91e, 0xa0},
	{0x1d91f, 0x9f},
	{0x1d920, 0x98},
	{0x1d921, 0x85},
	{0x1d922, 0x6d},
	{0x1d923, 0x50},

};

static struct isp_regb_vals isp_contrast_h0[] = {
	//Low gain Y-Curve
	{0x1c4c0, 0x06},
	{0x1c4c1, 0x0f},
	{0x1c4c2, 0x1b},
	{0x1c4c3, 0x2b},
	{0x1c4c4, 0x3d},
	{0x1c4c5, 0x50},
	{0x1c4c6, 0x65},
	{0x1c4c7, 0x7c},
	{0x1c4c8, 0x93},
	{0x1c4c9, 0xaa},
	{0x1c4ca, 0xbf},
	{0x1c4cb, 0xd3},
	{0x1c4cc, 0xe2},
	{0x1c4cd, 0xee},
	{0x1c4ce, 0xf7},
	//High gain Y-Curve
	{0x1d963, 0x06},
	{0x1d964, 0x0f},
	{0x1d965, 0x1b},
	{0x1d966, 0x2b},
	{0x1d967, 0x3d},
	{0x1d968, 0x50},
	{0x1d969, 0x65},
	{0x1d96a, 0x7c},
	{0x1d96b, 0x93},
	{0x1d96c, 0xaa},
	{0x1d96d, 0xbf},
	{0x1d96e, 0xd3},
	{0x1d96f, 0xe2},
	{0x1d970, 0xee},
	{0x1d971, 0xf7},
	//low gain UV curve 1/2
 	{0x1d904, 0x7a},//0x93
 	{0x1d905, 0x90},//0x9d
 	{0x1d906, 0x9f},//0xa0
 	{0x1d907, 0xa0},
 	{0x1d908, 0xa0},
 	{0x1d909, 0xa0},
 	{0x1d90a, 0xa0},
 	{0x1d90b, 0xa0},
 	{0x1d90c, 0xa0},
 	{0x1d90d, 0xa0},
 	{0x1d90e, 0xa0},
 	{0x1d90f, 0x9f},
 	{0x1d910, 0x98},
 	{0x1d911, 0x85},//0x89
 	{0x1d912, 0x6d},//0x73
 	{0x1d913, 0x50},//0x55

 //high gain UV curve 1/2
 	{0x1d914, 0x10},
 	{0x1d915, 0x2c},
 	{0x1d916, 0x3d},
 	{0x1d917, 0x5c},
 	{0x1d918, 0x7d},
 	{0x1d919, 0x9f},
 	{0x1d91a, 0xa0},
 	{0x1d91b, 0xa0},
 	{0x1d91c, 0xa0},
 	{0x1d91d, 0xa0},
 	{0x1d91e, 0xa0},
 	{0x1d91f, 0x9f},
 	{0x1d920, 0x98},
 	{0x1d921, 0x85},
 	{0x1d922, 0x6d},
 	{0x1d923, 0x50},

};

static struct isp_regb_vals isp_contrast_h1[] = {
//Low gain Y-Curve
	{0x1c4c0, 0x05},
	{0x1c4c1, 0x0d},
	{0x1c4c2, 0x17},
	{0x1c4c3, 0x26},
	{0x1c4c4, 0x38},
	{0x1c4c5, 0x4c},
	{0x1c4c6, 0x62},
	{0x1c4c7, 0x78},
	{0x1c4c8, 0x93},
	{0x1c4c9, 0xac},
	{0x1c4ca, 0xc4},
	{0x1c4cb, 0xd6},
	{0x1c4cc, 0xe5},
	{0x1c4cd, 0xf0},
	{0x1c4ce, 0xf7},
//High gain Y-Curve
	{0x1d963, 0x05},
	{0x1d964, 0x0d},
	{0x1d965, 0x17},
	{0x1d966, 0x26},
	{0x1d967, 0x38},
	{0x1d968, 0x4c},
	{0x1d969, 0x62},
	{0x1d96a, 0x78},
	{0x1d96b, 0x93},
	{0x1d96c, 0xac},
	{0x1d96d, 0xc4},
	{0x1d96e, 0xd6},
	{0x1d96f, 0xe5},
	{0x1d970, 0xf0},
	{0x1d971, 0xf7},
//low gain UV curve 1/2
	{0x1d904, 0x7a},//0x93
	{0x1d905, 0x90},//0x9d
	{0x1d906, 0x9f},//0xa0
	{0x1d907, 0xa0},
	{0x1d908, 0xa0},
	{0x1d909, 0xa0},
	{0x1d90a, 0xa0},
	{0x1d90b, 0xa0},
	{0x1d90c, 0xa0},
	{0x1d90d, 0xa0},
	{0x1d90e, 0xa0},
	{0x1d90f, 0x9f},
	{0x1d910, 0x98},
	{0x1d911, 0x85},//0x89
	{0x1d912, 0x6d},//0x73
	{0x1d913, 0x50},//0x55

//high gain UV curve 1/2
	{0x1d914, 0x10},
	{0x1d915, 0x2c},
	{0x1d916, 0x3d},
	{0x1d917, 0x5c},
	{0x1d918, 0x7d},
	{0x1d919, 0x9f},
	{0x1d91a, 0xa0},
	{0x1d91b, 0xa0},
	{0x1d91c, 0xa0},
	{0x1d91d, 0xa0},
	{0x1d91e, 0xa0},
	{0x1d91f, 0x9f},
	{0x1d920, 0x98},
	{0x1d921, 0x85},
	{0x1d922, 0x6d},
	{0x1d923, 0x50},
};
static struct isp_regb_vals isp_contrast_h2[] = {
//Low gain Y-Curve
	{0x1c4c0, 0x04},
	{0x1c4c1, 0x09},
	{0x1c4c2, 0x11},
	{0x1c4c3, 0x1f},
	{0x1c4c4, 0x2e},
	{0x1c4c5, 0x44},
	{0x1c4c6, 0x5e},
	{0x1c4c7, 0x76},
	{0x1c4c8, 0x94},
	{0x1c4c9, 0xb0},
	{0x1c4ca, 0xc9},
	{0x1c4cb, 0xde},
	{0x1c4cc, 0xeb},
	{0x1c4cd, 0xf3},
	{0x1c4ce, 0xfb},
//High gain Y-Curve
	{0x1d963, 0x04},
	{0x1d964, 0x09},
	{0x1d965, 0x11},
	{0x1d966, 0x1f},
	{0x1d967, 0x2e},
	{0x1d968, 0x44},
	{0x1d969, 0x5e},
	{0x1d96a, 0x76},
	{0x1d96b, 0x94},
	{0x1d96c, 0xb0},
	{0x1d96d, 0xc9},
	{0x1d96e, 0xde},
	{0x1d96f, 0xeb},
	{0x1d970, 0xf3},
	{0x1d971, 0xfb},
//low gain UV curve 1/2
	{0x1d904, 0x7a},//0x93
	{0x1d905, 0x90},//0x9d
	{0x1d906, 0x9f},//0xa0
	{0x1d907, 0xa0},
	{0x1d908, 0xa0},
	{0x1d909, 0xa0},
	{0x1d90a, 0xa0},
	{0x1d90b, 0xa0},
	{0x1d90c, 0xa0},
	{0x1d90d, 0xa0},
	{0x1d90e, 0xa0},
	{0x1d90f, 0x9f},
	{0x1d910, 0x98},
	{0x1d911, 0x85},//0x89
	{0x1d912, 0x6d},//0x73
	{0x1d913, 0x50},//0x55

//high gain UV curve 1/2
	{0x1d914, 0x10},
	{0x1d915, 0x2c},
	{0x1d916, 0x3d},
	{0x1d917, 0x5c},
	{0x1d918, 0x7d},
	{0x1d919, 0x9f},
	{0x1d91a, 0xa0},
	{0x1d91b, 0xa0},
	{0x1d91c, 0xa0},
	{0x1d91d, 0xa0},
	{0x1d91e, 0xa0},
	{0x1d91f, 0x9f},
	{0x1d920, 0x98},
	{0x1d921, 0x85},
	{0x1d922, 0x6d},
	{0x1d923, 0x50},

};

static struct isp_regb_vals isp_contrast_h3[] = {
//Low gain Y-Curve
	{0x1c4c0, 0x02},
	{0x1c4c1, 0x07},
	{0x1c4c2, 0x10},
	{0x1c4c3, 0x20},
	{0x1c4c4, 0x32},
	{0x1c4c5, 0x48},
	{0x1c4c6, 0x5e},
	{0x1c4c7, 0x78},
	{0x1c4c8, 0x91},
	{0x1c4c9, 0xb1},
	{0x1c4ca, 0xcb},
	{0x1c4cb, 0xe5},
	{0x1c4cc, 0xf2},
	{0x1c4cd, 0xfb},
	{0x1c4ce, 0xfd},
	//High gain Y-Curve
 	{0x1d963, 0x02},
 	{0x1d964, 0x07},
 	{0x1d965, 0x10},
 	{0x1d966, 0x20},
 	{0x1d967, 0x32},
 	{0x1d968, 0x48},
 	{0x1d969, 0x5e},
 	{0x1d96a, 0x78},
 	{0x1d96b, 0x91},
 	{0x1d96c, 0xb1},
 	{0x1d96d, 0xcb},
 	{0x1d96e, 0xe5},
 	{0x1d96f, 0xf2},
 	{0x1d970, 0xfb},
 	{0x1d971, 0xfd},
//low gain UV curve 1/2
	{0x1d904, 0x7a},//0x93
	{0x1d905, 0x90},//0x9d
	{0x1d906, 0x9f},//0xa0
	{0x1d907, 0xa0},
	{0x1d908, 0xa0},
	{0x1d909, 0xa0},
	{0x1d90a, 0xa0},
	{0x1d90b, 0xa0},
	{0x1d90c, 0xa0},
	{0x1d90d, 0xa0},
	{0x1d90e, 0xa0},
	{0x1d90f, 0x9f},
	{0x1d910, 0x98},
	{0x1d911, 0x85},//0x89
	{0x1d912, 0x6d},//0x73
	{0x1d913, 0x50},//0x55

//high gain UV curve 1/2
	{0x1d914, 0x10},
	{0x1d915, 0x2c},
	{0x1d916, 0x3d},
	{0x1d917, 0x5c},
	{0x1d918, 0x7d},
	{0x1d919, 0x9f},
	{0x1d91a, 0xa0},
	{0x1d91b, 0xa0},
	{0x1d91c, 0xa0},
	{0x1d91d, 0xa0},
	{0x1d91e, 0xa0},
	{0x1d91f, 0x9f},
	{0x1d920, 0x98},
	{0x1d921, 0x85},
	{0x1d922, 0x6d},
	{0x1d923, 0x50},
};
static struct isp_regb_vals isp_contrast_h4[] = {
//Low  gain Y-Curve
	{0x1c4c0, 0x38},
	{0x1c4c1, 0x47},
	{0x1c4c2, 0x51},
	{0x1c4c3, 0x5d},
	{0x1c4c4, 0x65},
	{0x1c4c5, 0x6f},
	{0x1c4c6, 0x76},
	{0x1c4c7, 0x7d},
	{0x1c4c8, 0x86},
	{0x1c4c9, 0x8f},
	{0x1c4ca, 0x99},
	{0x1c4cb, 0xa2},
	{0x1c4cc, 0xae},
	{0x1c4cd, 0xb8},
	{0x1c4ce, 0xd4},
//High gain Y-Curve
	{0x1d963, 0x38},
	{0x1d964, 0x47},
	{0x1d965, 0x51},
	{0x1d966, 0x5d},
	{0x1d967, 0x65},
	{0x1d968, 0x6f},
	{0x1d969, 0x76},
	{0x1d96a, 0x7d},
	{0x1d96b, 0x86},
	{0x1d96c, 0x8f},
	{0x1d96d, 0x99},
	{0x1d96e, 0xa2},
	{0x1d96f, 0xae},
	{0x1d970, 0xb8},
	{0x1d971, 0xd4},
//low gain UV curve 1/2
	{0x1d904, 0x7a},//0x93
	{0x1d905, 0x90},//0x9d
	{0x1d906, 0x9f},//0xa0
	{0x1d907, 0xa0},
	{0x1d908, 0xa0},
	{0x1d909, 0xa0},
	{0x1d90a, 0xa0},
	{0x1d90b, 0xa0},
	{0x1d90c, 0xa0},
	{0x1d90d, 0xa0},
	{0x1d90e, 0xa0},
	{0x1d90f, 0x9f},
	{0x1d910, 0x98},
	{0x1d911, 0x85},//0x89
	{0x1d912, 0x6d},//0x73
	{0x1d913, 0x50},//0x55

//high gain UV curve 1/2
	{0x1d914, 0x10},
	{0x1d915, 0x2c},
	{0x1d916, 0x3d},
	{0x1d917, 0x5c},
	{0x1d918, 0x7d},
	{0x1d919, 0x9f},
	{0x1d91a, 0xa0},
	{0x1d91b, 0xa0},
	{0x1d91c, 0xa0},
	{0x1d91d, 0xa0},
	{0x1d91e, 0xa0},
	{0x1d91f, 0x9f},
	{0x1d920, 0x98},
	{0x1d921, 0x85},
	{0x1d922, 0x6d},
	{0x1d923, 0x50},

};

static struct isp_regb_vals isp_saturation_l2[] = {
	{0x1c4eb, 0x60},
	{0x1c4ec, 0x50},
};

static struct isp_regb_vals isp_saturation_l1[] = {
	{0x1c4eb, 0x70},
	{0x1c4ec, 0x60},
};

static struct isp_regb_vals isp_saturation_h0[] = {
	{0x1c4eb, 0x80},
	{0x1c4ec, 0x50},
};

static struct isp_regb_vals isp_saturation_h1[] = {
	{0x1c4eb, 0x90},
	{0x1c4ec, 0x80},
};

static struct isp_regb_vals isp_saturation_h2[] = {
	{0x1c4eb, 0xa0},
	{0x1c4ec, 0x90},
};

static struct isp_regb_vals isp_white_balance_auto[] = {
	{0x1c17c, 0x01},
};

static struct isp_regb_vals isp_white_balance_daylight[] = {
	{0x1c17c, 0x03},
};

static struct isp_regb_vals isp_white_balance_cloudy_daylight[] = {
	{0x1c17c, 0x04},
};

static struct isp_regb_vals isp_white_balance_incandescent[] = {
	{0x1c17c, 0x05},
};

static struct isp_regb_vals isp_white_balance_fluorescent[] = {
	{0x1c17c, 0x06},
};

static struct isp_regb_vals isp_white_balance_warm_fluorescent[] = {
	{0x1c17c, 0x07},
};

static struct isp_regb_vals isp_white_balance_twilight[] = {
	{0x1c17c, 0x08},
};

static struct isp_regb_vals isp_white_balance_shade[] = {
	{0x1c17c, 0x09},
};

static struct isp_regb_vals isp_brightness_l3[] = {
	{0x1c5ac, 0x20},
	{0x1c5ad, 0x20},
	{0x1c5ae, 0x20},
};

static struct isp_regb_vals isp_brightness_l2[] = {
	{0x1c5ac, 0x40},
	{0x1c5ad, 0x40},
	{0x1c5ae, 0x40},
};

static struct isp_regb_vals isp_brightness_l1[] = {
	{0x1c5ac, 0x60},
	{0x1c5ad, 0x60},
	{0x1c5ae, 0x60},
};

static struct isp_regb_vals isp_brightness_h0[] = {
	{0x1c5ac, 0x80},
	{0x1c5ad, 0x80},
	{0x1c5ae, 0x80},
};

static struct isp_regb_vals isp_brightness_h1[] = {
	{0x1c5ac, 0xa0},
	{0x1c5ad, 0xa0},
	{0x1c5ae, 0xa0},
};

static struct isp_regb_vals isp_brightness_h2[] = {
	{0x1c5ac, 0xc0},
	{0x1c5ad, 0xc0},
	{0x1c5ae, 0xc0},
};

static struct isp_regb_vals isp_brightness_h3[] = {
	{0x1c5ac, 0xe0},
	{0x1c5ad, 0xe0},
	{0x1c5ae, 0xe0},
};

static struct isp_regb_vals isp_sharpness_l2[] = {
	{0x6560c, 0x00},
	{0x6560d, 0x0b},
};
static struct isp_regb_vals isp_sharpness_l1[] = {
	{0x6560c, 0x00},
	{0x6560d, 0x06},
};

static struct isp_regb_vals isp_sharpness_h0[] = {
	{0x6560c, 0x00},
	{0x6560d, 0x10},
};

static struct isp_regb_vals isp_sharpness_h1[] = {
	{0x6560c, 0x00},
	{0x6560d, 0x20},
};

static struct isp_regb_vals isp_sharpness_h2[] = {
	{0x6560c, 0x00},
	{0x6560d, 0x30},
};
#else
static struct isp_regb_vals isp_exposure_l2[] = {
	{0x1c146, 0x11},
};

static struct isp_regb_vals isp_exposure_l1[] = {
	{0x1c146, 0x22},
};

static struct isp_regb_vals isp_exposure_h0[] = {
	{0x1c146, 0x44},
};

static struct isp_regb_vals isp_exposure_h1[] = {
	{0x1c146, 0x88},
};

static struct isp_regb_vals isp_exposure_h2[] = {
	{0x1c146, 0xf0},
};

static struct isp_regb_vals isp_iso_100[] = {
	{0x1c150, 0x00}, //max gain
	{0x1c151, 0x14}, //max gain
	{0x1c154, 0x00}, //min gain
	{0x1c155, 0x10}, //min gain
};

static struct isp_regb_vals isp_iso_200[] = {
	{0x1c150, 0x00},
	{0x1c151, 0x23},
	{0x1c154, 0x00},
	{0x1c155, 0x1d},
};

static struct isp_regb_vals isp_iso_400[] = {
	{0x1c150, 0x00},
	{0x1c151, 0x44},
	{0x1c154, 0x00},
	{0x1c155, 0x3c},
};

static struct isp_regb_vals isp_iso_800[] = {
	{0x1c150, 0x00},
	{0x1c151, 0x80},
	{0x1c154, 0x00},
	{0x1c155, 0x70},
};

static struct isp_regb_vals isp_contrast_l3[] = {
	{0x67200, 0x00},
	{0x67201, 0x48},
	{0x67202, 0x00},
	{0x67203, 0x87},
	{0x67204, 0x00},
	{0x67205, 0xDD},
	{0x67206, 0x01},
	{0x67207, 0x1E},

	{0x67208, 0x01},
	{0x67209, 0x54},
	{0x6720A, 0x01},
	{0x6720B, 0x83}, //387
	{0x6720C, 0x01},
	{0x6720D, 0xAD},
	{0x6720E, 0x01},
	{0x6720F, 0xD3},
	{0x67210, 0x01},
	{0x67211, 0xF6},
	{0x67212, 0x02},
	{0x67213, 0x17}, //535

	{0x67214, 0x02},
	{0x67215, 0x36},
	{0x67216, 0x02},
	{0x67217, 0x53},
	{0x67218, 0x02},
	{0x67219, 0x6F}, //623
	{0x6721A, 0x02},
	{0x6721B, 0x8A},
	{0x6721C, 0x02},
	{0x6721D, 0xA3},
	{0x6721E, 0x02},
	{0x6721F, 0xBB},
	{0x67220, 0x02},
	{0x67221, 0xD3},
	{0x67222, 0x02},
	{0x67223, 0xEA}, //746
	{0x67224, 0x03},
	{0x67225, 0x00},
	{0x67226, 0x03},
	{0x67227, 0x15},
	{0x67228, 0x03},
	{0x67229, 0x2A}, //810
	{0x6722A, 0x03},
	{0x6722B, 0x3E},
	{0x6722C, 0x03},
	{0x6722D, 0x51},
	{0x6722E, 0x03},
	{0x6722F, 0x65}, //869
	{0x67230, 0x03},
	{0x67231, 0x77},
	{0x67232, 0x03},
	{0x67233, 0x89},
	{0x67234, 0x03},
	{0x67235, 0x9B}, //923
	{0x67236, 0x03},
	{0x67237, 0xAD},
	{0x67238, 0x03},
	{0x67239, 0xBE},
	{0x6723A, 0x03},
	{0x6723B, 0xCF},
	{0x6723C, 0x03},
	{0x6723D, 0xDF},
	{0x6723E, 0x03},
	{0x6723F, 0xEF},
	{0x67240, 0x03},
	{0x67241, 0xFF},
};

static struct isp_regb_vals isp_contrast_l2[] = {
	{0x67200, 0x00},
	{0x67201, 0x3C},
	{0x67202, 0x00},
	{0x67203, 0x75},
	{0x67204, 0x00},
	{0x67205, 0xC9}, //201
	{0x67206, 0x01},
	{0x67207, 0x0F},
	{0x67208, 0x01},
	{0x67209, 0x4F}, //335
	{0x6720A, 0x01},
	{0x6720B, 0x8A}, //394

	{0x6720C, 0x01},
	{0x6720D, 0xBE},
	{0x6720E, 0x01},
	{0x6720F, 0xEB}, //491
	{0x67210, 0x02},
	{0x67211, 0x15},
	{0x67212, 0x02},
	{0x67213, 0x3B}, //571

	{0x67214, 0x02},
	{0x67215, 0x5E}, //606
	{0x67216, 0x02},
	{0x67217, 0x7D},
	{0x67218, 0x02},
	{0x67219, 0x99}, //665
	{0x6721A, 0x02},
	{0x6721B, 0xB4},
	{0x6721C, 0x02},
	{0x6721D, 0xCE},
	{0x6721E, 0x02},
	{0x6721F, 0xE6}, //742
	{0x67220, 0x02},
	{0x67221, 0xFD},
	{0x67222, 0x03},
	{0x67223, 0x12}, //786
	{0x67224, 0x03},
	{0x67225, 0x26},
	{0x67226, 0x03},
	{0x67227, 0x39},
	{0x67228, 0x03},
	{0x67229, 0x4B}, //843

	{0x6722A, 0x03},
	{0x6722B, 0x5D},
	{0x6722C, 0x03},
	{0x6722D, 0x6E},
	{0x6722E, 0x03},
	{0x6722F, 0x7F}, //895
	{0x67230, 0x03},
	{0x67231, 0x8F},
	{0x67232, 0x03},
	{0x67233, 0x9E}, //926
	{0x67234, 0x03},
	{0x67235, 0xAD}, //941
	{0x67236, 0x03},
	{0x67237, 0xBC}, //956
	{0x67238, 0x03},
	{0x67239, 0xCA},
	{0x6723A, 0x03},
	{0x6723B, 0xD8},
	{0x6723C, 0x03},
	{0x6723D, 0xE5},
	{0x6723E, 0x03},
	{0x6723F, 0xF2},
	{0x67240, 0x03},
	{0x67241, 0xFF},
};

static struct isp_regb_vals isp_contrast_l1[] = {
	{0x67200, 0x00},
	{0x67201, 0x31},
	{0x67202, 0x00},
	{0x67203, 0x63},
	{0x67204, 0x00},
	{0x67205, 0xB6}, //182
	{0x67206, 0x01},
	{0x67207, 0x00},
	{0x67208, 0x01},
	{0x67209, 0x4A}, //330
	{0x6720A, 0x01},
	{0x6720B, 0x92}, //402

	{0x6720C, 0x01},
	{0x6720D, 0xCF},
	{0x6720E, 0x02},
	{0x6720F, 0x04}, //516
	{0x67210, 0x02},
	{0x67211, 0x34},
	{0x67212, 0x02},
	{0x67213, 0x5F}, //607

	{0x67214, 0x02},
	{0x67215, 0x85}, //645
	{0x67216, 0x02},
	{0x67217, 0xA7},
	{0x67218, 0x02},
	{0x67219, 0xC4}, //708
	{0x6721A, 0x02},
	{0x6721B, 0xDE}, //734
	{0x6721C, 0x02},
	{0x6721D, 0xF8},
	{0x6721E, 0x03},
	{0x6721F, 0x11}, //785
	{0x67220, 0x03},
	{0x67221, 0x26},
	{0x67222, 0x03},
	{0x67223, 0x3A}, //826
	{0x67224, 0x03},
	{0x67225, 0x4C},
	{0x67226, 0x03},
	{0x67227, 0x5D},
	{0x67228, 0x03},
	{0x67229, 0x6D}, //877

	{0x6722A, 0x03},
	{0x6722B, 0x7C},
	{0x6722C, 0x03},
	{0x6722D, 0x8B},
	{0x6722E, 0x03},
	{0x6722F, 0x99}, //921
	{0x67230, 0x03},
	{0x67231, 0xA7},
	{0x67232, 0x03},
	{0x67233, 0xB3}, //947
	{0x67234, 0x03},
	{0x67235, 0xC0}, //960
	{0x67236, 0x03},
	{0x67237, 0xCB}, //971
	{0x67238, 0x03},
	{0x67239, 0xD7},
	{0x6723A, 0x03},
	{0x6723B, 0xE2},
	{0x6723C, 0x03},
	{0x6723D, 0xEC},
	{0x6723E, 0x03},
	{0x6723F, 0xF6},
	{0x67240, 0x03},
	{0x67241, 0xFF},
};

static struct isp_regb_vals isp_contrast_h0[] = {
	{0x67200, 0x00},
	{0x67201, 0x25},
	{0x67202, 0x00},
	{0x67203, 0x52},
	{0x67204, 0x00},
	{0x67205, 0xA2}, //162
	{0x67206, 0x00},
	{0x67207, 0xF1},
	{0x67208, 0x01},
	{0x67209, 0x46}, //326
	{0x6720A, 0x01},
	{0x6720B, 0x99}, //409

	{0x6720C, 0x01},
	{0x6720D, 0xE0},
	{0x6720E, 0x02},
	{0x6720F, 0x1C}, //540
	{0x67210, 0x02},
	{0x67211, 0x54},
	{0x67212, 0x02},
	{0x67213, 0x82}, //642

	{0x67214, 0x02},
	{0x67215, 0xAD}, //685
	{0x67216, 0x02},
	{0x67217, 0xD0},
	{0x67218, 0x02},
	{0x67219, 0xEE}, //750
	{0x6721A, 0x03},
	{0x6721B, 0x09}, //777
	{0x6721C, 0x03},
	{0x6721D, 0x23},
	{0x6721E, 0x03},
	{0x6721F, 0x3B}, //827
	{0x67220, 0x03},
	{0x67221, 0x50},
	{0x67222, 0x03},
	{0x67223, 0x62}, //866
	{0x67224, 0x03},
	{0x67225, 0x72},
	{0x67226, 0x03},
	{0x67227, 0x81},
	{0x67228, 0x03},
	{0x67229, 0x8F}, //911

	{0x6722A, 0x03},
	{0x6722B, 0x9C},
	{0x6722C, 0x03},
	{0x6722D, 0xA8},
	{0x6722E, 0x03},
	{0x6722F, 0xB3}, //947
	{0x67230, 0x03},
	{0x67231, 0xBE},
	{0x67232, 0x03},
	{0x67233, 0xC8}, //968
	{0x67234, 0x03},
	{0x67235, 0xD2}, //978
	{0x67236, 0x03},
	{0x67237, 0xDB}, //987
	{0x67238, 0x03},
	{0x67239, 0xE3},
	{0x6723A, 0x03},
	{0x6723B, 0xEB},
	{0x6723C, 0x03},
	{0x6723D, 0xF2},
	{0x6723E, 0x03},
	{0x6723F, 0xF9},
	{0x67240, 0x03},
	{0x67241, 0xFF},
};

static struct isp_regb_vals isp_contrast_h1[] = {
	{0x67200, 0x00},
	{0x67201, 0x1D},
	{0x67202, 0x00},
	{0x67203, 0x46},
	{0x67204, 0x00},
	{0x67205, 0x95}, //149
	{0x67206, 0x00},
	{0x67207, 0xE7},
	{0x67208, 0x01},
	{0x67209, 0x43}, //323
	{0x6720A, 0x01},
	{0x6720B, 0x9E}, //414

	{0x6720C, 0x01},
	{0x6720D, 0xEB},
	{0x6720E, 0x02},
	{0x6720F, 0x2D}, //557
	{0x67210, 0x02},
	{0x67211, 0x68},
	{0x67212, 0x02},
	{0x67213, 0x9A}, //666

	{0x67214, 0x02},
	{0x67215, 0xC7}, //711
	{0x67216, 0x02},
	{0x67217, 0xEC},
	{0x67218, 0x03},
	{0x67219, 0x0A}, //778
	{0x6721A, 0x03},
	{0x6721B, 0x25}, //805
	{0x6721C, 0x03},
	{0x6721D, 0x3F},
	{0x6721E, 0x03},
	{0x6721F, 0x57}, //855
	{0x67220, 0x03},
	{0x67221, 0x6C},
	{0x67222, 0x03},
	{0x67223, 0x7D}, //893
	{0x67224, 0x03},
	{0x67225, 0x8C},
	{0x67226, 0x03},
	{0x67227, 0x99},
	{0x67228, 0x03},
	{0x67229, 0xA5}, //933

	{0x6722A, 0x03},
	{0x6722B, 0xB1},
	{0x6722C, 0x03},
	{0x6722D, 0x8B},
	{0x6722E, 0x03},
	{0x6722F, 0xC5}, //965
	{0x67230, 0x03},
	{0x67231, 0xCE},
	{0x67232, 0x03},
	{0x67233, 0xD6}, //982
	{0x67234, 0x03},
	{0x67235, 0xDE}, //990
	{0x67236, 0x03},
	{0x67237, 0xE5}, //997
	{0x67238, 0x03},
	{0x67239, 0xEC},
	{0x6723A, 0x03},
	{0x6723B, 0xF1},
	{0x6723C, 0x03},
	{0x6723D, 0xF6},
	{0x6723E, 0x03},
	{0x6723F, 0xFB},
	{0x67240, 0x03},
	{0x67241, 0xFF},
};

static struct isp_regb_vals isp_contrast_h2[] = {
	{0x67200, 0x00},
	{0x67201, 0x16},
	{0x67202, 0x00},
	{0x67203, 0x3A},
	{0x67204, 0x00},
	{0x67205, 0x87},
	{0x67206, 0x00},
	{0x67207, 0xDE},
	{0x67208, 0x01},
	{0x67209, 0x40},
	{0x6720A, 0x01},
	{0x6720B, 0xA3},

	{0x6720C, 0x01},
	{0x6720D, 0xF7},
	{0x6720E, 0x02},
	{0x6720F, 0x3D},
	{0x67210, 0x02},
	{0x67211, 0x7D},
	{0x67212, 0x02},
	{0x67213, 0xB2},

	{0x67214, 0x02},
	{0x67215, 0xE2},
	{0x67216, 0x03},
	{0x67217, 0x08},
	{0x67218, 0x03},
	{0x67219, 0x26},
	{0x6721A, 0x03},
	{0x6721B, 0x41},
	{0x6721C, 0x03},
	{0x6721D, 0x5C},
	{0x6721E, 0x03},
	{0x6721F, 0x74},
	{0x67220, 0x03},
	{0x67221, 0x88},
	{0x67222, 0x03},
	{0x67223, 0x98},
	{0x67224, 0x03},
	{0x67225, 0xA5},
	{0x67226, 0x03},
	{0x67227, 0xB1},
	{0x67228, 0x03},
	{0x67229, 0xBC},
	{0x6722A, 0x03},
	{0x6722B, 0xC5},
	{0x6722C, 0x03},
	{0x6722D, 0xCE},
	{0x6722E, 0x03},
	{0x6722F, 0xD6},
	{0x67230, 0x03},
	{0x67231, 0xDE},
	{0x67232, 0x03},
	{0x67233, 0xE4},
	{0x67234, 0x03},
	{0x67235, 0xEA},
	{0x67236, 0x03},
	{0x67237, 0xEF},
	{0x67238, 0x03},
	{0x67239, 0xF4},
	{0x6723A, 0x03},
	{0x6723B, 0xF8},
	{0x6723C, 0x03},
	{0x6723D, 0xFB},
	{0x6723E, 0x03},
	{0x6723F, 0xFD},
	{0x67240, 0x03},
	{0x67241, 0xFF},
};

static struct isp_regb_vals isp_contrast_h3[] = {
	{0x67200, 0x00},
	{0x67201, 0x0E},
	{0x67202, 0x00},
	{0x67203, 0x2E},
	{0x67204, 0x00},
	{0x67205, 0x7A},
	{0x67206, 0x00},
	{0x67207, 0xD4},
	{0x67208, 0x01},
	{0x67209, 0x3C},
	{0x6720A, 0x01},
	{0x6720B, 0xA8},

	{0x6720C, 0x02},
	{0x6720D, 0x02},
	{0x6720E, 0x02},
	{0x6720F, 0x4E},
	{0x67210, 0x02},
	{0x67211, 0x92},
	{0x67212, 0x02},
	{0x67213, 0xCA},

	{0x67214, 0x02},
	{0x67215, 0xFC},
	{0x67216, 0x03},
	{0x67217, 0x24},
	{0x67218, 0x03},
	{0x67219, 0x42},
	{0x6721A, 0x03},
	{0x6721B, 0x5E},
	{0x6721C, 0x03},
	{0x6721D, 0x78},
	{0x6721E, 0x03},
	{0x6721F, 0x90},
	{0x67220, 0x03},
	{0x67221, 0xA4},
	{0x67222, 0x03},
	{0x67223, 0xB3},
	{0x67224, 0x03},
	{0x67225, 0xBF},
	{0x67226, 0x03},
	{0x67227, 0xC9},
	{0x67228, 0x03},
	{0x67229, 0xD2},

	{0x6722A, 0x03},
	{0x6722B, 0xDA},
	{0x6722C, 0x03},
	{0x6722D, 0xE2},
	{0x6722E, 0x03},
	{0x6722F, 0xE8},
	{0x67230, 0x03},
	{0x67231, 0xEE},
	{0x67232, 0x03},
	{0x67233, 0xF2},
	{0x67234, 0x03},
	{0x67235, 0xF6},
	{0x67236, 0x03},
	{0x67237, 0xFA},
	{0x67238, 0x03},
	{0x67239, 0xFC},
	{0x6723A, 0x03},
	{0x6723B, 0xFE},
	{0x6723C, 0x03},
	{0x6723D, 0xFF},
	{0x6723E, 0x03},
	{0x6723F, 0xFF},
	{0x67240, 0x03},
	{0x67241, 0xFF},
};

static struct isp_regb_vals isp_saturation_l2[] = {
	{0x1c4eb, 0x60},
	{0x1c4ec, 0x50},
};

static struct isp_regb_vals isp_saturation_l1[] = {
	{0x1c4eb, 0x70},
	{0x1c4ec, 0x60},
};

static struct isp_regb_vals isp_saturation_h0[] = {
	{0x1c4eb, 0x80},
	{0x1c4ec, 0x70},
};

static struct isp_regb_vals isp_saturation_h1[] = {
	{0x1c4eb, 0x90},
	{0x1c4ec, 0x80},
};

static struct isp_regb_vals isp_saturation_h2[] = {
	{0x1c4eb, 0xa0},
	{0x1c4ec, 0x90},
};


static struct isp_regb_vals isp_white_balance_auto[] = {
	{0x1c190, 0x02}, //enable white balance
};

static struct isp_regb_vals isp_white_balance_daylight[] = {
	{0x1c190, 0x03}, //yueding
};

static struct isp_regb_vals isp_white_balance_cloudy_daylight[] = {
	{0x1c190, 0x04},
};
static struct isp_regb_vals isp_white_balance_incandescent[] = {
	{0x1c190, 0x05},
};

static struct isp_regb_vals isp_white_balance_fluorescent[] = {
	{0x1c190, 0x06},
};

static struct isp_regb_vals isp_white_balance_warm_fluorescent[] = {
	{0x1c190, 0x07},
};

static struct isp_regb_vals isp_white_balance_twilight[] = {
	{0x1c190, 0x08},
};

static struct isp_regb_vals isp_white_balance_shade[] = {
	{0x1c190, 0x09},
};

static struct isp_regb_vals isp_brightness_l3[] = {
	{0x1c1c5, 0x20},
	{0x1c1c6, 0x20},
	{0x1c1c7, 0x20},
};

static struct isp_regb_vals isp_brightness_l2[] = {
	{0x1c1c5, 0x40},
	{0x1c1c6, 0x40},
	{0x1c1c7, 0x40},
};

static struct isp_regb_vals isp_brightness_l1[] = {
	{0x1c1c5, 0x60},
	{0x1c1c6, 0x60},
	{0x1c1c7, 0x60},
};

static struct isp_regb_vals isp_brightness_h0[] = {
	{0x1c1c5, 0x80},
	{0x1c1c6, 0x80},
	{0x1c1c7, 0x80},
};

static struct isp_regb_vals isp_brightness_h1[] = {
	{0x1c1c5, 0xa0},
	{0x1c1c6, 0xa0},
	{0x1c1c7, 0xa0},
};

static struct isp_regb_vals isp_brightness_h2[] = {
	{0x1c1c5, 0xc0},
	{0x1c1c6, 0xc0},
	{0x1c1c7, 0xc0},
};

static struct isp_regb_vals isp_brightness_h3[] = {
	{0x1c1c5, 0xe0},
	{0x1c1c6, 0xe0},
	{0x1c1c7, 0xe0},
};

static struct isp_regb_vals isp_sharpness_l2[] = {
	{0x6560c, 0x00},
	{0x6560d, 0x0b},
};

static struct isp_regb_vals isp_sharpness_l1[] = {
	{0x6560c, 0x00},
	{0x6560d, 0x10},
};

static struct isp_regb_vals isp_sharpness_h0[] = {
	{0x6560c, 0x00},
	{0x6560d, 0x18},
};

static struct isp_regb_vals isp_sharpness_h1[] = {
	{0x6560c, 0x00},
	{0x6560d, 0x20},
};

static struct isp_regb_vals isp_sharpness_h2[] = {
	{0x6560c, 0x00},
	{0x6560d, 0x30},
};
#endif

static const struct v4l2_isp_regval gc2355_isp_setting[] = {
	// wingtech gc2355 isp performance setting
	// module No.5
	// rg ratio typical: 0x180
	// bg ratio typical: 0x16e
#if 1

	/* system register setting important*/
	{0x65004, 0x20},//disable edr, strange problem
	{0x1d998, 0x01},//disable manual blc trigger

	/* system initial */

	{0x60100, 0x01},//Software reset
	{0x6301b, 0xf0},//isp clock enable
	{0x63025, 0x40},//clock divider
	{0x63c12, 0x01},//data type
	{0x63c13, 0x22},//divider
	{0x63c14, 0x01},//men_thre
	{0x63c15, 0x53},//mem valid high&low number


	{0x1c5de, 0x00},//ov mode

		//chenshuang
	{0x65000, 0x3f},
	//chenshuang
	{0x65001, 0x6e},//turn off local boost
	//chenshuang
	{0x65002, 0x9b},//0x8b
	{0x65003, 0xff},
	{0x65004, 0x20},//disable edr
	{0x65005, 0x52},
	{0x65006, 0x02},//BGGR
	{0x65008, 0x00},
	{0x65009, 0x00},

	/*AECGC*/
	{0x1c174, 0x01},//bAECManualEnable:1: enable	 0: disable
	//{0x1c139, 0x01},//bFractalExposureExpressMode
	{0x1c179, 0x00},//nAdditonalExpo

	/* dynamic frame rate */
	{0x1c13a, 0x01},// turn on
	{0x1c17b, 0x02},// minimum 7.5 fps

	/*OV sensor需配置，非OVsensor不需要关注*/
	/* aecagc sensor address */
	{0x1c528, 0x20},//pSensorDeviceID[0]
	{0x1c529, 0x20},
	{0x1c52a, 0x09},//pSensorI2COption[0]
	{0x1c52b, 0x09},
	{0x1c52c, 0x02},
	{0x1c52d, 0x02},
	{0x1c52e, 0x02},
	{0x1c52f, 0x02},
	{0x1c530, 0x02},
	{0x1c531, 0x03},
	//{0x1c532, 0x00}, //about face detect
	//{0x1c533, 0x00}, // about face detect
	{0x1c534, 0x03},//vts 1
	{0x1c535, 0x40},
	{0x1c536, 0x03},//vts 2
	{0x1c537, 0x41},
	{0x1c538, 0x02},
	{0x1c539, 0x04},
	{0x1c53a, 0x02},
	{0x1c53b, 0x05},
	{0x1c53c, 0x00},
	{0x1c53d, 0x00},
	{0x1c53e, 0x00},
	{0x1c53f, 0x00},
	{0x1c540, 0x00},
	{0x1c541, 0x00},
	{0x1c542, 0x00},
	{0x1c543, 0x00},
	{0x1c55c, 0xff},
	{0x1c55d, 0xff},
	{0x1c55e, 0xff},
//	{0x1c55f, 0x00}, //about face detect
	{0x1c560, 0xff},
	{0x1c561, 0xff},
	{0x1c562, 0xff},
	{0x1c563, 0xff},
	{0x1c564, 0x00},
	{0x1c565, 0x00},
	{0x1c566, 0x00},
	{0x1c567, 0x00},
	/*由driver控制*/
	{0x1c140, 0x01},//banding
	{0x1c13f, 0x02},
	{0x1c164, 0x0f},//60Hz banding step
	{0x1c165, 0x80},
	{0x1c166, 0x21},//50Hz banding step
	{0x1c167, 0x4d},

	{0x1c13e, 0x02},//real gain mode

/* 自动曝光窗口部分*/
	{0x66401, 0x00},
	{0x66402, 0x30},//StatWin_Left
	{0x66403, 0x00},
	{0x66404, 0x2c},//StatWin_Top
	{0x66405, 0x00},
	{0x66406, 0x30},//StatWin_Right
	{0x66407, 0x00},
	{0x66408, 0x34},//StatWin_Bottom
	{0x66409, 0x00},//definiton ofthe center 3x3 window
	{0x6640a, 0xc8},//nWin_Left = 200
	{0x6640d, 0x00},
	{0x6640e, 0x96},//nWin_Top = 150
	{0x66411, 0x03},
	{0x66412, 0x70},//nWin_Width = 880
	{0x66415, 0x02},
	{0x66416, 0x94},//nWin_Height = 660
	{0x6642e, 0x01},//nWin_Weight_0 weight pass
	{0x6642f, 0x01},//nWin_Weight_1//01
	{0x66430, 0x01},//nWin_Weight_2//01
	{0x66431, 0x01},//nWin_Weight_3//01
	{0x66432, 0x02},//nWin_Weight_4 //02
	{0x66433, 0x02},//nWin_Weight_5//02
	{0x66434, 0x02},//nWin_Weight_6//02
	{0x66435, 0x02},//nWin_Weight_7//02
	{0x66436, 0x04},//nWin_Weight_8//04
	{0x66437, 0x02},//nWin_Weight_9//02
	{0x66438, 0x02},//nWin_Weight_10//02
	{0x66439, 0x02},//nWin_Weight_11//02
	{0x6643a, 0x02},//nWin_Weight_12//02
	{0x6644e, 0x03},//nWin_Weight_Shift
	{0x6644f, 0x00},//black level(BLC)//04
	{0x66450, 0xf8},//saturate level
	{0x6645b, 0x1a},//black weight1
	{0x6645d, 0x1a},//black weight2
	{0x66460, 0x04},//saturate per1
	{0x66464, 0x0a},//saturate per2
	{0x66467, 0x14},//saturate weight1
	{0x66469, 0x14},//saturate weight2
	//auto AE control
	{0x1c146, 0x30},//low AE target
	{0x1c5a0, 0x30},//high AE targe gt
	{0x1c14a, 0x03},
	{0x1c14b, 0x0a},//range
	//chenshuang
	//{0x1c14b, 0x0d},
	{0x1c14c, 0x0a},//aec fast step//
	{0x1c14e, 0x08},//slow step
	{0x1c138, 0x03},//frame number after sensor exp



	/* Raw Stretch */
	{0x65020, 0x01},//RAW Stretch Target
	{0x66500, 0x28},
	{0x66501, 0x00},//maxlowlevel
	{0x66502, 0xff},
	{0x66503, 0x1f},
	{0x1c1b0, 0xff},//nCurveAWBFixedGain[0]
	{0x1c1b1, 0xff},
	{0x1c1b2, 0x01},//nCurveAWBFixedGain[1]
	{0x65905, 0x08},
	{0x66301, 0x02},//high level step
	{0x66302, 0xd0},//ref bin
	{0x66303, 0x0a},//PsPer0
	{0x66304, 0x20},//PsPer1
	{0x1c5a2, 0x04},//target stable range
	{0x1c5a3, 0x06},//stretch target slow range
	{0x1c5a4, 0x01},//use new high stretch !!!
	{0x1c5a5, 0x20},//stretch low step
	{0x1c5a6, 0x20},//stretch high step
	{0x1c5a7, 0x08},//stretch slow range
	{0x1c5a8, 0x02},//stretch slow step
	{0x1c1b8, 0x08},//ratio scale


// add for hdr
	{0x1c470, 0x01},
	{0x1c471, 0x00},

	{0x6500c, 0x00},//HDR BLC
	{0x6500d, 0xff},//short
	{0x6500e, 0x00},//long
	{0x6500f, 0xff},
	// add end

	{0x1c174, 0x00},
	{0x1cd10, 0x00},//AF tracking affect 1c174

	/* De-noise */
	{0x65604, 0x00},//
	{0x65605, 0x00},//
	{0x65606, 0x00},//
	{0x65607, 0x00},//

	{0x65510, 0x0f},//G dns slope change from 0x4 to 0xf
	{0x6551a, 0x08},//Raw G Dns, 1x
	{0x6551b, 0x10},//2x
	{0x6551c, 0x40},//4x
	{0x6551d, 0xc0},//8x
	{0x6551e, 0xff},//16x
	{0x6551f, 0xff},//32x
	{0x65520, 0xff},//64x
	{0x65522, 0x00},//RAW BR De-noise
	{0x65523, 0x04},
	{0x65524, 0x00},
	{0x65525, 0x08},
	{0x65526, 0x00},
	{0x65527, 0x10},
	{0x65528, 0x00},
	{0x65529, 0x20},
	{0x6552a, 0x00},
	{0x6552b, 0x20},
	{0x6552c, 0x00},
	{0x6552d, 0x20},
	{0x6552e, 0x00},
	{0x6552f, 0x20},

	{0x65c00, 0x1f},//UV De-noise
	{0x65c01, 0x1f},
	{0x65c02, 0x1f},
	{0x65c03, 0x1f},
	{0x65c04, 0x1f},
	{0x65c05, 0x1f},

	{0x65614, 0x06},//high-frequency_denoise//M_threshold
	{0x65615, 0x14},//H_threshod
	{0x65619, 0xff},//L_threshold

/* sharpeness */
	{0x65600, 0x00},//min_gain[9:8]
	{0x65601, 0x10},//
	{0x65602, 0x00},//max_gain[9:8]
	{0x65603, 0x80},//
	{0x65608, 0x06},//unsharpen_mask
	{0x65609, 0x20},
	{0x6560c, 0x00},//min_sharpen
	{0x6560d, 0x0c},//max_sharpen
	{0x6560e, 0x10},//MinSharpenTp
	{0x6560f, 0x40},//MaxSharpenTp
	{0x65610, 0x10},//MinSharpenTm
	{0x65611, 0x60},//MaxSharpenTm
	{0x65613, 0x10},//SharpenAlpha
	{0x65615, 0x08},//HFreq_thre
	{0x65617, 0x06},//HFreq_coef

/* auto uv saturation */
	{0x1c4e8, 0x01},//Enable
	{0x1c4e9, 0x40},//bUVSaturateGainThre1
	{0x1c4ea, 0x80},//bUVSaturateGainThre2
	{0x1c4eb, 0x80}, //nUVMaxSaturation//0x80
	{0x1c4ec, 0x70}, //nUVMinSaturation//0x70

	/* Global Gamma */
	{0x1c49b, 0x01}, //Enable
	{0x1c49c, 0x02}, //manual gamma value [15:8]
	{0x1c49d, 0x01}, //gamma 2.0
	{0x1c49e, 0x02}, //RGB gamma value [15:8]
	{0x1c49f, 0x01}, //gamma 2.0
	{0x1c4a0, 0x00}, //nMaxCurveGain[15:8]
	{0x1c4a1, 0x18},
	{0x1c4a2, 0x00}, //nMaxRGBGammaGain[15:8]
	{0x1c4a3, 0x88}, //gamma 2.0 0310 //avoid false contour


	//dark boost
	{0x1c4b0, 0x02},//max gamma value for dark boost
	{0x1c4b1, 0x80},


/* BLC */
	{0x1c58b, 0x00},//avoid false contour
	{0x1c58c, 0x00},//avoid false contour
	/* Tone Mapping */
	//contrast curve 21

//Low gain Y-Curve
	{0x1c4c0, 0x06},
	{0x1c4c1, 0x0f},
	{0x1c4c2, 0x1b},
	{0x1c4c3, 0x2b},
	{0x1c4c4, 0x3d},
	{0x1c4c5, 0x50},
	{0x1c4c6, 0x65},
	{0x1c4c7, 0x7c},
	{0x1c4c8, 0x93},
	{0x1c4c9, 0xaa},
	{0x1c4ca, 0xbf},
	{0x1c4cb, 0xd3},
	{0x1c4cc, 0xe2},
	{0x1c4cd, 0xee},
	{0x1c4ce, 0xf7},

//High gain Y-Curve
	{0x1d963, 0x06},
	{0x1d964, 0x0f},
	{0x1d965, 0x1b},
	{0x1d966, 0x2b},
	{0x1d967, 0x3d},
	{0x1d968, 0x50},
	{0x1d969, 0x65},
	{0x1d96a, 0x7c},
	{0x1d96b, 0x93},
	{0x1d96c, 0xaa},
	{0x1d96d, 0xbf},
	{0x1d96e, 0xd3},
	{0x1d96f, 0xe2},
	{0x1d970, 0xee},
	{0x1d971, 0xf7},

	{0x1c4d4, 0x10},//EDR scale//min curve alpha
	{0x1c4d5, 0x20},//EDR scale //max curve alpha
	{0x1c4cf, 0x80}, //curve change step
	{0x65a00, 0x1b},
	{0x65a01, 0xc0},

//curve gain control
	{0x1c1b3, 0x60}, //Gain thre1
	{0x1c1b4, 0xd0},	//Gain thre2
	{0x1c1b5, 0x01}, //EDR gain control
	{0x1c1b6, 0x01}, //Curve Gain control
	{0x1c1b7, 0x40}, //after gamma cut ratio

//Manual UV curve 22
	//{0x1C998, 0x01},//16 registers
	{0x1c998, 0x00},//chenshuang
	{0x1C999, 0x27},
	{0x1C99A, 0x01},
	{0x1C99B, 0x3b},
	{0x1C99C, 0x01},
	{0x1C99D, 0x40},
	{0x1C99E, 0x01},
	{0x1C99F, 0x40},
	{0x1C9A0, 0x01},
	{0x1C9A1, 0x40},
	{0x1C9A2, 0x01},
	{0x1C9A3, 0x40},
	{0x1C9A4, 0x01},
	{0x1C9A5, 0x40},
	{0x1C9A6, 0x01},
	{0x1C9A7, 0x40},
	{0x1C9A8, 0x01},
	{0x1C9A9, 0x40},
	{0x1C9AA, 0x01},
	{0x1C9AB, 0x40},
	{0x1C9AC, 0x01},
	{0x1C9AD, 0x40},
	{0x1C9AE, 0x01},
	{0x1C9AF, 0x3E},
	{0x1C9B0, 0x01},
	{0x1C9B1, 0x31},
	{0x1C9B2, 0x01},
	{0x1C9B3, 0x13},
	{0x1C9B4, 0x00},
	{0x1C9B5, 0xE6},
	{0x1C9B6, 0x00},
	{0x1C9B7, 0xAA},

	{0x1d948, 0x00},//snapshot-preview RGB
	{0x1d94a, 0x00},//B
	{0x1d94b, 0x00},
	{0x1d94c, 0x00},//G
	{0x1d94d, 0x00},
	{0x1d94e, 0x00},//R
	{0x1d94f, 0x00},

/* LENC */
	{0x1c246, 0x01},//lens shading correction gain changes with camera gain
	{0x1c247, 0x02},//3 profile

	//chenshuang
	{0x1c254, 0x00},//threshold0
	{0x1c255, 0xdf},
	{0x1c256, 0x00},//threshold1
	{0x1c257, 0xf3},
	{0x1c258, 0x01},//threshold2
	{0x1c259, 0x8f},
	{0x1c25a, 0x01},//threshold3
	{0x1c25b, 0xde},

	{0x1c248, 0x40},//maxQ Long
	{0x1c249, 0x40},//maxQ Short
	{0x1c24a, 0x20},//minQ Long
	{0x1c24b, 0x20},//minQ Short

	{0x1c24c, 0x00}, //pLensGain[0][15:8]
	{0x1c24d, 0x40},
	{0x1c24e, 0x00},
	{0x1c24f, 0x80},

	{0x1c574, 0x00},//nMinLensOnlineGainThre[15:8]
	{0x1c575, 0x20},
	{0x1c576, 0x00},//nMaxLensOnlineGainThre[15:8]
	{0x1c577, 0xf0},
	{0x1c578, 0x40},//nMinLensOnlineApplyGain

	{0x65200, 0x0d},
	{0x65206, 0x3c},
	{0x65207, 0x04},
	{0x65208, 0x3e},
	{0x65209, 0x02},
	{0x6520a, 0x36},
	{0x6520b, 0x0c},
	{0x65214, 0x28},
	{0x65216, 0x20},


	/* OVISP LENC setting for A light Long Exposure (HDR/3D) */
	{0x1c310, 0x10}, //Y
	{0x1c311, 0x0c},
	{0x1c312, 0x0a},
	{0x1c313, 0x0a},
	{0x1c314, 0x0d},
	{0x1c315, 0x10},
	{0x1c316, 0x0b},
	{0x1c317, 0x06},
	{0x1c318, 0x05},
	{0x1c319, 0x05},
	{0x1c31a, 0x07},
	{0x1c31b, 0x0b},
	{0x1c31c, 0x08},
	{0x1c31d, 0x03},
	{0x1c31e, 0x00},
	{0x1c31f, 0x01},
	{0x1c320, 0x04},
	{0x1c321, 0x08},
	{0x1c322, 0x08},
	{0x1c323, 0x02},
	{0x1c324, 0x00},
	{0x1c325, 0x00},
	{0x1c326, 0x03},
	{0x1c327, 0x09},
	{0x1c328, 0x0b},
	{0x1c329, 0x07},
	{0x1c32a, 0x05},
	{0x1c32b, 0x05},
	{0x1c32c, 0x08},
	{0x1c32d, 0x0b},
	{0x1c32e, 0x12},
	{0x1c32f, 0x0c},
	{0x1c330, 0x0b},
	{0x1c331, 0x0b},
	{0x1c332, 0x0d},
	{0x1c333, 0x16},//cb
	{0x1c334, 0x22},
	{0x1c335, 0x21},
	{0x1c336, 0x21},
	{0x1c337, 0x1f},
	{0x1c338, 0x1e},
	{0x1c339, 0x1e},
	{0x1c33a, 0x1f},
	{0x1c33b, 0x1f},
	{0x1c33c, 0x1e},
	{0x1c33d, 0x1e},
	{0x1c33e, 0x1e},
	{0x1c33f, 0x1f},
	{0x1c340, 0x21},
	{0x1c341, 0x1f},
	{0x1c342, 0x1d},
	{0x1c343, 0x1d},
	{0x1c344, 0x1f},
	{0x1c345, 0x1f},
	{0x1c346, 0x1e},
	{0x1c347, 0x1e},
	{0x1c348, 0x1f},
	{0x1c349, 0x1e},
	{0x1c34a, 0x1f},
	{0x1c34b, 0x20},
	{0x1c34c, 0x1e},
	{0x1c34d, 0x28},//cr
	{0x1c34e, 0x2f},
	{0x1c34f, 0x2f},
	{0x1c350, 0x2f},
	{0x1c351, 0x27},
	{0x1c352, 0x2b},
	{0x1c353, 0x27},
	{0x1c354, 0x24},
	{0x1c355, 0x28},
	{0x1c356, 0x2d},
	{0x1c357, 0x28},
	{0x1c358, 0x21},
	{0x1c359, 0x1d},
	{0x1c35a, 0x22},
	{0x1c35b, 0x2b},
	{0x1c35c, 0x2b},
	{0x1c35d, 0x27},
	{0x1c35e, 0x24},
	{0x1c35f, 0x28},
	{0x1c360, 0x2d},
	{0x1c361, 0x27},
	{0x1c362, 0x2f},
	{0x1c363, 0x2f},
	{0x1c364, 0x2e},
	{0x1c365, 0x27},

	/* OVISP LENC setting for CWF light Long Exposure (HDR/3D) */
	{0x1c2ba, 0x0d},
	{0x1c2bb, 0x0a},
	{0x1c2bc, 0x08},
	{0x1c2bd, 0x09},
	{0x1c2be, 0x0a},
	{0x1c2bf, 0x10},
	{0x1c2c0, 0x09},
	{0x1c2c1, 0x06},
	{0x1c2c2, 0x04},
	{0x1c2c3, 0x04},
	{0x1c2c4, 0x07},
	{0x1c2c5, 0x0a},
	{0x1c2c6, 0x07},
	{0x1c2c7, 0x03},
	{0x1c2c8, 0x00},
	{0x1c2c9, 0x00},
	{0x1c2ca, 0x03},
	{0x1c2cb, 0x08},
	{0x1c2cc, 0x07},
	{0x1c2cd, 0x02},
	{0x1c2ce, 0x00},
	{0x1c2cf, 0x00},
	{0x1c2d0, 0x03},
	{0x1c2d1, 0x08},
	{0x1c2d2, 0x0a},
	{0x1c2d3, 0x06},
	{0x1c2d4, 0x04},
	{0x1c2d5, 0x04},
	{0x1c2d6, 0x07},
	{0x1c2d7, 0x0a},
	{0x1c2d8, 0x0f},
	{0x1c2d9, 0x0a},
	{0x1c2da, 0x09},
	{0x1c2db, 0x09},
	{0x1c2dc, 0x0b},
	{0x1c2dd, 0x15},
	{0x1c2de, 0x20},//cb
	{0x1c2df, 0x20},
	{0x1c2e0, 0x20},
	{0x1c2e1, 0x1f},
	{0x1c2e2, 0x1e},
	{0x1c2e3, 0x1e},
	{0x1c2e4, 0x1f},
	{0x1c2e5, 0x1f},
	{0x1c2e6, 0x1f},
	{0x1c2e7, 0x1e},
	{0x1c2e8, 0x1d},
	{0x1c2e9, 0x1f},
	{0x1c2ea, 0x21},
	{0x1c2eb, 0x1f},
	{0x1c2ec, 0x1d},
	{0x1c2ed, 0x1d},
	{0x1c2ee, 0x1e},
	{0x1c2ef, 0x1f},
	{0x1c2f0, 0x1e},
	{0x1c2f1, 0x1e},
	{0x1c2f2, 0x1e},
	{0x1c2f3, 0x1e},
	{0x1c2f4, 0x1e},
	{0x1c2f5, 0x1e},
	{0x1c2f6, 0x1f},
	{0x1c2f7, 0x24},//cr
	{0x1c2f8, 0x26},
	{0x1c2f9, 0x27},
	{0x1c2fa, 0x26},
	{0x1c2fb, 0x24},
	{0x1c2fc, 0x24},
	{0x1c2fd, 0x23},
	{0x1c2fe, 0x22},
	{0x1c2ff, 0x23},
	{0x1c300, 0x25},
	{0x1c301, 0x23},
	{0x1c302, 0x20},
	{0x1c303, 0x1f},
	{0x1c304, 0x21},
	{0x1c305, 0x25},
	{0x1c306, 0x24},
	{0x1c307, 0x23},
	{0x1c308, 0x21},
	{0x1c309, 0x23},
	{0x1c30a, 0x25},
	{0x1c30b, 0x22},
	{0x1c30c, 0x26},
	{0x1c30d, 0x25},
	{0x1c30e, 0x26},
	{0x1c30f, 0x25},

	/* OVISP LENC setting for DAY light Long Exposure (HDR/3D) */
	{0x1c264, 0x0e},
	{0x1c265, 0x0a},
	{0x1c266, 0x09},
	{0x1c267, 0x09},
	{0x1c268, 0x0b},
	{0x1c269, 0x10},
	{0x1c26a, 0x0a},
	{0x1c26b, 0x06},
	{0x1c26c, 0x04},
	{0x1c26d, 0x04},
	{0x1c26e, 0x07},
	{0x1c26f, 0x0a},
	{0x1c270, 0x07},
	{0x1c271, 0x03},
	{0x1c272, 0x00},
	{0x1c273, 0x00},
	{0x1c274, 0x03},
	{0x1c275, 0x08},
	{0x1c276, 0x07},
	{0x1c277, 0x02},
	{0x1c278, 0x00},
	{0x1c279, 0x00},
	{0x1c27a, 0x03},
	{0x1c27b, 0x09},
	{0x1c27c, 0x0a},
	{0x1c27d, 0x06},
	{0x1c27e, 0x04},
	{0x1c27f, 0x04},
	{0x1c280, 0x07},
	{0x1c281, 0x0b},
	{0x1c282, 0x10},
	{0x1c283, 0x0b},
	{0x1c284, 0x0a},
	{0x1c285, 0x0a},
	{0x1c286, 0x0c},
	{0x1c287, 0x15},
	{0x1c288, 0x20},
	{0x1c289, 0x20},
	{0x1c28a, 0x20},
	{0x1c28b, 0x1f},
	{0x1c28c, 0x1f},
	{0x1c28d, 0x1e},
	{0x1c28e, 0x1f},
	{0x1c28f, 0x1f},
	{0x1c290, 0x1f},
	{0x1c291, 0x1f},
	{0x1c292, 0x1e},
	{0x1c293, 0x20},
	{0x1c294, 0x21},
	{0x1c295, 0x20},
	{0x1c296, 0x1f},
	{0x1c297, 0x1d},
	{0x1c298, 0x1f},
	{0x1c299, 0x1f},
	{0x1c29a, 0x1f},
	{0x1c29b, 0x1f},
	{0x1c29c, 0x1e},
	{0x1c29d, 0x1e},
	{0x1c29e, 0x1f},
	{0x1c29f, 0x1f},
	{0x1c2a0, 0x1e},
	{0x1c2a1, 0x27},
	{0x1c2a2, 0x2a},
	{0x1c2a3, 0x2a},
	{0x1c2a4, 0x2a},
	{0x1c2a5, 0x25},
	{0x1c2a6, 0x27},
	{0x1c2a7, 0x25},
	{0x1c2a8, 0x23},
	{0x1c2a9, 0x25},
	{0x1c2aa, 0x29},
	{0x1c2ab, 0x25},
	{0x1c2ac, 0x20},
	{0x1c2ad, 0x1e},
	{0x1c2ae, 0x21},
	{0x1c2af, 0x27},
	{0x1c2b0, 0x27},
	{0x1c2b1, 0x25},
	{0x1c2b2, 0x23},
	{0x1c2b3, 0x25},
	{0x1c2b4, 0x28},
	{0x1c2b5, 0x25},
	{0x1c2b6, 0x2a},
	{0x1c2b7, 0x29},
	{0x1c2b8, 0x29},
	{0x1c2b9, 0x26},

	/* AWB */
	{0x66201, 0x52},
	{0x66203, 0x10},//crop window
	{0x66211, 0xe8},//awb top limit
	{0x66212, 0x04},//awb bottom limit

	{0x1c17c, 0x01},//CT mode
	{0x1c182, 0x08},
	{0x1c183, 0x00},//MinNum
	{0x1c184, 0x02},//AWB Step
	{0x1c58d, 0x00},//LimitAWBAtD65Enable

	{0x1c1be, 0x00},//AWB offset
	{0x1c1bf, 0x00},
	{0x1c1c0, 0x00},
	{0x1c1c1, 0x00},

	{0x1d948, 0x02}, //PreviewColorShiftType
	{0x1d94a, 0x00}, //AWBoffset2
	{0x1d94b, 0x00},
	{0x1d94c, 0x00},
	{0x1d94d, 0x00},
	{0x1d94e, 0x00},
	{0x1d94f, 0x00},

	{0x1c1aa, 0x00},//avgAllEnable
	{0x1c1ad, 0x02},//weight of A
	{0x1c1ae, 0x10},//weight of D65
	{0x1c1af, 0x04},//weight of CWF

	{0x1c5ac, 0x80},//pre-gain
	{0x1c5ad, 0x80},
	{0x1c5ae, 0x80},

	//{0x1ccce, 0x02},//awb shift
	{0x1ccce, 0x00},//chenshuang
	{0x1cccf, 0x00},//B gain for A
	{0x1ccd0, 0x10},//R gain for A
	{0x1c5b8, 0x00},//B gain for C outdoor
	{0x1c5b9, 0x40},//R gain for C outdoor
	{0x1ccd1, 0x00},//B gain for C indoor
	{0x1ccd2, 0x40},//R gain for C indoor
	{0x1ccd3, 0xe0},//B gain for D indoor
	{0x1ccd4, 0x50},//R gain for D indoor
	{0x1cccc, 0xe0},//B gain for D outdoor
	{0x1cccd, 0x50},//R gain for D outdoor

	{0x1c5b4, 0x02},//C indoor/outdoor switch lum 1 Richard@0517
	{0x1c5b5, 0xff},//C indoor/outdoor switch lum 1 Richard@0517
	{0x1c5b6, 0x04},//C indoor/outdoor switch lum 2 Richard@0517
	{0x1c5b7, 0xff},//C indoor/outdoor switch lum 2 Richard@0517

	{0x1ccd5, 0x3F},//CT_A
	{0x1ccd6, 0x59},//CT_C
	{0x1ccd7, 0x92},//CT_D

	//{0x1c5cd, 0x01},//high light awb shift, modified by Jiangtao to avoid blurish when high CT 0310
	{0x1c5cd, 0x00},//chenshuang
	{0x1c5ce, 0x00},
	{0x1c5cf, 0xf0},
	{0x1c5d0, 0x01},
	{0x1c5d1, 0x20},
	{0x1c5d2, 0x03},
	{0x1c5d3, 0x00},
	{0x1c5d4, 0x47},//
	{0x1c5d5, 0xa1},//
	{0x1c5d6, 0xb5},//
	{0x1c5d7, 0xf6},//
	{0x1c5d8, 0x40},//
	{0x1c1c2, 0x00},
	{0x1c1c3, 0x20},

	/* OVISP CTAWB setting for Long Exposure (HDR/3D)  h00206029 20120529 */
	{0x66206, 0x0c},
	{0x66207, 0x0d},
	{0x66208, 0x0c},
	{0x66209, 0x73},
	{0x6620a, 0x4c},
	{0x6620b, 0xca},
	{0x6620c, 0xad},
	{0x6620d, 0x49},
	{0x6620e, 0x2d},
	{0x6620f, 0x65},
	{0x66210, 0x41},
	{0x66201, 0x52},

	{0x1c1c8, 0x01},
	{0x1c1c9, 0x00},
	{0x1c1cc, 0x00},
	{0x1c1cd, 0xdd},
	{0x1c1d0, 0x02},
	{0x1c1d1, 0x5b},


		//dynamic CT AWB
	//chenshuang
	{0x1d924, 0x00}, //enable

	{0x1d950, 0x00}, //Br thres(supre highlight)
	{0x1d951, 0x20}, //Br thres
	{0x1d952, 0x30}, //Br Ratio
	{0x1d8dc, 0x00}, //Br thres0(high light)
	{0x1d8dd, 0xf0}, //Br thres0
	{0x1d8da, 0x10}, //Br Ratio0
	{0x1d8de, 0x44}, //Br thres1(middle light)
	{0x1d8df, 0x34}, //Br thres1
	{0x1d8db, 0x08}, //Br Ratio1

	{0x1d949, 0x0c}, //super highlight cwf thres //66206
	{0x1d925, 0x0c}, //highlight cwf thres
	{0x1d926, 0x0c}, //middlelight cwf thres
	{0x1d927, 0x0c}, //lowlight cwf thres

	{0x1d94a, 0x15}, //super highlight A thres //66207
	{0x1d928, 0x2f}, //highlight A thres
	{0x1d929, 0x2f}, //middlelight A thres
	{0x1d92a, 0x2f}, //lowlight A thres

	{0x1d94b, 0x0c}, //super highlight D thres //66208
	{0x1d92b, 0x0c}, //highlight D thres
	{0x1d92c, 0x0c}, //middlelight D thres
	{0x1d92d, 0x0c}, //lowlight D thres

	{0x1d94c, 0x4d}, //super highlight D limit //6620d
	{0x1d92e, 0x4d}, //highlight D limit
	{0x1d92f, 0x4d}, //middlelight D limit
	{0x1d930, 0x4d}, //lowlight D limit

	{0x1d94d, 0x33}, //super highlight A limit //6620e
	{0x1d931, 0x33}, //highlight A limit
	{0x1d932, 0x33}, //middlelight A limit
	{0x1d933, 0x33}, //lowlight A limit

	{0x1d94e, 0x74}, //super highlight D split //6620f
	{0x1d934, 0x74}, //highlight D split
	{0x1d935, 0x74}, //middlelight D split
	{0x1d936, 0x74}, //lowlight D split

	{0x1d94f, 0x48}, //super highlight A split //66210
	{0x1d937, 0x48}, //highlight A split
	{0x1d938, 0x48}, //middlelight A split
	{0x1d939, 0x48}, //lowlight A split


	/* Color matrix */
	{0x1C1C8, 0x01},//CT_C
	{0x1C1C9, 0x6D},//
	{0x1C1CC, 0x00},//CT_D
	{0x1C1CD, 0xDF},
	{0x1C1D0, 0x02},//CT_A
	{0x1C1D1, 0x01},

	/* Color matrix */
	{0x1C1D8, 0x01},//center matrix,pCCMCenterMatrix[0][0][0] [15:8]
	{0x1C1D9, 0xF0},
	{0x1C1DA, 0xFF},
	{0x1C1DB, 0x04},
	{0x1C1DC, 0x00},
	{0x1C1DD, 0x0C},
	{0x1C1DE, 0xFF},
	{0x1C1DF, 0x6B},
	{0x1C1E0, 0x02},
	{0x1C1E1, 0x1F},
	{0x1C1E2, 0xFF},
	{0x1C1E3, 0x76},
	{0x1C1E4, 0xFF},
	{0x1C1E5, 0x82},
	{0x1C1E6, 0xFE},
	{0x1C1E7, 0xFE},
	{0x1C1E8, 0x02},
	{0x1C1E9, 0x80},

	{0x1C1FC, 0xFF},//cmx left delta
	{0x1C1FD, 0xF3},
	{0x1C1FE, 0x00},
	{0x1C1FF, 0x30},
	{0x1C200, 0xFF},
	{0x1C201, 0xDD},
	{0x1C202, 0x00},
	{0x1C203, 0x27},
	{0x1C204, 0xFF},
	{0x1C205, 0xBC},
	{0x1C206, 0x00},
	{0x1C207, 0x1D},
	{0x1C208, 0x00},
	{0x1C209, 0x41},
	{0x1C20A, 0x00},
	{0x1C20B, 0x08},
	{0x1C20C, 0xFF},
	{0x1C20D, 0xB7},

	{0x1C220, 0x00},//cmx right delta
	{0x1C221, 0xb2},
	{0x1C222, 0xFF},
	{0x1C223, 0x76},
	{0x1C224, 0xff},
	{0x1C225, 0xd8},
	{0x1C226, 0x00},
	{0x1C227, 0x6e},
	{0x1C228, 0xFF},
	{0x1C229, 0xa7},
	{0x1C22A, 0xff},
	{0x1C22B, 0xeb},
	{0x1C22C, 0x00},
	{0x1C22D, 0x21},
	{0x1C22E, 0x00},
	{0x1C22F, 0xf0},
	{0x1C230, 0xFe},
	{0x1C231, 0xef},


	/* dpc */
	{0x65409, 0x04},//[6:0] wthre_list0, used for wthre calculation in auto-mode
	{0x6540a, 0x02},
	{0x6540b, 0x01},
	{0x6540c, 0x01},
	{0x6540d, 0x04},
	{0x6540e, 0x02},
	{0x6540f, 0x01},
	{0x65410, 0x01},
	{0x65408, 0x0b},

	{0x1d8fe, 0x01}, //UV cut gain control
	{0x1d8ff, 0x50}, //low gain thres
	{0x1d900, 0x70}, //high gain thres
	{0x1d97f, 0x14}, //UV cut low bright thres
	{0x1d973, 0x20}, //UV cut high bright thres
	{0x1d972, 0x00}, //adaptive gamma enable
	{0x1d974, 0x01}, //low gain gamma
	{0x1d975, 0xe6},
	{0x1d976, 0x01},//high gain gamma
	{0x1d977, 0xc0},
	{0x1d978, 0x01}, //dark image gamma
	{0x1d979, 0xb3},
	{0x1d97a, 0x88}, //low gain slope
	{0x1d97b, 0x68}, //high gain slope
	{0x1d97c, 0x48}, //dark image slope
	{0x1d97d, 0x14}, //low bright thres
	{0x1d97e, 0x20}, //high bright thres

	//chenshuang
	{0x1d99e, 0x01}, //dynamic UV curve
	//low gain UV curve 1/2
	{0x1d904, 0x7a},//0x930x93
	{0x1d905, 0x90},//0x9d0x9d
	{0x1d906, 0x9f},//0xa00xa0
	{0x1d907, 0xa0},
	{0x1d908, 0xa0},
	{0x1d909, 0xa0},
	{0x1d90a, 0xa0},
	{0x1d90b, 0xa0},
	{0x1d90c, 0xa0},
	{0x1d90d, 0xa0},
	{0x1d90e, 0xa0},
	{0x1d90f, 0x9f},
	{0x1d910, 0x98},
	{0x1d911, 0x85},//0x890x89
	{0x1d912, 0x6d},//0x730x73
	{0x1d913, 0x50},//0x550x55

//high gain UV curve 1/22
	{0x1d914, 0x10},
	{0x1d915, 0x2c},
	{0x1d916, 0x4d},
	{0x1d917, 0x6c},
	{0x1d918, 0x8d},
	{0x1d919, 0xa0},
	{0x1d91a, 0xa0},
	{0x1d91b, 0xa0},
	{0x1d91c, 0xa0},
	{0x1d91d, 0xa0},
	{0x1d91e, 0xa0},
	{0x1d91f, 0x9f},
	{0x1d920, 0x98},
	{0x1d921, 0x85},
	{0x1d922, 0x6d},
	{0x1d923, 0x50},

/* pre-set awb for D50(sunny day) D65(overcast/cloudy) CWF(fluorescent) and A(incandescent) */
	{0x1c4f0, 0x00},//BGain 0x1c17c -> 0x3
	{0x1c4f1, 0xab},
	{0x1c4f2, 0x00},//GGain
	{0x1c4f3, 0x80},
	{0x1c4f4, 0x00},//RGain
	{0x1c4f5, 0xe0},

	{0x1c4f6, 0x00},//BGain 0x1c17c -> 0x4(D65)
	{0x1c4f7, 0xA5},
	{0x1c4f8, 0x00},//GGain
	{0x1c4f9, 0x80},
	{0x1c4fa, 0x00},//RGain
	{0x1c4fb, 0xC5},

	{0x1c4fc, 0x01},//BGain 0x1c17c -> 0x5(Cwf)
	{0x1c4fd, 0x1f},
	{0x1c4fe, 0x00},//GGain
	{0x1c4ff, 0x80},
	{0x1c500, 0x00},//RGain
	{0x1c501, 0xB4},

	{0x1c502, 0x01},//BGain 0x1c17c -> 0x6(A)
	{0x1c503, 0x3A},
	{0x1c504, 0x00},//GGain
	{0x1c505, 0x80},
	{0x1c506, 0x00},//RGain
	{0x1c507, 0x80},

	{0x1c508, 0x01},//BGain 0x1c17c -> 0x7(H)
	{0x1c509, 0x6E},
	{0x1c50a, 0x00},//GGain
	{0x1c50b, 0x80},
	{0x1c50c, 0x00},//RGain
	{0x1c50d, 0x67},

	{0x1c50e, 0x01},//BGain 0x1c17c -> 0x8(TL84)
	{0x1c50f, 0x01},
	{0x1c510, 0x00},//GGain
	{0x1c511, 0x80},
	{0x1c512, 0x00},//RGain
	{0x1c513, 0x9C},


/* AF */
	{0x1cd0a, 0x01},
	{0x6502a, 0x2a},
	{0x66112, 0xC8},
	{0x66100, 0x00},	//AFHardwareControl//[2]:binning preview size =1,snapshot size =0

	{0x66104, 0x02},	//nX0
	{0x66105, 0x8e},
	{0x66106, 0x01},	//nY0
	{0x66107, 0xcb},
	{0x66108, 0x01},	//nW0
	{0x66109, 0x44},
	{0x6610a, 0x00},	//nH0
	{0x6610b, 0xf6},


	{0x1c5b0, 0x00},	//nWinMode
	{0x1cca4, 0x00},	//OffsetInit
	{0x1cca5, 0xa0},
	{0x1cca6, 0x02},	//FullRange
	{0x1cca7, 0x00},
	{0x1cca8, 0x00},	//AFCMinStep
	{0x1cca9, 0x0c},
	{0x1ccaa, 0x00},	//AFCCoarseStep
	{0x1ccab, 0x18},
	{0x1ccac, 0x00},	//AFCFineStep
	{0x1ccad, 0x0c},
	{0x1cd0b, 0x01},	//nAFMode:1:video servo AF;0:snapshot AF
	{0x1ccae, 0x00},	//nFrameRate
	{0x1ccaf, 0x1e},
	{0x1ccb2, 0x00},	//nMotorResTime
	{0x1ccb3, 0x20},
	{0x1cdc6, 0x09},	//nFocusI2COption:[3]:16 bit address;[2]:16bit data
	{0x1cdc7, 0x18},	//nFocusDeviceID
	{0x1cdc8, 0x30},	//pFocusMoveLensAddr_0
	{0x1cdc9, 0xec},
	{0x1cdca, 0x30},	//pFocusMoveLensAddr_1
	{0x1cdcb, 0xed},
	{0x1cddc, 0x01},	//nMotorDriveMode
	{0x1cddd, 0x00},	//bIfMotorCalibrate
	{0x1cd0e, 0x01},	//bIfPreMove
	{0x1cd05, 0x01},
	{0x1cd06, 0x01},


	{0x1cc92, 0x00},	//nT_Compare_1x:threshold for contrast compare@1x [15:8]
	{0x1cc93, 0x08},
	{0x1cc94, 0x00},	//nT_Compare_16x
	{0x1cc95, 0x0a},
	{0x1cc96, 0x00},	//nT_sad_1x:threshold for sad compare@1x [15:8]
	{0x1cc97, 0x00},
	{0x1cc98, 0x00},	//nT_sad_16x
	{0x1cc99, 0x00},
	{0x1cc9a, 0x00},	//nT_pre_sad_1x
	{0x1cc9b, 0x06},
	{0x1cc9c, 0x00},	//nT_pre_sad_16x
	{0x1cc9d, 0x04},
	{0x1cc9e, 0x00},	//nT_hist_1x
	{0x1cc9f, 0x02},
	{0x1cca0, 0x00},	//nT_hist_16x
	{0x1cca1, 0x02},
	{0x1cca2, 0x00},	//nT_contrast_diff
	{0x1cca3, 0x08},

	{0x1cd08, 0x00},
	{0x1ccef, 0x00},
	{0x1cdde, 0x04},
	{0x1cdcc, 0x0f},
	{0x1cdcd, 0xff},
	{0x1cdd8, 0x20},
	{0x1cdd9, 0x20},
	{0x1cdda, 0x38},
	{0x1ccca, 0x58},
	{0x1ccb1, 0x02},
	{0x1cd05, 0x00},
	{0x1cd0d, 0x36},
	{0x1ccc9, 0x1f},

#else
/* system register setting important*/
	{0x65004, 0x20},//disable edr, strange problem
	{0x1d998, 0x01},//disable manual blc trigger

	/* system initial */

	{0x60100, 0x01},//Software reset
	{0x6301b, 0xf0},//isp clock enable
	{0x63025, 0x40},//clock divider
	{0x63c12, 0x01},//data type
	{0x63c13, 0x22},//divider
	{0x63c14, 0x01},//men_thre
	{0x63c15, 0x53},//mem valid high&low number

	/* ISP TOP REG */
	{0x65000, 0x3f},
	{0x65001, 0x6f},//turn off local boost

	{0x65002, 0x9b},
	{0x65003, 0xff},
	{0x65004, 0x20},//disable edr
	{0x65005, 0x52},
	{0x65006, 0x00},
	{0x65008, 0x00},
	{0x65009, 0x00},

	/*AECGC*/
	{0x1c174, 0x01},//bAECManualEnable:1: enable     0: disable
	{0x1c139, 0x00},//bFractalExposureExpressMode
	{0x1c179, 0x00},//nAdditonalExpo

	/* dynamic frame rate */
	{0x1c13a, 0x01},// turn on
	{0x1c17b, 0x00},// minimum 7.5 fps

	/*OV sensor需配置，非OVsensor不需要关注*/
	/* aecagc sensor address */
	{0x1c528, 0x6c},//pSensorDeviceID[0]
	{0x1c529, 0x6c},
	{0x1c52a, 0x09},//pSensorI2COption[0]
	{0x1c52b, 0x09},
	{0x1c52c, 0x35},
	{0x1c52d, 0x00},
	{0x1c52e, 0x35},
	{0x1c52f, 0x01},
	{0x1c530, 0x35},
	{0x1c531, 0x02},
	{0x1c534, 0x38},//vts 1
	{0x1c535, 0x0e},
	{0x1c536, 0x38},//vts 2
	{0x1c537, 0x0f},
	{0x1c538, 0x35},
	{0x1c539, 0x0a},
	{0x1c53a, 0x35},
	{0x1c53b, 0x0b},
	{0x1c53c, 0x00},
	{0x1c53d, 0x00},
	{0x1c53e, 0x00},
	{0x1c53f, 0x00},
	{0x1c540, 0x00},
	{0x1c541, 0x00},
	{0x1c542, 0x00},
	{0x1c543, 0x00},
	{0x1c55c, 0xff},
	{0x1c55d, 0xff},
	{0x1c55e, 0xff},
	{0x1c560, 0xff},
	{0x1c561, 0xff},
	{0x1c562, 0xff},
	{0x1c563, 0xff},
	{0x1c564, 0x00},
	{0x1c565, 0x00},
	{0x1c566, 0x00},
	{0x1c567, 0x00},

	/*由driver控制*/
	{0x1c13f, 0x02},
	{0x1c164, 0x0f},//60Hz banding step
	{0x1c165, 0x80},
	{0x1c166, 0x21},//50Hz banding step
	{0x1c167, 0x33},

	{0x1c13e, 0x02},//real gain mode

/* 自动曝光窗口部分*/
	{0x66401, 0x00},
	{0x66402, 0x30},//StatWin_Left
	{0x66403, 0x00},
	{0x66404, 0x2c},//StatWin_Top
	{0x66405, 0x00},
	{0x66406, 0x30},//StatWin_Right
	{0x66407, 0x00},
	{0x66408, 0x34},//StatWin_Bottom
	{0x66409, 0x01},//definiton ofthe center 3x3 window
	{0x6640a, 0x38},//nWin_Left = 312
	{0x6640d, 0x00},
	{0x6640e, 0xea},//nWin_Top = 234
	{0x66411, 0x0a},
	{0x66412, 0x50},//nWin_Width = 2640
	{0x66415, 0x07},
	{0x66416, 0xbc},//nWin_Height = 1980
	{0x6642e, 0x01},//nWin_Weight_0 weight pass
	{0x6642f, 0x01},//nWin_Weight_1
	{0x66430, 0x01},//nWin_Weight_2
	{0x66431, 0x01},//nWin_Weight_3
	{0x66432, 0x02},//nWin_Weight_4
	{0x66433, 0x02},//nWin_Weight_5
	{0x66434, 0x02},//nWin_Weight_6
	{0x66435, 0x02},//nWin_Weight_7
	{0x66436, 0x04},//nWin_Weight_8
	{0x66437, 0x02},//nWin_Weight_9
	{0x66438, 0x02},//nWin_Weight_10
	{0x66439, 0x02},//nWin_Weight_11
	{0x6643a, 0x02},//nWin_Weight_12
	{0x6644e, 0x03},//nWin_Weight_Shift
	{0x6644f, 0x04},//black level(BLC)
	{0x66450, 0xf8},//saturate level
	{0x6645b, 0x1a},//black weight1
	{0x6645d, 0x1a},//black weight2
	{0x66460, 0x04},//saturate per1
	{0x66464, 0x0a},//saturate per2
	{0x66467, 0x14},//saturate weight1
	{0x66469, 0x14},//saturate weight2
	//auto AE control
	{0x1c146, 0x30},//low AE target
	{0x1c5a0, 0x44},//high AE target
	{0x1c14a, 0x03},
	{0x1c14b, 0x0a},
	{0x1c14c, 0x0a},//aec fast step//
	{0x1c14e, 0x08},//slow step
	{0x1c140, 0x01},//banding



	/* Raw Stretch */
	{0x65020, 0x01},//RAW Stretch Target
	{0x66500, 0x28},
	{0x66501, 0x03},//maxlowlevel
	{0x66502, 0xff},
	{0x66503, 0x1f},
	{0x1c1b0, 0xff},//nCurveAWBFixedGain[0]
	{0x1c1b1, 0xff},
	{0x1c1b2, 0x01},//nCurveAWBFixedGain[1]
	{0x65905, 0x08},
	{0x66301, 0x02},//high level step
	{0x66302, 0xd0},//ref bin
	{0x66303, 0x0a},//PsPer0
	{0x66304, 0x20},//PsPer1
	{0x1c5a2, 0x04},//target stable range
	{0x1c5a3, 0x06},//stretch target slow range
	{0x1c5a4, 0x01},//use new high stretch !!!
	{0x1c5a5, 0x20},//stretch low step
	{0x1c5a6, 0x20},//stretch high step
	{0x1c5a7, 0x08},//stretch slow range
	{0x1c5a8, 0x02},//stretch slow step
	{0x1c1b8, 0x08},//ratio scale

	// add for hdr
	{0x1c470, 0x01},
	{0x1c471, 0x00},
	// add end
	{0x1c174, 0x00},



	/* De-noise */
	{0x65604, 0x00},//
	{0x65605, 0x00},//
	{0x65606, 0x00},//
	{0x65607, 0x00},//

	{0x65510, 0x0f},//G dns slope change from 0x4 to 0xf
	{0x6551a, 0x01},//Raw G Dns, 1x
	{0x6551b, 0x02},//2x
	{0x6551c, 0x04},//4x
	{0x6551d, 0x05},//8x
	{0x6551e, 0x06},//16x
	{0x6551f, 0x07},//32x
	{0x65520, 0x08},//64x
	{0x65522, 0x00},//RAW BR De-noise
	{0x65523, 0x04},
	{0x65524, 0x00},
	{0x65525, 0x08},
	{0x65526, 0x00},
	{0x65527, 0x10},
	{0x65528, 0x00},
	{0x65529, 0x20},
	{0x6552a, 0x00},
	{0x6552b, 0x20},
	{0x6552c, 0x00},
	{0x6552d, 0x20},
	{0x6552e, 0x00},
	{0x6552f, 0x20},

	{0x65c00, 0x03},//UV De-noise
	{0x65c01, 0x05},
	{0x65c02, 0x08},
	{0x65c03, 0x1f},
	{0x65c04, 0x1f},
	{0x65c05, 0x1f},


/* sharpeness */
	{0x65600, 0x00},//min_gain[9:8]
	{0x65601, 0x10},//
	{0x65602, 0x00},//max_gain[9:8]
	{0x65603, 0x80},//
	{0x65608, 0x06},//unsharpen_mask
	{0x65609, 0x20},
	{0x6560c, 0x0a},//min_sharpen
	{0x6560d, 0x18},//max_sharpen
	{0x6560e, 0x10},//MinSharpenTp
	{0x6560f, 0x60},//MaxSharpenTp
	{0x65610, 0x10},//MinSharpenTm
	{0x65611, 0x60},//MaxSharpenTm
	{0x65613, 0x10},//SharpenAlpha
	{0x65615, 0x08},//HFreq_thre
	{0x65617, 0x06},//HFreq_coef

/* auto uv saturation */
	{0x1c4e8, 0x01},//Enable
	{0x1c4e9, 0x40},//bUVSaturateGainThre1
	{0x1c4ea, 0x78},//bUVSaturateGainThre2
	{0x1c4eb, 0x70}, //nUVMaxSaturation
	{0x1c4ec, 0x70}, //nUVMinSaturation

	/* Global Gamma */
	{0x1c49b, 0x01}, //Enable
	{0x1c49c, 0x02}, //manual gamma value [15:8]
	{0x1c49d, 0x01}, //gamma 2.0
	{0x1c49e, 0x02}, //RGB gamma value [15:8]
	{0x1c49f, 0x01}, //gamma 2.0
	{0x1c4a0, 0x00}, //nMaxCurveGain[15:8]
	{0x1c4a1, 0x18},
	{0x1c4a2, 0x00}, //nMaxRGBGammaGain[15:8]
	{0x1c4a3, 0x88}, //gamma 2.0 0310 //avoid false contour


	//dark boost
	{0x1c4b0, 0x02},//max gamma value for dark boost
	{0x1c4b1, 0x80},


/* BLC */
	{0x1c58b, 0xff},//avoid false contour
	{0x1c58c, 0xff},//avoid false contour

	/* Tone Mapping */
	//contrast curve 21
	{0x1C4C0, 0x1c},//15 registers
	{0x1C4C1, 0x2f},
	{0x1C4C2, 0x3e},
	{0x1C4C3, 0x4a},
	{0x1C4C4, 0x54},
	{0x1C4C5, 0x5d},
	{0x1C4C6, 0x66},
	{0x1C4C7, 0x70},
	{0x1C4C8, 0x7A},
	{0x1C4C9, 0x85},
	{0x1C4CA, 0x91},
	{0x1C4CB, 0xA0},
	{0x1C4CC, 0xB0},
	{0x1C4CD, 0xC5},
	{0x1C4CE, 0xDF},
	{0x1c4d4, 0x10},//EDR scale//min curve alpha
	{0x1c4d5, 0x20},//EDR scale //max curve alpha
	{0x1c4cf, 0x80}, //curve change step
	{0x65a00, 0x1b},
	{0x65a01, 0xc0},

//curve gain control
	{0x1c1b3, 0x60}, //Gain thre1
	{0x1c1b4, 0xd0},	//Gain thre2
	{0x1c1b5, 0x01}, //EDR gain control
	{0x1c1b6, 0x01}, //Curve Gain control
	{0x1c1b7, 0x40}, //after gamma cut ratio

//Manual UV curve 22
	{0x1C998, 0x01},//16 registers
	{0x1C999, 0x27},
	{0x1C99A, 0x01},
	{0x1C99B, 0x3b},
	{0x1C99C, 0x01},
	{0x1C99D, 0x40},
	{0x1C99E, 0x01},
	{0x1C99F, 0x40},
	{0x1C9A0, 0x01},
	{0x1C9A1, 0x40},
	{0x1C9A2, 0x01},
	{0x1C9A3, 0x40},
	{0x1C9A4, 0x01},
	{0x1C9A5, 0x40},
	{0x1C9A6, 0x01},
	{0x1C9A7, 0x40},
	{0x1C9A8, 0x01},
	{0x1C9A9, 0x40},
	{0x1C9AA, 0x01},
	{0x1C9AB, 0x40},
	{0x1C9AC, 0x01},
	{0x1C9AD, 0x40},
	{0x1C9AE, 0x01},
	{0x1C9AF, 0x3E},
	{0x1C9B0, 0x01},
	{0x1C9B1, 0x31},
	{0x1C9B2, 0x01},
	{0x1C9B3, 0x13},
	{0x1C9B4, 0x00},
	{0x1C9B5, 0xE6},
	{0x1C9B6, 0x00},
	{0x1C9B7, 0xAA},

/* LENC */
	{0x1c246, 0x01},//lens shading correction gain changes with camera gain
	{0x1c247, 0x02},//3 profile

	{0x1c248, 0x40},//maxQ
	{0x1c249, 0x40},
	{0x1c24a, 0x30},//minQ
	{0x1c24b, 0x20},

	{0x1c24c, 0x00}, //pLensGain[0][15:8]
	{0x1c24d, 0x40},
	{0x1c24e, 0x00},
	{0x1c24f, 0x80},
	{0x1c248, 0x40},
	{0x1c24a, 0x20},

	{0x1c574, 0x00},//nMinLensOnlineGainThre[15:8]
	{0x1c575, 0x20},
	{0x1c576, 0x00},//nMaxLensOnlineGainThre[15:8]
	{0x1c577, 0xf0},
	{0x1c578, 0x40},//nMinLensOnlineApplyGain

	{0x65200, 0x0d},
	{0x65206, 0x3c},
	{0x65207, 0x04},
	{0x65208, 0x3e},
	{0x65209, 0x02},
	{0x6520a, 0x36},
	{0x6520b, 0x0c},
	{0x65214, 0x28},
	{0x65216, 0x20},


	/* OVISP LENC setting for A light Long Exposure (HDR/3D) */
	{0x1c310, 0x06},
	{0x1c311, 0x03},
	{0x1c312, 0x03},
	{0x1c313, 0x03},
	{0x1c314, 0x03},
	{0x1c315, 0x06},
	{0x1c316, 0x02},
	{0x1c317, 0x02},
	{0x1c318, 0x02},
	{0x1c319, 0x02},
	{0x1c31a, 0x03},
	{0x1c31b, 0x02},
	{0x1c31c, 0x03},
	{0x1c31d, 0x01},
	{0x1c31e, 0x00},
	{0x1c31f, 0x00},
	{0x1c320, 0x01},
	{0x1c321, 0x03},
	{0x1c322, 0x03},
	{0x1c323, 0x01},
	{0x1c324, 0x00},
	{0x1c325, 0x00},
	{0x1c326, 0x01},
	{0x1c327, 0x03},
	{0x1c328, 0x03},
	{0x1c329, 0x03},
	{0x1c32a, 0x02},
	{0x1c32b, 0x02},
	{0x1c32c, 0x02},
	{0x1c32d, 0x03},
	{0x1c32e, 0x07},
	{0x1c32f, 0x04},
	{0x1c330, 0x04},
	{0x1c331, 0x03},
	{0x1c332, 0x04},
	{0x1c333, 0x07},
	{0x1c334, 0x20},
	{0x1c335, 0x1f},
	{0x1c336, 0x1f},
	{0x1c337, 0x1e},
	{0x1c338, 0x23},
	{0x1c339, 0x1d},
	{0x1c33a, 0x1e},
	{0x1c33b, 0x21},
	{0x1c33c, 0x20},
	{0x1c33d, 0x1d},
	{0x1c33e, 0x1d},
	{0x1c33f, 0x1f},
	{0x1c340, 0x21},
	{0x1c341, 0x21},
	{0x1c342, 0x20},
	{0x1c343, 0x1d},
	{0x1c344, 0x1e},
	{0x1c345, 0x1f},
	{0x1c346, 0x1f},
	{0x1c347, 0x1c},
	{0x1c348, 0x21},
	{0x1c349, 0x21},
	{0x1c34a, 0x1f},
	{0x1c34b, 0x1f},
	{0x1c34c, 0x25},
	{0x1c34d, 0x20},
	{0x1c34e, 0x24},
	{0x1c34f, 0x21},
	{0x1c350, 0x23},
	{0x1c351, 0x22},
	{0x1c352, 0x26},
	{0x1c353, 0x22},
	{0x1c354, 0x23},
	{0x1c355, 0x21},
	{0x1c356, 0x25},
	{0x1c357, 0x20},
	{0x1c358, 0x22},
	{0x1c359, 0x1f},
	{0x1c35a, 0x21},
	{0x1c35b, 0x21},
	{0x1c35c, 0x22},
	{0x1c35d, 0x23},
	{0x1c35e, 0x23},
	{0x1c35f, 0x22},
	{0x1c360, 0x24},
	{0x1c361, 0x21},
	{0x1c362, 0x24},
	{0x1c363, 0x24},
	{0x1c364, 0x25},
	{0x1c365, 0x1f},

	/* OVISP LENC setting for CWF light Long Exposure (HDR/3D) */
	{0x1c2ba, 0x05},
	{0x1c2bb, 0x03},
	{0x1c2bc, 0x02},
	{0x1c2bd, 0x03},
	{0x1c2be, 0x03},
	{0x1c2bf, 0x05},
	{0x1c2c0, 0x02},
	{0x1c2c1, 0x02},
	{0x1c2c2, 0x02},
	{0x1c2c3, 0x02},
	{0x1c2c4, 0x02},
	{0x1c2c5, 0x02},
	{0x1c2c6, 0x02},
	{0x1c2c7, 0x01},
	{0x1c2c8, 0x00},
	{0x1c2c9, 0x00},
	{0x1c2ca, 0x01},
	{0x1c2cb, 0x02},
	{0x1c2cc, 0x02},
	{0x1c2cd, 0x01},
	{0x1c2ce, 0x00},
	{0x1c2cf, 0x00},
	{0x1c2d0, 0x01},
	{0x1c2d1, 0x02},
	{0x1c2d2, 0x02},
	{0x1c2d3, 0x02},
	{0x1c2d4, 0x02},
	{0x1c2d5, 0x02},
	{0x1c2d6, 0x02},
	{0x1c2d7, 0x03},
	{0x1c2d8, 0x06},
	{0x1c2d9, 0x03},
	{0x1c2da, 0x03},
	{0x1c2db, 0x03},
	{0x1c2dc, 0x03},
	{0x1c2dd, 0x06},
	{0x1c2de, 0x1f},
	{0x1c2df, 0x20},
	{0x1c2e0, 0x1e},
	{0x1c2e1, 0x1f},
	{0x1c2e2, 0x22},
	{0x1c2e3, 0x1d},
	{0x1c2e4, 0x1e},
	{0x1c2e5, 0x21},
	{0x1c2e6, 0x20},
	{0x1c2e7, 0x1d},
	{0x1c2e8, 0x1e},
	{0x1c2e9, 0x20},
	{0x1c2ea, 0x21},
	{0x1c2eb, 0x21},
	{0x1c2ec, 0x20},
	{0x1c2ed, 0x1e},
	{0x1c2ee, 0x1f},
	{0x1c2ef, 0x1f},
	{0x1c2f0, 0x1f},
	{0x1c2f1, 0x1d},
	{0x1c2f2, 0x20},
	{0x1c2f3, 0x20},
	{0x1c2f4, 0x1f},
	{0x1c2f5, 0x1e},
	{0x1c2f6, 0x24},
	{0x1c2f7, 0x1c},
	{0x1c2f8, 0x20},
	{0x1c2f9, 0x1a},
	{0x1c2fa, 0x1f},
	{0x1c2fb, 0x1f},
	{0x1c2fc, 0x1f},
	{0x1c2fd, 0x1d},
	{0x1c2fe, 0x1f},
	{0x1c2ff, 0x1d},
	{0x1c300, 0x1f},
	{0x1c301, 0x1a},
	{0x1c302, 0x20},
	{0x1c303, 0x22},
	{0x1c304, 0x21},
	{0x1c305, 0x1a},
	{0x1c306, 0x1e},
	{0x1c307, 0x1d},
	{0x1c308, 0x1f},
	{0x1c309, 0x1d},
	{0x1c30a, 0x1f},
	{0x1c30b, 0x1f},
	{0x1c30c, 0x20},
	{0x1c30d, 0x1c},
	{0x1c30e, 0x1f},
	{0x1c30f, 0x1f},

	/* OVISP LENC setting for DAY light Long Exposure (HDR/3D) */
	{0x1c264, 0x06},
	{0x1c265, 0x03},
	{0x1c266, 0x03},
	{0x1c267, 0x03},
	{0x1c268, 0x03},
	{0x1c269, 0x05},
	{0x1c26a, 0x03},
	{0x1c26b, 0x02},
	{0x1c26c, 0x02},
	{0x1c26d, 0x02},
	{0x1c26e, 0x02},
	{0x1c26f, 0x02},
	{0x1c270, 0x02},
	{0x1c271, 0x01},
	{0x1c272, 0x00},
	{0x1c273, 0x00},
	{0x1c274, 0x01},
	{0x1c275, 0x02},
	{0x1c276, 0x03},
	{0x1c277, 0x01},
	{0x1c278, 0x00},
	{0x1c279, 0x00},
	{0x1c27a, 0x01},
	{0x1c27b, 0x02},
	{0x1c27c, 0x03},
	{0x1c27d, 0x02},
	{0x1c27e, 0x02},
	{0x1c27f, 0x02},
	{0x1c280, 0x02},
	{0x1c281, 0x03},
	{0x1c282, 0x06},
	{0x1c283, 0x04},
	{0x1c284, 0x03},
	{0x1c285, 0x03},
	{0x1c286, 0x03},
	{0x1c287, 0x06},
	{0x1c288, 0x1d},
	{0x1c289, 0x21},
	{0x1c28a, 0x1f},
	{0x1c28b, 0x20},
	{0x1c28c, 0x21},
	{0x1c28d, 0x22},
	{0x1c28e, 0x1f},
	{0x1c28f, 0x23},
	{0x1c290, 0x20},
	{0x1c291, 0x20},
	{0x1c292, 0x1f},
	{0x1c293, 0x21},
	{0x1c294, 0x20},
	{0x1c295, 0x22},
	{0x1c296, 0x21},
	{0x1c297, 0x21},
	{0x1c298, 0x20},
	{0x1c299, 0x21},
	{0x1c29a, 0x20},
	{0x1c29b, 0x20},
	{0x1c29c, 0x1e},
	{0x1c29d, 0x21},
	{0x1c29e, 0x20},
	{0x1c29f, 0x21},
	{0x1c2a0, 0x20},
	{0x1c2a1, 0x21},
	{0x1c2a2, 0x22},
	{0x1c2a3, 0x1f},
	{0x1c2a4, 0x21},
	{0x1c2a5, 0x23},
	{0x1c2a6, 0x20},
	{0x1c2a7, 0x1f},
	{0x1c2a8, 0x21},
	{0x1c2a9, 0x1f},
	{0x1c2aa, 0x21},
	{0x1c2ab, 0x1d},
	{0x1c2ac, 0x22},
	{0x1c2ad, 0x20},
	{0x1c2ae, 0x22},
	{0x1c2af, 0x1f},
	{0x1c2b0, 0x20},
	{0x1c2b1, 0x1f},
	{0x1c2b2, 0x22},
	{0x1c2b3, 0x1f},
	{0x1c2b4, 0x1f},
	{0x1c2b5, 0x25},
	{0x1c2b6, 0x22},
	{0x1c2b7, 0x20},
	{0x1c2b8, 0x22},
	{0x1c2b9, 0x24},


	/* AWB */
	{0x66201, 0x52},
	{0x66203, 0x10},//crop window
	{0x66211, 0xe8},//awb top limit
	{0x66212, 0x04},//awb bottom limit

	{0x1c17c, 0x01},//CT mode
	{0x1c182, 0x08},
	{0x1c183, 0x00},//MinNum
	{0x1c184, 0x02},//AWB Step
	{0x1c58d, 0x00},//LimitAWBAtD65Enable

	{0x1c1be, 0x00},//AWB offset
	{0x1c1bf, 0x00},
	{0x1c1c0, 0x00},
	{0x1c1c1, 0x00},

	{0x1d948, 0x02}, //PreviewColorShiftType
	{0x1d94a, 0x00}, //AWBoffset2
	{0x1d94b, 0x00},
	{0x1d94c, 0x00},
	{0x1d94d, 0x00},
	{0x1d94e, 0x00},
	{0x1d94f, 0x00},

	{0x1c1aa, 0x00},//avgAllEnable
	{0x1c1ad, 0x02},//weight of A
	{0x1c1ae, 0x10},//weight of D65
	{0x1c1af, 0x04},//weight of CWF

	{0x1c5ac, 0x80},//pre-gain
	{0x1c5ad, 0x80},
	{0x1c5ae, 0x80},

	{0x1ccce, 0x00},//awb shift
	{0x1cccf, 0x20},//B gain for A
	{0x1ccd0, 0x10},//R gain for A
	{0x1c5b8, 0x20},//B gain for C outdoor
	{0x1c5b9, 0x10},//R gain for C outdoor
	{0x1ccd1, 0x20},//B gain for C indoor
	{0x1ccd2, 0x10},//R gain for C indoor
	{0x1ccd3, 0x10},//B gain for D indoor
	{0x1ccd4, 0x10},//R gain for D indoor
	{0x1cccc, 0x10},//B gain for D outdoor
	{0x1cccd, 0x10},//R gain for D outdoor

	{0x1c5b4, 0x02},//C indoor/outdoor switch lum 1 Richard@0517
	{0x1c5b5, 0xff},//C indoor/outdoor switch lum 1 Richard@0517
	{0x1c5b6, 0x04},//C indoor/outdoor switch lum 2 Richard@0517
	{0x1c5b7, 0xff},//C indoor/outdoor switch lum 2 Richard@0517

	{0x1ccd5, 0x3b},//CT_A
	{0x1ccd6, 0x6b},//CT_C
	{0x1ccd7, 0xc9},//CT_D

	{0x1c5cd, 0x01},//high light awb shift, modified by Jiangtao to avoid blurish when high CT 0310
	{0x1c5ce, 0x00},
	{0x1c5cf, 0xf0},
	{0x1c5d0, 0x01},
	{0x1c5d1, 0x20},
	{0x1c5d2, 0x03},
	{0x1c5d3, 0x00},
	{0x1c5d4, 0x47},//
	{0x1c5d5, 0xa1},//
	{0x1c5d6, 0xb5},//
	{0x1c5d7, 0xf6},//
	{0x1c5d8, 0x40},//
	{0x1c1c2, 0x00},
	{0x1c1c3, 0x20},

	/* OVISP CTAWB setting for Long Exposure (HDR/3D)  h00206029 20120529 */
	{0x66206, 0x16},
	{0x66207, 0x19},
	{0x66208, 0x19},
	{0x66209, 0x81},
	{0x6620a, 0x6d},
	{0x6620b, 0xe0},
	{0x6620c, 0xa8},
	{0x6620d, 0x42},
	{0x6620e, 0x36},
	{0x6620f, 0x6d},
	{0x66210, 0x58},
	{0x66201, 0x52},

	{0x1c1c8, 0x01},
	{0x1c1c9, 0x2f},
	{0x1c1cc, 0x00},
	{0x1c1cd, 0xa2},
	{0x1c1d0, 0x02},
	{0x1c1d1, 0x24},
	{0x1c254, 0x00},
	{0x1c255, 0xd0},
	{0x1c256, 0x00},
	{0x1c257, 0xe8},
	{0x1c258, 0x01},
	{0x1c259, 0x98},
	{0x1c25a, 0x01},
	{0x1c25b, 0xd0},

		//dynamic CT AWB
	{0x1d924, 0x00}, //enable
	{0x1d8dc, 0x00}, //Br thres0
	{0x1d8dd, 0xf0}, //Br thres0
	{0x1d8de, 0x44}, //Br thres1
	{0x1d8df, 0x34}, //Br thres1
	{0x1d8da, 0x10}, //Br Ratio0
	{0x1d8db, 0x08}, //Br Ratio1
	{0x1d925, 0x0f}, //highlight cwf thres
	{0x1d926, 0x11}, //middlelight cwf thres
	{0x1d927, 0x1c}, //lowlight cwf thres
	{0x1d928, 0x0a}, //highlight A thres
	{0x1d929, 0x14}, //middlelight A thres
	{0x1d92a, 0x1c}, //lowlight A thres
	{0x1d92b, 0x14}, //highlight D thres
	{0x1d92c, 0x14}, //middlelight D thres
	{0x1d92d, 0x1c}, //lowlight D thres
	{0x1d92e, 0x54}, //highlight D limit
	{0x1d92f, 0x40}, //middlelight D limit
	{0x1d930, 0x40}, //lowlight D limit
	{0x1d931, 0x4b}, //highlight A limit
	{0x1d932, 0x32}, //middlelight A limit
	{0x1d933, 0x32}, //lowlight A limit
	{0x1d934, 0x6e}, //highlight D split
	{0x1d935, 0x6e}, //middlelight D split
	{0x1d936, 0x6e}, //lowlight D split
	{0x1d937, 0x60}, //highlight A split
	{0x1d938, 0x60}, //middlelight A split
	{0x1d939, 0x60}, //lowlight A split


	/* Color matrix */
	{0x1C1D8, 0x01},//center matrix,pCCMCenterMatrix[0][0][0] [15:8]

	{0x1C1D9, 0xF0},
	{0x1C1DA, 0xFF},
	{0x1C1DB, 0x04},
	{0x1C1DC, 0x00},
	{0x1C1DD, 0x0C},
	{0x1C1DE, 0xFF},
	{0x1C1DF, 0x6B},
	{0x1C1E0, 0x02},
	{0x1C1E1, 0x1F},
	{0x1C1E2, 0xFF},
	{0x1C1E3, 0x76},
	{0x1C1E4, 0xFF},
	{0x1C1E5, 0x82},
	{0x1C1E6, 0xFE},
	{0x1C1E7, 0xFE},
	{0x1C1E8, 0x02},
	{0x1C1E9, 0x80},

	{0x1C1FC, 0xFF},//cmx left delta
	{0x1C1FD, 0xF3},
	{0x1C1FE, 0x00},
	{0x1C1FF, 0x30},
	{0x1C200, 0xFF},
	{0x1C201, 0xDD},
	{0x1C202, 0x00},
	{0x1C203, 0x27},
	{0x1C204, 0xFF},
	{0x1C205, 0xBC},
	{0x1C206, 0x00},
	{0x1C207, 0x1D},
	{0x1C208, 0x00},
	{0x1C209, 0x41},
	{0x1C20A, 0x00},
	{0x1C20B, 0x08},
	{0x1C20C, 0xFF},
	{0x1C20D, 0xB7},

	{0x1C220, 0x00},//cmx right delta
	{0x1C221, 0xb2},
	{0x1C222, 0xFF},
	{0x1C223, 0x76},
	{0x1C224, 0xff},
	{0x1C225, 0xd8},
	{0x1C226, 0x00},
	{0x1C227, 0x6e},
	{0x1C228, 0xFF},
	{0x1C229, 0xa7},
	{0x1C22A, 0xff},
	{0x1C22B, 0xeb},
	{0x1C22C, 0x00},
	{0x1C22D, 0x21},
	{0x1C22E, 0x00},
	{0x1C22F, 0xf0},
	{0x1C230, 0xFe},
	{0x1C231, 0xef},


	/* dpc */
	{0x65409, 0x04},//[6:0] wthre_list0, used for wthre calculation in auto-mode
	{0x6540a, 0x02},
	{0x6540b, 0x01},
	{0x6540c, 0x01},
	{0x6540d, 0x04},
	{0x6540e, 0x02},
	{0x6540f, 0x01},
	{0x65410, 0x01},
	{0x65408, 0x0b},

	//high gain curve
	{0x1d963, 0x1c},
	{0x1d964, 0x2f},
	{0x1d965, 0x3e},
	{0x1d966, 0x4a},
	{0x1d967, 0x54},
	{0x1d968, 0x5d},
	{0x1d969, 0x66},
	{0x1d96a, 0x70},
	{0x1d96b, 0x7A},
	{0x1d96c, 0x85},
	{0x1d96d, 0x91},
	{0x1d96e, 0xA0},
	{0x1d96f, 0xB0},
	{0x1d970, 0xC5},
	{0x1d971, 0xDF},
	{0x1d8fe, 0x00}, //UV cut gain control
	{0x1d8ff, 0x50}, //low gain thres
	{0x1d900, 0x70}, //high gain thres
	{0x1d97f, 0x14}, //UV cut low bright thres
	{0x1d973, 0x20}, //UV cut high bright thres
	{0x1d972, 0x00}, //adaptive gamma enable
	{0x1d974, 0x01}, //low gain gamma
	{0x1d975, 0xe6},
	{0x1d976, 0x01},//high gain gamma
	{0x1d977, 0xc0},
	{0x1d978, 0x01}, //dark image gamma
	{0x1d979, 0xb3},
	{0x1d97a, 0x88}, //low gain slope
	{0x1d97b, 0x68}, //high gain slope
	{0x1d97c, 0x48}, //dark image slope
	{0x1d97d, 0x14}, //low bright thres
	{0x1d97e, 0x20}, //high bright thres

	{0x1d99e, 0x00}, //dynamic UV curve
	//low gain UV curve 1/2
	{0x1d904, 0x93},
	{0x1d905, 0x9d},
	{0x1d906, 0xa0},
	{0x1d907, 0xa0},
	{0x1d908, 0xa0},
	{0x1d909, 0xa0},
	{0x1d90a, 0xa0},
	{0x1d90b, 0xa0},
	{0x1d90c, 0xa0},
	{0x1d90d, 0xa0},
	{0x1d90e, 0xa0},
	{0x1d90f, 0x9f},
	{0x1d910, 0x98},
	{0x1d911, 0x89},
	{0x1d912, 0x73},
	{0x1d913, 0x55},
	//high gain UV curve 1/2
	{0x1d914, 0x46},
	{0x1d915, 0x70},
	{0x1d916, 0x89},
	{0x1d917, 0x98},
	{0x1d918, 0x9f},
	{0x1d919, 0xa0},
	{0x1d91a, 0xa0},
	{0x1d91b, 0xa0},
	{0x1d91c, 0xa0},
	{0x1d91d, 0xa0},
	{0x1d91e, 0xa0},
	{0x1d91f, 0x9f},
	{0x1d920, 0x98},
	{0x1d921, 0x89},
	{0x1d922, 0x73},
	{0x1d923, 0x55},



/* pre-set awb for D50(sunny day) D65(overcast/cloudy) CWF(fluorescent) and A(incandescent) */
	{0x1c4f0, 0x00},//BGain 0x1c17c -> 0x3
	{0x1c4f1, 0xab},
	{0x1c4f2, 0x00},//GGain
	{0x1c4f3, 0x80},
	{0x1c4f4, 0x00},//RGain
	{0x1c4f5, 0xd3},

	{0x1c4f6, 0x00},//BGain 0x1c17c -> 0x4
	{0x1c4f7, 0x91},
	{0x1c4f8, 0x00},//GGain
	{0x1c4f9, 0x80},
	{0x1c4fa, 0x00},//RGain
	{0x1c4fb, 0xeb},

	{0x1c4fc, 0x00},//BGain 0x1c17c -> 0x5
	{0x1c4fd, 0xf6},
	{0x1c4fe, 0x00},//GGain
	{0x1c4ff, 0x80},
	{0x1c500, 0x00},//RGain
	{0x1c501, 0xde},

	{0x1c502, 0x01},//BGain 0x1c17c -> 0x6
	{0x1c503, 0x16},
	{0x1c504, 0x00},//GGain
	{0x1c505, 0x80},
	{0x1c506, 0x00},//RGain
	{0x1c507, 0x8f},

/* AF */
	{0x1cd0a, 0x00},//01:Active AF procedure

	{0x6502a, 0x2a},
	{0x66112, 0xC8},
	{0x66100, 0x00},    //AFHardwareControl//[2]:binning preview size =1,snapshot size =0
	{0x66104, 0x02},    //nX0
	{0x66105, 0x67},
	{0x66106, 0x01},    //nY0
	{0x66107, 0x92},
	{0x66108, 0x01},    //nW0
	{0x66109, 0xa4},
	{0x6610a, 0x01},    //nH0
	{0x6610b, 0xa4},

	{0x1c5b0, 0x00},    //nWinMode
	{0x1cca4, 0x00},    //OffsetInit
	{0x1cca5, 0xa0},
	{0x1cca6, 0x02},    //FullRange
	{0x1cca7, 0x00},
	{0x1cca8, 0x00},    //AFCMinStep
	{0x1cca9, 0x0c},
	{0x1ccaa, 0x00},    //AFCCoarseStep
	{0x1ccab, 0x18},
	{0x1ccac, 0x00},    //AFCFineStep
	{0x1ccad, 0x0c},
	{0x1cd0b, 0x01},    //nAFMode:1:video servo AF;0:snapshot AF
	{0x1ccae, 0x00},    //nFrameRate
	{0x1ccaf, 0x14},
	{0x1ccb2, 0x00},    //nMotorResTime
	{0x1ccb3, 0x20},
	{0x1cdc6, 0x09},    //nFocusI2COption:[3]:16 bit address;[2]:16bit data
	{0x1cdc7, 0x18},    //nFocusDeviceID
	{0x1cdc8, 0x30},    //pFocusMoveLensAddr_0
	{0x1cdc9, 0xec},
	{0x1cdca, 0x30},    //pFocusMoveLensAddr_1
	{0x1cdcb, 0xed},
	{0x1cddc, 0x01},    //nMotorDriveMode
	{0x1cddd, 0x00},    //bIfMotorCalibrate
	{0x1cd0e, 0x01},    //bIfPreMove
	{0x1cd05, 0x01},
	{0x1cd06, 0x01},


	{0x1cc92, 0x00},    //nT_Compare_1x:threshold for contrast compare@1x [15:8]
	{0x1cc93, 0x08},
	{0x1cc94, 0x00},    //nT_Compare_16x
	{0x1cc95, 0x0a},
	{0x1cc96, 0x00},    //nT_sad_1x:threshold for sad compare@1x [15:8]
	{0x1cc97, 0x00},
	{0x1cc98, 0x00},    //nT_sad_16x
	{0x1cc99, 0x00},
	{0x1cc9a, 0x00},    //nT_pre_sad_1x
	{0x1cc9b, 0x06},
	{0x1cc9c, 0x00},    //nT_pre_sad_16x
	{0x1cc9d, 0x04},
	{0x1cc9e, 0x00},    //nT_hist_1x
	{0x1cc9f, 0x02},
	{0x1cca0, 0x00},    //nT_hist_16x
	{0x1cca1, 0x02},
	{0x1cca2, 0x00},    //nT_contrast_diff
	{0x1cca3, 0x08},

	{0x1cd08, 0x00},
	{0x1ccef, 0x00},
	{0x1cdde, 0x04},
	{0x1cdcc, 0x0f},
	{0x1cdcd, 0xff},
	{0x1cdd8, 0x20},
	{0x1cdd9, 0x20},
	{0x1cdda, 0x38},
	{0x1ccca, 0x58},
	{0x1ccb1, 0x02},
	{0x1cd05, 0x00},
	{0x1cd0d, 0x36},
	{0x1ccc9, 0x1f},
#endif
};
#elif defined(CONFIG_CPU_LC1813)

static struct v4l2_isp_parm gc2355_isp_parm = {
	.min_exposure = 0x03,
	.default_snapshot_exposure_ratio = 0x00b0,
	.default_preview_exposure_ratio = 0x0173,
	.sensor_vendor = SENSOR_VENDOR_SAMSUMG,
	.sensor_main_type = SAMSUNG_GC2355,
	.down_frm_small_thres = 50,
	.down_frm_large_thres = 25,
	.up_frm_small_thres = 100,
	.up_frm_large_thres = 120,
};

static const struct v4l2_isp_regval gc2355_isp_setting[] = {
	{0x60100, 0x01},//Software reset
	{0x6301b, 0xf0},//isp clock enable
	{0x63025, 0x40},//clock divider

	{0x63c0a, 0x01},
	{0x63c0b, 0x53},
	{0x63c0c, 0x02},
};

#endif

#endif /* __GC2355_H__ */
