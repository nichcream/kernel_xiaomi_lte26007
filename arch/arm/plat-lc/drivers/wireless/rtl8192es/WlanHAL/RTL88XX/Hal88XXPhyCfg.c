...
)
{
    ...
    else if(Band == RF88XX_BAND_ON_5G)    //5G band
    {
		...
		if (HAL_VAR_INTERLPA_8881A == 1 && (get_bonding_type_8881A() != BOND_8881AB)){			 
			...
			panic_printk("8881A internal PA PIN control\n");
		}else{
#endif			
...
)
{
	...
	{	// use default value
		...
		if(!Adapter->pshare->rf_ft_var.disable_txpwrlmt)
		{
			if (HAL_TX2PATH && !Adapter->pshare->rf_ft_var.disable_txpwrlmt2path) {
				Adapter->pshare->txpwr_lmt_OFDM -= 6;
				Adapter->pshare->txpwr_lmt_HT1S -= 6;
			}
...
)
{
	...
	{	
		...
		{
			if (HAL_TX2PATH && !Adapter->pshare->rf_ft_var.disable_txpwrlmt2path)
				Adapter->pshare->txpwr_lmt_CCK -= 6;
