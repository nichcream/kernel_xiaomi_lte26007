...
s32 rtl8723bs_xmit_buf_handler(PADAPTER padapter)
{
	...
	_queue *pqueue_main, *pqueue_buddy;
	u32 empty_queue;
	...
	pqueue_main = &padapter->xmitpriv.pending_xmitbuf_queue;
	empty_queue = _rtw_queue_empty(pqueue_main);
#ifdef CONFIG_CONCURRENT_MODE
	if (rtw_buddy_adapter_up(padapter)){
		pqueue_buddy = &padapter->pbuddy_adapter->xmitpriv.pending_xmitbuf_queue;
		empty_queue &= _rtw_queue_empty(pqueue_buddy);
	}
#endif
	if (empty_queue) {
		//DBG_871X("+%s: Adapter's pending_xmitbuf_queue is Empty!\n", __func__);
		return _SUCCESS;
	}
