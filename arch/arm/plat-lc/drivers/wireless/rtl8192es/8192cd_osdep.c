...
//mark_dma
#else
#ifdef RTK_NL80211_DMA  
#ifdef CONCURRENT_MODE
static int wlandev_idx = 0;
static struct rtl8192cd_hw hw_info[NUM_WLAN_IFACE];
extern unsigned char desc_buf[2][340304]; 
#else
static struct rtl8192cd_hw hw_info;
extern unsigned char desc_buf[205120];
#endif
#endif
#endif
...
static int rtl8192cd_init_sw(struct rtl8192cd_priv *priv)
{
...
#endif
		{
...
#endif
			{
...
#endif
				{
					...
					if(!priv->pmib->dot11StationConfigEntry.sc_enabled || ((priv->simple_config_status==0) && (IS_ROOT_INTERFACE(priv) && (GET_VXD_PRIV(priv)->simple_config_status==0))))
#endif
					{
#ifdef PCIE_POWER_SAVING
	...
	if ( !rtw_is_list_empty(&priv->pshare->beacon_timer_event.list) )
		printk("beacon timer evt not empty\n");
...
#ifdef REVO_MIFI
		priv->pshare->wps_wake_thread= kthread_run(rtw_wps_wake_thread, priv, "rtw_wps_wk/%s", priv->dev->name);
		if (IS_ERR(priv->pshare->wps_wake_thread)) {
			priv->pshare->wps_wake_thread = NULL;
			printk("[%s] create wps_wake_thread fail\n", __FUNCTION__);
			return 1;
		}
#endif	
...
#include <linux/gpio.h>
	//set GPIO and wake isr
#define RTK_WIFI_WAKE_I_PIN 203
static int rtk_wifi_wake_i = mfp_to_gpio(RTK_WIFI_WAKE_I_PIN);

#ifdef __LINUX_2_6__
irqreturn_t leadcore_gpio_wakeup_isr(int irq, void *dev_instance)
#else
void leadcore_gpio_wakeup_isr(int irq, void *dev_instance, struct pt_regs *regs)
#endif
{
	 ...
	 printk("===>leadcore_gpio_wakeup_isr\n");
...
int  set_leadcore_wakeup_pin(struct net_device *dev, struct rtl8192cd_priv *priv)
{
 	 int err;
	  int level;
	  if (! IS_ROOT_INTERFACE(priv))
	 ...
	 printk("===>set_leadcore_wakeup_pin\n");
	err= gpio_request(rtk_wifi_wake_i, "wifi_wakeup_host");
 	 if(err<0){
  		printk("set_leadcore_wakeup_pin request gpio is busy!");
		return err;
	 }

	gpio_direction_input(rtk_wifi_wake_i);
	dev->irq = gpio_to_irq(RTK_WIFI_WAKE_I_PIN);
	err=request_irq(dev->irq, leadcore_gpio_wakeup_isr, IRQ_TYPE_EDGE_RISING, "wifi_wakeup_host", priv);
	if(err<0){
		printk("set_leadcore_wakeup_pin request_irq fail\n");
...
#define INT_GPIO_GP5 114

#define BALONG_GPIO_WIFI_WAKEUP_PIN 15
#define BALONG_GPIO_WIFI_WAKEUP BALONG_GPIO_2(BALONG_GPIO_WIFI_WAKEUP_PIN)
...
#endif
{
	 ...
	 printk("balong_gpio_wakeup_isr acli: get intr %d\n", ucData);
...
void set_balong_wakeup_pin(struct net_device *dev, struct rtl8192cd_priv *priv)
{
 	 int rc;
	  int level;
 	 dev->irq = INT_GPIO_GP5;
	 printk("===>set_balong_wakeup_pin\n");
 	 if(gpio_request(BALONG_GPIO_WIFI_WAKEUP, "wifi_wakeup_host"))
  		printk("set_balong_wakeup_pin request gpio is busy!");

	gpio_direction_input(BALONG_GPIO_WIFI_WAKEUP);
 	gpio_int_mask_set(BALONG_GPIO_WIFI_WAKEUP);
	gpio_set_function(BALONG_GPIO_WIFI_WAKEUP,1);
 	rc=gpio_int_trigger_set(BALONG_GPIO_WIFI_WAKEUP,IRQ_TYPE_EDGE_RISING);//IRQ_TYPE_EDGE_FALLING
	if(rc)
		printk("set_balong_wakeup_pin gpio_int_trigger_set fail\n");
 	gpio_int_state_clear(BALONG_GPIO_WIFI_WAKEUP);
	rc=request_irq(dev->irq, balong_gpio_wakeup_isr, IRQF_SHARED, "wifi_wakeup_host", priv);
	if(rc)
		printk("set_balong_wakeup_pin request_irq fail\n");
	gpio_int_unmask_set(BALONG_GPIO_WIFI_WAKEUP);
   return;
}
        ...
        switch ( ACT_STS(act, sts) )     
        {
        	  case ACT_STS(RTW_ACT_IDLE, RTW_STS_NORMAL):
	      	  	if ( is_ps_cond_match(priv)) {
	    	  	    ...
	    	  	    GET_ROOT(priv)->offload_function_ctrl = 1;
			    priv->pshare->pwr_state = RTW_STS_SUSPEND;
    			
		            printk("starting offload \n");	    
			    /* change reserved page: move to update_beacon */
...
#endif
#define RTW_SUSPEND_LOCK_NAME "rtw_wifi_suspend"
int rtl8192cd_open(struct net_device *dev)
{
	...
	{
		printk(" [dev->name] DO NOT double OPEN !!\n", dev->name);
		return 0;
	}
	else
	{
		printk("+++ OPEN[%s] for priv = 0x%x (root=%d vxd=%d vap=%d) +++ \n\n", 
				dev->name, 
	...
	if(!IS_ROOT_INTERFACE(priv))
	{
		...
		{
			printk("Root interface NOT OPEN yet !!\n");
			return 0;
		}

		if(IS_VXD_INTERFACE(priv) && (priv->rtk->num_vxd < 1))
		{
			printk("VXD if already deleted or NOT added !!\n");
			return 0;
		}
		...
		if(IS_VAP_INTERFACE(priv) && (priv->rtk->num_vap < 1))
		{
			...
			{
				printk("VAP already deleted or NOT added !!\n");
				return 0;
			}
		}
	...
	{
		printk("No need to call open for scan iface !!\n");
		return 0;
	}
...
#if !defined(CONFIG_RTL_8881A_SELECTIVE) && !defined(CONFIG_RTL_8196E) && !defined(CONFIG_RTL_8196D)
	{
		if(strcmp(dev->name, "wlan0")==0) //wlan0 always be 5G, for 5G site survey issue
		{
			if((priv->pmib->dot11BssType.net_work_type & (WIRELESS_11B|WIRELESS_11G)) ||
				(priv->pmib->dot11RFEntry.phyBandSelect == PHY_BAND_2G))
			{
				printk("Correct Wlan0 as 5G Band !!\n");
...
#ifdef SDIO_AP_PS
#ifndef MBSSID_OFFLOAD
		priv->pshare->ps_ctrl = RTW_ACT_SETREP;   //test by zyj 20141218 
#endif
#endif		
	}
#endif // UNIVERSAL_REPEATER
...
//#ifdef CONFIG_RTL865X_AC
...
#endif
	{
        ...
        priv->pshare->pwr_state = RTW_STS_NORMAL;
        priv->pshare->ps_ctrl = RTW_ACT_IDLE;
#ifdef PLATFORM_ARM_BALONG	    //zyj
   		BSP_PWRCTRL_WIFI_LowPowerExit();
    	set_balong_wakeup_pin(dev, priv);
#endif
#ifdef CONFIG_POWER_SUSPEND
	wake_lock_init(&priv->pshare->rtw_suspend_lock, WAKE_LOCK_SUSPEND, RTW_SUSPEND_LOCK_NAME);
#endif
#ifdef CONFIG_POWER_SUSPEND
	wake_lock(&priv->pshare->rtw_suspend_lock);
	printk("==>rtl8192cd_open wake_lock(&rtw_suspend_lock)!!\n");
	rc=set_leadcore_wakeup_pin(dev, priv);
...
#ifdef MBSSID
		/*add the MAC address of vxd to checking bssid cams*/
		camData[0] = BIT(31) | BIT(30) | BIT(23) |      
		(8 & 0x1f)<<24 |
		(macAddr[5] << 8) | macAddr[4];
		camData[1] = (macAddr[3] << 24) | (macAddr[2] << 16) | (macAddr[1] << 8) | macAddr[0];
		for (j=1; j>=0; j--) {
			RTL_W32((0x0628+4)-4*j, camData[j]);
		}
#endif
	}
#endif
...
priv->drv_state |= DRV_STATE_OPEN;      // set driver as has been opened, david
	...
	for (i=0; i<MAX_MAC_CLONE_NUM; i++) {
	    if (priv->pshare->mclone_sta[i].isTimerInit == 0) {// if it is used, do not re-init
			...
			priv->pshare->mclone_sta[i].reauth_timer.data = (unsigned long) i+1;
			...
			priv->pshare->mclone_sta[i].reassoc_timer.data = (unsigned long) i+1;
...
#else
				mod_timer(&priv->ss_timer, jiffies + RTL_MILISECONDS_TO_JIFFIES(1000)); 	//5000		   
#endif
                }
            }
		}
...
int rtl8192cd_close(struct net_device *dev)
{
	...
	SMP_LOCK(flags);
	DBFENTER;
	printk("===>rtl8192cd_close\n");
#ifdef SDIO_AP_PS
	printk("rtl8192cd_close  priv->pshare->pwr_state:%x\n", priv->pshare->pwr_state);
	if ( priv->pshare->pwr_state == RTW_STS_SUSPEND ) {
#ifdef PLATFORM_ARM_BALONG
	 		BSP_PWRCTRL_WIFI_LowPowerExit();
#endif
...
#endif
	{
#if defined(PRIV_STA_BUF) || defined(RTK_NL80211_DMA) //mark_dma , hw struct may over 128k kmalloc limit
#ifdef CONCURRENT_MODE
		phw = &hw_info[wlandev_idx];
		#if defined(RTK_NL80211_DMA) //mark_dma
		wlandev_idx++; 
		#endif
#else
		phw = &hw_info;
...
#endif
		{
#ifndef RTK_NL80211_DMA//mark_dma
