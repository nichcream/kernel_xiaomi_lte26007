...
//
// 1. If STATION mode, then return.
// 2. If buddy is scanning and I am AP, then return.
// 3. If P2P Mode
//     3.1 Check if p2p_state is not Listen and my role is not GO, then return.
//     3.2 Check if CCK Rate, then return.
//     3.3 Check OFDM Rate, SSID and ServiceNameHash.
//     3.4 If support WiFi Direct Service, then send Probe Request to wpa_supplicant and return.
//     3.5 Send Probe Response according to P2P Role.
// 4. If ADHOC or AP mode
//     4.1 Check if not ADHOC or AP, then return.
//     4.2 Check if fw_state is not Linked or join_res is _FALSE, then return.
//     4.3 Check if missed SSID IE, then return.
//     4.4 Check if SSID not matched, then return.
//     4.5 Send Probe Response.
//
unsigned int OnProbeReq(_adapter *padapter, union recv_frame *precv_frame)
{
	struct mlme_priv *pmlmepriv = &padapter->mlmepriv;
	struct mlme_ext_info	*pmlmeinfo = &(padapter->mlmeextpriv.mlmext_info);
	WLAN_BSSID_EX 	*pcur_net = &(pmlmeinfo->network);
#ifdef CONFIG_P2P
	struct wifidirect_info	*pwdinfo = &(padapter->wdinfo);
	struct rx_pkt_attrib	*pattrib = &precv_frame->u.hdr.attrib;
	u8 brpt_uplayer = _FALSE;
#endif
	u8 *pframe = precv_frame->u.hdr.rx_data;
	...
	u8 *pssid_ie = NULL;
	u32	ielen = 0;
#if defined(CONFIG_WFDS) && defined(CONFIG_IOCTL_CFG80211)
	int channel = 1;
	int freq = 0;
#endif
	...
	{
		DBG_871X("%s(): return because fwstate(0x%x)!=WIFI_STATION_STATE\n", __func__, pmlmepriv->fw_state);
		return _SUCCESS;
	}
	...
	if (pwdinfo->p2p_state > P2P_STATE_NONE)
	{
		...
		if (!(rtw_p2p_chk_state(pwdinfo, P2P_STATE_LISTEN) ||
			rtw_p2p_chk_state(pwdinfo, P2P_STATE_FIND_PHASE_LISTEN) ||
			rtw_p2p_chk_role(pwdinfo, P2P_ROLE_GO)))
		{
			//DBG_871X("%s(): return because p2p_state(%d) or p2p_role(%d) error!\n", __func__, pwdinfo->p2p_state, pwdinfo->role);
			return _SUCCESS;
		}
		...
		if (pattrib->mcs_rate == DESC_RATE1M || pattrib->mcs_rate == DESC_RATE1M
			|| pattrib->mcs_rate == DESC_RATE1M || pattrib->mcs_rate == DESC_RATE1M)
		{
			return _SUCCESS;
		}
		...
		// 3.3 Check OFDM Rate, SSID and ServiceNameHash
		if(process_probe_req_p2p_ie(pwdinfo, pframe, len) != _TRUE)
		{
			//DBG_871X("%s(): return because parse p2p ie error!\n", __func__);
			return _SUCCESS;
		}
		...
		if(brpt_uplayer)
		{
			...
			rtw_cfg80211_rx_mgmt(padapter, freq, 0, pframe, len, GFP_ATOMIC);
#else
			cfg80211_rx_action(padapter->pnetdev, freq, pframe, len, GFP_ATOMIC);
#endif
			return _SUCCESS;
		}
#endif //CONFIG_WFDS && CONFIG_IOCTL_CFG80211

		// 3.5 Send Probe Response according to P2P Role.
		if(rtw_p2p_chk_role(pwdinfo, P2P_ROLE_DEVICE))
		{
			//DBG_871X("%s(): Issue Device Probe Response!DA="MAC_FMT"\n", __func__, MAC_ARG(get_sa(pframe)));
			issue_probersp_p2p( padapter, get_sa(pframe));
			return _SUCCESS;
		}
		else if(rtw_p2p_chk_role(pwdinfo, P2P_ROLE_GO))
		{
			//DBG_871X("%s(): Issue GO Probe Response!\n", __func__);
			issue_probersp(padapter, get_sa(pframe), _TRUE);;
			return _SUCCESS;
		}
	}
	else
#endif //CONFIG_P2P
	// 4. WIFI_FW_AP_STATE ||WIFI_ADHOC_MASTER_STATE
	{
		// 4.1 Check if not ADHOC or AP, then return.
		if (check_fwstate(pmlmepriv, WIFI_ADHOC_MASTER_STATE|WIFI_AP_STATE|WIFI_ADHOC_STATE) == _FALSE)
		{
			return _SUCCESS;
		}

		// 4.2 Check if fw_state is not Linked or join_res is _FALSE, then return.
		if(check_fwstate(pmlmepriv, _FW_LINKED) == _FALSE || pmlmepriv->cur_network.join_res == _FALSE)
		{
			return _SUCCESS;
		}

		// 4.3 Check if missed SSID IE, then return.
		pssid_ie = rtw_get_ie(pframe + WLAN_HDR_A3_LEN + _PROBEREQ_IE_OFFSET_, _SSID_IE_, (int *)&ielen,
		...
		// 4.4 Check if SSID not matched, then return.
		if ( (ielen != 0 && _FALSE ==_rtw_memcmp((void *)(pssid_ie+2), (void *)pcur_net->Ssid.Ssid, pcur_net->Ssid.SsidLength))
...
static int _issue_deauth(_adapter *padapter, unsigned char *da, unsigned short reason, u8 wait_ack)
{
	...
	DBG_871X("%s to "MAC_FMT" wait_ack:%d\n", __func__, MAC_ARG(da),wait_ack);
/*
#ifdef CONFIG_P2P
...
#else
	DBG_871X("%s to "MAC_FMT"\n", __func__, MAC_ARG(da));
	 _issue_deauth(padapter, da, reason, _FALSE);
#endif
}
	...
	int wait_ms)
{
	...
	DBG_871X("===>%s try_cnt:%d wait_ms:%d\n",__FUNCTION__,try_cnt,wait_ms);
	do
	{	DBG_871X("===>%s i:%d _issue_deauth ret:%d\n",__FUNCTION__,i,ret);
...
//#endif
	...
	do{
		...
		do {
			...
			poll++;
		}while((poll%3)!=0 && _FALSE == bxmitok && !padapter->bSurpriseRemoved && !padapter->bDriverStopped);

	}while(_FALSE == bxmitok && issue<2 && !padapter->bSurpriseRemoved && !padapter->bDriverStopped);
...
#endif
			{
#if((defined PLATFORM_LEADCORE_1813)  || (defined CONFIG_PLATFORM_LEADCORE))
				if((scan_index == 0)  ){
					pmlmeext->chan_scan_time=50;  //in order to open wifi faster
					scan_index++;
				}
#endif
...
bool rtw_port_switch_chk(_adapter *adapter)
{
	...
	if (if_port0 == NULL) {
		DBG_871X("rtw_port_switch_chk: if_port0 is NULL \n");
		goto exit;
	}

	if (if_port1 == NULL) {
		DBG_871X("rtw_port_switch_chk: if_port1 is NULL \n");
		goto exit;
	}
...
u8 setopmode_hdl(_adapter *padapter, u8 *pbuf)
{
	...
	if (rtw_port_switch_chk(padapter) == _TRUE)
	{
		...
		if(check_fwstate(&padapter->mlmepriv, WIFI_FW_AP_STATE))
		{
			DBG_871X("setopmode_hdl: send beacon after port switch \n");
	...
	for(k=0;k<in_num;k++)
     	{
			...
			&&!(in[k].flags & RTW_IEEE80211_CHAN_DISABLED))
     		{
     			set_idx_cur = rtw_ch_set_search_ch(pmlmeext->channel_set, pmlmeext->cur_channel);
     			if(set_idx_cur  > 0)
     			{
...
int rtw_chk_start_clnt_join(_adapter *padapter)
{
...
#ifdef CONFIG_REFUSE_CONNECTION_FOR_WIFIDISPLAY
	if(rtw_linked_check(pbuddy_adapter))
	{
		DBG_871X("start_clnt_join:  link fail when wifi display is running \n");
		...
		check_fwstate(pbuddy_mlmepriv, WIFI_STATION_STATE) == _TRUE) //for Client Mode/p2p client
	{
		...
		if (connect_allow == _TRUE && chbw_allow == _FALSE) {
			...
			rtw_disassoc_cmd(pbuddy_adapter,_FALSE);
			rtw_indicate_disconnect(pbuddy_adapter,1);
