...
EXTERN void rtl8192cd_mclone_reauth_timer(unsigned long acctive_id);
EXTERN void rtl8192cd_mclone_reassoc_timer(unsigned long acctive_id);
...
#ifdef SDIO_AP_PS
EXTERN void handle_ap_cmd( struct work_struct *work );
EXTERN void cmd_set_ap_offload(struct rtl8192cd_priv *priv, u8 en);
EXTERN int set_ap_ps_mode(struct rtl8192cd_priv *priv, unsigned char *data);
#endif
#endif // CONFIG_USB_HCI || CONFIG_SDIO_HCI
...
#ifdef CONFIG_SDIO_HCI
EXTERN int rtl_siwap(struct net_device *dev, struct iw_request_info *info, struct sockaddr *awrq, char *extra);
EXTERN int rtl_giwap(struct net_device *dev, struct iw_request_info *info, struct sockaddr *awrq, char *extra);
#else
...
// EDCA
#ifdef USE_OUT_SRC
EXTERN void odm_EdcaParaInit(IN	PVOID		pDM_VOID);
