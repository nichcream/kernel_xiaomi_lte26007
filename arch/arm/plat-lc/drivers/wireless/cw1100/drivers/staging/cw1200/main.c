...
#endif /* CONFIG_PM */
	.get_drv_signal = cw1200_get_signal,
...
struct ieee80211_hw *cw1200_init_common(size_t priv_data_len)
{
	...
	int band;
	mm_segment_t old_fs;
	char buf_mac_line[256];
	u8 temp = 0;
	struct file *file_read_mac= NULL;
	...
	file_read_mac = filp_open(CONFIG_STE_WIFI_MAC_PATH,O_RDONLY,0);
	if(IS_ERR(file_read_mac)) 
		goto open_wifi_nvram_failed;
	
	old_fs = get_fs();
	set_fs(get_ds());
	if(file_read_mac && file_read_mac->f_op &&  file_read_mac->f_op->read)
	{
		int i = 0;
		while ( file_read_mac->f_op->read(file_read_mac, &buf_mac_line[i], 1,&file_read_mac->f_pos) == 1)
		{       
			if (buf_mac_line[i] == '\n')
			{
				buf_mac_line[i] ='\0';//one line string read
				i=0;

				if(17 == strlen(buf_mac_line))
				{
					int j=0;
					char *p = buf_mac_line;
					u8 cw1200_mac[ETH_ALEN] = {0};
					for(j=0;j<6;j++)
					{
						char buf_Byte_Hex[3] = {0};
						memcpy(buf_Byte_Hex,p,2);
						sscanf(buf_Byte_Hex,"%x",(unsigned int *)&temp);
						cw1200_mac[j] = temp;
						p += 3;
					}
					SET_IEEE80211_PERM_ADDR(hw, cw1200_mac);
				}
				break;
			}
			if (i< 254)
		...
		filp_close(file_read_mac, NULL);
	}
	set_fs(old_fs); //rest fs

	open_wifi_nvram_failed:
	{
