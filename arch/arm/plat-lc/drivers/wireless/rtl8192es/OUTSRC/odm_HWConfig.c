	...
	)
{
	...
	if(pPktinfo->DataRate <= DESC8812_RATE54M)
	{
	...
	else //is OFDM rate
	{
		...
		{//pMgntInfo->CustomerID != RT_CID_819x_Lenovo
			...
			if(	(pPktinfo->DataRate>=DESC8812_RATEMCS8) &&
		 		...
		 		Max_spatial_stream = 2;
			else if(	(pPktinfo->DataRate >= DESC8812_RATEMCS16) &&
				(pPktinfo->DataRate <= DESC8812_RATEMCS23))
				Max_spatial_stream = 3;
			else if(	(pPktinfo->DataRate >= DESC8812_RATEVHTSS3MCS0) &&
				(pPktinfo->DataRate <= DESC8812_RATEVHTSS3MCS9))
				Max_spatial_stream = 3;
			else
    ...
    )
{
	...
	if(pDM_Odm->SupportICType == ODM_RTL8814A) 
	{
		READ_AND_CONFIG(8814A,_TxPowerTrack);			
	}
#endif	
