...
#define CHIP_SHUTDOWN_TIMEOUT			3000	/* ms */
...
/** CG2900_GNSS_EXTCLK- Channel for keeping ST-Ericsson CG2900 enabled.
 * Opening this channel forces the chip to stay powered.
 * No data can be written to or read from this channel.
 * This will be used for GPS external clock reference
 */
#define CG2900_GNSS_EXTCLK			"ste_gnss_extclk"
 ...
 */
#define CG2900_HCI_RAW				"ste_hci_raw"
...
static void decrement_flow_cnt(struct cg2900_chip_dev *dev)
{
	struct cg2900_chip_info *info = dev->c_data;

	spin_lock_bh(&info->tx_bt_lock);
	if (info->main_state == CG2900_ACTIVE)
		--info->bt_tx_in_queue;
	spin_unlock_bh(&info->tx_bt_lock);
}


inline void increment_flow_cnt(struct cg2900_chip_info *info)
{
	if (info->main_state == CG2900_ACTIVE)
		++info->bt_tx_in_queue;
}
...
static void transmit_skb_from_tx_queue_bt(struct cg2900_chip_dev *dev)
{
	...
	while (skb) {
		...
		dev_dbg(dev->dev, "skb(read) tx_nr_pkts_allowed_bt = %d\n",
				...
				const struct sk_buff * const skb)
{
	...
	if (HCI_EV_CMD_COMPLETE == event->evt) {
		...
		dev_dbg(dev->dev, "cmd compl: bt_tx_in_queue= %d complete->ncmd:%d \n", info->bt_tx_in_queue, complete->ncmd);
		if (complete->ncmd > info->bt_tx_in_queue) {
			info->tx_nr_pkts_allowed_bt = complete->ncmd - info->bt_tx_in_queue;
			dev_dbg(dev->dev, "cmd compl: tx_nr_pkts_allowed_bt = %d\n", info->tx_nr_pkts_allowed_bt);
		}
		...
		spin_lock_bh(&info->tx_bt_lock);
		//fix L1813_Bug00002266 by Chengpenghui,BEGIN,fix hci command credit issue
		dev_dbg(dev->dev, "cmd status: bt_tx_in_queue= %d status->ncmd:%d \n", info->bt_tx_in_queue, status->ncmd);
		if (status->ncmd > info->bt_tx_in_queue) {
			info->tx_nr_pkts_allowed_bt = status->ncmd - info->bt_tx_in_queue;
			dev_dbg(dev->dev, "cmd status: tx_nr_pkts_allowed_bt = %d\n", info->tx_nr_pkts_allowed_bt);
		}
		//fix L1813_Bug00002266 by Chengpenghui,END,fix hci command credit issue
		if (!skb_queue_empty(&info->tx_queue_bt))
			transmit_skb_from_tx_queue_bt(dev);
...
shut_down_chip:
	...
	if (info->startup) {
		...
		err = mfd_add_devices(dev->dev, main_info->cell_base_id,
				cg2900_devs, info->mfd_size, NULL, 0,NULL);
		if (err) {
		...
		if (dev->chip.hci_revision == CG2905_PG2_REV) {
			err = mfd_add_devices(dev->dev, main_info->cell_base_id,
					cg2905_extra_devs,
					info->mfd_extra_size, NULL, 0,NULL);
			if (err) {
			...
			err = mfd_add_devices(dev->dev, main_info->cell_base_id,
					...
					info->mfd_extra_char_size, NULL, 0,NULL);
			if (err) {
					   ...
					   struct sk_buff *skb)
{
	...
	if (info->tx_nr_pkts_allowed_bt > 0) {
		dev_dbg(user->dev, "tx skb(write) tx_nr_pkts_allowed_bt = %d\n",
			info->tx_nr_pkts_allowed_bt);
		info->tx_nr_pkts_allowed_bt--;
...
static void chip_startup_finished(struct cg2900_chip_info *info, int err)
{
	...
	if (err)
		/* Shutdown the chip */
		cg2900_create_work_item(info->wq, work_power_off_chip, info->chip_dev);
	else {
...
static int cg2900_open(struct cg2900_user_data *user)
{
	...
	if (CG2900_ACTIVE != info->main_state &&
		...
		dev_dbg(user->dev, "[ZHBBINGGE]set_chip_power ok\n");
		/* Wait to be sure that the chip is ready */
		schedule_timeout_killable(
				msecs_to_jiffies(CHIP_READY_TIMEOUT));
		dev_dbg(user->dev, "[ZHBBINGGE]schedule_timeout_killable ok\n");

		if (dev->t_cb.open) {
			err = dev->t_cb.open(dev);
			dev_dbg(user->dev, "[ZHBBINGGE]dev->t_cb.open ERR=%d ok\n", err);
			if (err) {
				if (dev->t_cb.set_chip_power)
		...
		dev_dbg(user->dev, "[ZHBBINGGE]dev->t_cb.open ok\n");
	...
	return 0;
err_free_list_item:
	dev_dbg(user->dev, "[ZHBBINGGE]err_free_list_item\n");
	kfree(tmp);
err_free_mutex:
	dev_dbg(user->dev, "[ZHBBINGGE]err_free_mutex\n");
	mutex_unlock(&main_info->man_mutex);
	return err;
}
...
static void cg2900_close(struct cg2900_user_data *user)
{
	...
	dev_dbg(user->dev, "Wait up to 3 seconds for chip to shut-down\n");
	wait_event_timeout(main_wait_queue,
				(CG2900_IDLE == info->main_state),
...
static struct cg2900_user_data char_gnss_extclk_data = {
	.channel_data = {
		.char_dev_name = CG2900_GNSS_EXTCLK,
	},
	.h4_channel = CHANNEL_CORE,
	.write = cg2900_no_write,
};
...
static struct mfd_cell cg2900_char_devs[] = {
	...
	},
	{
		.name = "cg2900-chardev",
		.id = 8,
		.platform_data = &char_gnss_extclk_data,
		.pdata_size = sizeof(char_gnss_extclk_data),
	},
	{
		.name = "cg2900-chardev",
		.id = 9,
...
static int fetch_firmware_files(struct cg2900_chip_dev *dev,
		struct cg2900_chip_info *info)
{
	int filename_size_ptc = strlen("CG29XX_XXXX_patch.fw");
	int filename_size_ssf = strlen("CG29XX_XXXX_settings.fw");
	int err;
	...
	err = snprintf(info->patch_file_name, filename_size_ptc + 1,
			"CG29XX_%04X_patch.fw", dev->chip.hci_revision);
	...
	err = snprintf(info->settings_file_name, filename_size_ssf + 1,
			"CG29XX_%04X_settings.fw", dev->chip.hci_revision);
...
static bool check_chip_support(struct cg2900_chip_dev *dev)
{
	...
	dev->c_cb.decrement_flow_cnt = decrement_flow_cnt; 
...
module_platform_driver(cg2900_chip_driver);
