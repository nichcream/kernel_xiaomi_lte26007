...
#include "zet6251_fw_hover_20141119_V3.h"

//#define TPD_PROXIMITY

#ifdef CONFIG_TOUCHSCREEN_ZET62XX_PROXIMITY
...
static u8 last_tpd_proximity_state = 1;
...
static struct ZET9151_data *zet9151_datap;
...
///=============================================================================================///
/// 17. 硬件腳位設定: 請根據主芯片進行調整
///=============================================================================================///
#ifdef CONFIG_ARCH_SUN4I
#define CTP_IRQ_NO_A10			(IRQ_EINT21)	///< 設定A10 sun4i 的INT pin 中斷號碼
#else ///< for CONFIG_ARCH_SUN4I
#ifdef CONFIG_ARCH_SUN5I
#define CTP_IRQ_NO_A10		(IRQ_EINT9)	///< 設定A10 sun5i的INT pin 中斷號碼
#endif ///< for CONFIG_ARCH_SUN5I
#endif ///< for CONFIG_ARCH_SUN4I

#define CTP_IRQ_NO_A13				(IRQ_EINT11)	///< 設定A13 的INT pin 中斷號碼
...
/// X, Y Resolution
#define FW_X_RESOLUTION				(800)		///< the FW setting X resolution
#define FW_Y_RESOLUTION				(1280)		///< the FW setting Y resolution
#define X_MAX	 				(800)		///< the X resolution of TP AA(Action Area)
#define Y_MAX 					(1280)		///< the Y resolution of TP AA(Action Area)
...
#define DRIVER_VERSION "$Revision: 66 $"
//static char const *revision="$Revision: 66 $";
...
static struct zet622x_tsdrv *zet62xx_ts;
...
#define FW_FILE_NAME 			"/system/zet62xx.bin"
...
///      "/mnt/extsd/"
///-------------------------------------///
#define TRAN_MODE_FILE_PATH		"/mnt/extsd/"
...
static unsigned int zet62xx_rst_gpio;
...
{
//	ctp_wakeup(0, 0);
	gpio_direction_output(zet62xx_rst_gpio, 0);
}
...
{
	gpio_direction_output(zet62xx_rst_gpio, 1);
}

///**********************************************************************
///   [function]:  ctp_get_system_config
///   [parameters]: void
///   [return]: int
///**********************************************************************
/*
static int ctp_get_system_config(void)
{
	...
	if((twi_id == 0) || (screen_max_x == 0) || (screen_max_y == 0))
        {
		printk("[ZET] : %s:read config error!\n",__func__);
		return 0;
	}
	return 1;
}
...
///**********************************************************************
///   [function]:  zet622x_ts_tran_user_setting_data
///   [parameters]: client
///   [return]: u8
///**********************************************************************
u8 zet622x_ts_tran_user_setting_data(struct i2c_client *client)
{
	...
	printk("[ZET] : zet622x_ts_tran_user_setting_data Path \n");
...
//printk("zhouzf tpd_proximity_flag is  tpd_proximity_flag==%d  hover_status=%d \n",tpd_proximity_flag,hover_status);

	if (tpd_proximity_flag == 1) {

		if (zet622x_ts_hover_status_get()==1) {
			tpd_proximity_detect = 0;  //close  lcd
			//  printk("zhouzf  close lcd  hover_status=%d \n",hover_status);
		} else {
			//printk("zhouzf  open lcd  hover_status=%d \n",hover_status);

			tpd_proximity_detect = 1;   //open lcd led
		}

		if(last_tpd_proximity_state != tpd_proximity_detect) {
			//printk("wake_up_interruptible\n");
			//	printk("zhouzf tpd_proximity_detect is %d\n",tpd_proximity_detect);
			zet9151_datap->prox_event = 1;
			//printk("zhouzf %s,ZET9151_datap->prox_event is %d\n",__func__,zet9151_datap->prox_event);
			wake_up_interruptible(&zet9151_datap->proximity_event_wait);
		}
...
#ifdef CONFIG_TOUCHSCREEN_ZET62XX_PROXIMITY
	//printk("@@-------%s  tpd_proximity_flag=%d \n",__FUNCTION__,tpd_proximity_flag);
...
{
	struct zet622x_tsdrv *ts
	=  container_of(handler, struct zet622x_tsdrv, early_suspend);
...
static long zet_fops_ioctl(struct file *file, unsigned int cmd, unsigned long arg )
{
	...
	if(cmd == ZET_IOCTL_CMD_FLASH_READ) {
		...
		printk("[ZET]: zet_ioctl: Set tran type =%d\n", transfer_type);
		zet622x_cmd_ioctl_write_data(this_client, buf[0], &buf[1]);
...
static int tpd_enable_ps(int enable)
{
	...
	if (enable) {
		...
		//printk("zhouzf tpd-ps function is on\n");
	} else {
		tpd_proximity_flag = 0;
		wake_unlock(&ps_lock);
		//printk("zhouzf tpd-ps function is off\n");
	}
	printk("zhouzf tpd_proximity_flag is %d\n",tpd_proximity_flag);
	return 0;
}
                         ...
                         char *buf, size_t count, loff_t *ppos)
{
	printk("zhouzf %s\n",__func__);
	int len, err;
	struct ZET9151_data *data = (struct ZET9151_data*)file->private_data;
	...
	if (!data->prox_event|| !buf)
		return 0;
	data->prox_state = tpd_proximity_detect; //zhouzf

	len = sizeof(data->prox_state);
	printk("%s: lux_state=0x%x\n", __func__, data->prox_state);
...
static unsigned int prox_poll(struct file *file, struct poll_table_struct *poll)
{
	...
	//printk("zhouzf %s step2, data->prox_event is %d\n",__func__,data->prox_event);
	if (data->prox_event) {
		mask |= POLLIN | POLLRDNORM;
		printk("zhouzf %s, data->prox_event is %d\n",__func__,data->prox_event);
	}
	return mask;
}
...
//	printk("zhouzf %s\n",__func__);
	int retval = 0;
	unsigned int flag = 0;
	struct ZET9151_data * data = file->private_data;
	int ret;
	int err = 0;
	switch (cmd) {
...
{
	file->private_data = zet9151_datap;
	// Do nothing!!!
	printk("%s\n",__func__);
	return 0;
}
...
const struct file_operations zet9151_prox_fops = {
	.owner = THIS_MODULE,
	.read = prox_read,
	.poll = prox_poll,
...
static int  zet622x_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	struct zet62xx_platform_data *info;
	struct i2c_dev *i2c_dev;
	struct device *dev;
	char name[64];
...
#ifdef CONFIG_TOUCHSCREEN_ZET62XX_PROXIMITY
	zet9151_datap = kmalloc(sizeof(struct ZET9151_data), GFP_KERNEL);
	if (!zet9151_datap) {
		printk("%s:Cannot get memory!\n", __func__);
		return -ENOMEM;
	}
	memset(zet9151_datap,0,sizeof(struct ZET9151_data));

	init_waitqueue_head(&zet9151_datap->proximity_event_wait);

	zet9151_datap->prox_dev.minor = MISC_DYNAMIC_MINOR;
	zet9151_datap->prox_dev.name = "proximity";
	zet9151_datap->prox_dev.fops = &zet9151_prox_fops;

	ret = misc_register(&(zet9151_datap->prox_dev));
	if (ret) {
...
//setup reset gpio
	zet62xx_rst_gpio = info->reset_gpio;
	err = gpio_request(zet62xx_rst_gpio, "zet62xx_reset_gpio");
	if(err) {
		printk(KERN_ERR "et62xx_ts reset_gpio_request fail \n");
	}
	gpio_direction_output(zet62xx_rst_gpio, 1);//default high
//setup reset gpio
...
//	zet_tran_type_set_file_name(TRAN_MODE_FILE_PATH);
	...
	///-----------------------------------------------///
	resolution_x = info->max_scr_x;
	resolution_y = info->max_scr_y;

	finger_num   = FINGER_NUMBER;//change later
	key_num      = KEY_NUMBER;   //
	if(key_num == 0) {
		finger_packet_size  = 3 + 4*finger_num;
	} else {
	...
	///-----------------------------------------------///
	input_mt_init_slots(input_dev, finger_num, INPUT_MT_DIRECT);
#endif ///< for FEATURE_MT_TYPE_B
	...
	zet6221_ts->gpio =  info->irq_gpio;
	zet6221_ts->irq = gpio_to_irq(info->irq_gpio);
	if(gpio_request(info->irq_gpio, "zet62xx")) {
		printk(KERN_ERR "zet62xx_ts gpio_request fail \n");
		goto LABEL_REQUEST_IRQ_FAIL;
	}
	gpio_direction_input(info->irq_gpio);
	err = request_irq(zet6221_ts->irq, zet6221_ts_interrupt, IRQF_TRIGGER_FALLING, "zet6221_ts", zet6221_ts);
	if (err<0) {
		printk(KERN_ERR "[ZET] : zet6221_ts_probe.request_irq failed. err %d=\n",err);
		goto LABEL_REQUEST_IRQ_FAIL;
	}
#endif ///< for FEATURE_INT_FREE
...
{
	return i2c_add_driver(&zet622x_i2c_driver);
}
