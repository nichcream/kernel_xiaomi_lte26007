...
bool rtw_p2p_check_action_frames(_adapter *padapter, const u8 *buf, u32 len, u8 tx, u8 *category, u8 *action, u8 *OUI_Subtype)
{
	bool is_p2p_frame = _FALSE;
	unsigned char	*frame_body;
	u8 dialogToken=0;
	struct rtw_wdev_priv *pwdev_priv = wdev_to_priv(padapter->rtw_wdev);
...
#ifdef CONFIG_WFDS
		else if(*action == ACT_PUBLIC_GAS_INITIAL_REQ || *action == ACT_PUBLIC_GAS_INITIAL_RSP)
		{
			is_p2p_frame = _TRUE;
		}
#endif
