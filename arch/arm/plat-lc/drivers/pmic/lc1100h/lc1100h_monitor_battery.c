...
#include <plat/lc1100h_monitor_battery.h>
#include <plat/comip-battery.h>
#include <plat/lc1100h_adc.h>
#include <plat/usb.h>

#include "lc1100h_capacity_lookup.h"
...
#define PMIC_BATTERY_TEMP_CHANNEL    (1)
...
static int pmic_write_byte(u8 reg, u8 value)
{
    int ret = 0;
    ret = lc1100h_reg_write(reg, value);
    if(ret)
        pr_err("%s: Error access to reg = (%x)\n", __func__, reg);
    return ret;
}

static int pmic_read_byte(u8 reg,u8 *value)
{
    int ret = 0;

    ret = lc1100h_reg_read(reg,value);
    if(ret)
        pr_err("%s: Error access to reg = (%x)\n", __func__, reg);
    return ret;
}
...
static void lc1100h_charger_control_reg0x30_set(struct monitor_battery_device_info *di)
{
    u8 data = 0;

    data = (di->param.enable_iterm << LC1100H_EOC_SHIFT) | (CHGPROT_EN << LC1100H_CHGPROT_SHIFT)|
        (TEMPON_EN << LC1100H_TEMPONEN_SHIFT) | di->param.enable_charger;
    pmic_write_byte(LC1100H_CHARGER_CTRL_REG0x30, data);
    return;
}

static void lc1100h_charger_voltage_reg0x31_set(struct monitor_battery_device_info *di)
{
    u8 data = 0;
    u8 volt = 0,rechg = 0;
    int rechgmV = 0;
    ...
    data = (volt << LC1100H_VOLT_SHIFT)|
        (VHYSSEL_100 << LC1100H_VHYSSEL_SHIFT)|rechg;
  
    pmic_write_byte(LC1100H_CHARGER_VOLT_REG0x31, data);
    return;
}

static void lc1100h_charger_current_reg0x32_set(struct monitor_battery_device_info *di)
{
    u8 data = 0;
   ...
   if(term_current > TERM_CURRENT_MAX)
    ...
    data = (iterm << LC1100H_ITERM_SHIFT) | ibat;  
    pmic_write_byte(LC1100H_CHARGER_CURRENT_REG0x32, data);
    return;
}

static void lc1100h_charger_timer_reg0x33_set(struct monitor_battery_device_info *di)
{
    u8 data = 0;
    u8 timenum = 0;
   ...
   data = (timenum << LC1100H_CHARGE_TIMER_SHIFT) |
        (di->param.timer_clear << 3) | (di->param.timer_stop);

    pmic_write_byte(LC1100H_CHARGER_TIMER_REG0x33, data);
    return;
}
...
{
    lc1100h_charger_control_reg0x30_set(di);
    lc1100h_charger_voltage_reg0x31_set(di);
    lc1100h_charger_current_reg0x32_set(di);
    lc1100h_charger_timer_reg0x33_set(di);
    return 0;
}
...
static void monitor_battery_enable_charger(struct monitor_battery_device_info *di,bool enable)
{
    ...
    di->param.enable_charger = di->param.enable_charger & di->hand_flag;
    lc1100h_charger_control_reg0x30_set(di);
    //lc1100h_charger_current_reg0x32_set(di);
#else
...
static int monitor_adc_channel_voltage(int channel)
{
    ...
    /* unit is mV */
    temp = pmic_get_adc_conversion(PMIC_BATTERY_VOLT_CHANNEL);
...
#define ADC_SAMPLE_VOLT_TIMES  10
static int pmic_get_filter_battery_voltage(int channel)
{
    int i = 0,j = 0,t;
    u32 sum = 0;
    int rbuf_volt[ADC_SAMPLE_VOLT_TIMES] = {0};

    for(i=0; i < ADC_SAMPLE_VOLT_TIMES; i++){
        rbuf_volt[i] = monitor_adc_channel_voltage(PMIC_BATTERY_VOLT_CHANNEL);
    }
    for(j=1; j<ADC_SAMPLE_VOLT_TIMES; j++)
        for(i=0; i<ADC_SAMPLE_VOLT_TIMES-j; i++)
    ...
    for(i=1; i<ADC_SAMPLE_VOLT_TIMES-1; i++)
        sum= sum + rbuf_volt[i];
...
static int monitor_get_battery_voltage(void)
{
    ...
    di->rbuf_volt[index] = pmic_get_filter_battery_voltage(PMIC_BATTERY_VOLT_CHANNEL);
     for(i=0; i < VOLT_FILTER_LEN; i++){
        sum = sum + di->rbuf_volt[i];
     }
...
static int monitor_get_battery_health(void)
{
    ...
    ret = pmic_read_byte(LC1100H_POWER_STATUS_REG0x35, &data);
    if (ret)
        return ret;
...
static int monitor_get_battery_capacity(void)
{
    ...
    if((di->system_state & LCD_IS_ON) == LCD_IS_ON){
        time_pulse =  _now.tv_sec - di->history_timeval;
        if(time_pulse < 60)
            goto out2;
        ...
        if(di->charge_status == POWER_SUPPLY_STATUS_CHARGING){
            if(di->charger_source == POWER_SUPPLY_TYPE_MAINS) {
                if(di->history_capacity > curr_capacity){
                    goto out1;
                }else if (curr_capacity > di->history_capacity + 5){
                    if(time_pulse < 300)
                        curr_capacity = di->history_capacity + 5;
                }
            }else{
                if((curr_capacity - di->history_capacity) > 1){
                    curr_capacity = di->history_capacity + 1;
...
static int pmic_check_vbus_present(struct monitor_battery_device_info *di)
{
    ...
    ret = pmic_read_byte(LC1100H_POWER_STATUS_REG0x35, &present_charge_state);
    if(ret){
        dev_err(di->dev, "check vbus present err %x\n",LC1100H_POWER_STATUS_REG0x35);
    }
...
static void monitor_charger_interrupt(int event, void *puser, void* pargs)
{
...
#ifndef CONFIG_EXTERN_CHARGER
    ret = pmic_read_byte(LC1100H_CHARGER_STATUS_REG0x34, &pmic_charge_sts);
    if(ret){
        goto err;
    }

    if(pmic_charge_sts & LC1100H_CHARGE_TIMEOUT){
        dev_info(di->dev, "charge over time\n");
    }

    if(pmic_charge_sts & LC1100H_ADPOV_STATUS){
    ...
    if(pmic_charge_sts & LC1100H_BATOT_STATUS){
        dev_dbg(di->dev, "battery temperature fault\n");
    }

    if(pmic_charge_sts & LC1100H_RECHG_STATUS){
        dev_dbg(di->dev, "recharge\n");
        di->charge_status = POWER_SUPPLY_STATUS_CHARGING;
    }

    if(!(pmic_charge_sts & LC1100H_CHGFAULT_STATUS)){
        dev_dbg(di->dev, "charge fault\n");
        charger_fault = 1;
    }

    if(pmic_charge_sts & LC1100H_CHARGE_DONE){
...
static void monitor_battery_temp_charging(struct monitor_battery_device_info *di,bool enable)
{
    ...
    pmic_read_byte(LC1100H_CHARGER_STATUS_REG0x34, &pmic_charge_sts);

    if((di->param.enable_charger)&&
            ((pmic_charge_sts & LC1100H_ADPOV_STATUS)!= LC1100H_ADPOV_STATUS)){
        if(di->capacity < 100)
            di->charge_status = POWER_SUPPLY_STATUS_CHARGING;
    }else{
        di->charge_status = POWER_SUPPLY_STATUS_NOT_CHARGING;
...
static void monitor_battery_update_data(struct monitor_battery_device_info *di)
{
    ...
    if(di->battery_info->battery_current){
        di->bat_current_mA = di->battery_info->battery_current();
    }
                ...
                struct device_attribute *attr, char *buf)
{
    ...
    int i =0;
    for(i=0;i<6;i++){
        pmic_read_byte(i+48,&read_reg[i]);
    }
...
static const struct file_operations monitor_battery_operations = {
        .owner = THIS_MODULE,
        .read  = monitor_battery_proc_read,
...
static int __init monitor_battery_probe(struct platform_device *pdev)
{
    ...
    di->lcvbus = lcusb_get_transceiver();
    if (di->lcvbus) {
        ...
        dev_err(&pdev->dev, "lcusb_get_transceiver failed \n");
    }
    ...
    dev_info(&pdev->dev,"Initialized LC1100h Monitor Battery module\n");
    return 0;
