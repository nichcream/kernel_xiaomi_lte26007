	...
	)
{
	...
	u4Tmp = pBtCoexist->fBtcRead4Byte(pBtCoexist, 0x68);
	BTC_PRINT(BTC_MSG_ALGORITHM, ALGO_BT_MONITOR, ("[BTCoex], check bt disable or not, 0x68=0x%x!!\n", u4Tmp));
	if(BIT28 != (u4Tmp & BIT28))
	{
		bBtDisableBy68 = TRUE;
		bBtActive = FALSE;
		BTC_PRINT(BTC_MSG_ALGORITHM, ALGO_BT_MONITOR, ("[BTCoex], check bt is disabled by 0x68[28]=0!!\n"));
	}
	...
	)
{
	...
	BTC_PRINT(BTC_MSG_ALGORITHM, ALGO_TRACE, ("[BTCoex], numOfDiffProfile:%d\n",numOfDiffProfile));
...
VOID
halbtc8723b2ant_SetFwBtLnaConstrain(
	IN	PBTC_COEXIST		pBtCoexist,
...
VOID
halbtc8723b2ant_SetBtLnaConstrain(
	IN	PBTC_COEXIST		pBtCoexist,
	IN	BOOLEAN			bForceExec,
	IN	BOOLEAN			bBtLnaConsOn
	)
{
	...
	halbtc8723b2ant_SetFwBtLnaConstrain(pBtCoexist, pCoexDm->bCurBtLnaConstrain);

	pCoexDm->bPreBtLnaConstrain = pCoexDm->bCurBtLnaConstrain;
}

VOID
halbtc8723b2ant_SetFwBtPsdMode(
	IN	PBTC_COEXIST		pBtCoexist,
	IN	u1Byte			btPsdMode
	)
{
	...
	)
{
	...
	halbtc8723b2ant_SetFwBtPsdMode(pBtCoexist, pCoexDm->bCurBtPsdMode);
	...
	)
{
	...
	pBtCoexist->fBtcWrite1ByteBitMask(pBtCoexist, 0x883, 0xf8, val);
}
	...
	)
{
	...
	//halbtc8723b2ant_LowPenaltyRa(pBtCoexist, NORMAL_EXEC, bLowPenaltyRA);
  	...
  	//no limited DIG
	//halbtc8723b2ant_SetBtLnaConstrain(pBtCoexist, NORMAL_EXEC, bBTLNAConstrain);
}
	...
	)
{
	...
	if(bTurnOn)
	{
		switch(type)
		{
			...
			case 3:
				...
				break;
			case 4:
				halbtc8723b2ant_SetFwPstdma(pBtCoexist, 0xa3, 0x5, 0x5, 0xe1, 0x90);
				break;
			case 5:
			...
			case 7:
				...
				break;
			case 8:
				halbtc8723b2ant_SetFwPstdma(pBtCoexist, 0xa3, 0x5, 0x5, 0x60, 0x90);
				break;
			case 9:
				halbtc8723b2ant_SetFwPstdma(pBtCoexist, 0xe3, 0x1a, 0x1a, 0xe1, 0x90);
				break;
	...
	else
	{
		...
		{
			case 0:
				halbtc8723b2ant_SetFwPstdma(pBtCoexist, 0x0, 0x0, 0x0, 0x0, 0x0);
				break;
			case 1:
				halbtc8723b2ant_SetFwPstdma(pBtCoexist, 0x0, 0x0, 0x0, 0x8, 0x0);
				break;
			default:
				halbtc8723b2ant_SetFwPstdma(pBtCoexist, 0x0, 0x0, 0x0, 0x0, 0x0);
				break;
	...
	)
{
	...
	// hw all off
	halbtc8723b2ant_CoexTable(pBtCoexist, NORMAL_EXEC, 0x55555555, 0x55555555, 0xffff, 0x3);
}
	...
	)
{
	...
	halbtc8723b2ant_CoexTable(pBtCoexist, FORCE_EXEC, 0x55555555, 0x55555555, 0xffff, 0x3);
...
VOID
halbtc8723b2ant_BtInquiryPage(
	IN	PBTC_COEXIST		pBtCoexist
	)
{
	halbtc8723b2ant_CoexTable(pBtCoexist, NORMAL_EXEC, 0x55ff55ff, 0x5afa5afa, 0xffff, 0x3);
	...
	)
{
	...
	if(!bWifiConnected &&
		BT_8723B_2ANT_BT_STATUS_IDLE == pCoexDm->btStatus)
	{
			...
			(BT_8723B_2ANT_BT_STATUS_IDLE == pCoexDm->btStatus) )
	{
		...
		(BT_8723B_2ANT_BT_STATUS_CONNECTED_IDLE == pCoexDm->btStatus) )
	{
		...
		(BT_8723B_2ANT_BT_STATUS_CONNECTED_IDLE == pCoexDm->btStatus) )
	{
		...
		halbtc8723b2ant_SwMechanism1(pBtCoexist,TRUE,TRUE,TRUE,TRUE);
		halbtc8723b2ant_SwMechanism2(pBtCoexist,FALSE,FALSE,FALSE,0x18);

		bCommon = TRUE;
	}
	else if(!bWifiConnected &&
			(BT_8723B_2ANT_BT_STATUS_NON_IDLE == pCoexDm->btStatus) )
	{
	...
	)
{
	...
	// when halbtc8723b2ant_TdmaDurationAdjust() is called, fw dac swing is included in the function.
	//if(pCoexDm->psTdmaDuAdjType == 71)
	//	halbtc8723b2ant_FwDacSwingLvl(pBtCoexist, NORMAL_EXEC, 0xc);
	//else
		halbtc8723b2ant_FwDacSwingLvl(pBtCoexist, NORMAL_EXEC, 0x6);
}
	...
	)
{
	...
	{
		halbtc8723b2ant_CoexTable(pBtCoexist, NORMAL_EXEC, 0x5a5a5a5a, 0x5a5a5a5a, 0xffff, 0x3);
	}
	else  //for SCO quality & wifi performance balance at 11n mode
	{
		halbtc8723b2ant_CoexTable(pBtCoexist, NORMAL_EXEC, 0x5aea5aea, 0x5aea5aea, 0xffff, 0x3);
	}
	...
	if(BTC_WIFI_BW_HT40 == wifiBw)
	{
		...
		if( (wifiRssiState == BTC_RSSI_STATE_HIGH) ||
			(wifiRssiState == BTC_RSSI_STATE_STAY_HIGH) )
		{
			halbtc8723b2ant_SwMechanism2(pBtCoexist,TRUE,TRUE,FALSE,0x18);
		}
		else
		{
			halbtc8723b2ant_SwMechanism2(pBtCoexist,FALSE,TRUE,FALSE,0x18);
		}
	}
	else
	{
	...
	)
{
	...
	{
		halbtc8723b2ant_CoexTable(pBtCoexist, NORMAL_EXEC, 0x55ff55ff, 0x5a5a5a5a, 0xffff, 0x3);
	}
	else  //for HID quality & wifi performance balance at 11n mode
	{
		halbtc8723b2ant_CoexTable(pBtCoexist, NORMAL_EXEC, 0x55ff55ff, 0x5aea5aea, 0xffff, 0x3);
	}
	...
	)
{
	...
	//fw dac swing is called in halbtc8723b2ant_TdmaDurationAdjust()
	//halbtc8723b2ant_FwDacSwingLvl(pBtCoexist, NORMAL_EXEC, 6);
	...
	)
{
	...
	if (BTC_WIFI_BW_LEGACY == wifiBw) //for HID at 11b/g mode
	{
		halbtc8723b2ant_CoexTable(pBtCoexist, NORMAL_EXEC, 0x55ff55ff, 0x5aff5aff, 0xffff, 0x3);
	}
	else  //for HID quality & wifi performance balance at 11n mode
	{
		halbtc8723b2ant_CoexTable(pBtCoexist, NORMAL_EXEC, 0x55ff55ff, 0x5aff5aff, 0xffff, 0x3);
	...
	)
{
	...
	if (BTC_WIFI_BW_LEGACY == wifiBw) //for HID at 11b/g mode
	{
		halbtc8723b2ant_CoexTable(pBtCoexist, NORMAL_EXEC, 0x55ff55ff, 0x5a5f5a5f, 0xffff, 0x3);
	}
	else  //for HID quality & wifi performance balance at 11n mode
	{
		halbtc8723b2ant_CoexTable(pBtCoexist, NORMAL_EXEC, 0x55ff55ff, 0x5a5f5a5f, 0xffff, 0x3);
	...
	)
{
	...
	//if(pStackInfo->bProfileNotified)
	{
		...
		if(pCoexSta->bC2hBtInquiryPage && (BT_8723B_2ANT_COEX_ALGO_PANHS!=algorithm))
		{
			...
			halbtc8723b2ant_BtInquiryPage(pBtCoexist);
			return;
		}
	...
	)
{
	...
	u4Tmp = pBtCoexist->fBtcRead4Byte(pBtCoexist, 0x38); //Ant select enable
	u4Tmp |= BIT11;
	pBtCoexist->fBtcWrite4Byte(pBtCoexist, 0x38, u4Tmp);

	pBtCoexist->fBtcWrite1Byte(pBtCoexist, 0x974, 0xff);
	pBtCoexist->fBtcWrite1ByteBitMask(pBtCoexist, 0x944, 0x3, 0x3);
	pBtCoexist->fBtcWrite1Byte(pBtCoexist, 0x930, 0x77);
	pBtCoexist->fBtcWrite1ByteBitMask(pBtCoexist, 0x92c, 0x3, 0x1); //Main Anernna
	// 0x790[5:0]=0x5
	...
	)
{
	...
	CL_SPRINTF(cliBuf, BT_TMP_BUF_SIZE, "\r\n %-35s = [%s/ %d/ %d] ", "BT [status/ rssi/ retryCnt]", \
		((pCoexSta->bC2hBtInquiryPage)?("inquiry/page scan"):((BT_8723B_2ANT_BT_STATUS_IDLE == pCoexDm->btStatus)? "idle":(  (BT_8723B_2ANT_BT_STATUS_CONNECTED_IDLE == pCoexDm->btStatus)? "connected-idle":"busy"))),
	...
	//u1Tmp[2] = pBtCoexist->fBtcRead1Byte(pBtCoexist, 0x790);
	//u1Tmp[3] = pBtCoexist->fBtcRead1Byte(pBtCoexist, 0x796);
	//CL_SPRINTF(cliBuf, BT_TMP_BUF_SIZE, "\r\n %-35s = 0x%x/ 0x%x/ 0x%x/ 0x%x", "0x778/ 0x783/ 0x790/ 0x796", u1Tmp[0], u1Tmp[1], u1Tmp[2], u1Tmp[3]);

	CL_SPRINTF(cliBuf, BT_TMP_BUF_SIZE, "\r\n %-35s = 0x%x/ 0x%x", "0x778/ 0x880", \
		u1Tmp[0], u4Tmp[0]);
	CL_PRINTF(cliBuf);
	...
	//CL_SPRINTF(cliBuf, BT_TMP_BUF_SIZE, "\r\n %-35s = 0x%x", "0x8db[30:29]", 	((u1Tmp[0]&0x60)>>5));
	//CL_PRINTF(cliBuf);
	...
	u4Tmp[1] = pBtCoexist->fBtcRead4Byte(pBtCoexist, 0x64);
	CL_SPRINTF(cliBuf, BT_TMP_BUF_SIZE, "\r\n %-35s = 0x%x/ 0x%x/ 0x%x/ 0x%x", "0x38[11]/ 0x40/ 0x4c[24:23]/ 0x64", \
		((u1Tmp[0] & 0x8)>>3), u1Tmp[1], ((u4Tmp[0]&0x01800000)>>23), u4Tmp[1]);
	CL_PRINTF(cliBuf);
	...
	//CL_SPRINTF(cliBuf, BT_TMP_BUF_SIZE, "\r\n %-35s = 0x%x", "0x41b (mgntQ hang chk == 0xf)", 	u1Tmp[0]);
	//CL_PRINTF(cliBuf);
	...
	)
{
	...
	BTC_PRINT(BTC_MSG_INTERFACE, INTF_NOTIFY, ("[BTCoex], 2Ant Bt info[%d], length=%d, hex data=[", rspSource, length));
	for(i=0; i<length; i++)
	{
	...
	if(BT_INFO_SRC_8723B_2ANT_WIFI_FW != rspSource)
	{
		...
		if(setBtPsdMode <= 3)
		{
			halbtc8723b2ant_SetBtPsdMode(pBtCoexist, FORCE_EXEC, 0x0);
			setBtPsdMode++;
		}
		...
		if(pCoexDm->bCurBtLnaConstrain)
		{
			...
			else
			{
				if(setBtLnaCnt <= 3)
				{
					halbtc8723b2ant_SetBtLnaConstrain(pBtCoexist, FORCE_EXEC, TRUE);
					setBtLnaCnt++;
				}
			}
		}
	...
	if(btInfo & BT_INFO_8723B_2ANT_B_INQ_PAGE)
	{
		...
		pCoexDm->btStatus = BT_8723B_2ANT_BT_STATUS_NON_IDLE;
	}
	else
	{
		...
		if(btInfo == 0x1)	// connection exists but no busy
		{
			...
			pStackInfo->bBtLinkExist = TRUE;

			pCoexDm->btStatus = BT_8723B_2ANT_BT_STATUS_CONNECTED_IDLE;
		}
		else if(btInfo & BT_INFO_8723B_2ANT_B_CONNECTION)	// connection exists and some link is busy
		{
			pCoexSta->bBtLinkExist = TRUE;
			pStackInfo->bBtLinkExist = TRUE;
			if(btInfo & BT_INFO_8723B_2ANT_B_FTP){
				pCoexSta->bPanExist = TRUE;
				pStackInfo->bPanExist = TRUE;
			}
			else{
				pCoexSta->bPanExist = FALSE;
				pStackInfo->bPanExist = FALSE;
			}
			if(btInfo & BT_INFO_8723B_2ANT_B_A2DP){
				pCoexSta->bA2dpExist = TRUE;
				pStackInfo->bA2dpExist = TRUE;
			}
			else{
				pCoexSta->bA2dpExist = FALSE;
				pStackInfo->bA2dpExist = FALSE;
			}
			if(btInfo & BT_INFO_8723B_2ANT_B_HID){
				pCoexSta->bHidExist = TRUE;
				pStackInfo->bHidExist = TRUE;
			}
			else{
				pCoexSta->bHidExist = FALSE;
				pStackInfo->bHidExist = FALSE;
			}
			if(btInfo & BT_INFO_8723B_2ANT_B_SCO_ESCO){
				pCoexSta->bScoExist = TRUE;
				pStackInfo->bScoExist = TRUE;
			}
			else{
				pCoexSta->bScoExist = FALSE;
				pStackInfo->bScoExist = FALSE;
			}
			pCoexDm->btStatus = BT_8723B_2ANT_BT_STATUS_NON_IDLE;
		}
		else
		{
			...
			pCoexSta->bHidExist = FALSE;
			pCoexSta->bScoExist = FALSE;
			pStackInfo->bBtLinkExist = FALSE;
			pStackInfo->bPanExist = FALSE;
			pStackInfo->bA2dpExist = FALSE;
			pStackInfo->bHidExist = FALSE;
			pStackInfo->bScoExist = FALSE;
			pCoexDm->btStatus = BT_8723B_2ANT_BT_STATUS_IDLE;
		}
...
BTC_PRINT(BTC_MSG_INTERFACE, INTF_NOTIFY, ("[BTCoex], pCoexSta->bPanExist=%d \n",pCoexSta->bPanExist));
BTC_PRINT(BTC_MSG_INTERFACE, INTF_NOTIFY, ("[BTCoex], pCoexSta->bA2dpExist=%d \n",pCoexSta->bA2dpExist));
BTC_PRINT(BTC_MSG_INTERFACE, INTF_NOTIFY, ("[BTCoex], pCoexSta->bScoExist=%d \n",pCoexSta->bScoExist));
BTC_PRINT(BTC_MSG_INTERFACE, INTF_NOTIFY, ("[BTCoex], pCoexDm->btStatus=%d \n",pCoexDm->btStatus));

	if(BT_8723B_2ANT_BT_STATUS_NON_IDLE == pCoexDm->btStatus)
	{
		bBtBusy = TRUE;
	}
	else
	...
	if(BT_8723B_2ANT_BT_STATUS_IDLE != pCoexDm->btStatus)
	{
		bLimitedDig = TRUE;
	}
	else
	...
	)
{
	...
	if(bWifiUnder5G)
	{
		BTC_PRINT(BTC_MSG_ALGORITHM, ALGO_TRACE, ("[BTCoex], Wifi is under 5G, coexist All Off!!!\n"));
		halbtc8723b2ant_CoexAllOff(pBtCoexist);
		return;
	}
