	...
	)
{
	...
	rtw_set_ps_mode(Adapter, PS_MODE_MIN, 0, pBtCoexist->btInfo.lpsVal);
}
	...
	)
{
	...
	//	smart_ps = 0;
	//rtw_set_ps_mode(Adapter, PS_MODE_MIN, smart_ps, pBtCoexist->btInfo.lpsVal);
}

/* we have leave 32k when we call this function */
/* so just set bBtDisableLowPwr is ok */
VOID
	...
	)
{
	...
	switch(getType)
	{
		...
		case BTC_GET_BL_WIFI_CONNECTED:
			{
				...
				if (check_fwstate(&Adapter->mlmepriv, WIFI_ASOC_STATE) == _TRUE)
					bTmp = TRUE; /* AP always true here */
#ifdef CONFIG_CONCURRENT_MODE
				if (_FALSE == bTmp)
				{
					if (check_fwstate(&Adapter->pbuddy_adapter->mlmepriv, WIFI_ASOC_STATE))
						bTmp = TRUE;
				}
#endif // CONFIG_CONCURRENT_MODE
				*pBoolean = bTmp;
		...
		case BTC_GET_BL_WIFI_BUSY:
			if(halbtcoutsrc_IsWifiBusy(Adapter))
				*pBoolean = TRUE;
			else
				*pBoolean = FALSE;
			break;
		case BTC_GET_BL_WIFI_BUSY_TDMA:
			if(halbtcoutsrc_IsWifiBusy(Adapter) &&
				(/*Adapter->mlmepriv.LinkDetectInfo.MaxRxBytesInPeriod < 300000 || */
				pwrctrlpriv->in_early_suspend)) {
				if (Adapter->drv_in_test == CMCC_RX_PWR_TEST)
					*pBoolean = TRUE;
				else
					*pBoolean = FALSE;
			} else {
		...
		case BTC_GET_BL_WIFI_SCAN:
			...
			if (FALSE == *pBoolean)
			{
				if (check_fwstate(&Adapter->pbuddy_adapter->mlmepriv, WIFI_SITE_MONITOR))
					*pBoolean = TRUE;
				else
					*pBoolean = FALSE;
			}
		...
		case BTC_GET_BL_WIFI_LINK:
			...
			if (_FALSE == *pBoolean)
			{
				if (check_fwstate(&Adapter->pbuddy_adapter->mlmepriv, WIFI_UNDER_LINKING))
					*pBoolean = TRUE;
				else
					*pBoolean = FALSE;
			}
		...
		case BTC_GET_BL_WIFI_4_WAY_PROGRESS:
			...
			if(*pBoolean == FALSE)
			{
				if(check_fwstate(&Adapter->pbuddy_adapter->mlmepriv, WIFI_STATION_STATE) &&
					(Adapter->pbuddy_adapter->securitypriv.dot11AuthAlgrthm == dot11AuthAlgrthm_8021X ||
					Adapter->pbuddy_adapter->securitypriv.dot11AuthAlgrthm == dot11AuthAlgrthm_WAPI ||
					check_fwstate(&Adapter->pbuddy_adapter->mlmepriv, WIFI_UNDER_WPS) == _TRUE) &&
					(Adapter->pbuddy_adapter->securitypriv.bStaInstallPairwiseKey == _FALSE)){
					*pBoolean = TRUE;
				}
				else
					*pBoolean = FALSE;
		...
		case BTC_GET_BL_WIFI_AP_MODE_ENABLE:
			...
			if (_FALSE == *pBoolean)
			{
				if (check_fwstate(&Adapter->pbuddy_adapter->mlmepriv, WIFI_AP_STATE))
					*pBoolean = TRUE;
				else
					*pBoolean = FALSE;
			}
		...
		case BTC_GET_U4_WIFI_FW_VER:
			*pU4Tmp = (pHalData->FirmwareVersion << 16 |pHalData->FirmwareSubVersion);//Adapter->MgntInfo.FirmwareVersion;
			break;
	...
	)
{
	...
	switch(setType)
	{
		...
		case BTC_SET_BL_INC_SCAN_DEV_NUM:
			pBtCoexist->btInfo.bIncreaseScanDevNum = *pBoolean;
			break;
		...
		case BTC_SET_U1_LPS_VAL:
			if (check_fwstate(&Adapter->mlmepriv, WIFI_AP_STATE) == _TRUE)
				pBtCoexist->btInfo.lpsVal = 0;
			else
				pBtCoexist->btInfo.lpsVal = *pU1Tmp;
			break;
		case BTC_SET_U1_RPWM_VAL:
			if (check_fwstate(&Adapter->mlmepriv, WIFI_AP_STATE) == _TRUE)
				pBtCoexist->btInfo.rpwmVal = 0;
			else
		...
		case BTC_SET_ACT_DISABLE_LOW_POWER:
			halbtcoutsrc_DisableLowPower(pBtCoexist, *pBoolean);
			break;
...
VOID
halbtcoutsrc_tdma_change(
	IN	PBTC_COEXIST		pBtCoexist,
	IN	pu1Byte				pCmdBuffer
	)
{
	PADAPTER		Adapter=pBtCoexist->Adapter;
	u1Byte			byte1 = pCmdBuffer[0];
	u1Byte			byte2 = pCmdBuffer[1];
	u1Byte			byte3 = pCmdBuffer[2];
	u1Byte			byte4 = pCmdBuffer[3];
	u1Byte			byte5 = pCmdBuffer[4];

	// byte1[1:0] != 0 means enable pstdma
	// for 2Ant bt coexist, if byte1 != 0 means enable pstdma
	if(byte1 & BIT0) {
		/* change pspoll mode here */
		if (rtw_check_iot_peer_ps_should_pspoll(Adapter)) {
			if(!(byte1 & BIT5)) { //NoPS = 0, psmode open
				BTC_PRINT(BTC_MSG_INTERFACE, INTF_NOTIFY, ("[BTCoex], TDMA parameter change pspoll mode\n"));
				byte1 &= ~BIT4;//AutoWakeUp = 0, close null
				byte1 |= BIT2;//TxPspollIntheMedium = 1, open pspoll
			}
		}
	}
	...
	)
{
	...
	/* TDMA should open EN_BCN_FUNCTION because TDMA and power save all need it */
	if (elementId == 0x60) {
		u1Byte	u1Tmp = 0;
		u1Tmp = rtw_read8(Adapter, 0x550) | BIT(3);
		rtw_write8(Adapter, 0x550, u1Tmp);
	}

	//TDMA parameter should changed some times
	if (elementId == 0x60) {
		printk("%x%x%x%x%x\n", pCmdBuffer[0],pCmdBuffer[1],pCmdBuffer[2],pCmdBuffer[3],pCmdBuffer[4]);
		halbtcoutsrc_tdma_change(pBtcContext, pCmdBuffer);
		printk("%x%x%x%x%x\n", pCmdBuffer[0],pCmdBuffer[1],pCmdBuffer[2],pCmdBuffer[3],pCmdBuffer[4]);
	}

	//if(elementId != 0x66 && elementId != 0x61)
	rtw_hal_fill_h2c_cmd(Adapter, elementId, cmdLen, pCmdBuffer);
}
		...
		)
{
	...
	BTC_PRINT(BTC_MSG_ALGORITHM, ALGO_TRACE_FW_EXEC, ("[BTCoex], FW write 0x25(1bytes)=%x\n",
		H2C_Parameter[0]));
	...
	)
{
	...
	if (scanType == BTC_SCAN_FINISH)
		halbtcoutsrc_SetPsScan(pBtCoexist, FALSE);
	...
	if (scanType == BTC_SCAN_START)
		halbtcoutsrc_SetPsScan(pBtCoexist, TRUE);
}
	...
	)
{
	...
	{
		if(pBtCoexist->boardInfo.btdmAntNum == 1)
			EXhalbtc8723b1ant_DownloadFwNotify(pBtCoexist,DLFwState);
	}
	else if(IS_HARDWARE_TYPE_8812(pBtCoexist->Adapter))
	{
	}
