				    ...
				    struct net_device *dev)
{
	...
	if ((int)(skb->data) & 1) {
        //printk("\n Unaligned data:%s skb->head: %p skb->data: %p skb->tail:%p \n", __func__, skb->head, skb->data, skb->tail);
	u8 tmp_offset = (int)(skb->data) & 3;
	u8 *tmp_skb_data = skb->data;
	if(skb_headroom(skb) < tmp_offset) {
		printk(KERN_ERR "Not enough headroom to move data %d\n", skb_headroom(skb));
		}
	else {
		skb_push(skb, tmp_offset);
		memmove(skb->data, tmp_skb_data, skb->len - tmp_offset);
		skb_trim(skb, skb->len - tmp_offset);
	     }
	}
