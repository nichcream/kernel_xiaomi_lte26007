...
static void lis3dh_acc_resume(struct early_suspend *h);
static void lis3dh_acc_suspend(struct early_suspend *h);
#endif
...
static int lis3dh_acc_hw_init(struct lis3dh_acc_data *acc)
{
	...
	if (err < 0) {
	printk(" lis3dh Error reading WHO_AM_I: is device "
		"available/working?\n");//dev_warn
		goto err_firstread;
	} else
		acc->hw_working = 1;
	if (buf[0] != WHOAMI_LIS3DH_ACC) {
	printk("lis3dh device unknown. Expected: 0x%x,"
...
err_resume_state:
	...
	printk("lis3dh hw init error 0x%x,0x%x: %d\n", buf[0],
			buf[1], err);
	return err;
}
...
static void lis3dh_acc_device_power_off(struct lis3dh_acc_data *acc)
{
	...
	err = lis3dh_acc_i2c_write(acc, buf, 1);
	if (err < 0)
		printk("%s:soft power off failed: %d\n", __func__, err);//
...
int lis3dh_acc_update_g_range(struct lis3dh_acc_data *acc, u8 new_g_range)
{
	...
	case LIS3DH_ACC_G_16G:
		...
		break;
	default:
		printk("%s:invalid g range requested: %u\n",
	...
	return err;
error:
	printk("%s:update g range failed 0x%x,0x%x: %d\n",
...
error:
	printk("%s:update odr failed 0x%x,0x%x: %d\n", __func__,
...
#endif
static DEVICE_ATTR(pollrate_ms, 0664, attr_get_polling_rate, attr_set_polling_rate);
...
static DEVICE_ATTR(int1_threshold, 0664, attr_get_thresh1, attr_set_thresh1);
static DEVICE_ATTR(int1_source, 0444, attr_get_source1, NULL);
static DEVICE_ATTR(click_config, 0664, attr_get_click_cfg, attr_set_click_cfg);
static DEVICE_ATTR(click_source, 0444, attr_get_click_source, NULL);
static DEVICE_ATTR(click_threshold, 0664, attr_get_click_ths, attr_set_click_ths);
static DEVICE_ATTR(click_timelimit, 0664, attr_get_click_tlim, attr_set_click_tlim);
static DEVICE_ATTR(click_timelatency, 0664, attr_get_click_tlat,attr_set_click_tlat);
static DEVICE_ATTR(click_timewindow, 0664, attr_get_click_tw, attr_set_click_tw);
#ifdef DEBUG
...
static int lis3dh_acc_validate_pdata(struct lis3dh_acc_data *acc)
{
	...
	if (acc->pdata->poll_interval < acc->pdata->min_interval) {
		printk("minimum poll interval violated\n");//
		return -EINVAL;
	}
		...
		const struct i2c_device_id *id)
{
	...
	printk("%s: probe start.\n", LIS3DH_ACC_DEV_NAME);//pr_info

	if (client->dev.platform_data == NULL) {
		printk("%s platform data is NULL. exiting.\n", __func__);//dev_err
		err = -ENODEV;
		goto exit_check_functionality_failed;
	}

	if (!i2c_check_functionality(client->adapter, I2C_FUNC_I2C)) {
		printk("%s client not i2c capable\n", __func__);//dev_err
		err = -ENODEV;
		goto exit_check_functionality_failed;
	}
	...
	*/
	...
	if (acc == NULL) {
		err = -ENOMEM;
		printk("failed to allocate memory for lis3sh module data: "
					"%d\n", err);//dev_err
		goto exit_check_functionality_failed;
	}
	...
	if (acc->pdata == NULL) {
		err = -ENOMEM;
		printk("%s:failed to allocate memory for pdata: %d\n", __func__,
				err);//dev_err
	...
	err = lis3dh_acc_device_power_on(acc);
	if (err < 0) {
		printk("lis3dh power on failed: %d\n", err);//dev_err
		goto err_pdata_init;
	}
	...
	if (err < 0) {
		printk("lis3dh update_g_range failed\n");//dev_err
		goto  err_power_off;
	}

	err = lis3dh_acc_update_odr(acc, acc->pdata->poll_interval);
	if (err < 0) {
		printk("lis3dh update_odr failed\n");//
		goto  err_power_off;
	}

	err = lis3dh_acc_input_init(acc);
	if (err < 0) {
		printk("lis3dh input init failed\n");//
		goto err_power_off;
	}
/*
	...
	if (err < 0) {
		printk(
		   "lis3dh device LIS3DH_ACC_DEV_NAME sysfs register failed\n");//
		goto err_input_cleanup;
	}
*/
	err = sysfs_create_group(&acc->input_dev->dev.kobj,
			&lis3dh_attribute_group);
	if (err < 0) {
		printk(
		   "lis3dh device LIS3DH_ACC_DEV_NAME sysfs register failed\n");//
		goto err_input_cleanup;
	...
	acc->early_suspend.suspend = lis3dh_acc_suspend;
	acc->early_suspend.resume = lis3dh_acc_resume;
...
static void lis3dh_acc_resume(struct early_suspend *h)
{
	...
	if (acc->on_before_suspend)
		lis3dh_acc_enable(acc);
	printk(KERN_DEBUG "lis3dh late_resume \n");
}
...
static void lis3dh_acc_suspend(struct early_suspend *h)
{
	...
	lis3dh_acc_disable(acc);
	printk(KERN_DEBUG "lis3dh early_suspend \n");
