							...
							u8 new_fs)
{
	...
	if (res < 0) {
		printk(KERN_DEBUG "%s : failed to update fs:0x%02x\n",
							__func__, new_fs);
		return res;
	}
...
static int lsm330_gyr_device_power_on(struct lsm330_gyr_status *stat)
{
	...
	if (stat->hw_initialized) {
              dev_dbg(&stat->client->dev, "stat->pdata->gpio_int1 = %d\n",
...
#endif
static struct device_attribute pollrate_ms_attribute = __ATTR(pollrate_ms, S_IRUGO|S_IWUSR, attr_polling_rate_show,attr_polling_rate_store);
static struct device_attribute range_attribute = __ATTR(range, S_IRUGO|S_IWUSR, attr_range_show, attr_range_store);
static struct device_attribute enable_device_attribute = __ATTR(enable_device, S_IRUGO|S_IWUSR, attr_enable_show, attr_enable_store);
static struct device_attribute fifo_samples_attribute = __ATTR(fifo_samples, S_IRUGO|S_IWUSR, attr_watermark_show,attr_watermark_store);
static struct device_attribute fifo_mode_attribute = __ATTR(fifo_mode, S_IRUGO|S_IWUSR, attr_fifomode_show,attr_fifomode_store);
#ifdef DEBUG							
static struct device_attribute reg_value_attribute = __ATTR(reg_value, S_IRUGO|S_IWUSR, attr_reg_get, attr_reg_set);
static struct device_attribute reg_addr_attribute = __ATTR(reg_addr, S_IWUSR, NULL, attr_addr_set);
#endif
static struct attribute *lsm330_gyr_attrs[] =
{
    &pollrate_ms_attribute.attr,
    &range_attribute.attr,
    &enable_device_attribute.attr,
    &fifo_samples_attribute.attr, 
    &fifo_mode_attribute.attr,
    #ifdef DEBUG	
...
static struct attribute_group lsm330_gyr_attribute_group = {
	.attrs = lsm330_gyr_attrs,
};
...
static void lsm330_gyr_irq1_fifo(struct lsm330_gyr_status *stat)
{
	int err;
	u8 buf[2];
	u8 int_source;
...
static void lsm330_gyr_irq1_work_func(struct work_struct *work)
{

	struct lsm330_gyr_status *stat;
	stat = container_of(work, struct lsm330_gyr_status, irq1_work);
	/* TODO  add interrupt service procedure.
		 ie:lsm330_gyr_irq2_XXX(stat); */
	lsm330_gyr_irq1_fifo(stat);
	/*  */
	pr_debug("%s %s: IRQ1 served\n", LSM330_GYR_DEV_NAME, __func__);
/* exit: */
	enable_irq(stat->irq1);
}
...
static int lsm330_gyr_input_init(struct lsm330_gyr_status *stat)
{
	...
	err = sysfs_create_group(&stat->input_dev->dev.kobj, &lsm330_gyr_attribute_group);
	if (err < 0) {
	...
	return 0;
err2:
	sysfs_remove_group(&stat->input_dev->dev.kobj, &lsm330_gyr_attribute_group);
err1:
					...
					const struct i2c_device_id *devid)
{
	...
	if (err < 0) {
		printk(KERN_DEBUG "lsm330_gyr update_fs_range failed\n");
		goto err2;
	}
	...
	atomic_set(&stat->enabled, 0);
       ...
       if (stat->pdata->gpio_int1 > 0) {
		...
		dev_info(&client->dev, "%s: %s has set irq1 to irq:"
						" %d mapped on gpio:%d\n",
			LSM330_GYR_DEV_NAME, __func__, stat->irq1,
							stat->pdata->gpio_int1);

		INIT_WORK(&stat->irq1_work, lsm330_gyr_irq1_work_func);
		stat->irq1_work_queue =
			create_singlethread_workqueue("lsm330_gyr_irq1_wq");
		...
		err = request_irq(stat->irq1, lsm330_gyr_isr1,
				IRQF_TRIGGER_HIGH, "lsm330_gyr_irq1", stat);
...
//#define SLEEP
	...
	//dev_info(&client->dev, "suspend, stat->enabled:0x%x\n",atomic_read(&stat->enabled));
...
static int lsm330_gyr_resume(struct device *dev)
{
	...
	//dev_info(&client->dev, "resume,stat->enabled:0x%x\n",atomic_read(&stat->enabled));
