...
#define HAS_PUT_TASK_STRUCT

#if defined(CONFIG_CW1200_BH_DEBUG)
#define bh_printk(...) do{\
	if(priv->bh_enable_bh_debug)\
		printk(__VA_ARGS__);\
}while(0)
#else
#define bh_printk(...)
			     ...
			     bool enable)
{
	...
	priv->powersave_enabled = enable; // Previously disabled to fix throughput and interrupt missing.
}
...
static int cw1200_bh(void *arg)
{
	...
	for (;;) {
		...
		if (priv->hw_bufs_used) {
			cw1200_bh_read_ctrl_reg(priv, &ctrl_reg);
		...
		if (status < 0 || term || priv->bh_error) {
			printk(KERN_DEBUG "status:%ld, term:%d, priv->bh_error:%d\n", status, term, priv->bh_error);
			break;
		}
#ifdef INTERRUPT_WORKAROUND		
		/* Read the control register to avoid the race condition when
		   nothing has been transmitted to the device or there is nothing
		   to be transmitted and the interrupt miss has also occurred.
		   this will avoid the deadlock on receiving if there is no further
		   packet to be txed */
		if (!status) {
			cw1200_bh_read_ctrl_reg(priv, &ctrl_reg);
			if(ctrl_reg & ST90TDS_CONT_NEXT_LEN_MASK)
			{
				printk(KERN_DEBUG "Manoj: Interrupt Miss occurred\n");
				rx = 1;
				goto test;
			}
		}
...
tx:
	...
	if (!term) {
		...
		if (priv->vif) {
			printk(KERN_DEBUG "%s, vif->type:%d, vif->addr[0]:%x, vif->addr[5]:%x, vif->p2p:%d\n",
					__func__, priv->vif->type, priv->vif->addr[0], priv->vif->addr[5],
					priv->vif->p2p);
		}
