...
_func_enter_;
	...
	{
		DBG_8192C("FillH2CCmd8723B(): return H2C cmd because fw is not ready\n");
		return ret;
	}
...
void rtl8723b_set_FwPwrMode_cmd(PADAPTER padapter, u8 psmode)
{
	...
	u8 PowerState=0, awake_intvl = 1, pwrModeByte5 = pwrpriv->bcn_ant_mode, rlbm = 0;
	u8 Mode = psmode;
	u8 smart_ps = pwrpriv->smart_ps;

_func_enter_;
	DBG_871X("%s(): FW LPS mode = %d\n", __func__, psmode);

	// when bt is enabled and wifi is 1ant and under LPS, set Mode
	...
	SET_8723B_H2CCMD_PWRMODE_PARM_MODE(u1H2CPwrModeParm, (Mode>0)?1:0);
	...
	if(psmode == PS_MODE_ACTIVE) {
		PowerState = PS_STATE_S4;
		pwrModeByte5 = 0x40;
		pwrModeByte5 |= BIT(6);
		SET_8723B_H2CCMD_PWRMODE_PARM_TWO_ANTENNA_EN(u1H2CPwrModeParm, pwrModeByte5);
	} else {
		if(BT_IsBtControlLps(padapter) && BT_IsBtLpsOn(padapter))
		{
			PowerState = BT_CoexRpwmVal(padapter);
			pwrModeByte5 = BT_CoexLpsVal(padapter);

			/* BIT6 if set 1, fw will not set ant & PAT based */
			/* on Byte5 under recv PwrMode H2C, we should always set it to 1 */
			/* BIT4 to close SmartPs0 issue pspoll(1) under NULL mode TDMA */
			pwrModeByte5 |= BIT(6);
#ifdef CONFIG_POWER_SAVING
			/* this AP should not close pspoll */
			if (!rtw_check_iot_peer_ps_should_pspoll(padapter))
				pwrModeByte5 |= BIT(4); //BIT4 is close pspoll
			else
				pwrModeByte5 &= ~BIT(4);
#endif
			SET_8723B_H2CCMD_PWRMODE_PARM_TWO_ANTENNA_EN(u1H2CPwrModeParm, pwrModeByte5);
		...
		{
			PowerState = 0; // AllON(0x0C), RFON(0x04), RFOFF(0x00)
			pwrModeByte5 = 0x40;
			pwrModeByte5 |= BIT(6);
			SET_8723B_H2CCMD_PWRMODE_PARM_TWO_ANTENNA_EN(u1H2CPwrModeParm, pwrModeByte5);
		}
...
_func_enter_;
	...
	if(enable)
	{
		...
		if(!(ppwrpriv->wowlan_wake_reason & FWDecisionDisconnect))
		{
			...
			rtl8723b_set_FwMediaStatusRpt_cmd(padapter, RT_MEDIA_CONNECT, mac_id);
		}
		else
...
void rtl8723b_set_FwJoinBssRpt_cmd(PADAPTER padapter, u8 mstatus)
{
	...
	rtl8723b_download_rsvd_page(padapter, RT_MEDIA_CONNECT);
	rtl8723b_set_FwMediaStatusRpt_cmd(padapter, mstatus, mac_id);
}
