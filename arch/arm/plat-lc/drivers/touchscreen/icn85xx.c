...
#include "icn85xx_fw.h"
       ...
       static char firmware[128] = {"/system/ICN87xx.bin"};
    #endif
#endif
...
#if SUPPORT_ESD_CHECK
#include <linux/hrtimer.h>
int esd_timer_start(void);
int esd_timer_cancel(void);
...
static ssize_t icn85xx_tool_write(struct file *file, const char __user * buffer, size_t count, loff_t * ppos)
{
    ...
    {        
       ...
       #if SUPPORT_ESD_CHECK	   
        esd_timer_cancel();
	  flush_workqueue(icn85xx_ts->esd_monitor_workqueue);
	  cancel_delayed_work_sync(&icn85xx_ts->esd_monitor_work);
	  mdelay(100);   // wait  
	 #endif
        if((icn85xx_ts->ictype == ICN85XX_WITH_FLASH_87) || (icn85xx_ts->ictype == ICN85XX_WITHOUT_FLASH_87)) 
        {
...
{  
    ...
    err = gpio_request(CTP_IRQ_PORT, "request irq port");
    if ( err ) {
        err = -EINVAL;
        icn85xx_trace("error : =%s=====CTP_IRQ_PORT  request fail\n", __func__); 
        return err;
    }
    
    //set gpio0 low
    err = gpio_direction_output(CTP_IRQ_PORT, 1);
    if ( err ) {
        err = -EINVAL;
        return err;
    }
    gpio_set_value_cansleep(CTP_IRQ_PORT, 0);
    msleep(30);
    icn85xx_ts_reset();
    //set gpio0 high
    gpio_set_value_cansleep(CTP_IRQ_PORT, 1);
    err = gpio_direction_input(CTP_IRQ_PORT);
    if ( err ) {
        err = -EINVAL;
        return err;
    }
    
    gpio_free(CTP_IRQ_PORT);
...
//  char buf_check = 0,i,j;
    ...
    if (icn85xx_ts->point_num > POINT_NUM)
     {
     	printk(" %s : pint num is error , rebutn\n",__func__);
       	return ;
     }
...
#endif
      icn85xx_trace("=========icn85xx_ts_pen_irq_work\n");
...
int esd_timer_start(void)
{
     hrtimer_start(This_Hrtimer, ktime_set(CTP_ESD_TIMER/1000, (CTP_ESD_TIMER%1000)*1000000), HRTIMER_MODE_REL);
	return 0;
}
...
//static void esd_monitor_worker(struct delayed_work  esd_monitor_work)
static void esd_monitor_worker(struct work_struct *esd_work)
{
	//disable_irq_nosync(icn85xx_ts->irq);
	 struct icn85xx_ts_data *icn85xx_ts = i2c_get_clientdata(this_client);
    int ret = -1;
	...
	while (retry-- && !need_update_fw)
		{
		    ret = icn85xx_read_reg(0xa, &value);
	            ...
	            { 
	                ...
	                break;         // communication ok, 
	            }
		}
    
	if (retry <= 0) 
		need_update_fw = true;   // need reset or update
      if (need_update_fw)    
      	{
      	       if((icn85xx_ts->ictype == ICN85XX_WITHOUT_FLASH_85) || (icn85xx_ts->ictype == ICN85XX_WITHOUT_FLASH_86)) 
              {
                   icn85xx_ts_reset();      // reset
                   icn85xx_bootfrom_sram();
		      //--------
		      // is need update
		      //--------
              ...
              {
              	printk("%s-----------ICN85XX_WITHOUT_FLASH_87 reset\n",__func__);
                       icn85xx_set_prog_addr();    // reset
                       icn87xx_boot_sram();
			    //--------
		          // is need update
		         //--------
...
static enum hrtimer_restart esd_timer_func(struct hrtimer *timer)
{
	...
	if (!delayed_work_pending(&icn85xx_ts->esd_monitor_work)) {
		queue_delayed_work(icn85xx_ts->esd_monitor_workqueue, &icn85xx_ts->esd_monitor_work, msecs_to_jiffies(20));
	} else { // this may cause dada lost??
		icn85xx_error("*** esd_monitor_workqueue is full! ***\n");
	}
	return HRTIMER_NORESTART;
}
#endif
...
//report touch release    
	...
	printk("==%s  :  icn85xx_ts->ictype=%x\n",__func__,icn85xx_ts->ictype);
    ...
    {
    	printk("==%s  :   ICN85XX_WITHOUT_FLASH_87\n",__func__);
        while (retry-- && !need_update_fw) 
        {
            //icn85xx_ts_reset();
...
//   icn85xx_irq_enable();
//   printk("icn85xx_ts_resume  open irq enable  %d \n",icn85xx_ts->use_irq);
...
static void icn85xx_request_io_port(struct icn85xx_ts_data *icn85xx_ts)
{
    ...
    icn85xx_ts->irq = gpio_to_irq(CTP_IRQ_PORT);
    return ;
}
...
static int icn85xx_request_irq(struct icn85xx_ts_data *icn85xx_ts)
{
   ...
   // err = request_irq(icn85xx_ts->irq, icn85xx_ts_interrupt, IRQ_TYPE_EDGE_FALLING, "icn85xx_ts", icn85xx_ts);
    err = request_irq(icn85xx_ts->irq, icn85xx_ts_interrupt, IRQF_TRIGGER_RISING, "icn85xx_ts", icn85xx_ts);
    if (err < 0) 
    {
    ...
    icn85xx_trace("icn85xx_request_irq  quest OK \n");
    icn85xx_irq_disable();
...
static void icn85xx_update(struct icn85xx_ts_data *icn85xx_ts)
{
    ...
    {
        printk("icn87xx_update 87 without flash\n");
        #if (COMPILE_FW_WITH_DRIVER && !SUPPORT_SENSOR_ID)
        	 printk("icn87xx_update 87   COMPILE_FW_WITH_DRIVER 1   SUPPORT_SENSOR_ID 0   \n");
            icn85xx_set_fw(sizeof(icn85xx_fw), &icn85xx_fw[0]);
        #endif
...
{  
	  ...
	  #if SUPPORT_ESD_CHECK
	   This_Hrtimer = &icn85xx_ts->esd_timer;	//Initial This_Hrtimer
	   hrtimer_init(&icn85xx_ts->esd_timer, CLOCK_MONOTONIC, HRTIMER_MODE_REL); // Initial the struc hrtimer
		  icn85xx_ts->esd_timer.function = esd_timer_func;
		 hrtimer_start(&icn85xx_ts->esd_timer, ktime_set(CTP_ESD_TIMER/1000, (CTP_ESD_TIMER%1000)*1000000), HRTIMER_MODE_REL);
		INIT_DELAYED_WORK(&icn85xx_ts->esd_monitor_work, esd_monitor_worker);
		icn85xx_ts->esd_monitor_workqueue = create_singlethread_workqueue("esd_monitor_workqueue");
	#endif
     if(icn85xx_ts->use_irq)
        icn85xx_irq_enable();
     icn85xx_trace("====%s over1111=====.  \n", __func__);
}
#endif
...
static int icn85xx_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    if ( probe_flag == 1 )
    {
    	printk("%s  second into ....\n",__func__);
//    	  err = icn85xx_iic_test();
//    	  if ( err <= 0 )
//    	  {
//    	  	printk("icn85xx_iic_test  second run error \n");
//		return  err;    	  	
//    	  }
//    	  else
//    	  {
//    	  	printk("icn85xx_iic_test  second run OK \n");
//    	  	return 0;
//    	  }
	return 0;
    }
...
static int /*__devexit */ icn85xx_ts_remove(struct i2c_client *client)
{
 ...
 #if SUPPORT_DELAYED_WORK
		cancel_delayed_work_sync(&icn85xx_ts->icn_delayed_work);
#endif
 #if SUPPORT_ESD_CHECK
	esd_timer_cancel();
		cancel_delayed_work_sync(&icn85xx_ts->esd_monitor_work);
		destroy_workqueue(icn85xx_ts->esd_monitor_workqueue);
#endif
...
static int icn85xx_hw_init(void)
{
    ...
    if ( err ) {
        err = -EINVAL;
        icn85xx_trace("error : =%s=====CTP_RST_PORT  request fail\n", __func__); 
        return err;
    }
    ...
    err = gpio_direction_output(CTP_IRQ_PORT, 0);
    if ( err ) {
        err = -EINVAL;
        icn85xx_trace("error : =%s=====CTP_IRQ_PORT  request fail\n", __func__); 
        return err;
...
{ 
    int ret = -1;
    struct i2c_adapter *adap = i2c_get_adapter(2);     // i2c bus num
    printk(KERN_ALERT "==icn85xx_init %d    20150319!!!!!!!!\n",__LINE__);
