...
#if 0
	[NL80211_ATTR_DYNAMIC_PS_TIMEOUT] = { .type = NLA_U32 },
#endif
...
static int nl80211_set_hw_scan_disable(struct sk_buff *skb, struct genl_info *info)
{
...
#if 0
	if (info->attrs[NL80211_ATTR_DYNAMIC_PS_TIMEOUT]) {
	dynamic_ps_timeout =
		(int) nla_get_u32(info->attrs[NL80211_ATTR_DYNAMIC_PS_TIMEOUT]);
	}
#endif
	...
	err = rdev->ops->set_hw_scan_disable(wdev->wiphy, dev, state);
...
static int nl80211_set_power_save(struct sk_buff *skb, struct genl_info *info)
{
	...
	if (!info->attrs[NL80211_ATTR_PS_STATE] &&
	    !info->attrs[NL80211_ATTR_DYNAMIC_PS_TIMEOUT])
		return -EINVAL;
#endif
	...
	if (state == wdev->ps && dynamic_ps_timeout == wdev->ps_timeout)
		return 0;
	...
	if (!err) {
		wdev->ps = state;
		wdev->ps_timeout = dynamic_ps_timeout;
	}
	return err;
}
