...
#ifdef CONFIG_WOWLAN
	//Add for ping reply delayed when suspend/resume.
	if (check_fwstate(&padapter->mlmepriv, WIFI_STATION_STATE) && (check_fwstate(&padapter->mlmepriv, _FW_LINKED)))
	{
		if (!padapter->mlmepriv.linked_lock_flag)
		{
			rtw_lock_suspend_timeout(1 * HZ);
		}
	}
#endif
