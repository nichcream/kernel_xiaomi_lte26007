...
static int rtw_wifi_open(struct net_device *pnetdev)
{
	_adapter *padapter = (_adapter *)rtw_netdev_priv(pnetdev);
	int ret = 0;
	wifi_set_powersave(1,0);

	if (!check_fwstate(&padapter->mlmepriv, WIFI_STATION_STATE)) {
		DBG_871X_LEVEL(_drv_always_, "%s No Station Mode Open Lock Suspend\n",
				__func__);
		rtw_lock_suspend();
	}

#if defined(CONFIG_HAS_EARLYSUSPEND) || defined(CONFIG_ANDROID_POWER)
	if (adapter_to_pwrctl(padapter)->in_early_suspend) {
		DBG_871X_LEVEL(_drv_err_, "%s under early suspend\n", __func__);
		return 0;
	}
#endif

	DBG_871X_LEVEL(_drv_always_, "%s start\n", __func__);
	ret = netdev_open(pnetdev);
	DBG_871X_LEVEL(_drv_always_, "%s ok\n", __func__);

	return ret;
}


static int rtw_wifi_close(struct net_device *pnetdev)
{
	_adapter *padapter = (_adapter *)rtw_netdev_priv(pnetdev);
	int ret = 0;

	if (!check_fwstate(&padapter->mlmepriv, WIFI_STATION_STATE)) {
		DBG_871X_LEVEL(_drv_always_, "%s No Station Mode close Unlock Suspend\n",
				__func__);
		rtw_unlock_suspend();
	}
	wifi_set_powersave(0,0);
#if defined(CONFIG_HAS_EARLYSUSPEND) || defined(CONFIG_ANDROID_POWER)
	if (adapter_to_pwrctl(padapter)->in_early_suspend) {
		DBG_871X_LEVEL(_drv_err_, "%s under early suspend\n", __func__);
		return 0;
...
#else
	pnetdev->open = rtw_wifi_open;
	pnetdev->stop = rtw_wifi_close;
...
u32 rtw_start_drv_threads(_adapter *padapter)
{
	...
	padapter->evtThread = kthread_run(rtw_evt_thread, padapter, "RTW_EVENT_THREAD");
...
int _netdev_open(struct net_device *pnetdev)
{
	...
	if(padapter->bup == _FALSE)
	{
		...
		if(status ==_FAIL)
		{
			...
			goto netdev_open_error_without_thread;
		}
...
netdev_open_error:
	rtw_stop_drv_threads(padapter);
netdev_open_error_without_thread:
...
static int netdev_close(struct net_device *pnetdev)
{
	...
	if(pwrctrlpriv->bInternalAutoSuspend == _TRUE)
	{
		//rtw_pwr_wakeup(padapter);
		if(pwrctrlpriv->rf_pwrstate == rf_off)
			pwrctrlpriv->ps_flag = _TRUE;
	}
	padapter->net_closed = _TRUE;
