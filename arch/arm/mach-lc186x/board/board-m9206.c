...
struct extern_charger_platform_data exchg_data = {
	...
	.termination_currentmA  = 128,
	.dpm_voltagemV          = 4360,
	.usb_cin_limit_currentmA= 500,
	.usb_battery_currentmA  = 500,
	.ac_cin_limit_currentmA = 1600,
	.ac_battery_currentmA   = 1500,
};
#endif
...
static struct cw_bat_platform_data cw_bat_platdata = {
	.bat_low_pin    = mfp_to_gpio(EXTERN_FG_INT_PIN),
	.battery_tmp_tbl	= batt_temp_table,
	.tmptblsize = ARRAY_SIZE(batt_temp_table),
...
static struct mfp_pin_cfg comip_mfp_cfg[] = {
	...
	{EXTERN_FG_INT_PIN,	MFP_PIN_MODE_GPIO},
#endif

#if defined(CONFIG_CHARGER_NCP1851)||defined(CONFIG_CHARGER_BQ24158)||defined(CONFIG_CHARGER_BQ24296)
...
#endif
#if defined(CONFIG_TOUCHSCREEN_ICN8528F)
	{ICN8528F_RST_PIN,                MFP_PIN_MODE_GPIO},
	{ICN8528F_INT_PIN,                MFP_PIN_MODE_GPIO},
#endif
	...
	{SGM3141_EN_PIN,		MFP_PIN_MODE_GPIO},
	{SGM3141_MODE_PIN,		MFP_PIN_MODE_GPIO},
#endif
...
#endif
	/*mfp low power config*/
#if defined(CONFIG_COMIP_BOARD_M9206_V1_0)
...
static struct pmic_power_ctrl_map lc1160_power_ctrl_map[] = {
	...
	{PMIC_DLDO7,	PMIC_POWER_CTRL_REG,	PMIC_POWER_CTRL_GPIO_ID_NONE,	3300},
...
#elif defined(CONFIG_TOUCHSCREEN_ICN8528F)
static int ICN8528F_rst = ICN8528F_RST_PIN;
static int ICN8528F_i2c_power(struct device *dev, unsigned int vdd)
{
	if (vdd=1) {
		pmic_voltage_set(PMIC_POWER_TOUCHSCREEN_IO, 0, PMIC_POWER_VOLTAGE_ENABLE);
		printk("ICN8528F_i2c on\n");
	} else {
		pmic_voltage_set(PMIC_POWER_TOUCHSCREEN_IO, 0, PMIC_POWER_VOLTAGE_DISABLE);
		printk("ICN8528F_i2c off\n");
	}

	return 0;
}
static int ICN8528F_ic_power(struct device *dev, unsigned int vdd)
{
	int retval = 0;
	int gpio_rst = ICN8528F_RST_PIN;
	if (vdd=1) {
		printk("ICN8528F_ic_power on\n");
		retval = gpio_request(gpio_rst, "ICN8528F_ts_rst");
		if(retval) {
			pr_err("ICN8528F_ts request rst error\n");
			return retval;
		}
		gpio_direction_output(gpio_rst, 0);
		mdelay(2);
...
static int ICN8528F_reset(struct device *dev)
{
	printk("ICN8528F_reset\n");
	gpio_request(ICN8528F_rst, "ICN8528F Reset");
	gpio_direction_output(ICN8528F_rst, 1);
	mdelay(2);
	gpio_direction_output(ICN8528F_rst, 0);
	mdelay(10);
	gpio_direction_output(ICN8528F_rst, 1);
	mdelay(200);
	gpio_free(ICN8528F_rst);
	return 0;
}
static struct icn85xx_info comip_i2c_ICN8528F_info = {
	.reset = ICN8528F_reset,
	.power_i2c = ICN8528F_i2c_power,
	.power_ic = ICN8528F_ic_power,
	.irq_gpio = mfp_to_gpio(ICN8528F_INT_PIN),
	.power_i2c_flag = 1,
	.power_ic_flag = 1,
	.max_scrx = 1024,
	.max_scry = 600,
...
#endif
#if defined(CONFIG_RGB_LVDS_ZA7783)
static int za7783_ic_power(  int onoff)
{
	printk("za7783_ic_power:%d",onoff);
	if (onoff) {
		...
		pmic_voltage_set(PMIC_POWER_LCD_ANALOG, 0, PMIC_POWER_VOLTAGE_ENABLE);
	} else {
		...
		pmic_voltage_set(PMIC_POWER_LCD_ANALOG, 0, PMIC_POWER_VOLTAGE_DISABLE);
	}
	return 0;
}
#endif
#if defined(CONFIG_RGB_LVDS_ZA7783)
static struct za7783_platform_data comip_i2c_za7783_info = {
	.irq_gpio = 0,
	.mclk_parent_name = "pll1_mclk",
	.mclk_name = "clkout2_clk",//"clkout2_clk"
	.mclk_rate = 26000000,	//ref clk
	.power = za7783_ic_power,
};
#endif
...
static struct i2c_board_info comip_i2c0_board_info[] = {
#if defined(CONFIG_RGB_LVDS_ZA7783)
	{
		.type = ZA7783_NAME,
		.addr = 0x37, //0x2c
		.platform_data = &comip_i2c_za7783_info,
	},
#endif
};
...
static struct i2c_board_info comip_i2c2_board_info[] = {
...
#if defined(CONFIG_TOUCHSCREEN_ICN8528F)
	{
		.type = "icn85xx",
		.addr = 0x48,
		.platform_data = &comip_i2c_ICN8528F_info,
	},
#endif
};
...
#endif
#if defined(CONFIG_CHARGER_BQ24296)
	{
		I2C_BOARD_INFO("bq24296_charger", 0x6b),
		.platform_data = &exchg_data,
		.irq = mfp_to_gpio(EXTERN_CHARGER_INT_PIN),
	},
#endif
...
#else defined(CONFIG_LCD_AUO_B079XAN01)

static int comip_lcd_power(int onoff)
{

	printk("comip_lcd_power:%d\n",onoff);
	if (onoff) {
		pmic_voltage_set(PMIC_POWER_LCD_CORE, 0, PMIC_POWER_VOLTAGE_ENABLE);
		pmic_voltage_set(PMIC_POWER_LCD_IO, 0, PMIC_POWER_VOLTAGE_ENABLE);
		pmic_voltage_set(PMIC_POWER_LCD_ANALOG, 0, PMIC_POWER_VOLTAGE_ENABLE);
...
static int gc2035_power(int onoff)
{
	...
	gpio_request(gc2035_rst, "GC2355 Reset");
	if (onoff) {
		mdelay(50);
...
#endif
#if defined(CONFIG_VIDEO_SGM3141)
static int sgm3141_gpio_en = mfp_to_gpio(SGM3141_EN_PIN);
static int sgm3141_gpio_mode = mfp_to_gpio(SGM3141_MODE_PIN);
...
static int sgm3141_set(enum camera_led_mode mode, int onoff)
{
	...
	if (!init_flag) {
		gpio_request(sgm3141_gpio_en, "sgm3141 EN");
		gpio_request(sgm3141_gpio_mode, "sgm3141 MODE");
		gpio_direction_output(sgm3141_gpio_en, 0);
		gpio_direction_output(sgm3141_gpio_mode, 0);
		gpio_free(sgm3141_gpio_en);
		gpio_free(sgm3141_gpio_mode);
		init_flag = 1;
	}
	...
	if (mode == FLASH) {
		...
		gpio_request(sgm3141_gpio_mode, "sgm3141 MODE");
		if (onoff) {
			gpio_direction_output(sgm3141_gpio_mode, 1);
			gpio_direction_output(sgm3141_gpio_en, 1);
		} else {
			gpio_direction_output(sgm3141_gpio_en, 0);
			gpio_direction_output(sgm3141_gpio_mode, 0);
		}
