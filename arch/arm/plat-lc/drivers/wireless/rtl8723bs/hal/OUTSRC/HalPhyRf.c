	...
	)
{
	...
	for (p = ODM_RF_PATH_A; p < ODM_RF_PATH_MAX; ++p)
	{
	...
	)
{
	...
	//s1Byte			 CCK_index=0/*, OFDM_index_old[2]={0,0}, CCK_index_old=0*/;
	//BOOLEAN 		is2T = (pDM_Odm->RFType == ODM_2T2R) ? TRUE : FALSE;

	u1Byte			OFDM_min_index = 6;  // OFDM BB Swing should be less than +3.0dB, which is required by Arthur
	...
	if( ! pDM_Odm->RFCalibrateInfo.TxPowerTrackControl || ThermalValue == 0 ||
	...
	ODM_RT_TRACE(pDM_Odm, ODM_COMP_TX_PWR_TRACK, ODM_DBG_LOUD, ("(delta_LCK, delta_IQK) = (%d, %d)\n", delta_LCK, delta_IQK));
	...
	if (delta > 0 && pDM_Odm->RFCalibrateInfo.TxPowerTrackControl)
	{
	    ...
	    for (p = ODM_RF_PATH_A; p < c.RfPathCount; p++)
        {
		    ...
		    if (pDM_Odm->RFCalibrateInfo.DeltaPowerIndex[p] == pDM_Odm->RFCalibrateInfo.DeltaPowerIndexLast[p])
		    	pDM_Odm->RFCalibrateInfo.PowerIndexOffset[p] = 0;
		    else
