...
#endif
#include "board-yongda.h"

#include "board-common.c"

#if defined(CONFIG_MIPI_LVDS_ICN6201)
...
#if defined(CONFIG_MIPI_LVDS_ICN6201)
	{ICN6201_IRQ_PIN,       MFP_PIN_MODE_GPIO},
	{ICN6201_EN,            MFP_PIN_MODE_GPIO},
#endif
...
static struct pmic_power_module_map lc1160_power_module_map[] = {
	{PMIC_DCDC9,	PMIC_POWER_LCD_IO,		0,	1},
	...
	{PMIC_ALDO14,	PMIC_POWER_USB_ETH,			0,	0},
#endif
	{PMIC_ISINK3,	PMIC_POWER_VIBRATOR,		0,	0}
};

static struct pmic_power_ctrl_map lc1160_power_ctrl_map[] = {
	{PMIC_DCDC9,	PMIC_POWER_CTRL_REG,	PMIC_POWER_CTRL_GPIO_ID_NONE,	3300},
#if defined(CONFIG_COMIP_BOARD_LC1860_EVB3)
...
#if defined(CONFIG_MIPI_LVDS_ICN6201)
static void lvds_icn6201_power_set(int onoff)
{
	if (onoff)
...
static void lvds_icn6201_en(int enable)
{
	gpio_request(ICN6201_EN, "icn6201_en");
	gpio_direction_output(ICN6201_EN, enable);
	gpio_free(ICN6201_EN);
}

static struct lvds_icn6201_platform_data icn6201_pdata = {
	.gpio_irq = ICN6201_IRQ_PIN,
	.mclk_name = "clkout0_clk",
	.mclk_parent_name = "pll1_mclk",
	.clk_rate = 26000000,
	.power = lvds_icn6201_power_set,
	.lvds_en = lvds_icn6201_en,
};
#endif
...
#if defined(CONFIG_LCD_LVDS_1280X800)
static int comip_lcd_power(int onoff)
{
	if (onoff) {
		pmic_voltage_set(PMIC_POWER_LCD_CORE, 0, PMIC_POWER_VOLTAGE_ENABLE);
