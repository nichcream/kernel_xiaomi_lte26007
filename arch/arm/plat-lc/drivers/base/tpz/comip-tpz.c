 ...
 *	1.0.0		2013-08-13	gaochong	created
 *
 */
...
int comip_tpz_enabe(int id, int enable)
{
	...
	if (id == TPZ_CP) {
		tpz_writel(tpz, TPZCTL_CP_RGN_EN, !!enable);
		tpz_writel(tpz, TPZCTL_CP_RGN_UPDT, 1);
		...
		tpz_irq_enable(tpz, enable,
			(1 << TPZCTL_AP_TPZ_INTR) | (1 << TPZCTL_AXI_TPZ_INTR)
			| (1 << TPZCTL_LCDC_TPZ_INTR));
	}
...
int comip_tpz_config(int id, int attr, int start, int end, int def_addr)
{
	...
	val = (start << 3) | (end << 14) | (attr & TPZ_ATTR_MASK);
	if (id == TPZ_CP) {
		tpz_writel(tpz, TPZCTL_CP_TPZ_CFG, val);
...
static irqreturn_t comip_tpz_irq_handle(int irq, void* dev_id)
{
	...
	if(status & (1 << TPZCTL_AP_TPZ_INTR))
		printk(KERN_DEBUG "ap tpz interrupt\n");

	if(status & (1 << TPZCTL_AXI_TPZ_INTR))
		printk(KERN_DEBUG "axi tpz interrupt\n");

	if(status & (1 << TPZCTL_LCDC_TPZ_INTR))
		printk(KERN_DEBUG "lcdc tpz interrupt\n");
...
MODULE_AUTHOR("gaochong<gaochong2@leadcoretech.com>");
